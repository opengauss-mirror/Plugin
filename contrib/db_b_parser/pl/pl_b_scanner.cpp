...
void plbsql_yyerror(const char* message, bool isError)
{
    ...
    if (rc != PLPGSQL_COMPILE_NULL) {
        lines = GetProcedureLineNumberInPackage(u_sess->plsql_cxt.curr_compile_context->core_yy->scanbuf, plbsql_yylloc);
...
static PLpgSQL_package* compilePackageSpec(Oid pkgOid)
{
    ...
    YYSTYPE temp_lval = plbsql_yylval;
    YYLTYPE temp_lloc = plbsql_yylloc;
    ...
    plbsql_yylval = temp_lval;
    plbsql_yylloc = temp_lloc;
    return pkg;
}
...
{
    return plbsql_yylval.wdatum.ident != NULL &&
        strncasecmp(plbsql_yylval.wdatum.ident, "BULK_EXCEPTIONS", strlen("BULK_EXCEPTIONS")) == 0;
}
...
void CheckSaveExceptionsDML(int errstate)
{
    ...
    if (tok3 != K_INSERT && tok3 != K_UPDATE && tok3 != K_DELETE && tok3 != K_SELECT &&
        ...
        InsertErrorMessage(message, plbsql_yylloc);
        ereport(errstate,
