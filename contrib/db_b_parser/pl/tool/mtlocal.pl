...
#      3. plsql parser: (src/pl/plbsql/src/) pl_gram.c pl_gram.h
#
...
while (my $indata = <$F>)
{
	...
	$indata =~ s/(char \*plbsql_base_yytext;)/THR_LOCAL $1/;
	...
	$indata =~ s/(YYSTYPE plbsql_yylval;)/THR_LOCAL $1/;
	$indata =~ s/(YYLTYPE plbsql_yylloc;)/THR_LOCAL $1/;
