...
-- Build all three contexts under one snapshot so concurrent xid activity
-- cannot invalidate a context created by an earlier statement in this phase.
...
-- load contexts again
-- Keep the rebuild phase on one stable snapshot for the same reason.
BEGIN ISOLATION LEVEL REPEATABLE READ;
SELECT * FROM cypher('ag_graph_3', $$ MATCH (u) RETURN vertex_stats(u) ORDER BY id(u) $$) AS (result agtype);
SELECT * FROM cypher('ag_graph_2', $$ MATCH (u) RETURN vertex_stats(u) ORDER BY id(u) $$) AS (result agtype);
SELECT * FROM cypher('ag_graph_1', $$ MATCH (u) RETURN vertex_stats(u) ORDER BY id(u) $$) AS (result agtype);
...
SELECT * FROM cypher('ag_graph_1', $$ MATCH (u)-[]->(v) CREATE (v)-[:stalks]->(u) $$) AS (result agtype);
\pset tuples_only on
