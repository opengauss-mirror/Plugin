...
-- identifiers when the unquoted form is also a keyword. Quoted keyword
-- aliases must also remain readable after a WITH projection.
SELECT * FROM cypher('issue_2355', $$ RETURN 1 AS `count` $$) AS (a agtype);
SELECT * FROM cypher('issue_2355', $$ WITH 1 AS `count` RETURN `count` $$) AS (a agtype);

-- Known limitation: the equivalent unquoted keyword still follows the
-- keyword token path and cannot be read back as a variable (issue #2416).
SELECT * FROM cypher('issue_2355', $$ WITH 1 AS exists RETURN exists $$) AS (a agtype);
