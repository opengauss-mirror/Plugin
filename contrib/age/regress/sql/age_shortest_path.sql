...
-- an explicitly unknown relationship type must not be widened to any type
-- when the start=end zero-hop result triggers the minimum-hop fallback
SELECT count(*) FROM age_shortest_path(
    '"sp_tri"'::agtype,
    (SELECT id FROM cypher('sp_tri', $$ MATCH (n {name:'A'}) RETURN id(n) $$) AS (id agtype)),
    (SELECT id FROM cypher('sp_tri', $$ MATCH (n {name:'A'}) RETURN id(n) $$) AS (id agtype)),
    '"NOSUCHLABEL"'::agtype, '"out"'::agtype, 3::agtype);
SELECT count(*) FROM age_all_shortest_paths(
    '"sp_tri"'::agtype,
    (SELECT id FROM cypher('sp_tri', $$ MATCH (n {name:'A'}) RETURN id(n) $$) AS (id agtype)),
    (SELECT id FROM cypher('sp_tri', $$ MATCH (n {name:'A'}) RETURN id(n) $$) AS (id agtype)),
