...
-- GIN keys must preserve the sign of infinite float values.
CREATE TEMP TABLE agtype_infinity_gin_probe (value agtype);
INSERT INTO agtype_infinity_gin_probe
VALUES ('{"value": Infinity}'), ('{"value": -Infinity}');
CREATE INDEX agtype_infinity_gin_probe_idx
ON agtype_infinity_gin_probe USING gin (value);
CREATE FUNCTION agtype_infinity_gin_has_false_positives(query_value agtype)
RETURNS boolean
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    FOR plan_line IN EXECUTE
        ...
        'SELECT value FROM agtype_infinity_gin_probe WHERE value @> $1'
        USING query_value
    LOOP
        IF plan_line LIKE '  Rows Removed by Index Recheck:%' THEN
            RETURN true;
        END IF;
    END LOOP;
    RETURN false;
END;
$function$;
SELECT agtype_infinity_gin_has_false_positives(
           '{"value": Infinity}'::agtype) AS positive_infinity_false_positives;
SELECT agtype_infinity_gin_has_false_positives(
           '{"value": -Infinity}'::agtype) AS negative_infinity_false_positives;
DROP FUNCTION agtype_infinity_gin_has_false_positives(agtype);
DROP TABLE agtype_infinity_gin_probe;
...
-- Anonymous regular-edge endpoints must still validate vertex existence.
SELECT * FROM cypher('cypher_index', $$
    CREATE (:City {city_id: 999, name: "orphan"})
           -[:has_city]->(:Country {country_code: "ZZ"})
$$) AS (result agtype);
DELETE FROM cypher_index."City"
WHERE properties @> '{"city_id": 999}'::agtype;
SELECT COUNT(*) FROM cypher('cypher_index', $$
    MATCH (:Country {country_code: "ZZ"})<-[e:has_city]-()
    RETURN e
$$) AS (edge agtype);
