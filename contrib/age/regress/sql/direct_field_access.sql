...
--
-- Section 7: #2424 direct access and TOAST ownership
--
CREATE TEMP TABLE agtype_2424_access (
    object_value agtype,
    key_value agtype
);
ALTER TABLE agtype_2424_access
    ALTER COLUMN object_value SET STORAGE EXTERNAL;
ALTER TABLE agtype_2424_access
    ALTER COLUMN key_value SET STORAGE EXTERNAL;
INSERT INTO agtype_2424_access
SELECT agtype_build_map(repeat('k', 6000), 42),
       text_to_agtype(repeat('k', 6000));

SELECT agtype_access_operator(object_value, key_value) = '42'::agtype
       AS toasted_direct_access
FROM agtype_2424_access;

SELECT count(*) = 128 AS repeated_toasted_direct_access
FROM agtype_2424_access, generate_series(1, 128)
WHERE agtype_access_operator(object_value, key_value) = '42'::agtype;

SELECT agtype_access_operator(VARIADIC ARRAY[
           '{"outer": {"inner": 7}}'::agtype,
           '"outer"'::agtype,
           '"inner"'::agtype
       ]) = '7'::agtype AS variadic_chained_access;
