...
-- A volatile initial expression is evaluated once, not once per list element.
CREATE SEQUENCE reduce_init_counter;
CREATE FUNCTION reduce_init_tick() RETURNS agtype
LANGUAGE SQL VOLATILE
AS $$
    SELECT nextval('reduce_init_counter')::text::agtype
$$;

SELECT * FROM cypher('reduce', $$
    RETURN reduce(s = reduce_init_tick(), x IN [1, 2, 3] | s + x)
$$) AS (result agtype);
SELECT last_value AS init_evaluations FROM reduce_init_counter;

DROP FUNCTION reduce_init_tick();
DROP SEQUENCE reduce_init_counter;
...
-- the initial value may reference an outer variable independently of body
-- captures; both are evaluated through their respective outer-query boundary
SELECT * FROM cypher('reduce', $$
    WITH 5 AS base
...
-- two distinct outer variables occupy independent capture slots
SELECT * FROM cypher('reduce', $$
...
-- an outer map property used together with the current element
SELECT * FROM cypher('reduce', $$
    WITH {factor: 10} AS m
    RETURN reduce(s = 0, x IN [1, 2, 3] | s + x * m.factor)
$$) AS (result agtype);

-- the outer value changes per input row and must be rebound for each group
SELECT * FROM cypher('reduce', $$
    UNWIND [1, 2, 3] AS m
...
-- a NULL outer value is represented as Cypher null in its capture slot
SELECT * FROM cypher('reduce', $$
    WITH null AS w
    RETURN reduce(s = 0, x IN [1, 2, 3] | w)
$$) AS (result agtype);

-- mixed NULL and non-NULL captures remain in their own slots
SELECT * FROM cypher('reduce', $$
    WITH 3 AS a, null AS b
    RETURN reduce(s = [], x IN [1] | s + [a, b])
$$) AS (result agtype);

-- only outer leaves are captured, so CASE still short-circuits inside the body
SELECT * FROM cypher('reduce', $$
    WITH 0 AS w
    RETURN reduce(s = 0, x IN [1, 2, 3] |
                  CASE WHEN false THEN s + 1 / w ELSE s + x END)
$$) AS (result agtype);

-- an empty list returns init without evaluating a body that references outside
SELECT * FROM cypher('reduce', $$
    WITH 0 AS w
    RETURN reduce(s = 7, x IN [] | s + 1 / w)
$$) AS (result agtype);
...
--
-- cypher() parameters referenced in the fold body are captured per execution
--
...
-- openGauss: float constants must keep full precision through the serialized
-- clause data (float8out() would truncate to 15 significant digits).
SELECT * FROM cypher('reduce', $$
    RETURN reduce(acc = 0, x IN [1] | acc + 3.141592653589793) = 3.141592653589793
$$) AS (exact agtype);
SELECT * FROM cypher('reduce', $$
    RETURN reduce(acc = 0.0, x IN [0.5, 0.25] | acc + x)
$$) AS (sum agtype);
