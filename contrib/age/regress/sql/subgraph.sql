...
-- openGauss note: a count(*) placed directly over a data-modifying cypher()
-- SRF crashes the distributed planner (make_agg -> ng_copy_distribution with a
-- NULL distribution). An OFFSET 0 subquery fence forces an intermediate node so
-- the aggregate is not planned straight onto the function scan. The CREATE
-- cypher returns no rows, so the count is 0 either way.
SELECT count(*) FROM (SELECT * FROM cypher('sg_src', $$
    UNWIND range(1, 2000) AS i
    CREATE (:Person {pid: i, age: i % 100})-[:KNOWS {w: i}]->(:Friend {pid: i})
$$) AS (a agtype) OFFSET 0) t;
...
-- Mixed-case labels Person/Friend/Company/KNOWS force quoted destination
-- relation and sequence names, covering regclass resolution in both staging passes.
--
...
-- VERBOSITY terse suppresses the PL/pgSQL CONTEXT line, whose reported
-- function-body line number is sensitive to the openGauss-specific comments
-- added to create_subgraph and is not the point of this assertion.
\set VERBOSITY terse
