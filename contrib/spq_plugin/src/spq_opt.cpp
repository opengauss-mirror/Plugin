...
#include "spq/spq_mutate.h" /* apply_shareinput */
...
static void log_optimizer(PlannedStmt *plan, bool fUnexpectedFailure)
{
    ...
    // if ((OPTIMIZER_ALL_FAIL == spq_optimizer_log_failure) ||
    //    (fUnexpectedFailure && OPTIMIZER_UNEXPECTED_FAIL == spq_optimizer_log_failure) || /* unexpected fall back */
    //    (!fUnexpectedFailure && OPTIMIZER_EXPECTED_FAIL == spq_optimizer_log_failure)) {  /* expected fall back */
    //    if (fUnexpectedFailure) {
    //        elog(LOG, "OpenGauss SPQ Optimizer failed to produce plan (unexpected)");
    //    } else {
    //        elog(LOG, "OpenGauss SPQ Optimizer failed to produce plan");
    //    }
    //    return;
    // }
}
