...
//		SPQDB cost model
//---------------------------------------------------------------------------
#ifndef SPQDBCOST_CCostModelSPQDB_H
#define SPQDBCOST_CCostModelSPQDB_H
...
private:
	...
	CCostModelParamsSPQDB *m_cost_model_params;
    ...
    /* SPQ: for ShareScan */
    static CCost CostShareScan(CMemoryPool *mp, CExpressionHandle &exprhdl,
	...
	static CCost CostBitmapSmallNDV(const CCostModelSPQDB *pcmspqdb,
									const SCostingInfo *pci, CDouble dNDV);

	// cost of bitmap scan when the NDV is large
	static CCost CostBitmapLargeNDV(const CCostModelSPQDB *pcmspqdb,
									const SCostingInfo *pci, CDouble dNDV);
...
public:
	// ctor
	CCostModelSPQDB(CMemoryPool *mp, ULONG ulSegments,
				   CCostModelParamsSPQDB *pcp = NULL);

	// dtor
	virtual ~CCostModelSPQDB();
	...
	Ecmt() const
	{
		return ICostModel::EcmtSPQDBCalibrated;
	}

};	// class CCostModelSPQDB

}  // namespace spqdbcost

#endif	// !SPQDBCOST_CCostModelSPQDB_H
