...
//---------------------------------------------------------------------------
#ifndef SPQNAUCRATES_CJoinCardinalityTest_H
#define SPQNAUCRATES_CJoinCardinalityTest_H
...
class CJoinCardinalityTest
{
	...
	typedef CStatsPredJoinArray *(FnPdrspqstatjoin)(CMemoryPool *mp);
...
private:
	...
	static CStatsPredJoinArray *PdrspqstatspredjoinSingleJoinPredicate(
		CMemoryPool *mp);

	// helper method to generate generate multiple join predicates
	static CStatsPredJoinArray *PdrspqstatspredjoinMultiplePredicates(
		CMemoryPool *mp);

	// helper method to generate join predicate over columns that contain null values
	static CStatsPredJoinArray *PdrspqstatspredjoinNullableCols(CMemoryPool *mp);
...
public:
	...
	static SPQOS_RESULT EresUnittest_JoinNDVRemain();

	// join buckets tests
	static SPQOS_RESULT EresUnittest_Join();

};	// class CJoinCardinalityTest
}  // namespace spqnaucrates

#endif	// !SPQNAUCRATES_CJoinCardinalityTest_H
