										   ...
										   CExpression **ppexprInner)
{
	...
	*ppexprOuter = CTestUtils::PexprLogicalGet(mp, ptabdescR, &strNameR);
	...
	// inner expression
	CWStringConst strNameS(SPQOS_WSZ_LIT("Rel2"));
	CMDIdSPQDB *pmdidS =
...
CSubqueryTestUtils::PexprJoinWithAggSubquery(CMemoryPool *mp, BOOL fCorrelated)
{
	...
	CExpression *pexpr = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CLogicalInnerJoin(mp), (*pexprSelect)[0],
													   ...
													   BOOL fCorrelated)
{
	...
	CWStringConst strNameT(SPQOS_WSZ_LIT("Rel3"));
	...
	pdrspqexpr->Append(pexprR);
	pdrspqexpr->Append(pexprS);
	pdrspqexpr->Append(pexprPred);
	CExpression *pexprJoin = CTestUtils::PexprLogicalNAryJoin(mp, pdrspqexpr);

	CExpression *pexprSubq = SPQOS_NEW(mp) CExpression(
		mp,
	...
	CMemoryPool *mp, COperator::EOperatorId op_id, BOOL fCorrelated)
{
	...
	CWStringConst strAlias1(SPQOS_WSZ_LIT("Rel1Alias"));
	CExpression *pexprOuter1 = CTestUtils::PexprLogicalGetNullable(
		mp, SPQOPT_TEST_REL_OID1, &strName1, &strAlias1);

	CWStringConst strName2(SPQOS_WSZ_LIT("Rel2"));
	CWStringConst strAlias2(SPQOS_WSZ_LIT("Rel2Alias"));
	CExpression *pexprOuter2 = CTestUtils::PexprLogicalGetNullable(
		mp, SPQOPT_TEST_REL_OID2, &strName2, &strAlias2);

	CWStringConst strName3(SPQOS_WSZ_LIT("Rel3"));
	CWStringConst strAlias3(SPQOS_WSZ_LIT("Rel3Alias"));
	CExpression *pexprOuter3 = CTestUtils::PexprLogicalGetNullable(
		mp, SPQOPT_TEST_REL_OID3, &strName3, &strAlias3);

	CWStringConst strName4(SPQOS_WSZ_LIT("Rel4"));
	CWStringConst strAlias4(SPQOS_WSZ_LIT("Rel4Alias"));
	CExpression *pexprInner = CTestUtils::PexprLogicalGetNullable(
		mp, SPQOPT_TEST_REL_OID4, &strName4, &strAlias4);
													   ...
													   BOOL fCorrelated)
{
	...
	if (fCorrelated)
	{
		...
		CExpressionArray *pdrspqexpr = (*pexprInnerSelect)[1]->PdrgPexpr();
...
)
{
	...
	if (fCorrelated)
	{
		...
		CExpressionArray *pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
		pdrspqexpr->Append(pexprCorrelated);
		pdrspqexpr->Append(pexprNonCorrelated);
		pexprPred =
...
)
{
	...
	if (COperator::EopScalarSubqueryAny == op_id)
	{
		const CWStringConst *str =
			SPQOS_NEW(mp) CWStringConst(SPQOS_WSZ_LIT("="));
	...
	return SPQOS_NEW(mp) CExpression(
		...
		SPQOS_NEW(mp) CScalarSubqueryAll(
			mp, SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, SPQDB_INT4_NEQ_OP),
			str, pcrInner),
		pexprSelect, CUtils::PexprScalarIdent(mp, pcrOuter));
}
												  ...
												  BOOL fCorrelated)
{
	...
	CWStringConst strNameR(SPQOS_WSZ_LIT("Rel1"));
	CMDIdSPQDB *pmdidR =
		SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidRel, SPQOPT_TEST_REL_OID1,
	...
	switch (op_id)
	{
		...
		default:
			SPQOS_ASSERT(!"Invalid subquery type");
	}
	...
	CExpressionArray *pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	pdrspqexpr->Append(pexprSubquery);
	pdrspqexpr->Append(pexprPred);

	return CUtils::PexprLogicalSelect(
		mp, pexprOuter, CPredicateUtils::PexprDisjunction(mp, pdrspqexpr));
}
...
)
{
	...
	{
		return SPQOS_NEW(mp) CExpression(
			mp, SPQOS_NEW(mp) CScalarSubqueryExists(mp), pexprSelect);
	}

	return SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CScalarSubqueryNotExists(mp), pexprSelect);
}
	...
	BOOL fCorrelated, CScalarBoolOp::EBoolOperator eboolop)
{
	...
	CExpressionArray *pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	pdrspqexpr->Append(pexprPred1);
	pdrspqexpr->Append(pexprPred2);
	pdrspqexpr->Append(pexprSubqueryExists);
	pdrspqexpr->Append(pexprSubqueryAll);

	CExpression *pexprPred = CUtils::PexprScalarBoolOp(mp, eboolop, pdrspqexpr);
											   ...
											   BOOL fCorrelated)
{
	...
	pdrspqexpr->Append(pexprPrjElem);
	...
	CMemoryPool *mp, COperator::EOperatorId op_id, BOOL fCorrelated)
{
	...
	if (COperator::EopScalarSubqueryAny == op_id)
	{
		const CWStringConst *str =
			SPQOS_NEW(mp) CWStringConst(SPQOS_WSZ_LIT("="));
		pexprSubqueryQuantified = SPQOS_NEW(mp) CExpression(
			mp,
			SPQOS_NEW(mp) CScalarSubqueryAny(
				mp,
				SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, SPQDB_INT4_EQ_OP),
...
)
{
	...
	CExpression *pexprGbAgg = CUtils::PexprLogicalGbAggGlobal(
		mp, SPQOS_NEW(mp) CColRefArray(mp), (*pexprInner)[0], (*pexprInner)[1]);
	pexprInner->Release();
	...
	if (COperator::EopScalarSubqueryExists == op_id)
	{
		pexprSubqueryExistential = SPQOS_NEW(mp)
			CExpression(mp, SPQOS_NEW(mp) CScalarSubqueryExists(mp), pexprGbAgg);
	}
	else
	{
		pexprSubqueryExistential = SPQOS_NEW(mp) CExpression(
			mp, SPQOS_NEW(mp) CScalarSubqueryNotExists(mp), pexprGbAgg);
	}
