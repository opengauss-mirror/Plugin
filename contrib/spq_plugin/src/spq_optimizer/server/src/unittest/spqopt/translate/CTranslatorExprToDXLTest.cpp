...
{
	CUnittest rgut[] = {
		SPQOS_UNITTEST_FUNC(CTranslatorExprToDXLTest::EresUnittest_RunTests),
		SPQOS_UNITTEST_FUNC(
...
CTranslatorExprToDXLTest::EresUnittest_RunTests()
{
	...
	{
		SPQOS_RESULT eres = CTestUtils::EresTranslate(mp, rgtc[ul].szInputFile,
...
CTranslatorExprToDXLTest::EresUnittest_RunMinidumpTests()
{
	CAutoMemoryPool amp;
	CMemoryPool *mp = amp.Pmp();
	const ULONG ulTests = SPQOS_ARRAY_SIZE(rgszMinidumpFileNames);
