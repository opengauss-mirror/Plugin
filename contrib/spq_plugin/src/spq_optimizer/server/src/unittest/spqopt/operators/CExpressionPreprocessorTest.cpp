...
{
	CUnittest rgut[] = {
		SPQOS_UNITTEST_FUNC(EresUnittest_UnnestSubqueries),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcess),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessWindowFunc),
		SPQOS_UNITTEST_FUNC(EresUnittest_InferPredsOnLOJ),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessWindowFuncWithLOJ),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessWindowFuncWithOuterRefs),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessWindowFuncWithDistinctAggs),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessNestedScalarSubqueries),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessOuterJoin),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessOuterJoinMinidumps),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessOrPrefilters),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessOrPrefiltersPartialPush),
		SPQOS_UNITTEST_FUNC(EresUnittest_CollapseInnerJoin),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessConvert2InPredicate),
		SPQOS_UNITTEST_FUNC(EresUnittest_PreProcessConvertArrayWithEquals),
		SPQOS_UNITTEST_FUNC(
...
CExpressionPreprocessorTest::EresUnittest_PreProcessWindowFunc()
{
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed) && "unexpected outer join");
	...
	BOOL fPredBelowWindow)
{
	...
	CColRefArray *pdrspqcrPartitionBy = SPQOS_NEW(mp) CColRefArray(mp);
	pdrspqcrPartitionBy->Append(pcrPartitionBy);
	...
	if (!fOuterChildPred && fCascadedLOJ)
	{
		...
		{
			pdrspqcrPartitionBy->Append(pcrInner);
		}
...
)
{
...

	{
		...
		SPQOS_ASSERT(!FHasNoOuterJoin(pexpr) && "expected outer join");
		...
		if (fAllowOuterJoin)
		{
			SPQOS_ASSERT(!FHasNoOuterJoin(pexprPreprocessed) &&
						"expected outer join");
		}
		else
		{
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOuterJoinMinidumps()
{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgszOuterJoinPositiveTests); ul++)
	{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgszOuterJoinNegativeTests); ul++)
	{
													...
													ULONG size)
{
	...
	for (ULONG ul = 0; ul < size; ul += 2)
	{
		...
		SPQOS_ASSERT(fEqual && "expected equal expressions");

		SPQOS_DELETE(pstrFst);
		SPQOS_DELETE(pstrSnd);
...
)
{
...

	{
		...
		// read query expression
		CExpression *pexpr = CTestUtils::PexprReadQuery(mp, szFilePath);
		SPQOS_ASSERT(HasOuterRefs(pexpr) && "expected outer references");
		...
		{
			SPQOS_ASSERT(HasOuterRefs(pexprPreprocessed) &&
...
CExpressionPreprocessorTest::EresUnittest_PreProcessWindowFuncWithOuterRefs()
{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgszTestsNoOuterRefs); ul++)
	{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgszTestsOuterRefs); ul++)
	{
...
)
{
	// read query expression
	CExpression *pexpr = CTestUtils::PexprReadQuery(mp, szFilePath);
	SPQOS_ASSERT(FHasSeqPrj(pexpr) && "expected sequence project");
	...
	if (fAllowSeqPrj)
	{
		SPQOS_ASSERT(FHasSeqPrj(pexprPreprocessed) &&
	...
	if (fAllowIDF)
	{
		SPQOS_ASSERT(FHasIDF(pexprPreprocessed) &&
...
CExpressionPreprocessorTest::EresUnittest_PreProcessWindowFuncWithDistinctAggs()
{
...

	{
		...
		for (ULONG ul = 0;
			 ul < SPQOS_ARRAY_SIZE(rgszTestsDistinctAggsRemoveWindow); ul++)
		{
		...
		for (ULONG ul = 0;
			 ul < SPQOS_ARRAY_SIZE(rgszTestsDistinctAggsRemoveWindowINDF); ul++)
		{
			PreprocessWinFuncWithDistinctAggs(
		...
		for (ULONG ul = 0;
			 ul < SPQOS_ARRAY_SIZE(rgszTestsDistinctAggsDoNotRemoveWindow); ul++)
		{
			PreprocessWinFuncWithDistinctAggs(
		...
		for (ULONG ul = 0;
			 ul < SPQOS_ARRAY_SIZE(rgszTestsDistinctAggsDoNotRemoveWindowINDF);
			 ul++)
		{
			PreprocessWinFuncWithDistinctAggs(
...
CExpressionPreprocessorTest::EresUnittest_UnnestSubqueries()
{
	...
	const ULONG ulTestCases = SPQOS_ARRAY_SIZE(rgunnesttc);
	for (ULONG ul = 0; ul < ulTestCases && (SPQOS_OK == eres); ul++)
	{
		...
		SPQOS_ASSERT(NULL != pfFst);
		SPQOS_ASSERT(NULL != pfSnd);
		...
		CExpressionArray *pdrspqexprAndOr = SPQOS_NEW(mp) CExpressionArray(mp);

		if (fNegateChildren)
		{
			CExpressionArray *pdrspqexprFst = SPQOS_NEW(mp) CExpressionArray(mp);
			pdrspqexprFst->Append(pexprPredFst);
			pdrspqexprAndOr->Append(CUtils::PexprScalarBoolOp(
				mp, CScalarBoolOp::EboolopNot, pdrspqexprFst));

			CExpressionArray *pdrspqexprSnd = SPQOS_NEW(mp) CExpressionArray(mp);
			pdrspqexprSnd->Append(pexprPredSnd);
			pdrspqexprAndOr->Append(CUtils::PexprScalarBoolOp(
				mp, CScalarBoolOp::EboolopNot, pdrspqexprSnd));
		}
		else
		{
			pdrspqexprAndOr->Append(pexprPredFst);
			pdrspqexprAndOr->Append(pexprPredSnd);
		}

		CScalarBoolOp::EBoolOperator eboolop = elem.m_eboolop;
		CExpression *pexprAndOr =
			CUtils::PexprScalarBoolOp(mp, eboolop, pdrspqexprAndOr);

		CExpressionArray *pdrspqexprNot = SPQOS_NEW(mp) CExpressionArray(mp);
		pdrspqexprNot->Append(pexprAndOr);

		CExpression *pexpr = SPQOS_NEW(mp)
			CExpression(mp, SPQOS_NEW(mp) CLogicalSelect(mp), pexprGet,
	...
	CExpression *pexpr, COperator::EOperatorId eopidPresent)
{
	...
	if (SPQOS_OK == EresCheckExistsSubqueryType(pexpr, eopidPresent))
	{
		return SPQOS_OK;
	}

	if (SPQOS_OK == EresCheckQuantifiedSubqueryType(pexpr, eopidPresent))
	{
		return SPQOS_OK;
	}
...
CExpressionPreprocessorTest::EresUnittest_PreProcessNestedScalarSubqueries()
{
	...
	CExpression *pexprSubqInner = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp) CScalarSubquery(mp, pcrInner, false /*fGeneratedByExist*/,
	...
	CExpression *pexprSubqOuter = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp)
	...
	SPQOS_ASSERT(1 == UlScalarSubqs(pexprPreprocessed) &&
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOuterJoin()
{
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed1) && "unexpected outer join");
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed2) && "unexpected outer join");
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed3) && "unexpected outer join");
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed4) && "unexpected outer join");
	...
	SPQOS_ASSERT(!FHasNoOuterJoin(pexprPreprocessed5) && "expected outer join");
	...
	SPQOS_ASSERT(FHasNoOuterJoin(pexprPreprocessed6) && "unexpected outer join");
													...
													CColRefArray *colref_array)
{
	...
	for (ULONG ul = 0; ul < length; ++ul)
	{
		...
		pdrspqexpr->Append(pexprComparison);
	}
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOrPrefilters()
{
	...
	SPQOS_ASSERT(pdrspqcrInner != NULL);
	SPQOS_ASSERT(3 <= pdrspqcrInner->Size());

	CColRefSet *outer_refs = (*pexprJoin)[0]->DeriveOutputColumns();
	CColRefArray *pdrspqcrOuter = outer_refs->Pdrspqcr(mp);
	SPQOS_ASSERT(pdrspqcrOuter != NULL);
	SPQOS_ASSERT(3 <= pdrspqcrOuter->Size());
	...
	CColRefArray *pdrspqcrDisjunct = SPQOS_NEW(mp) CColRefArray(mp);
	CColRef *pcr0_0 = (*pdrspqcrOuter)[0];
	pdrspqcrDisjunct->Append(pcr0_0);
	CColRef *pcr1_0 = (*pdrspqcrInner)[0];
	pdrspqcrDisjunct->Append(pcr1_0);
	pdrspqexpr->Append(PexprCreateConjunction(mp, pdrspqcrDisjunct));
	pdrspqcrDisjunct->Release();

	pdrspqcrDisjunct = SPQOS_NEW(mp) CColRefArray(mp);
	CColRef *pcr0_1 = (*pdrspqcrOuter)[1];
	pdrspqcrDisjunct->Append(pcr0_1);
	CColRef *pcr0_2 = (*pdrspqcrOuter)[2];
	pdrspqcrDisjunct->Append(pcr0_2);
	pdrspqcrDisjunct->Append(pcr1_0);
	pdrspqexpr->Append(PexprCreateConjunction(mp, pdrspqcrDisjunct));
	pdrspqcrDisjunct->Release();
	...
	CWStringConst strExpectedDebugPrintForSelect(SPQOS_WSZ_LIT(
		// clang-format off
			"+--CLogicalSelect\n"
			"   |--CLogicalInnerJoin\n"
			"   |  |--CLogicalGet \"BaseTableAlias\" (\"BaseTable\"), Columns: [\"column_0000\" (3), \"column_0001\" (4), \"column_0002\" (5)] Key sets: {[0]}\n"
	...
	SPQOS_ASSERT(strSelect.Equals(&strExpectedDebugPrintForSelect));
	...
	CWStringConst strExpectedDebugPrintForPreprocessed(SPQOS_WSZ_LIT(
		// clang-format off
			"+--CLogicalNAryJoin\n"
			"   |--CLogicalSelect\n"
			"   |  |--CLogicalGet \"BaseTableAlias\" (\"BaseTable\"), Columns: [\"column_0000\" (3), \"column_0001\" (4), \"column_0002\" (5)] Key sets: {[0]}\n"
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOrPrefiltersPartialPush()
{
	...
	pdrspqcrDisjunct = SPQOS_NEW(mp) CColRefArray(mp);
	CColRef *pcr1_2 = (*pdrspqcrInner)[2];
	pdrspqcrDisjunct->Append(pcr1_2);
	pdrspqexpr->Append(PexprCreateConjunction(mp, pdrspqcrDisjunct));
	pdrspqcrDisjunct->Release();
...
{
	SPQOS_ASSERT(NULL != popJoin);

	// (1) generate two nested outer joins
	CExpressionArray *pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	rspqexpr[0]->AddRef();
	pdrspqexpr->Append(rspqexpr[0]);
	rspqexpr[1]->AddRef();
	pdrspqexpr->Append(rspqexpr[1]);
	CTestUtils::EqualityPredicate(mp, rspqdprel[0]->GetOutputColumns(),
								  rspqdprel[1]->GetOutputColumns(), pdrspqexpr);
	CExpression *pexprLOJ1 = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CLogicalLeftOuterJoin(mp), pdrspqexpr);

	pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	rspqexpr[2]->AddRef();
	pdrspqexpr->Append(rspqexpr[2]);
	pdrspqexpr->Append(pexprLOJ1);
	CTestUtils::EqualityPredicate(mp, rspqdprel[0]->GetOutputColumns(),
								  rspqdprel[2]->GetOutputColumns(), pdrspqexpr);
	CExpression *pexprLOJ2 = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CLogicalLeftOuterJoin(mp), pdrspqexpr);

	// (2) add Inner/NAry Join on top of outer join
	pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	pdrspqexpr->Append(pexprLOJ2);
	rspqexpr[3]->AddRef();
	pdrspqexpr->Append(rspqexpr[3]);
	CTestUtils::EqualityPredicate(mp, rspqdprel[0]->GetOutputColumns(),
								  rspqdprel[3]->GetOutputColumns(), pdrspqexpr);
	popJoin->AddRef();
	CExpression *pexprJoin1 = SPQOS_NEW(mp) CExpression(mp, popJoin, pdrspqexpr);

	// (3) add another Inner/NAry Join on top of Inner/NAry Join
	pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	pdrspqexpr->Append(pexprJoin1);
	rspqexpr[4]->AddRef();
	pdrspqexpr->Append(rspqexpr[4]);
	CTestUtils::EqualityPredicate(mp, rspqdprel[0]->GetOutputColumns(),
								  rspqdprel[4]->GetOutputColumns(), pdrspqexpr);
	popJoin->AddRef();
	CExpression *pexprJoin2 = SPQOS_NEW(mp) CExpression(mp, popJoin, pdrspqexpr);

	// (4) add another Inner/NAry Join on top of Inner/NAry Join
	pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	pdrspqexpr->Append(pexprJoin2);
	rspqexpr[5]->AddRef();
	pdrspqexpr->Append(rspqexpr[5]);
	CTestUtils::EqualityPredicate(mp, rspqdprel[0]->GetOutputColumns(),
								  rspqdprel[5]->GetOutputColumns(), pdrspqexpr);
	popJoin->AddRef();
	CExpression *pexprJoin3 = SPQOS_NEW(mp) CExpression(mp, popJoin, pdrspqexpr);
	...
	CExpression *pexprCmpLOJInner = CUtils::PexprScalarEqCmp(
		mp, rspqdprel[1]->GetOutputColumns()->PcrFirst(),
...
CExpressionPreprocessorTest::EresUnittest_CollapseInnerJoin()
{
	...
	CWStringConst rgscRel[] = {
		...
		SPQOS_WSZ_LIT("Rel4"), SPQOS_WSZ_LIT("Rel5"), SPQOS_WSZ_LIT("Rel6"),
	...
	ULONG rgulRel[] = {
		...
		SPQOPT_TEST_REL_OID4, SPQOPT_TEST_REL_OID5, SPQOPT_TEST_REL_OID6,
	};
	...
	CExpression *rspqexpr[SPQOS_ARRAY_SIZE(rgscRel)];
	CDrvdPropRelational *rspqdprel[SPQOS_ARRAY_SIZE(rgscRel)];
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgscRel); ul++)
	{
		rspqexpr[ul] = CTestUtils::PexprLogicalGet(mp, &rgscRel[ul],
												  &rgscRel[ul], rgulRel[ul]);
		rspqdprel[ul] = CDrvdPropRelational::GetRelationalProperties(
			rspqexpr[ul]->PdpDerive());
	}
	...
	SPQOS_RESULT eres = SPQOS_OK;
	for (ULONG ulInput = 0; eres == SPQOS_OK && ulInput < 2; ulInput++)
	{
		...
		eres = EresUnittest_CollapseInnerJoinHelper(mp, popJoin, rspqexpr,
													rspqdprel);
		popJoin->Release();
	}
...
CExpressionPreprocessorTest::EresUnittest_PreProcessConvert2InPredicate()
{
	...
	CScalarBoolOp *pscboolop =
		SPQOS_NEW(mp) CScalarBoolOp(mp, CScalarBoolOp::EboolopOr);
	CExpression *pexprDisjunct = SPQOS_NEW(mp) CExpression(
		mp, pscboolop,
	...
	CAutoRef<CExpression> apexprGetWithChildren(
		SPQOS_NEW(mp) CExpression(mp, popGet, pexprDisjunct));

	SPQOS_ASSERT(3 == CUtils::UlCountOperator(apexprGetWithChildren.Value(),
	...
	SPQOS_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(),
	...
	CMemoryPool *mp, BOOL fCreateInStatement)
{
	...
	CExpressionArray *pdrgexprDisjChildren = SPQOS_NEW(mp) CExpressionArray(mp);
	...
	CScalarBoolOp *pscboolop = SPQOS_NEW(mp) CScalarBoolOp(mp, eboolop);
	CExpression *pexprDisjConj =
		SPQOS_NEW(mp) CExpression(mp, pscboolop, pdrgexprDisjChildren);
	pexprArrayComp->AddRef();  // needed for Replace()
	pexpr->PdrgPexpr()->Replace(1, pexprDisjConj);

	SPQOS_ASSERT(2 == CUtils::UlCountOperator(pexpr, COperator::EopScalarCmp));
	return pexpr;
}
...
CExpressionPreprocessorTest::EresUnittest_PreProcessConvertArrayWithEquals()
{
	...
	SPQOS_RTL_ASSERT(0 == CUtils::UlCountOperator(apexprInConverted.Value(),
												 COperator::EopScalarCmp));
	SPQOS_RTL_ASSERT(7 == CUtils::UlCountOperator(apexprInConverted.Value(),
												 COperator::EopScalarConst));
	SPQOS_RTL_ASSERT(1 == CUtils::UlCountOperator(apexprInConverted.Value(),
												 COperator::EopScalarArrayCmp));
	...
	SPQOS_ASSERT(NULL != pexprArrayInCmp);
	CScalarArrayCmp *popCmpInArray =
		CScalarArrayCmp::PopConvert(pexprArrayInCmp->Pop());
	SPQOS_RTL_ASSERT(CScalarArrayCmp::EarrcmpAny == popCmpInArray->Earrcmpt());
	...
	SPQOS_RTL_ASSERT(0 == CUtils::UlCountOperator(apexprNotInConverted.Value(),
												 COperator::EopScalarCmp));
	SPQOS_RTL_ASSERT(7 == CUtils::UlCountOperator(apexprNotInConverted.Value(),
												 COperator::EopScalarConst));
	SPQOS_RTL_ASSERT(1 == CUtils::UlCountOperator(apexprNotInConverted.Value(),
												 COperator::EopScalarArrayCmp));
	...
	EresUnittest_PreProcessConvert2InPredicateDeepExpressionTree()
{
	...
	CAutoRef<CColRefArray> apdrspqcr(
		apexprGet->DeriveOutputColumns()->Pdrspqcr(mp));
	SPQOS_ASSERT(1 < apdrspqcr->Size());
	CColRef *pcrLeft = (*apdrspqcr)[0];
	CColRef *pcrRight = (*apdrspqcr)[1];

	// inner most OR
	CScalarBoolOp *pscboolopOrInner =
		SPQOS_NEW(mp) CScalarBoolOp(mp, CScalarBoolOp::EboolopOr);
	CExpression *pexprDisjunctInner = SPQOS_NEW(mp) CExpression(
		mp, pscboolopOrInner,
	...
	CScalarBoolOp *pscboolopAnd =
		SPQOS_NEW(mp) CScalarBoolOp(mp, CScalarBoolOp::EboolopAnd);
	CExpression *pexprConjunct = SPQOS_NEW(mp)
		CExpression(mp, pscboolopAnd, pexprDisjunctInner,
					CUtils::PexprScalarEqCmp(mp, pcrLeft, pcrRight));
	// outer most OR
	CScalarBoolOp *pscboolopOr =
		SPQOS_NEW(mp) CScalarBoolOp(mp, CScalarBoolOp::EboolopOr);
	CExpression *pexprDisjunct = SPQOS_NEW(mp)
	...
	SPQOS_ASSERT(5 == CUtils::UlCountOperator(apexprGetWithChildren.Value(),
											 COperator::EopScalarCmp));
	...
	SPQOS_ASSERT(2 == CUtils::UlCountOperator(apexprConvert.Value(),
											 COperator::EopScalarArrayCmp));
	SPQOS_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(),
											 COperator::EopScalarCmp));
