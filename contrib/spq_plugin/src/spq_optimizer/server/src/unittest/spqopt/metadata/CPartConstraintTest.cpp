...
{
	CUnittest rgut[] = {
		SPQOS_UNITTEST_FUNC(CPartConstraintTest::EresUnittest_Basic),
		SPQOS_UNITTEST_FUNC(CPartConstraintTest::EresUnittest_DateIntervals),
	};
	CAutoMemoryPool amp;
...
CPartConstraintTest::EresUnittest_Basic()
{
	...
	CPartConstraint *ppartcnstr13Default = SPQOS_NEW(mp) CPartConstraint(
		mp, pcnstr13, true /*fDefaultPartition*/, false /*is_unbounded*/);
	CPartConstraint *ppartcnstr15Default = SPQOS_NEW(mp) CPartConstraint(
		mp, pcnstr15, true /*fDefaultPartition*/, false /*is_unbounded*/);

	pcnstr13->AddRef();
	CPartConstraint *ppartcnstr13NoDefault = SPQOS_NEW(mp) CPartConstraint(
		mp, pcnstr13, false /*fDefaultPartition*/, false /*is_unbounded*/);

	pcnstr13->AddRef();
	CPartConstraint *ppartcnstr13DefaultUnbounded =
		SPQOS_NEW(mp) CPartConstraint(mp, pcnstr13, true /*fDefaultPartition*/,
									 true /*is_unbounded*/);

	pcnstr15->AddRef();
	CPartConstraint *ppartcnstr15DefaultUnbounded =
		SPQOS_NEW(mp) CPartConstraint(mp, pcnstr15, true /*fDefaultPartition*/,
									 true /*is_unbounded*/);

	// tests

	// equivalence
	SPQOS_ASSERT(ppartcnstr13Default->FEquivalent(ppartcnstr13Default));
	SPQOS_ASSERT(ppartcnstr13DefaultUnbounded->FEquivalent(
		ppartcnstr13DefaultUnbounded));
	SPQOS_ASSERT(ppartcnstr13DefaultUnbounded->FEquivalent(
		ppartcnstr15DefaultUnbounded));
	SPQOS_ASSERT(
		!ppartcnstr13DefaultUnbounded->FEquivalent(ppartcnstr13Default));
	SPQOS_ASSERT(!ppartcnstr13Default->FEquivalent(ppartcnstr13NoDefault));
	SPQOS_ASSERT(!ppartcnstr13Default->FEquivalent(ppartcnstr15Default));

	// unboundedness
	SPQOS_ASSERT(!ppartcnstr13Default->IsConstraintUnbounded());
	SPQOS_ASSERT(ppartcnstr13DefaultUnbounded->IsConstraintUnbounded());

	// subsumption & overlap
	SPQOS_ASSERT(ppartcnstr13DefaultUnbounded->FSubsume(ppartcnstr13Default));
	SPQOS_ASSERT(ppartcnstr13DefaultUnbounded->FSubsume(ppartcnstr15Default));
	SPQOS_ASSERT(ppartcnstr15Default->FSubsume(ppartcnstr13Default));
	SPQOS_ASSERT(!ppartcnstr13Default->FSubsume(ppartcnstr15Default));
	SPQOS_ASSERT(ppartcnstr13Default->FOverlap(mp, ppartcnstr15Default));
	SPQOS_ASSERT(ppartcnstr13NoDefault->FOverlap(mp, ppartcnstr13Default));
...
CPartConstraintTest::EresUnittest_DateIntervals()
{
	...
	CPartConstraint *ppartcnstr1Default = SPQOS_NEW(mp) CPartConstraint(
		mp, pciFirst, true /*fDefaultPartition*/, false /*is_unbounded*/);
	CPartConstraint *ppartcnstr2Default = SPQOS_NEW(mp) CPartConstraint(
		mp, pciSecond, true /*fDefaultPartition*/, false /*is_unbounded*/);

	pciFirst->AddRef();
	CPartConstraint *ppartcnstr1NoDefault = SPQOS_NEW(mp) CPartConstraint(
		mp, pciFirst, false /*fDefaultPartition*/, false /*is_unbounded*/);

	pciFirst->AddRef();
	CPartConstraint *ppartcnstr1DefaultUnbounded = SPQOS_NEW(mp) CPartConstraint(
		mp, pciFirst, true /*fDefaultPartition*/, true /*is_unbounded*/);

	pciSecond->AddRef();
	CPartConstraint *ppartcnstr2DefaultUnbounded = SPQOS_NEW(mp) CPartConstraint(
		mp, pciSecond, true /*fDefaultPartition*/, true /*is_unbounded*/);

	// tests

	// equivalence
	SPQOS_ASSERT(ppartcnstr1Default->FEquivalent(ppartcnstr1Default));
	SPQOS_ASSERT(
	...
	SPQOS_ASSERT(!ppartcnstr1DefaultUnbounded->FEquivalent(ppartcnstr1Default));
	SPQOS_ASSERT(!ppartcnstr1Default->FEquivalent(ppartcnstr1NoDefault));
	SPQOS_ASSERT(!ppartcnstr1Default->FEquivalent(ppartcnstr2Default));

	// unboundedness
	SPQOS_ASSERT(!ppartcnstr1Default->IsConstraintUnbounded());
	SPQOS_ASSERT(ppartcnstr1DefaultUnbounded->IsConstraintUnbounded());

	// subsumption & overlap
	SPQOS_ASSERT(ppartcnstr1DefaultUnbounded->FSubsume(ppartcnstr1Default));
	SPQOS_ASSERT(ppartcnstr1DefaultUnbounded->FSubsume(ppartcnstr2Default));
	SPQOS_ASSERT(pciSecond->FContainsInterval(mp, pciFirst));
	SPQOS_ASSERT(ppartcnstr2Default->FSubsume(ppartcnstr1Default));
	SPQOS_ASSERT(!ppartcnstr1Default->FSubsume(ppartcnstr2Default));
	SPQOS_ASSERT(ppartcnstr1Default->FOverlap(mp, ppartcnstr2Default));
	SPQOS_ASSERT(ppartcnstr1NoDefault->FOverlap(mp, ppartcnstr1Default));
