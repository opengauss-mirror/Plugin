...
#define SPQDB_INT4_GT_OP OID(521)
#define SPQDB_INT4_ADD_OP OID(551)
...
CTranslatorDXLToExprTest::EresUnittest()
{
	CUnittest rgut[] = {
		...
		SPQOS_UNITTEST_FUNC(CTranslatorDXLToExprTest::EresUnittest_SelectQuery),
		SPQOS_UNITTEST_FUNC(
		...
		SPQOS_UNITTEST_FUNC(CTranslatorDXLToExprTest::EresUnittest_Limit),
		SPQOS_UNITTEST_FUNC(
			CTranslatorDXLToExprTest::EresUnittest_LimitNoOffset),
		SPQOS_UNITTEST_FUNC(
			CTranslatorDXLToExprTest::EresUnittest_ScalarSubquery),
		SPQOS_UNITTEST_FUNC(CTranslatorDXLToExprTest::EresUnittest_TVF),
		SPQOS_UNITTEST_FUNC(
...
CTranslatorDXLToExprTest::Pexpr(CMemoryPool *mp, const CHAR *dxl_filename)
{
	...
	SPQOS_ASSERT(NULL != ptroutput->CreateDXLNode() &&
	...
	CTranslatorDXLToExpr *pdxltr =
		SPQOS_NEW(mp) CTranslatorDXLToExpr(mp, md_accessor);
	...
	const CWStringDynamic *pstrExpected)
{
	...
	SPQOS_ASSERT(NULL != pstrExpected && NULL != pstrTranslated);
	...
	SPQOS_DELETE(pstrExpected);
	SPQOS_DELETE(pstrTranslated);
...
CTranslatorDXLToExprTest::Pstr(CMemoryPool *mp, CExpression *pexpr)
{
	...
	CWStringDynamic str(mp);
	COstreamString *poss = SPQOS_NEW(mp) COstreamString(&str);

	*poss << std::endl;
	pexpr->OsPrint(*poss);

	CWStringDynamic *pstrExpr =
		SPQOS_NEW(mp) CWStringDynamic(mp, str.GetBuffer());
...
public:
		...
		: m_mp(mp), m_strTableName(strTableName)
	{
		...
		CMDIdSPQDB *mdid =
			SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidRel, oidTableOid, 1, 1);

		const BOOL convert_hash_to_random = false;
		const ULONG ulExecuteAsUser = 0;
		m_ptabdesc = SPQOS_NEW(mp) CTableDescriptor(
			mp, mdid, CName(&strTableName), convert_hash_to_random,
			CMDRelationSPQDB::EreldistrMasterOnly,
			CMDRelationSPQDB::ErelstorageHeap, ulExecuteAsUser);
	}
...
{
	CWStringConst strTblName(SPQOS_WSZ_LIT("r"));
	const BOOL is_nullable = true;

	GetBuilder gb(mp, strTblName, SPQOPT_TEST_REL_OID21);
	gb.AddIntColumn(CWStringConst(SPQOS_WSZ_LIT("a")), 1, is_nullable);
	gb.AddIntColumn(CWStringConst(SPQOS_WSZ_LIT("b")), 2, is_nullable);
...
CTranslatorDXLToExprTest::EresUnittest_SelectQuery()
{
	...
	// manually create the Expr Tree
	{
		...
		CColRefArray *colref_array = popGet->PdrspqcrOutput();

		SPQOS_ASSERT(NULL != colref_array && 2 == colref_array->Size());
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithBoolExpr()
{
	...
	// manually create the Expr Tree
	{
		...
		CColRef *pcrRight = (*colref_array)[1];
		CExpressionArray *pdrspqexprInput = SPQOS_NEW(mp) CExpressionArray(mp, 2);
		pdrspqexprInput->Append(pexprScCmp);
		pexprScCmp = CUtils::PexprScalarEqCmp(mp, pcrLeft, pcrRight);

		pdrspqexprInput->Append(pexprScCmp);
		CExpression *pexprScBool = CUtils::PexprScalarBoolOp(
			mp, CScalarBoolOp::EboolopAnd, pdrspqexprInput);
		CExpression *pexprExpected =
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithScalarOp()
{
	...
	// manually create the Expr Tree
	{
		...
		CExpression *pexprScOp = CUtils::PexprScalarOp(
			mp, pcrLeft, pexprScConst, CWStringConst(SPQOS_WSZ_LIT("+")),
			SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, SPQDB_INT4_ADD_OP));

		// create a scalar compare for a > b + 2
		CColRef *pcrRight = (*colref_array)[0];
		CExpression *pexprScCmp = CUtils::PexprScalarCmp(
			mp, pcrRight, pexprScOp, CWStringConst(SPQOS_WSZ_LIT(">")),
			SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, SPQDB_INT4_GT_OP));
