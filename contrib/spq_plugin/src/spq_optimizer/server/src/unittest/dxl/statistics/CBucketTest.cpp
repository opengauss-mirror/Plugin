...
CBucketTest::EresUnittest()
{
	...
	CUnittest rgutSharedOptCtxt[] = {
		SPQOS_UNITTEST_FUNC(CBucketTest::EresUnittest_CBucketInt4),
		SPQOS_UNITTEST_FUNC(CBucketTest::EresUnittest_CBucketBool),
		SPQOS_UNITTEST_FUNC(CBucketTest::EresUnittest_CBucketScale),
		SPQOS_UNITTEST_FUNC(CBucketTest::EresUnittest_CBucketDifference),
		SPQOS_UNITTEST_FUNC(CBucketTest::EresUnittest_CBucketIntersect),
		SPQOS_UNITTEST_FUNC(
...
CBucketTest::EresUnittest_CBucketInt4()
{
	...
	SPQOS_RTL_ASSERT_MSG(bucket1->Contains(point1), "[1,1] must contain 1");
	SPQOS_RTL_ASSERT_MSG(CDouble(1.0) == bucket1->GetOverlapPercentage(
											point1, true /*include_point*/),
						"overlap of 1 in [1,1] must 1.0");

	SPQOS_RTL_ASSERT_MSG(!bucket1->Contains(point2), "[1,1] must not contain 2");
	...
	SPQOS_RTL_ASSERT(0.49 <= overlap && overlap <= 0.51);

	// bucket [1,10)
	CBucket *bucket3 = SPQOS_NEW(mp)
	...
	SPQOS_RTL_ASSERT(0.49 <= overlap_closed2 && overlap_closed2 <= 0.51);

	// subsumption
	SPQOS_RTL_ASSERT(bucket1->Subsumes(bucket1));
	SPQOS_RTL_ASSERT(bucket2->Subsumes(bucket1));

	// width
	CDouble width = bucket1->Width();
	SPQOS_RTL_ASSERT(0.99 <= width && width <= 1.01);
	...
	// point IsBefore
	SPQOS_RTL_ASSERT_MSG(pbucket4->IsBefore(point2), "2 must be before (2,4)");

	// bucket IsBefore
	SPQOS_RTL_ASSERT_MSG(pbucket3->IsBefore(pbucket4),
	...
	point4->Release();
	point10->Release();
	SPQOS_DELETE(bucket1);
	SPQOS_DELETE(bucket2);
	SPQOS_DELETE(bucket3);
	SPQOS_DELETE(bucket4);
	SPQOS_DELETE(bucket5);
	SPQOS_DELETE(pbucket3);
	SPQOS_DELETE(pbucket4);
...
CBucketTest::EresUnittest_CBucketBool()
{
	...
	SPQOS_RTL_ASSERT_MSG(bucket->Contains(p1), "true bucket must contain true");
	SPQOS_RTL_ASSERT_MSG(CDouble(1.0) == bucket->GetOverlapPercentage(p1),
						"overlap must 1.0");

	SPQOS_RTL_ASSERT_MSG(!bucket->Contains(p2),
						"true bucket must not contain false");
	SPQOS_RTL_ASSERT_MSG(CDouble(0.0) == bucket->GetOverlapPercentage(p2),
						"overlap must 0.0");
...
CBucketTest::EresUnittest_CBucketScale()
{
	...
	SPQOS_RTL_ASSERT(!bucket2->Contains(point1));

	// new bucket [1, 10) must contain 9
	CPoint *point2 = CTestUtils::PpointInt4(mp, 9);
	SPQOS_RTL_ASSERT(bucket2->Contains(point2));
	point2->Release();

	// new bucket's frequency and distinct values must be lesser than original
	SPQOS_RTL_ASSERT(bucket2->GetFrequency() < bucket1->GetFrequency());
	SPQOS_RTL_ASSERT(bucket2->GetNumDistinct() < bucket1->GetNumDistinct());

	// scale lower
	CBucket *pbucket3 =
		bucket1->MakeBucketScaleLower(mp, point1, true /* include_lower */);
	SPQOS_RTL_ASSERT(pbucket3->Contains(point1));

	// scale to a singleton
	CBucket *pbucket4 = bucket1->MakeBucketSingleton(mp, point1);
	SPQOS_RTL_ASSERT(pbucket4->GetNumDistinct() < 2.0);
...
CBucketTest::EresUnittest_CBucketDifference()
{
	...
	SPQOS_RTL_ASSERT(NULL != pbucket4);
	SPQOS_RTL_ASSERT(NULL != pbucket5);
	...
	SPQOS_RTL_ASSERT(NULL != pbucket6);
	SPQOS_RTL_ASSERT(NULL == pbucket7);
	...
	SPQOS_DELETE(pbucket3);
	SPQOS_DELETE(pbucket4);
	SPQOS_DELETE(pbucket5);
	SPQOS_DELETE(pbucket6);
...
CBucketTest::EresUnittest_CBucketIntersect()
{
	...
	const ULONG length = SPQOS_ARRAY_SIZE(rgBucketsIntersectTestElem);
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (true == result)
		{
			...
			SPQOS_DELETE(pbucketExpected);
			SPQOS_DELETE(pbucketOuput);
		}
		// clean up
		SPQOS_DELETE(bucket1);
		SPQOS_DELETE(bucket2);
...
{
	SPQOS_ASSERT(NULL != bucket1);
	SPQOS_ASSERT(NULL != bucket2);
...
CBucketTest::EresUnittest_CBucketMergeCommutativityUnion()
{
	...
	SPQOS_ASSERT(result1->Equals(result2));

	if (NULL != bucket1_new1)
	{
		SPQOS_ASSERT(bucket1_new1->Equals(bucket2_new2));
	}
	else if (NULL != bucket2_new1)
	{
		SPQOS_ASSERT(bucket2_new1->Equals(bucket1_new2));
	}
	...
	SPQOS_DELETE(bucket1_new1);
	SPQOS_DELETE(bucket2_new1);
	SPQOS_DELETE(bucket1_new2);
	SPQOS_DELETE(bucket2_new2);
...
CBucketTest::EresUnittest_CBucketMergeCommutativitySameLowerBounds()
{
	...
	SPQOS_ASSERT(bucket2->Equals(bucket2_new1));
	...
	SPQOS_ASSERT(result1->Equals(result2));
	SPQOS_ASSERT(result1->IsSingleton());
	SPQOS_ASSERT(result2->IsSingleton());
...
CBucketTest::EresUnittest_CBucketMergeCommutativitySameUpperBounds()
{
	...
	SPQOS_ASSERT(bucket2_new1->IsSingleton());
	...
	SPQOS_ASSERT(bucket1_new2->IsSingleton());
...
CBucketTest::EresUnittest_CBucketMergeCommutativityDoubleDatum()
{
	...
	// [0.0, 100.0)
	CPoint *ppLower1 =
		CCardinalityTestUtils::PpointDouble(mp, SPQDB_FLOAT8, CDouble(0.0));
	CPoint *ppUpper1 =
		CCardinalityTestUtils::PpointDouble(mp, SPQDB_FLOAT8, CDouble(100.0));
	...
	// [50.0, 150.0)
	CPoint *ppLower2 =
		CCardinalityTestUtils::PpointDouble(mp, SPQDB_FLOAT8, CDouble(50.0));
	CPoint *ppUpper2 =
		CCardinalityTestUtils::PpointDouble(mp, SPQDB_FLOAT8, CDouble(150.0));
	CBucket *bucket2 = SPQOS_NEW(mp)
