													  ...
													  CDouble distinct)
{
	...
	return SPQOS_NEW(mp) CBucket(ppLower, ppUpper, true /* is_lower_closed */,
									  ...
									  CDouble distinct)
{
	...
	return SPQOS_NEW(mp) CBucket(ppLower, ppUpper, is_lower_closed,
								is_upper_closed, frequency, distinct);
}
											   ...
											   CDouble frequency)
{
	...
	ppLower->AddRef();
	return SPQOS_NEW(mp) CBucket(ppLower, ppLower, true /* fClosedUpper */,
									   ...
									   CDouble num_NDV_remain)
{
	...
	return SPQOS_NEW(mp) CHistogram(mp, histogram_buckets, true, null_freq,
									 ...
									 LINT value)
{
	...
	IMDId *mdid = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, oid);
...
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	CMDIdSPQDB *mdid = SPQOS_NEW(mp) CMDIdSPQDB(CMDIdSPQDB::m_mdid_numeric);
	...
	CDXLDatumStatsDoubleMappable *dxl_datum = SPQOS_NEW(mp)
