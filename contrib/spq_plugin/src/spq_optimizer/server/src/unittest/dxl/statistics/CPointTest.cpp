...
CPointTest::EresUnittest()
{
	...
	CUnittest rgutSharedOptCtxt[] = {
		SPQOS_UNITTEST_FUNC(CPointTest::EresUnittest_CPointInt4),
		SPQOS_UNITTEST_FUNC(CPointTest::EresUnittest_CPointBool),
	};
...
CPointTest::EresUnittest_CPointInt4()
{
	...
	SPQOS_RTL_ASSERT_MSG(point1->Equals(point1), "1 == 1");
	SPQOS_RTL_ASSERT_MSG(point1->IsLessThan(point2), "1 < 2");
	SPQOS_RTL_ASSERT_MSG(point2->IsGreaterThan(point1), "2 > 1");
	SPQOS_RTL_ASSERT_MSG(point1->IsLessThanOrEqual(point2), "1 <= 2");
	SPQOS_RTL_ASSERT_MSG(point2->IsGreaterThanOrEqual(point2), "2 >= 2");

	CDouble dDistance = point2->Distance(point1);

	// should be 1.0
	SPQOS_RTL_ASSERT_MSG(0.99 < dDistance && dDistance < 1.01,
...
CPointTest::EresUnittest_CPointBool()
{
	...
	// true == true
	SPQOS_RTL_ASSERT_MSG(point1->Equals(point1), "true must be equal to true");

	// true != false
	SPQOS_RTL_ASSERT_MSG(point1->IsNotEqual(point2),
