...
{
	CUnittest rgut[] = {
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_ScalarExpr),
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_Statistics),
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_Metadata),
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_MDRequest),
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_RunPlanTests),
		// tests involving dxl representation of queries.
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_RunQueryTests),

		// test that throw XML validation exception
		SPQOS_UNITTEST_FUNC_THROW(
			CParseHandlerTest::EresUnittest_ErrSAXParseException,
			spqdxl::ExmaDXL, spqdxl::ExmiDXLValidationError),

		// tests that should throw an exception
		SPQOS_UNITTEST_FUNC(CParseHandlerTest::EresUnittest_RunAllNegativeTests),
	};
...
CParseHandlerTest::EresUnittest_Metadata()
{
	return EresUnittest_RunAllPositiveTests(
		m_rgszMetadataDXLFileNames, SPQOS_ARRAY_SIZE(m_rgszMetadataDXLFileNames),
...
{
	return EresUnittest_RunAllPositiveTests(
		m_rgszStatsDXLFileNames, SPQOS_ARRAY_SIZE(m_rgszStatsDXLFileNames),
...
{
	return EresUnittest_RunAllPositiveTests(
		m_rgszPlanDXLFileNames, SPQOS_ARRAY_SIZE(m_rgszPlanDXLFileNames),
...
{
	return EresUnittest_RunAllPositiveTests(
		m_rgszQueryDXLFileNames, SPQOS_ARRAY_SIZE(m_rgszQueryDXLFileNames),
...
CParseHandlerTest::EresUnittest_ScalarExpr()
{
	return EresUnittest_RunAllPositiveTests(
		...
		SPQOS_ARRAY_SIZE(m_rgszScalarExprDXLFileNames),
...
CParseHandlerTest::EresUnittest_RunAllPositiveTests(
	...
	SPQOS_RESULT (*testFunc)(CMemoryPool *, const CHAR *, BOOL), BOOL fValidate)
{
	// create memory pool
	CAutoMemoryPool amp;
	CMemoryPool *mp = amp.Pmp();
	...
	{
		if (SPQOS_FAILED == (*testFunc)(mp, rgszFileNames[ul], fValidate))
		{
			return SPQOS_FAILED;
		}
	}
...
CParseHandlerTest::EresUnittest_RunAllNegativeTests()
{
	...
	for (ULONG ulFileNum = 0;
		 ulFileNum < SPQOS_ARRAY_SIZE(m_rgszNegativeTestsFileNames); ulFileNum++)
	{
		SPQOS_TRY
		{
											 ...
											 BOOL fValidate)
{
	...
	dstrExpected.AppendFormat(SPQOS_WSZ_LIT("%s"), dxl_string);

	if (!dstrExpected.Equals(&strPlan))
	{
		SPQOS_TRACE(dstrExpected.GetBuffer());
		SPQOS_TRACE(strPlan.GetBuffer());

		SPQOS_ASSERT(!"Not matching");
	}
											  ...
											  BOOL fValidate)
{
	...
	SPQOS_ASSERT(NULL != pq2dxlresult);
	...
	{
		SPQOS_TRACE(wstrQuery.GetBuffer());
	}

	// cleanup
	SPQOS_DELETE(pq2dxlresult);
	SPQOS_DELETE_ARRAY(dxl_string);
												 ...
												 BOOL fValidate)
{
	...
	{
		SPQOS_TRACE(metadata_str->GetBuffer());
		SPQOS_ASSERT(false);
	}

	mdcache_obj_array->Release();
	SPQOS_DELETE(metadata_str);
	SPQOS_DELETE_ARRAY(dxl_string);
												   ...
												   BOOL fValidate)
{
	...
	SPQOS_ASSERT(dstrExpected.Equals(statistics_str));

	statistics_array->Release();

	SPQOS_DELETE_ARRAY(dxl_string);
	SPQOS_DELETE(statistics_str);
	return SPQOS_OK;
}
												   ...
												   BOOL fValidate)
{
	...
	{
		SPQOS_TRACE(dstrExpected.GetBuffer());
		SPQOS_TRACE(scalar_expr_str->GetBuffer());
