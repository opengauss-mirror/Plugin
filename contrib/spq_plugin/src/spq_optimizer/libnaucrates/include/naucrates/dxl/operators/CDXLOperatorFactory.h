...
#ifndef SPQDXL_CDXLOperatorFactory_H
#define SPQDXL_CDXLOperatorFactory_H
...
public:
	...
	static CDXLPhysical *MakeDXLTblScan(CDXLMemoryManager *dxl_memory_manager,
										const Attributes &attrs);
    /* SPQ: for ShareScan */
    static CDXLPhysical *MakeDXLTblShareScan(CDXLMemoryManager *dxl_memory_manager,
                                              const Attributes &attrs);
	...
	// parse a SPQDB mdid object from an array of its components
	static CMDIdSPQDB *GetSPQDBMdId(
		CDXLMemoryManager *dxl_memory_manager, XMLChArray *remaining_tokens,
		Edxltoken target_attr, Edxltoken target_elem,
		IMDId::EMDIdType mdidType = IMDId::EmdidGeneral);

	// parse a SPQDB CTAS mdid object from an array of its components
	static CMDIdSPQDB *GetSPQDBCTASMdId(CDXLMemoryManager *dxl_memory_manager,
										...
										Edxltoken target_elem)
{
	...
	CDynamicPtrArray<T, CleanupFn> *pdrspqt =
		SPQOS_NEW(mp) CDynamicPtrArray<T, CleanupFn>(mp);
	...
	for (ULONG ul = 0; ul < num_tokens; ul++)
	{
		...
		SPQOS_ASSERT(NULL != xmlszNext);

		T *pt = SPQOS_NEW(mp) T(ValueFromXmlstr(dxl_memory_manager, xmlszNext,
...
#endif	// !SPQDXL_CDXLOperatorFactory_H
