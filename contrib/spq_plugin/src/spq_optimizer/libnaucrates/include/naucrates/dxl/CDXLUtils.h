...
#ifndef SPQDXL_CDXLUtils_H
#define SPQDXL_CDXLUtils_H
					 ...
					 const CDynamicPtrArray<T, CleanupFn> *dynamic_ptr_array)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (ul == length - 1)
		{
			...
			string_var->AppendFormat(SPQOS_WSZ_LIT("%d"), value);
		}
		else
		{
			string_var->AppendFormat(
				SPQOS_WSZ_LIT("%d%ls"), value,
