...
//---------------------------------------------------------------------------
#ifndef SPQOPT_traceflags_H
#define SPQOPT_traceflags_H
...
enum EOptTraceFlag
{
	// reserve range 100000-199999 for SPQOPT
	...
	// enforce evaluating subqueries using correlated joins (subplans in SPQDB)
	...
	// is nestloop params enabled, it is only enabled in SPQDB 6.x onwards.
	EopttraceIndexedNLJOuterRefAsParams = 106000,

    EopttracePrintOptimizationCost = 108000,
...
void SetTraceflags(spqos::CMemoryPool *mp, const spqos::CBitSet *input_bitset,
				   spqos::CBitSet **enable_bitset,
				   spqos::CBitSet **disabled_bitset);

// restore trace flags values based on given bit sets
void ResetTraceflags(spqos::CBitSet *enable_bitset,
					 spqos::CBitSet *disabled_bitset);
