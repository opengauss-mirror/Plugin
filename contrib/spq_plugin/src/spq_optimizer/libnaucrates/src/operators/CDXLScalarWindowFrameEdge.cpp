...
{
	SPQOS_ASSERT(EdxlfbSentinel > frame_boundary);
	...
	const ULONG arity = SPQOS_ARRAY_SIZE(dxl_frame_boundary_token_mapping);
	...
	SPQOS_ASSERT(!"Unrecognized window frame boundary");
	return NULL;
}
									   ...
									   BOOL validate_children) const
{
	...
	SPQOS_ASSERT_IMP((m_dxl_frame_boundary == EdxlfbBoundedPreceding ||
					...
					1 == arity);
	SPQOS_ASSERT_IMP((m_dxl_frame_boundary == EdxlfbUnboundedPreceding ||
