...
//---------------------------------------------------------------------------
CDXLLogicalGroupBy::CDXLLogicalGroupBy(CMemoryPool *mp,
									   ULongPtrArray *pdrspqulGrpColIds)
	: CDXLLogical(mp), m_grouping_colid_array(pdrspqulGrpColIds)
{
	SPQOS_ASSERT(NULL != pdrspqulGrpColIds);
}
...
{
	SPQOS_ASSERT(NULL != grouping_colid_array);
...
CDXLLogicalGroupBy::SerializeGrpColsToDXL(CXMLSerializer *xml_serializer) const
{
	if (NULL != m_grouping_colid_array)
	{
		...
		{
			SPQOS_ASSERT(NULL != (*m_grouping_colid_array)[idx]);
