	  ...
	  m_execute_as_user_id(ulExecuteAsUser)
{
	...
	m_dxl_column_descr_array = SPQOS_NEW(mp) CDXLColDescrArray(mp);
}
...
{
	SPQOS_ASSERT(NULL != m_dxl_column_descr_array);
	SPQOS_ASSERT(NULL != column_descr_dxl);
...
CDXLTableDescr::SerializeToDXL(CXMLSerializer *xml_serializer) const
{
	...
	if (SPQDXL_DEFAULT_USERID != m_execute_as_user_id)
	{
