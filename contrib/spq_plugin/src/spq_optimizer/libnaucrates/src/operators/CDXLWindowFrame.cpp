	  ...
	  m_dxlnode_trailing(dxlnode_trailing)
{
	...
	SPQOS_ASSERT(EdxlfsSentinel > m_dxl_win_frame_spec);
	SPQOS_ASSERT(EdxlfesSentinel > m_dxl_frame_exclusion_strategy);
	SPQOS_ASSERT(NULL != dxlnode_leading);
	SPQOS_ASSERT(NULL != dxlnode_trailing);
}
...
{
	SPQOS_ASSERT(EdxlfesSentinel > edxles);
	...
	SPQOS_ASSERT(!"Unrecognized window frame exclusion strategy");
	return NULL;
}
...
{
	SPQOS_ASSERT(EdxlfsSentinel > edxlfs &&
