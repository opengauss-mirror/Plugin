	  ...
	  m_win_spec_pos(ulWinspecPosition)
{
	...
	SPQOS_ASSERT(EdxlwinstageSentinel != m_dxl_win_stage);
}
...
CDXLScalarWindowRef::GetWindStageStr() const
{
	...
	const ULONG arity = SPQOS_ARRAY_SIZE(win_stage_token_mapping);
	...
	SPQOS_ASSERT(!"Unrecognized window stage");
	return NULL;
}
								 ...
								 BOOL validate_children) const
{
	...
	SPQOS_ASSERT((EdxlwinstageSentinel >= edxlwinrefstage));
