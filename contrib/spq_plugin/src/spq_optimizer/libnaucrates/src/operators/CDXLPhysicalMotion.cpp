...
{
	SPQOS_ASSERT(NULL == m_input_segids_array);
	SPQOS_ASSERT(NULL != input_segids_array);
...
{
	SPQOS_ASSERT(NULL == m_output_segids_array);
	SPQOS_ASSERT(NULL != output_segids_array);
...
{
	SPQOS_ASSERT(NULL == m_output_segids_array && NULL == m_input_segids_array);
	SPQOS_ASSERT(NULL != output_segids_array && NULL != input_segids_array);
...
{
	SPQOS_ASSERT(segment_ids_array != NULL && 0 < segment_ids_array->Size());
	...
	for (ULONG idx = 0; idx < num_of_segments; idx++)
	{
		...
		if (idx == num_of_segments - 1)
		{
			...
			str->AppendFormat(SPQOS_WSZ_LIT("%d"), segment_id);
		}
		else
		{
			str->AppendFormat(
				SPQOS_WSZ_LIT("%d%ls"), segment_id,
	...
	CXMLSerializer *xml_serializer) const
{
	...
	SPQOS_DELETE(input_segment_ids_str);
	SPQOS_DELETE(output_segment_ids_str);
}
