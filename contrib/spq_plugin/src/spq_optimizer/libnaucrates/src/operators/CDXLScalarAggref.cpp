...
CDXLScalarAggref::CDXLScalarAggref(CMemoryPool *mp, IMDId *agg_func_mdid,
								   ...
								   EdxlAggrefKind agg_kind, IMDId *spq_agg_mdid)
	: CDXLScalar(mp),
...
{
	SPQOS_ASSERT(NULL != agg_func_mdid);
	SPQOS_ASSERT_IMP(NULL != resolved_rettype_mdid,
					resolved_rettype_mdid->IsValid());
	SPQOS_ASSERT(m_agg_func_mdid->IsValid());
}
...
CDXLScalarAggref::GetDXLStrAggStage() const
{
	switch (m_agg_stage)
	{
		...
		default:
			SPQOS_ASSERT(!"Unrecognized aggregate stage");
			return NULL;
	}
}
...
CDXLScalarAggref::GetDXLStrAggKind() const
{
	switch (m_agg_kind)
	{
		...
		default:
			SPQOS_ASSERT(!"Unrecognized aggregate kind");
			return NULL;
	}
}
								 ...
								 const CDXLNode *dxlnode) const
{
	...
	if (NULL != m_spq_agg_mdid)
	{
		m_spq_agg_mdid->Serialize(xml_serializer, CDXLTokens::GetDXLTokenStr(
							  ...
							  BOOL validate_children) const
{
	...
	SPQOS_ASSERT((EdxlaggstageFinal >= aggrefstage) &&
