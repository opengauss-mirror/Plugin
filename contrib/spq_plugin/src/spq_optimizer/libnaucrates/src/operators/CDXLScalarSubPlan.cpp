	  ...
	  m_outer_param(outer_param)
{
	...
	SPQOS_ASSERT_IMP(EdxlSubPlanTypeAny == dxl_subplan_type ||
...
CDXLScalarSubPlan::GetSubplanTypeStr() const
{
	switch (m_dxl_subplan_type)
	{
		...
		default:
			SPQOS_ASSERT(!"Unrecognized subplan type");
			return NULL;
	}
}
								  ...
								  const CDXLNode *dxlnode) const
{
	...
	SPQOS_ASSERT(1 == dxlnode->GetChildDXLNodeArray()->Size());
...
{
	SPQOS_ASSERT(EdxlSubPlanIndexSentinel == dxlnode->Arity());
