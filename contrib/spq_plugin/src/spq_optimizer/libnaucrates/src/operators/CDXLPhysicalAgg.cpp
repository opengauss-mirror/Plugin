...
{
	SPQOS_ASSERT_IMP(stream_safe, (EdxlaggstrategyHashed == dxl_agg_strategy));
}
...
CDXLPhysicalAgg::GetAggStrategyNameStr() const
{
	switch (m_dxl_agg_strategy)
	{
		...
		default:
			SPQOS_ASSERT(!"Unrecognized aggregation strategy");
			return NULL;
	}
}
...
{
	SPQOS_ASSERT(NULL != grouping_colids_array);
...
{
	SPQOS_ASSERT(NULL != m_grouping_colids_array);
	...
	{
		SPQOS_ASSERT(NULL != (*m_grouping_colids_array)[idx]);
...
CDXLPhysicalAgg::AssertValid(const CDXLNode *node, BOOL validate_children) const
{
	...
	SPQOS_ASSERT((EdxlaggstrategySentinel > m_dxl_agg_strategy) &&
				(EdxlaggstrategyPlain <= m_dxl_agg_strategy));

	SPQOS_ASSERT(EdxlaggIndexSentinel == node->Arity());
	SPQOS_ASSERT(NULL != m_grouping_colids_array);
