...
CDXLPhysicalAssert::CDXLPhysicalAssert(CMemoryPool *mp, const CHAR *sql_state)
	: CDXLPhysical(mp)
{
	SPQOS_ASSERT(NULL != sql_state);
	SPQOS_ASSERT(SPQOS_SQLSTATE_LENGTH == clib::Strlen(sql_state));
	clib::Strncpy(m_sql_state, sql_state, SPQOS_SQLSTATE_LENGTH);
	m_sql_state[SPQOS_SQLSTATE_LENGTH] = '\0';
}
