...
{
	SPQOS_ASSERT(NULL != m_col_descr_array);
	SPQOS_ASSERT(NULL != m_input_colids_arrays);
	SPQOS_ASSERT(EdxlsetopSentinel > edxlsetoptype);
	...
	for (ULONG idx = 0; idx < length; idx++)
	{
		...
		SPQOS_ASSERT(num_of_cols == input_colids_array->Size());
	}
								 ...
								 const CDXLNode *node) const
{
	...
	SPQOS_DELETE(input_colids_array_str);
...
{
	SPQOS_ASSERT(2 <= node->Arity());
	SPQOS_ASSERT(NULL != m_col_descr_array);

	// validate output columns
	const ULONG num_of_output_cols = m_col_descr_array->Size();
	SPQOS_ASSERT(0 < num_of_output_cols);
