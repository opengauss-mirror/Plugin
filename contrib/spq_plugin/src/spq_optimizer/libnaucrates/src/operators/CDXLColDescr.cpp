...
{
	SPQOS_ASSERT_IMP(m_is_dropped, 0 == m_md_name->GetMDName()->Length());
}
...
//
//	@doc:
//		Returns the column attribute number in SPQDB
//
//---------------------------------------------------------------------------
INT
...
CDXLColDescr::SerializeToDXL(CXMLSerializer *xml_serializer) const
{
	...
	if (spqos::ulong_max != m_column_width)
	{
		xml_serializer->AddAttribute(
