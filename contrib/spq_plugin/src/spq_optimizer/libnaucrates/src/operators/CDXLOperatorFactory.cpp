...
#define SPQDXL_SPQDB_MDID_COMPONENTS 3
#define SPQDXL_DEFAULT_USERID 0
//---------------------------------------------------------------------------
//	@function:
...
)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalTableScan(mp);
}
									 ...
									 const Attributes &attrs)
{
	...
	CMDName *subquery_name = SPQOS_NEW(mp) CMDName(mp, subquery_name_str);

	SPQOS_DELETE(subquery_name_str);

	return SPQOS_NEW(mp) CDXLPhysicalSubqueryScan(mp, subquery_name);
}
...
CDXLOperatorFactory::MakeDXLResult(CDXLMemoryManager *dxl_memory_manager)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalResult(mp);
}
									 ...
									 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalHashJoin(mp, join_type);
}
									  ...
									  const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalMergeJoin(mp, join_type, is_unique_outer);
}
										 ...
										 const Attributes &attrs)
{
	...
	CDXLPhysicalGatherMotion *dxl_op =
		SPQOS_NEW(mp) CDXLPhysicalGatherMotion(mp);
	...
	CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	CDXLPhysicalBroadcastMotion *dxl_op =
		SPQOS_NEW(mp) CDXLPhysicalBroadcastMotion(mp);
	SetSegmentInfo(dxl_memory_manager, dxl_op, attrs,
	...
	CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	CDXLPhysicalRedistributeMotion *dxl_op =
		SPQOS_NEW(mp) CDXLPhysicalRedistributeMotion(mp, is_duplicate_sensitive);
	SetSegmentInfo(dxl_memory_manager, dxl_op, attrs,
										 ...
										 const Attributes &attrs)
{
	...
	CDXLPhysicalRoutedDistributeMotion *dxl_op =
		SPQOS_NEW(mp) CDXLPhysicalRoutedDistributeMotion(mp, segment_colid);
	SetSegmentInfo(dxl_memory_manager, dxl_op, attrs,
				   EdxltokenPhysicalRoutedDistributeMotion);
										 ...
										 const Attributes &attrs)
{
	...
	CDXLPhysicalRandomMotion *dxl_op =
		SPQOS_NEW(mp) CDXLPhysicalRandomMotion(mp, is_duplicate_sensitive);
	SetSegmentInfo(dxl_memory_manager, dxl_op, attrs,
								   ...
								   const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalAppend(mp, is_target, is_zapped);
}
...
)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalLimit(mp);
}
...
)
{
	...
	return SPQOS_NEW(mp) CDXLScalarLimitCount(mp);
}
...
)
{
	...
	return SPQOS_NEW(mp) CDXLScalarLimitOffset(mp);
}
								...
								const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalAgg(mp, dxl_agg_strategy, stream_safe);
}
								 ...
								 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLPhysicalSort(mp, discard_duplicates);
}
										...
										const Attributes &attrs)
{
	...
	if (1 == attrs.getLength())
	{
		...
		materialize_dxlnode =
			SPQOS_NEW(mp) CDXLPhysicalMaterialize(mp, eager_materialize);
	}
	else
	{
		// parse spool id
		...
		materialize_dxlnode = SPQOS_NEW(mp)
									  ...
									  const Attributes &attrs)
{
	...
	CWStringConst *comp_op_name_copy =
		SPQOS_NEW(mp) CWStringConst(mp, comp_op_name->GetBuffer());

	// cleanup
	SPQOS_DELETE(comp_op_name);

	return SPQOS_NEW(mp) CDXLScalarComp(mp, op_id, comp_op_name_copy);
}
										...
										const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarDistinctComp(mp, op_id);
}
								   ...
								   const Attributes &attrs)
{
	...
	CWStringConst *value_copy =
		SPQOS_NEW(mp) CWStringConst(mp, value->GetBuffer());
	SPQOS_DELETE(value);
									  ...
									  const Attributes &attrs)
{
	...
	CWStringConst *opname_copy =
		SPQOS_NEW(mp) CWStringConst(mp, opname->GetBuffer());
	SPQOS_DELETE(opname);
									 ...
									 const EdxlBoolExprType edxlboolexprType)
{
	...
	return SPQOS_NEW(mp) CDXLScalarBoolExpr(mp, edxlboolexprType);
}
	...
	const EdxlBooleanTestType edxlbooleantesttype)
{
	...
	return SPQOS_NEW(mp) CDXLScalarBooleanTest(mp, edxlbooleantesttype);
}
									...
									CDXLNode *dxlnode_test_expr)
{
	...
	return SPQOS_NEW(mp) CDXLScalarSubPlan(mp, mdid, dxl_colref_array,
									 ...
									 const BOOL is_null)
{
	...
	return SPQOS_NEW(mp) CDXLScalarNullTest(mp, is_null);
}
								 ...
								 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarCast(mp, mdid_type, mdid_func);
}
	...
	CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarArrayCoerceExpr(
								   ...
								   const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarIfStmt(mp, mdid_type);
}
									 ...
									 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarFuncExpr(mp, mdid_func, mdid_return_type,
									...
									const Attributes &attrs)
{
	...
	IMDId *spq_agg_oid = ExtractConvertAttrValueToMdId(
	...
	if (NULL != spq_agg_oid)
	{
		return SPQOS_NEW(mp)
			CDXLScalarAggref(mp, agg_mdid, resolved_rettype, is_distinct,
							 agg_stage, agg_kind, spq_agg_oid);
	}

	return SPQOS_NEW(mp) CDXLScalarAggref(mp, agg_mdid, resolved_rettype,
								  ...
								  const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarArray(mp, elem_type_mdid, array_type_mdid,
										...
										const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarIdent(mp, dxl_colref);
}
									 ...
									 const Attributes &attrs)
{
	...
	// create a copy of the string in the CMDName constructor
	CMDName *mdname = SPQOS_NEW(mp) CMDName(mp, alias);

	SPQOS_DELETE(alias);

	return SPQOS_NEW(mp) CDXLScalarProjElem(mp, id, mdname);
}
									 ...
									 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLScalarHashExpr(mp, mdid_type);
}
									...
									const Attributes &attrs)
{
	...
	CWStringConst *sort_op_name_copy =
		SPQOS_NEW(mp) CWStringConst(mp, sort_op_name->GetBuffer());

	SPQOS_DELETE(sort_op_name);

	return SPQOS_NEW(mp) CDXLScalarSortCol(mp, colid, sort_op_id,
										 ...
										 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLOperatorCost(startup_cost_str, total_cost_str,
									   ...
									   const Attributes &attrs)
{
	...
	ULONG user_id = SPQDXL_DEFAULT_USERID;
	...
	return SPQOS_NEW(mp) CDXLTableDescr(mp, mdid, mdname, user_id);
}
									   ...
									   const Attributes &attrs)
{
	...
	// create a copy of the string in the CMDName constructor
	CMDName *mdname = SPQOS_NEW(mp) CMDName(mp, index_name);
	SPQOS_DELETE(index_name);

	return SPQOS_NEW(mp) CDXLIndexDescr(mp, mdid, mdname);
}
									 ...
									 const Attributes &attrs)
{
	...
	if (0 == attno)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLInvalidAttributeValue,
	...
	// create a copy of the string in the CMDName constructor
	CMDName *mdname = SPQOS_NEW(mp) CMDName(mp, col_name);

	SPQOS_DELETE(col_name);

	return SPQOS_NEW(mp) CDXLColDescr(mp, mdname, id, attno, mdid_type,
								   ...
								   Edxltoken target_elem)
{
	...
	if (NULL == colid_xml)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLMissingAttribute,
	...
	return SPQOS_NEW(mp) CDXLColRef(mp, mdname, id, mdid_type, type_modifier);
}
									  ...
									  const Attributes &attrs)
{
	...
	{
		// turn Xerces exception into SPQOS exception
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLInvalidAttributeValue,
...
{
	SPQOS_ASSERT(attribute_val_xml != NULL);
	ULONG attr = 0;
	try
	{
	...
	Edxltoken target_attr, Edxltoken target_elem)
{
	...
	SPQOS_ASSERT(NULL != attr);

	CHAR **end = NULL;
	LINT converted_val = clib::Strtoll(attr, end, 10 /*ulBase*/);

	if ((NULL != end && attr == *end) || spqos::lint_max == converted_val ||
		spqos::lint_min == converted_val || 0 > converted_val)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLInvalidAttributeValue,
				   CDXLTokens::GetDXLTokenStr(target_attr)->GetBuffer(),
				   CDXLTokens::GetDXLTokenStr(target_elem)->GetBuffer());
									 ...
									 Edxltoken target_elem)
{
	...
	switch (typ)
	{
		...
		case IMDId::EmdidCheckConstraint:
			mdid = GetSPQDBMdId(dxl_memory_manager, remaining_tokens,
							   target_attr, target_elem, typ);
			break;

		case IMDId::EmdidSPQDBCtas:
			mdid = GetSPQDBCTASMdId(dxl_memory_manager, remaining_tokens,
								   target_attr, target_elem);
			break;
		...
		default:
			SPQOS_ASSERT(!"Unrecognized mdid type");
	}
...
//---------------------------------------------------------------------------
//	@function:
//		CDXLOperatorFactory::GetSPQDBMdId
//
//	@doc:
//		Construct a SPQDB mdid from an array of XML string components.
//
//---------------------------------------------------------------------------
CMDIdSPQDB *
CDXLOperatorFactory::GetSPQDBMdId(CDXLMemoryManager *dxl_memory_manager,
								 XMLChArray *remaining_tokens,
								 Edxltoken target_attr, Edxltoken target_elem,
								 IMDId::EMDIdType mdidType)
{
	SPQOS_ASSERT(SPQDXL_SPQDB_MDID_COMPONENTS <= remaining_tokens->Size());
	...
	return SPQOS_NEW(dxl_memory_manager->Pmp())
		CMDIdSPQDB(mdidType, oid_colid, version_major, version_minor);
}

//---------------------------------------------------------------------------
//	@function:
//		CDXLOperatorFactory::GetSPQDBCTASMdId
//
//	@doc:
//		Construct a SPQDB CTAS mdid from an array of XML string components.
//
//---------------------------------------------------------------------------
CMDIdSPQDB *
CDXLOperatorFactory::GetSPQDBCTASMdId(CDXLMemoryManager *dxl_memory_manager,
									 XMLChArray *remaining_tokens,
									 Edxltoken target_attr,
									 Edxltoken target_elem)
{
	...
	// construct metadata id object
	return SPQOS_NEW(dxl_memory_manager->Pmp()) CMDIdSPQDBCtas(oid_colid);
}
...
CDXLOperatorFactory::GetColStatsMdId(CDXLMemoryManager *dxl_memory_manager,
									 ...
									 Edxltoken target_elem)
{
	SPQOS_ASSERT(SPQDXL_SPQDB_MDID_COMPONENTS + 1 == remaining_tokens->Size());

	CMDIdSPQDB *rel_mdid =
		GetSPQDBMdId(dxl_memory_manager, remaining_tokens, target_attr,
	...
	// construct metadata id object
	return SPQOS_NEW(dxl_memory_manager->Pmp()) CMDIdColStats(rel_mdid, attno);
}
									 ...
									 Edxltoken target_elem)
{
	...
	// construct metadata id object
	return SPQOS_NEW(dxl_memory_manager->Pmp()) CMDIdRelStats(rel_mdid);
}
...
CDXLOperatorFactory::GetCastFuncMdId(CDXLMemoryManager *dxl_memory_manager,
									 ...
									 Edxltoken target_elem)
{
	SPQOS_ASSERT(2 * SPQDXL_SPQDB_MDID_COMPONENTS == remaining_tokens->Size());

	CMDIdSPQDB *mdid_src = GetSPQDBMdId(dxl_memory_manager, remaining_tokens,
									  target_attr, target_elem);
	XMLChArray *dest_xml = SPQOS_NEW(dxl_memory_manager->Pmp())
		XMLChArray(dxl_memory_manager->Pmp());

	for (ULONG ul = SPQDXL_SPQDB_MDID_COMPONENTS;
		 ul < SPQDXL_SPQDB_MDID_COMPONENTS * 2; ul++)
	{
		dest_xml->Append((*remaining_tokens)[ul]);
	}

	CMDIdSPQDB *mdid_dest =
		GetSPQDBMdId(dxl_memory_manager, dest_xml, target_attr, target_elem);
	dest_xml->Release();

	return SPQOS_NEW(dxl_memory_manager->Pmp()) CMDIdCast(mdid_src, mdid_dest);
}
...
CDXLOperatorFactory::GetScCmpMdId(CDXLMemoryManager *dxl_memory_manager,
								  ...
								  Edxltoken target_attr, Edxltoken target_elem)
{
	SPQOS_ASSERT(2 * SPQDXL_SPQDB_MDID_COMPONENTS + 1 == remaining_tokens->Size());

	CMDIdSPQDB *left_mdid = GetSPQDBMdId(dxl_memory_manager, remaining_tokens,
									   target_attr, target_elem);
	XMLChArray *right_xml = SPQOS_NEW(dxl_memory_manager->Pmp())
		XMLChArray(dxl_memory_manager->Pmp());

	for (ULONG ul = SPQDXL_SPQDB_MDID_COMPONENTS;
		 ul < SPQDXL_SPQDB_MDID_COMPONENTS * 2 + 1; ul++)
	{
		right_xml->Append((*remaining_tokens)[ul]);
	}

	CMDIdSPQDB *right_mdid =
		GetSPQDBMdId(dxl_memory_manager, right_xml, target_attr, target_elem);
								 ...
								 const Attributes &attrs, Edxltoken target_elem)
{
	...
	CMDIdSPQDB *spqdb_mdid = CMDIdSPQDB::CastMdid(mdid);
	...
	SDXLDatumFactoryElem translators_mapping[] = {
		// native support
		{CMDIdSPQDB::m_mdid_int2.Oid(), &CDXLOperatorFactory::GetDatumInt2},
		{CMDIdSPQDB::m_mdid_int4.Oid(), &CDXLOperatorFactory::GetDatumInt4},
		{CMDIdSPQDB::m_mdid_int8.Oid(), &CDXLOperatorFactory::GetDatumInt8},
		{CMDIdSPQDB::m_mdid_bool.Oid(), &CDXLOperatorFactory::GetDatumBool},
		{CMDIdSPQDB::m_mdid_oid.Oid(), &CDXLOperatorFactory::GetDatumOid},
		// types with long int mapping
		{CMDIdSPQDB::m_mdid_bpchar.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		{CMDIdSPQDB::m_mdid_varchar.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		{CMDIdSPQDB::m_mdid_text.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		{CMDIdSPQDB::m_mdid_cash.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		{CMDIdSPQDB::m_mdid_uuid.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		// non-integer numeric types
		{CMDIdSPQDB::m_mdid_numeric.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_float4.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_float8.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		// network-related types
		{CMDIdSPQDB::m_mdid_inet.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_cidr.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_macaddr.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		// time-related types
		{CMDIdSPQDB::m_mdid_date.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsLintMappable},
		{CMDIdSPQDB::m_mdid_time.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_timeTz.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_timestamp.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_timestampTz.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_abs_time.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_relative_time.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_interval.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable},
		{CMDIdSPQDB::m_mdid_time_interval.Oid(),
		 &CDXLOperatorFactory::GetDatumStatsDoubleMappable}};
								 ...
								 IMDId *mdid, BOOL is_const_null)
{
	...
	return SPQOS_NEW(mp) CDXLDatumOid(mp, mdid, is_const_null, val);
}
								  ...
								  BOOL is_const_null)
{
	...
	return SPQOS_NEW(mp) CDXLDatumInt2(mp, mdid, is_const_null, val);
}
								  ...
								  BOOL is_const_null)
{
	...
	return SPQOS_NEW(mp) CDXLDatumInt4(mp, mdid, is_const_null, val);
}
								  ...
								  BOOL is_const_null)
{
	...
	return SPQOS_NEW(mp) CDXLDatumInt8(mp, mdid, is_const_null, val);
}
								  ...
								  BOOL is_const_null)
{
	...
	return SPQOS_NEW(mp) CDXLDatumBool(mp, mdid, is_const_null, value);
}
	...
	Edxltoken target_attr, Edxltoken target_elem)
{
	...
	IMdIdArray *mdid_array = SPQOS_NEW(mp) IMdIdArray(mp);
	...
	Edxltoken target_attr, Edxltoken target_elem)
{
	...
	ULongPtr2dArray *array_2D = SPQOS_NEW(mp) ULongPtr2dArray(mp);
	...
	Edxltoken target_attr, Edxltoken target_elem)
{
	...
	SPQOS_ASSERT(NULL != seg_id_list_xml);

	IntPtrArray *seg_ids = SPQOS_NEW(mp) IntPtrArray(mp);

	XMLStringTokenizer mdid_components(seg_id_list_xml,
									   CDXLTokens::XmlstrToken(EdxltokenComma));

	const ULONG num_of_segments = mdid_components.countTokens();
	SPQOS_ASSERT(0 < num_of_segments);

	for (ULONG ul = 0; ul < num_of_segments; ul++)
	{
		XMLCh *seg_id_xml = mdid_components.nextToken();

		SPQOS_ASSERT(NULL != seg_id_xml);

		INT *seg_id = SPQOS_NEW(mp) INT(ConvertAttrValueToInt(
	...
	CDXLMemoryManager *dxl_memory_manager, const XMLCh *xml_val)
{
	...
	StringPtrArray *array_strs = SPQOS_NEW(mp) StringPtrArray(mp);
									 ...
									 const Attributes &attrs)
{
	...
	return SPQOS_NEW(mp) CDXLLogicalJoin(mp, join_type);
}
...
{
	SPQOS_ASSERT(NULL != attribute_val_xml);
	CHAR *sz = XMLString::transcode(attribute_val_xml, dxl_memory_manager);
	CHAR *szEnd = NULL;
						   ...
						   Edxltoken target_elem)
{
	...
	SPQOS_ASSERT(2 == sys_id_components.countTokens());
								   ...
								   const Attributes &attrs)
{
	...
	SPQOS_ASSERT(EdxlwinstageSentinel != dxl_win_stage);

	return SPQOS_NEW(mp) CDXLScalarWindowRef(
...
CDXLOperatorFactory::ParseCmpType(const XMLCh *xml_str_comp_type)
{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(parse_cmp_type_mapping); ul++)
	{
	...
	SPQOS_RAISE(
		spqdxl::ExmaDXL, spqdxl::ExmiDXLInvalidAttributeValue,
		CDXLTokens::GetDXLTokenStr(EdxltokenSPQDBScalarOpCmpType)->GetBuffer(),
		CDXLTokens::GetDXLTokenStr(EdxltokenSPQDBScalarOp)->GetBuffer());
...
CDXLOperatorFactory::ParseRelationStorageType(const XMLCh *xml_val)
{
	...
	SPQOS_ASSERT(!"Unrecognized storage type");
...
CDXLOperatorFactory::ParseOnCommitActionSpec(const Attributes &attrs)
{
	...
	{
		SPQOS_RAISE(
			spqdxl::ExmaDXL, spqdxl::ExmiDXLMissingAttribute,
    ...
    )
{
    ...
    return SPQOS_NEW(mp) CDXLPhysicalTableShareScan(mp);
}
