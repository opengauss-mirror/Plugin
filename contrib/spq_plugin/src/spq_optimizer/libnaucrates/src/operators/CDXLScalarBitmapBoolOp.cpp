...
{
	SPQOS_ASSERT(EdxlbitmapSentinel > bitmap_op_type);
	SPQOS_ASSERT(IMDId::IsValid(mdid_type));
}
									   ...
									   const CDXLNode *dxlnode) const
{
	...
	SPQOS_ASSERT(NULL != element_name);
									...
									BOOL validate_children) const
{
	...
	SPQOS_ASSERT((bitmap_bool_dxlop == EdxlbitmapAnd) ||
	...
	for (ULONG ul = 0; ul < arity; ++ul)
	{
		...
		SPQOS_ASSERT(EdxlopScalarBitmapBoolOp == dxl_operator ||
