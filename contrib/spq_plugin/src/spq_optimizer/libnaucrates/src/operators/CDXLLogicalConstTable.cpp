...
{
	SPQOS_ASSERT(NULL != col_descr_array);
	SPQOS_ASSERT(NULL != const_tuples_datum_array);

#ifdef SPQOS_DEBUG
	const ULONG length = const_tuples_datum_array->Size();
	for (ULONG idx = 0; idx < length; idx++)
	{
		CDXLDatumArray *pdrspqdxldatum = (*const_tuples_datum_array)[idx];
		SPQOS_ASSERT(pdrspqdxldatum->Size() == col_descr_array->Size());
	}
#endif
}
...
{
	SPQOS_ASSERT(m_col_descr_array->Size() > idx);
...
) const
{
	...
	for (ULONG tuple_idx = 0; tuple_idx < num_of_tuples; tuple_idx++)
	{
		...
		CDXLDatumArray *pdrspqdxldatum =
			(*m_const_tuples_datum_array)[tuple_idx];

		const ULONG num_of_cols = pdrspqdxldatum->Size();
		for (ULONG idx = 0; idx < num_of_cols; idx++)
		{
			CDXLDatum *dxl_datum = (*pdrspqdxldatum)[idx];
...
) const
{
	...
	SPQOS_ASSERT(m_col_descr_array != NULL);
	SPQOS_ASSERT(0 < m_col_descr_array->Size());
