									...
									const Attributes &attrs)
{
								 ...
								 CDXLTokens::XmlstrToken(EdxltokenMetadata)))
	{
		...
		SPQOS_ASSERT(NULL == m_mdid_cached_obj_array);

		m_mdid_cached_obj_array = SPQOS_NEW(m_mp) IMDCacheObjectArray(m_mp);
		m_mdid_array = SPQOS_NEW(m_mp) IMdIdArray(m_mp);
	...
	{
		// start of the metadata section in the DXL document
		SPQOS_ASSERT(NULL != m_mdid_array);
...
)
{
	...
	for (ULONG idx = 0; idx < size; idx++)
	{
		...
		SPQOS_ASSERT(NULL != metadata_obj_parse_handler->GetImdObj());
										...
										Edxltoken dxl_token_element)
{
	...
	CSystemIdArray *src_sys_id_array = SPQOS_NEW(m_mp) CSystemIdArray(m_mp);
	...
	while (NULL != (xml_str_sys_id = xml_str_tokenizer.nextToken()))
	{
		...
		SPQOS_ASSERT(2 == xml_str_sys_id_tokenizer.countTokens());
		...
		src_sys_id_array->Append(SPQOS_NEW(m_mp) CSystemId(
