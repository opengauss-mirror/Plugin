								  ...
								  const Attributes &attrs)
{
	...
	if (NULL == error_code || SPQOS_SQLSTATE_LENGTH != clib::Strlen(error_code))
	{
		SPQOS_RAISE(
	...
	m_dxl_op = SPQOS_NEW(m_mp) CDXLPhysicalAssert(m_mp, error_code);

	// ctor created a copy of the error code
	SPQOS_DELETE_ARRAY(error_code);
