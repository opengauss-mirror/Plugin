	  ...
	  m_mdid_array_type(NULL)
{
	...
	m_mdid_min_op = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, 0);
	m_mdid_max_op = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, 0);
	m_mdid_avg_op = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, 0);
	m_mdid_sum_op = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, 0);
	m_mdid_count_op = SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, 0);
}
								  ...
								  const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		if (!IsBuiltInType(m_mdid))
		{
			...
			m_mdname = SPQOS_NEW(m_mp) CMDName(m_mp, pstrTypeName);
			SPQOS_DELETE(pstrTypeName);
							   ...
							   const Attributes &attrs)
{
	...
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(rgmdidmap); ul++)
	{
	...
	SPQOS_ASSERT(EdxltokenSentinel != token_type);
	SPQOS_ASSERT(NULL != token_mdid);
...
CParseHandlerMDType::IsBuiltInType(const IMDId *mdid) const
{
	...
	const CMDIdSPQDB *mdidSPQDB = CMDIdSPQDB::CastMdid(mdid);

	switch (mdidSPQDB->Oid())
	{
		case SPQDB_INT2:
		case SPQDB_INT4:
		case SPQDB_INT8:
...
CParseHandlerMDType::GetTokenMDid(Edxltoken token_type)
{
	...
	SPQOS_ASSERT(!"Unexpected DXL token when parsing MDType");
	return NULL;
}
...
)
{
									  ...
									  element_local_name))
	{
		...
		const CMDIdSPQDB *pmdidSPQDB = CMDIdSPQDB::CastMdid(m_mdid);

		switch (pmdidSPQDB->Oid())
		{
			case SPQDB_INT2:
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeInt2SPQDB(m_mp);
				break;

			case SPQDB_INT4:
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeInt4SPQDB(m_mp);
				break;

			case SPQDB_INT8:
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeInt8SPQDB(m_mp);
				break;

			case SPQDB_BOOL:
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeBoolSPQDB(m_mp);
				break;

			case SPQDB_OID:
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeOidSPQDB(m_mp);
				break;
			...
			default:
				...
				m_imd_obj = SPQOS_NEW(m_mp) CMDTypeGenericSPQDB(
