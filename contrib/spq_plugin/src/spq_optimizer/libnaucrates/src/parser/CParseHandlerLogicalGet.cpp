									...
									Edxltoken token_type)
{
	...
	if (EdxltokenLogicalGet == token_type)
	{
		m_dxl_node = SPQOS_NEW(m_mp)
			CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalGet(m_mp, table_descr));
	}
	else
	{
		SPQOS_ASSERT(EdxltokenLogicalExternalGet == token_type);
		m_dxl_node = SPQOS_NEW(m_mp) CDXLNode(
			m_mp, SPQOS_NEW(m_mp) CDXLLogicalExternalGet(m_mp, table_descr));
	}
