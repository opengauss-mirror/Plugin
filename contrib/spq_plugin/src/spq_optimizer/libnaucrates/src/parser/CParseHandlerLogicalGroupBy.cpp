...
)
{
								 ...
								 element_local_name))
	{
		m_dxl_node = SPQOS_NEW(m_mp)
			CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalGroupBy(m_mp));
...
)
{
	...
	SPQOS_ASSERT(NULL != m_dxl_node);
	SPQOS_ASSERT(3 == this->Length());
	...
	SPQOS_ASSERT(NULL != lg_op_parse_handler->CreateDXLNode());
	...
	SPQOS_ASSERT(NULL != grouping_col_parse_handler->GetGroupingColidArray());
