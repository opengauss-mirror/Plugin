										  ...
										  const Attributes &attrs)
{
	...
	{
		SPQOS_ASSERT(NULL != m_dxl_node);
		SPQOS_ASSERT(2 > m_parse_index_lists);
					  ...
					  element_local_name))
	{
		SPQOS_ASSERT(NULL != m_dxl_node);
		SPQOS_ASSERT(2 == m_parse_index_lists);
		SPQOS_ASSERT(!m_parsing_ref_expr);
		SPQOS_ASSERT(!m_parsing_assign_expr);
	...
	{
		// parse scalar child
		SPQOS_ASSERT(m_parsing_ref_expr || m_parsing_assign_expr);
