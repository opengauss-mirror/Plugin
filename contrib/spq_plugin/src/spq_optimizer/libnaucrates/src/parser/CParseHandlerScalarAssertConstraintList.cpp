	...
	const Attributes &attrs)
{
				 ...
				 element_local_name))
	{
		...
		SPQOS_ASSERT(NULL == m_dxl_op_assert_constraint);
		SPQOS_ASSERT(NULL == m_dxlnode_assert_constraints_parsed_array);

		m_dxl_op = SPQOS_NEW(m_mp) CDXLScalarAssertConstraintList(m_mp);
					  ...
					  element_local_name))
	{
		...
		m_dxl_op_assert_constraint =
			SPQOS_NEW(m_mp) CDXLScalarAssertConstraint(m_mp, pstrErrorMsg);
...
)
{
					  ...
					  element_local_name))
	{
		...
		CDXLNode *pdxlnAssertConstraint = SPQOS_NEW(m_mp)
