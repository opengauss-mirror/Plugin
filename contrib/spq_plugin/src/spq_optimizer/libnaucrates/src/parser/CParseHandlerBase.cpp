	  ...
	  m_parse_handler_root(parse_handler_root)
{
	...
	SPQOS_ASSERT(NULL != parse_handler_mgr);

	m_parse_handler_base_array = SPQOS_NEW(m_mp) CParseHandlerBaseArray(m_mp);
}
	...
	CParseHandlerBase *parse_handler_base_new)
{
	...
	SPQOS_ASSERT(NULL != m_parse_handler_base_array);
	...
	SPQOS_ASSERT(idx < m_parse_handler_base_array->Size());
...
CParseHandlerBase::error(const SAXParseException &to_catch)
{
	...
	SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLValidationError, message);
}
