	...
	const Attributes &attrs)
{
				 ...
				 element_local_name))
	{
		...
		SPQOS_ASSERT(0 == this->Length());
		SPQOS_ASSERT(NULL == m_const_tuples_datum_array);

		// initialize the array of const tuples (datum arrays)
		m_const_tuples_datum_array = SPQOS_NEW(m_mp) CDXLDatum2dArray(m_mp);
					  ...
					  element_local_name))
	{
		...
		SPQOS_ASSERT(NULL == m_dxl_datum_array);

		// initialize the array of datums (const tuple)
		m_dxl_datum_array = SPQOS_NEW(m_mp) CDXLDatumArray(m_mp);
	}
	else if (0 ==
...
)
{
				 ...
				 element_local_name))
	{
		...
		CDXLLogicalConstTable *lg_const_table_get_dxl_op =
			SPQOS_NEW(m_mp) CDXLLogicalConstTable(m_mp, dxl_col_descr_array,
												 m_const_tuples_datum_array);
		m_dxl_node = SPQOS_NEW(m_mp) CDXLNode(m_mp, lg_const_table_get_dxl_op);
