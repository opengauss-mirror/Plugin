										 ...
										 ULONG part_idx_id_printable)
{
	...
	{
		dxl_op = SPQOS_NEW(m_mp) CDXLPhysicalIndexScan(
			m_mp, dxl_table_descr, dxl_index_descr, m_index_scan_dir);
		m_dxl_node = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
	}
	else
	{
		SPQOS_ASSERT(EdxltokenPhysicalDynamicIndexScan == token_type);

		dxl_op = SPQOS_NEW(m_mp) CDXLPhysicalDynamicIndexScan(
