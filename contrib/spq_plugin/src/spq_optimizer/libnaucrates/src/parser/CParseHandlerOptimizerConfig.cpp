...
)
{
	...
	CParseHandlerWindowOids *pphDefoidsSPQDB =
		dynamic_cast<CParseHandlerWindowOids *>((*this)[3]);
	CWindowOids *pwindowoidsSPQDB = pphDefoidsSPQDB->GetWindowOids();
	SPQOS_ASSERT(NULL != pwindowoidsSPQDB);
	pwindowoidsSPQDB->AddRef();
	...
	m_optimizer_config = SPQOS_NEW(m_mp) COptimizerConfig(
		pec, stats_config, pcteconfig, pcm, phint, pwindowoidsSPQDB);
