									  ...
									  const Attributes &attrs)
{
								 ...
								 element_local_name))
	{
		...
		SPQOS_ASSERT(NULL == m_dxl_window_spec_gen);
		...
		if (NULL != xml_alias)
		{
			...
			m_mdname = SPQOS_NEW(m_mp) CMDName(m_mp, str_alias);
			SPQOS_DELETE(str_alias);
		}
		...
		SPQOS_ASSERT(NULL != m_part_by_colid_array);
	}
...
)
{
	...
	if (1 == this->Length())
	{
		...
		{
			// In SPQDB 5 and before, window specification cannot have a window frame specification
			// without sort columns. This changed in SPQDB 6/Postgres 8.4+ where a query
	...
	m_dxl_window_spec_gen =
		SPQOS_NEW(m_mp) CDXLWindowSpec(m_mp, m_part_by_colid_array, m_mdname,
