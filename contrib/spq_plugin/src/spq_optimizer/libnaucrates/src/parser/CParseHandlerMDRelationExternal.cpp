...
#define SPQDXL_DEFAULT_REJLIMIT -1
...
CParseHandlerMDRelationExternal::CParseHandlerMDRelationExternal(
	...
	: CParseHandlerMDRelation(mp, parse_handler_mgr, parse_handler_root),
	  m_reject_limit(SPQDXL_DEFAULT_REJLIMIT),
...
)
{
				 ...
				 element_local_name))
	{
		...
		//			m_part_constraint = SPQOS_NEW(m_mp) CMDPartConstraintSPQDB(m_mp, m_level_with_default_part_array, m_part_constraint_unbounded, NULL);
		//		}
		return;
	}
	...
	SPQOS_ASSERT(NULL != mdid_check_constraint_parse_handler->GetMdIdArray());
	...
	m_imd_obj = SPQOS_NEW(m_mp) CMDRelationExternalSPQDB(
