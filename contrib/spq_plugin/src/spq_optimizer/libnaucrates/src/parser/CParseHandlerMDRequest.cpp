									 ...
									 const Attributes &attrs)
{
								 ...
								 element_local_name))
	{
		...
		m_mdtype_request_array =
			SPQOS_NEW(m_mp) CMDRequest::SMDTypeRequestArray(m_mp);
	...
	SPQOS_ASSERT(0 == XMLString::compareString(
						 CDXLTokens::XmlstrToken(EdxltokenMDTypeRequest),
						 element_local_name));
	SPQOS_ASSERT(NULL != m_mdtype_request_array);
				 ...
				 element_local_name))
	{
		...
		m_mdtype_request_array->Append(
			SPQOS_NEW(m_mp) CMDRequest::SMDTypeRequest(sysid, type_info));
	}
}
