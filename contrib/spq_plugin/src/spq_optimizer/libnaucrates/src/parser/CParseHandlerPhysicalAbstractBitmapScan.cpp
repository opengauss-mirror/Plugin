	...
	ULONG part_idx_id, ULONG part_idx_id_printable)
{
	...
	{
		dxl_op = SPQOS_NEW(m_mp) CDXLPhysicalBitmapTableScan(m_mp, table_descr);
	}
	else
	{
		SPQOS_ASSERT(EdxltokenPhysicalDynamicBitmapTableScan == token_type);
		dxl_op = SPQOS_NEW(m_mp) CDXLPhysicalDynamicBitmapTableScan(
