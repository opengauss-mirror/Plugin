...
)
{
				 ...
				 element_local_name))
	{
		if (m_part_constraint_contains_expr)
		{
			...
			m_part_constraint = SPQOS_NEW(m_mp) CMDPartConstraintSPQDB(
		...
		else
		{
			...
			m_part_constraint = SPQOS_NEW(m_mp)
				CMDPartConstraintSPQDB(m_mp, m_level_with_default_part_array,
	...
	SPQOS_ASSERT(NULL != md_cols_parse_handler->GetMdColArray());
	SPQOS_ASSERT(NULL != pphMdlIndexInfo->GetMdIndexInfoArray());
	SPQOS_ASSERT(NULL != pphMdidlCheckConstraints->GetMdIdArray());
	...
	m_imd_obj = SPQOS_NEW(m_mp) CMDRelationSPQDB(
												 ...
												 Edxltoken dxl_token_element)
{
	...
	m_mdname = SPQOS_NEW(m_mp) CMDName(m_mp, str_table_name);
	SPQOS_DELETE(str_table_name);
	...
	{
		// construct an empty keyset
		m_key_sets_arrays = SPQOS_NEW(m_mp) ULongPtr2dArray(m_mp);
	}
