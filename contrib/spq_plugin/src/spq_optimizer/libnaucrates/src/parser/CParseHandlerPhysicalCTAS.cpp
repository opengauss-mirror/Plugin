...
)
{
	...
	SPQOS_ASSERT(NULL != proj_list_parse_handler->CreateDXLNode());
	CParseHandlerPhysicalOp *child_parse_handler =
		dynamic_cast<CParseHandlerPhysicalOp *>((*this)[5]);

	SPQOS_ASSERT(NULL != prop_parse_handler->GetProperties());
	SPQOS_ASSERT(NULL != opclasses_parse_handler->GetMdIdArray());
	SPQOS_ASSERT(NULL != col_descr_parse_handler->GetDXLColumnDescrArray());
	SPQOS_ASSERT(NULL != ctas_options_parse_handler->GetDxlCtasStorageOption());
	SPQOS_ASSERT(NULL != proj_list_parse_handler->CreateDXLNode());
	SPQOS_ASSERT(NULL != child_parse_handler->CreateDXLNode());
	...
	m_dxl_node = SPQOS_NEW(m_mp) CDXLNode(
		m_mp, SPQOS_NEW(m_mp) CDXLPhysicalCTAS(
