...
{
	SPQOS_ASSERT(NULL != m_colid_histogram_mapping);
	SPQOS_ASSERT(NULL != m_colid_width_mapping);
	SPQOS_ASSERT(CDouble(0.0) <= m_rows);

	// hash map for source id -> max source cardinality mapping
	m_src_upper_bound_NDVs = SPQOS_NEW(mp) CUpperBoundNDVPtrArray(mp);
										   ...
										   ULongPtrArray *colids, CDouble rows)
{
	...
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
	CColRefSet *colrefs = SPQOS_NEW(mp) CColRefSet(mp);
	...
	if (0 < colrefs->Size())
	{
		stats->AddCardUpperBound(SPQOS_NEW(mp) CUpperBoundNDVs(colrefs, rows));
	}
	else
	{
		colrefs->Release();
...
{
	SPQOS_ASSERT(NULL != col_histogram_mapping);
	SPQOS_ASSERT(NULL != col_width_mapping);
	...
	for (ULONG ul = 0; ul < num_col_hist; ul++)
	{
		...
		result_col_histogram_mapping->Insert(SPQOS_NEW(mp) ULONG(colid),
											 histogram);
	}
...
{
	SPQOS_ASSERT(NULL != outer_stats);
	SPQOS_ASSERT(NULL != inner_side_stats);
...
{
	SPQOS_ASSERT(NULL != excluded_cols);
	SPQOS_ASSERT(NULL != col_histogram_mapping);
...
{
	UlongToDoubleMap *widths_copy = SPQOS_NEW(mp) UlongToDoubleMap(mp);
...
CStatistics::CopyHistograms(CMemoryPool *mp) const
{
	...
	while (col_hist_mapping.Advance())
	{
		...
		histograms_copy->Insert(SPQOS_NEW(mp) ULONG(colid), histogram_copy);
	}
...
CStatistics::GetReqdRelationalProps(CMemoryPool *mp) const
{
	...
	return SPQOS_NEW(mp) CReqdPropRelational(colrefs);
}
...
{
	UlongToHistogramMap *histograms_new = SPQOS_NEW(mp) UlongToHistogramMap(mp);
	UlongToDoubleMap *widths_new = SPQOS_NEW(mp) UlongToDoubleMap(mp);
	...
	CStatistics *scaled_stats = SPQOS_NEW(mp) CStatistics(
								...
								BOOL must_exist) const
{
	...
	CStatistics *stats_copy = SPQOS_NEW(mp) CStatistics(
...
)
{
	...
	while (colref_iterator.Advance())
	{
		...
		SPQOS_ASSERT_IMP(must_exist, NULL != dest_colref);
...
{
	SPQOS_ASSERT(NULL != upper_bound_NDVs);
									 ...
									 CMDAccessor *md_accessor) const
{
	CDXLStatsDerivedColumnArray *dxl_stats_derived_col_array =
		SPQOS_NEW(mp) CDXLStatsDerivedColumnArray(mp);
	...
	return SPQOS_NEW(mp) CDXLStatsDerivedRelation(m_rows, IsEmpty(),
