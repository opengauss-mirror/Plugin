...
#define SPQOPT_SKEW_SAMPLE_SIZE 1000
...
CHistogram::CHistogram(CMemoryPool *mp, BOOL is_well_defined)
	: m_mp(mp),
	  ...
	  m_is_col_stats_missing(false)
{
	m_histogram_buckets = SPQOS_NEW(m_mp) CBucketArray(m_mp);
}
...
{
	SPQOS_ASSERT(m_histogram_buckets);
	SPQOS_ASSERT(CDouble(0.0) <= null_freq);
	SPQOS_ASSERT(CDouble(1.0) >= null_freq);
	SPQOS_ASSERT(CDouble(0.0) <= distinct_remaining);
	SPQOS_ASSERT(CDouble(0.0) <= freq_remaining);
	SPQOS_ASSERT(CDouble(1.0) >= freq_remaining);
	// if distinct_remaining is 0, freq_remaining must be 0 too
	SPQOS_ASSERT_IMP(distinct_remaining < CStatistics::Epsilon,
...
{
	SPQOS_ASSERT(CDouble(0.0) <= null_freq && CDouble(1.0) >= null_freq);
	m_null_freq = null_freq;
}

FORCE_GENERATE_DBGSTR(spqnaucrates::CHistogram);
...
{
	SPQOS_ASSERT(CStatsPred::EstatscmptL == stats_cmp_type ||
				CStatsPred::EstatscmptLEq == stats_cmp_type);

	CBucketArray *new_buckets = SPQOS_NEW(m_mp) CBucketArray(m_mp);
	...
	for (ULONG bucket_index = 0; bucket_index < num_buckets; bucket_index++)
	{
		...
		{
			SPQOS_ASSERT(bucket->Contains(point));
	...
	return SPQOS_NEW(m_mp) CHistogram(m_mp, new_buckets,
...
CHistogram::MakeBucketsWithEqualityFilter(CPoint *point) const
{
	...
	CBucketArray *histogram_buckets = SPQOS_NEW(m_mp) CBucketArray(m_mp);
...
CHistogram::MakeHistogramEqualFilter(CPoint *point) const
{
	...
	if (point->GetDatum()->IsNull())
	{
		return SPQOS_NEW(m_mp) CHistogram(
			m_mp, SPQOS_NEW(m_mp) CBucketArray(m_mp), true /* is_well_defined */,
	...
	return SPQOS_NEW(m_mp) CHistogram(m_mp, histogram_buckets);
}
...
CHistogram::MakeHistogramGreaterThanOrGreaterThanEqualFilter(
	CStatsPred::EStatsCmpType stats_cmp_type, CPoint *point) const
{
	SPQOS_ASSERT(CStatsPred::EstatscmptGEq == stats_cmp_type ||
...
{
	CBucketArray *histogram_buckets =
		SPQOS_NEW(mp) CBucketArray(mp, buckets->Size());
	...
	CDouble *scale_factor) const
{
	...
	if (!IsWellDefined())
	{
		CHistogram *result_histogram =
			SPQOS_NEW(m_mp) CHistogram(m_mp, false /* is_well_defined */);
	...
	*scale_factor = result_histogram->NormalizeHistogram();
	SPQOS_ASSERT(result_histogram->IsValid());
...
CHistogram::MakeJoinHistogramNormalize(CStatsPred::EStatsCmpType stats_cmp_type,
									   ...
									   CDouble *scale_factor) const
{
	SPQOS_ASSERT(NULL != other_histogram);
										 ...
										 CDouble rows_other) const
{
	...
	SPQOS_DELETE(equijoin_histogram);

	CDouble cartesian_product_num_rows = rows * rows_other;

	SPQOS_ASSERT(CDouble(1.0) <= scale_factor);
								...
								CPoint *point) const
{
	...
	SPQOS_ASSERT(IsOpSupportedForFilter(stats_cmp_type));
	...
	switch (stats_cmp_type)
	{
		...
		default:
		{
			SPQOS_RTL_ASSERT_MSG(false, "Not supported. Must not be called.");
			break;
		}
	}
	return result_histogram;
...
{
	SPQOS_ASSERT(JoinPredCmpTypeIsSupported(stats_cmp_type));
	...
	return SPQOS_NEW(m_mp) CHistogram(m_mp, false /* is_well_defined */);
}
							  ...
							  const CHistogram *histogram) const
{
	...
	while (idx1 < buckets1 && idx2 < buckets2)
	{
		...
		SPQOS_ASSERT(NULL == candidate_bucket);
...
CHistogram::CopyHistogram() const
{
	...
	CHistogram *histogram_copy = SPQOS_NEW(m_mp)
...
CHistogram::MakeJoinHistogramEqualityFilter(const CHistogram *histogram) const
{
	...
	CBucketArray *join_buckets = SPQOS_NEW(m_mp) CBucketArray(m_mp);
	while (idx1 < buckets1 && idx2 < buckets2)
	{
		...
		{
			SPQOS_ASSERT(bucket2->IsBefore(bucket1));
			idx2++;
		}
	}
...
{
	SPQOS_ASSERT(NULL != histogram1);
	SPQOS_ASSERT(NULL != histogram2);
									 ...
									 CDouble *result_freq_remain)
{
	SPQOS_ASSERT(NULL != histogram1);
	SPQOS_ASSERT(NULL != histogram2);
	SPQOS_ASSERT(NULL != join_buckets);
										  ...
										  CDouble *result_distinct_values) const
{
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CBucket *new_bucket = SPQOS_NEW(m_mp) CBucket(
	...
	CHistogram *result_histogram = SPQOS_NEW(m_mp)
										   ...
										   CDouble rows_other) const
{
	...
	while (NULL != bucket1 && NULL != bucket2)
	{
		...
		{
			SPQOS_ASSERT(bucket1->Intersects(bucket2));
	...
	SPQOS_ASSERT_IFF(NULL == bucket1, idx1 == num_buckets1);
	SPQOS_ASSERT_IFF(NULL == bucket2, idx2 == num_buckets2);
...
{
	SPQOS_ASSERT(NULL != src_buckets);
	SPQOS_ASSERT(NULL != dest_buckets);
	SPQOS_ASSERT(begin <= end);
	SPQOS_ASSERT(end <= src_buckets->Size());
...
CHistogram::CombineBuckets(CMemoryPool *mp, CBucketArray *buckets,
						   ULONG desired_num_buckets)
{
	SPQOS_ASSERT(desired_num_buckets >= 1);
	...
	SPQOS_ASSERT(start_frequency <= CDouble(1.0) + CStatistics::Epsilon);
#endif

	CBucketArray *result_buckets = SPQOS_NEW(mp) CBucketArray(mp);
	...
	CKHeap<SAdjBucketBoundaryArray, SAdjBucketBoundary> *boundary_factors =
		SPQOS_NEW(mp) CKHeap<SAdjBucketBoundaryArray, SAdjBucketBoundary>(
			mp, bucketsToCombine);
	...
	for (ULONG ul = 0; ul < buckets->Size() - 1; ++ul)
	{
		...
		{
			SPQOS_ASSERT(bucket1->IsUpperClosed() ^ bucket2->IsLowerClosed());
			...
			SAdjBucketBoundary *elem =
				SPQOS_NEW(mp) SAdjBucketBoundary(ul, factor1 + factor2);
	...
	CBitSet *indexes_to_merge = SPQOS_NEW(mp) CBitSet(mp);
		   ...
		   (candidate_to_remove = boundary_factors->RemoveBestElement()))
	{
		...
		SPQOS_DELETE(candidate_to_remove);
	}
	...
	SPQOS_ASSERT(start_frequency - end_frequency <=
				CDouble(0.0) + CStatistics::Epsilon);
#endif

	SPQOS_ASSERT(result_buckets->Size() == desired_num_buckets);
										...
										CDouble *num_output_rows) const
{
	SPQOS_ASSERT(NULL != other_histogram);
	SPQOS_ASSERT(this->IsValid());
	SPQOS_ASSERT(other_histogram->IsValid());
	...
	CDoubleArray *num_tuples_per_bucket = SPQOS_NEW(m_mp) CDoubleArray(m_mp);
	...
	while (NULL != bucket1 && NULL != bucket2)
	{
		if (bucket1->IsBefore(bucket2))
		{
			...
			num_tuples_per_bucket->Append(
				SPQOS_NEW(m_mp) CDouble(bucket1->GetFrequency() * rows));
			CleanupResidualBucket(bucket1, bucket1_is_residual);
			idx1++;
			bucket1 = (*this)[idx1];
			bucket1_is_residual = false;
		...
		else if (bucket2->IsBefore(bucket1))
		{
			...
			num_tuples_per_bucket->Append(
				SPQOS_NEW(m_mp) CDouble(bucket2->GetFrequency() * rows_other));
			CleanupResidualBucket(bucket2, bucket2_is_residual);
			idx2++;
		...
		else
		{
			...
			num_tuples_per_bucket->Append(SPQOS_NEW(m_mp) CDouble(
	...
	SPQOS_ASSERT(num_tuples_per_bucket->Size() == histogram_buckets->Size());
	...
	*num_output_rows = std::max(CStatistics::MinRows, total_output_rows);
	...
	CHistogram *result_histogram = SPQOS_NEW(m_mp) CHistogram(
...
{
	SPQOS_ASSERT(NULL != histogram_buckets);
	SPQOS_ASSERT(NULL != dest_bucket_freqs);
	...
	dest_bucket_freqs->Append(SPQOS_NEW(m_mp)
					   ...
					   CDoubleArray *dest_bucket_freqs, ULONG begin, ULONG end)
{
	...
	for (ULONG ul = begin; ul < end; ul++)
	{
		...
		dest_bucket_freqs->Append(
			SPQOS_NEW(mp) CDouble(new_bucket->GetFrequency() * rows));
	}
}
											 ...
											 ULONG colid, CDouble width) const
{
	CDXLBucketArray *dxl_stats_bucket_array =
		SPQOS_NEW(m_mp) CDXLBucketArray(m_mp);
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CDXLBucket *dxl_bucket = SPQOS_NEW(m_mp) CDXLBucket(
						  ...
						  ULONG index)
{
	...
	if (index >= SPQOPT_SKEW_SAMPLE_SIZE)
	{
		return;
	}
...
CHistogram::ComputeSkew()
{
	...
	DOUBLE samples[SPQOPT_SKEW_SAMPLE_SIZE];
	DOUBLE sample_rate[SPQOPT_SKEW_SAMPLE_SIZE];
	sample_rate[0] = 0;
	sample_rate[1] = 1;
	ULONG index = 1;
	...
	CDouble accumulated_freq = 0;
	for (ULONG ul = 0; ul < SPQOPT_SKEW_SAMPLE_SIZE; ul++)
	{
		CBucket *bucket = (*m_histogram_buckets)[bucket_index];
		...
		sample_index++;
		DOUBLE filling_rate = (DOUBLE) ul / SPQOPT_SKEW_SAMPLE_SIZE;
	...
	sample_mean = (DOUBLE) sample_mean / SPQOPT_SKEW_SAMPLE_SIZE;
	...
	DOUBLE moment2 = (DOUBLE)(num2 / SPQOPT_SKEW_SAMPLE_SIZE);
	// third moment: a/symmetry
	DOUBLE moment3 = (DOUBLE)(num3 / SPQOPT_SKEW_SAMPLE_SIZE);

	if (moment2 == 0)
	{
		m_skew = pow(SPQOPT_SKEW_SAMPLE_SIZE, 0.5);
	}
	else
	{
								 ...
								 BOOL is_empty)
{
	...
	return SPQOS_NEW(mp) CHistogram(mp, false /* is_well_defined */);
}
...
CHistogram::MakeDefaultBoolHistogram(CMemoryPool *mp)
{
	...
	return SPQOS_NEW(mp) CHistogram(
...
CHistogram::NeedsNDVBasedCardEstimationForEq(const CHistogram *histogram)
{
	...
	if (datum->StatsMappable())
	{
		...
		{
			SPQOS_ASSERT(datum->IsDatumMappableToLINT());
	...
	const ULongPtrArray *columns, BOOL is_empty)
{
	...
	for (ULONG ul = 0; ul < count; ul++)
	{
		...
		output_histograms->Insert(SPQOS_NEW(mp) ULONG(colid), histogram);

		CDouble width =
			CStatisticsUtils::DefaultColumnWidth(col_ref->RetrieveType());
		output_col_widths->Insert(SPQOS_NEW(mp) ULONG(colid),
...
{
	SPQOS_ASSERT(NULL != output_histograms);
	SPQOS_ASSERT(NULL != input_histograms);
	...
	while (col_hist_mapping.Advance())
	{
		...
		// empty histogram
		CHistogram *histogram =
			SPQOS_NEW(mp) CHistogram(mp, false /* is_well_defined */);
		output_histograms->Insert(SPQOS_NEW(mp) ULONG(colid), histogram);
	}
}
