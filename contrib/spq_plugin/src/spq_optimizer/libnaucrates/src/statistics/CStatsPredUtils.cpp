...
{
	SPQOS_ASSERT(NULL != str_opname);

	CStatsPred::EStatsCmpType stats_cmp_type = CStatsPred::EstatscmptOther;

	CWStringConst str_eq(SPQOS_WSZ_LIT("="));
	CWStringConst str_lt(SPQOS_WSZ_LIT("<"));
	CWStringConst str_leq(SPQOS_WSZ_LIT("<="));
	CWStringConst str_geq(SPQOS_WSZ_LIT(">="));
	CWStringConst str_gt(SPQOS_WSZ_LIT(">"));
	CWStringConst str_neq(SPQOS_WSZ_LIT("<>"));
...
{
	return SPQOS_NEW(mp)
		CStatsPredUnsupported(spqos::ulong_max, CStatsPred::EstatscmptOther);
}
...
{
	SPQOS_ASSERT(NULL != predicate_expr);
	SPQOS_ASSERT(IsPredScalarIdentIsNull(predicate_expr) ||
	...
	CStatsPredPoint *pred_stats =
		SPQOS_NEW(mp) CStatsPredPoint(col_ref->Id(), stats_cmp_type, point);
...
CStatsPredUtils::GetPredStats(CMemoryPool *mp, CExpression *expr)
{
	...
	else
	{
		...
		SPQOS_ASSERT(CPredicateUtils::FIdentCompareConstIgnoreCast(
	...
	SPQOS_ASSERT(COperator::EopScalarIdent == expr_left->Pop()->Eopid() ||
				CScalarIdent::FCastedScId(expr_left));
	SPQOS_ASSERT(COperator::EopScalarConst == expr_right->Pop()->Eopid() ||
	...
	SPQOS_ASSERT(NULL != scalar_const_op);
	...
	return SPQOS_NEW(mp) CStatsPredPoint(mp, col_ref, stats_cmp_type, datum);
}
...
{
	SPQOS_ASSERT(NULL != col_ref_outer);
	SPQOS_ASSERT(NULL != col_ref_inner);
	SPQOS_ASSERT(NULL == *col_ref_outer);
	SPQOS_ASSERT(NULL == *col_ref_inner);
	...
	CExpression **inner_expr)
{
	...
	if (spqos::ulong_max == child_index_1 || spqos::ulong_max == child_index_2)
	{
	...
	if (child_index_1 < child_index_2)
	{
		SPQOS_ASSERT(0 == child_index_1 && 1 == child_index_2);
		*outer_expr = expr_1;
		*inner_expr = expr_2;
	}
	else
	{
		SPQOS_ASSERT(0 == child_index_2 && 1 == child_index_1);
		*outer_expr = expr_2;
		*inner_expr = expr_1;
	}
								  ...
								  CColRefSet *outer_refs)
{
	...
	return SPQOS_NEW(mp) CStatsPredConj(SPQOS_NEW(mp) CStatsPredPtrArry(mp));
}
									 ...
									 CColRefSet *outer_refs)
{
	...
	CExpressionArray *pred_expr_conjuncts =
		CPredicateUtils::PdrspqexprConjuncts(mp, scalar_expr);
	const ULONG size = pred_expr_conjuncts->Size();

	CStatsPredPtrArry *pred_stats_array = SPQOS_NEW(mp) CStatsPredPtrArry(mp);
	...
	{
		return SPQOS_NEW(mp) CStatsPredConj(pred_stats_array);
	}
...
{
	SPQOS_ASSERT(NULL != predicate_expr);
	SPQOS_ASSERT(CPredicateUtils::FOr(predicate_expr));

	CStatsPredPtrArry *pred_stats_disj_child =
		SPQOS_NEW(mp) CStatsPredPtrArry(mp);
	...
	CExpressionArray *disjunct_expr =
		CPredicateUtils::PdrspqexprDisjuncts(mp, expr_copy);
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		{
			CStatsPredPtrArry *pred_stats = SPQOS_NEW(mp) CStatsPredPtrArry(mp);
			AddSupportedStatsFilters(mp, pred_stats, expr, outer_refs);
			pred_stats_disj_child->Append(SPQOS_NEW(mp)
	...
	{
		return SPQOS_NEW(mp) CStatsPredDisj(pred_stats_disj_child);
	}
										  ...
										  CColRefSet *outer_refs)
{
	...
	{
		pred_stats_array->Append(SPQOS_NEW(mp) CStatsPredUnsupported(
			spqos::ulong_max, CStatsPred::EstatscmptOther,
	...
	else
	{
		...
		SPQOS_ASSERT(CStatsPredUtils::EptSentinel != ept);
...
CStatsPredUtils::IsConjunction(CMemoryPool *mp, CExpression *predicate_expr)
{
	...
	CExpressionArray *expr_conjuncts =
		CPredicateUtils::PdrspqexprConjuncts(mp, predicate_expr);
...
{
	SPQOS_ASSERT(NULL != predicate_expr);
	SPQOS_ASSERT(CPredicateUtils::FLikePredicate(predicate_expr));
	...
	{
		return SPQOS_NEW(mp)
			CStatsPredUnsupported(spqos::ulong_max, CStatsPred::EstatscmptLike);
	}
...
{
	SPQOS_ASSERT(NULL != result_pred_stats);
	SPQOS_ASSERT(NULL != predicate_expr);
	SPQOS_ASSERT(2 == predicate_expr->Arity());
	...
	if (!is_supported_array_cmp)
	{
		...
		result_pred_stats->Append(SPQOS_NEW(mp) CStatsPredUnsupported(
			spqos::ulong_max, CStatsPred::EstatscmptOther));
		return;
	}
	...
	{
		SPQOS_ASSERT(CCastUtils::FBinaryCoercibleCast((*predicate_expr)[0]));
	...
	if (!CHistogram::IsOpSupportedForFilter(stats_cmp_type))
	{
		...
		result_pred_stats->Append(
			SPQOS_NEW(mp) CStatsPredUnsupported(col_ref->Id(), stats_cmp_type));
		return;
	}
	...
	if (is_array_cmp_any)
	{
		...
		pred_stats = SPQOS_NEW(mp) CStatsPredPtrArry(mp);
		...
		if (is_array_cmp_eq)
		{
			points = SPQOS_NEW(mp) CPointArray(mp);
		}
	}
	else
	{
		SPQOS_ASSERT(CScalarArrayCmp::EarrcmpAll ==
	...
	for (ULONG ul = 0; ul < num_array_elems; ++ul)
	{
		...
		if (COperator::EopScalarConst == child_expr->Pop()->Eopid())
		{
			...
			{
				// stats calculations on such datums unsupported
				CStatsPred *child_pred_stats = SPQOS_NEW(mp)
					CStatsPredUnsupported(col_ref->Id(), stats_cmp_type);
	...
	if (is_array_cmp_any)
	{
		...
		{
			// "a = ANY (ARRAY[...])"
			CStatsPredArrayCmp *pred_stats_array_cmp = SPQOS_NEW(mp)
		...
		CStatsPredDisj *pred_stats_disj =
			SPQOS_NEW(mp) CStatsPredDisj(pred_stats);
...
{
	SPQOS_ASSERT(NULL != predicate_expr);
	SPQOS_ASSERT(CPredicateUtils::FBooleanScalarIdent(predicate_expr) ||
				CPredicateUtils::FNegatedBooleanScalarIdent(predicate_expr));
	...
	SPQOS_ASSERT(NULL != datum && spqos::ulong_max != colid);

	return SPQOS_NEW(mp) CStatsPredPoint(colid, CStatsPred::EstatscmptEq,
										SPQOS_NEW(mp) CPoint(datum));
}
...
{
	SPQOS_ASSERT(NULL != join_pred_expr);
	SPQOS_ASSERT(NULL != output_col_refsets);
	SPQOS_ASSERT(NULL != unsupported_expr_array);
		...
		CStatsPred::EstatscmptOther != stats_cmp_type)
	{
		...
		ULONG outer_id =
			(NULL != col_ref_outer ? col_ref_outer->Id() : spqos::ulong_max);
		ULONG inner_id =
			(NULL != col_ref_inner ? col_ref_inner->Id() : spqos::ulong_max);

		return SPQOS_NEW(mp) CStatsPredJoin(outer_id, stats_cmp_type, inner_id);
	}
												...
												BOOL is_semi_or_anti_join)
{
	...
	{
		return SPQOS_NEW(mp) CStatsPredJoinArray(mp);
	}

	CColRefSetArray *output_col_refsets = SPQOS_NEW(mp) CColRefSetArray(mp);
...
CStatsPredUtils::IsUnsupportedPredOnDefinedCol(CStatsPred *pred_stats)
{
	return ((CStatsPred::EsptUnsupported == pred_stats->GetPredStatsType()) &&
			(spqos::ulong_max == pred_stats->GetColId()));
}
