	...
	ULongPtrArray *first_child_colids, ULongPtrArray *second_child_colids)
{
	...
	SPQOS_ASSERT(NULL != stats_second_child);

	// lengths must match
	SPQOS_ASSERT(output_colids->Size() == first_child_colids->Size());
	SPQOS_ASSERT(output_colids->Size() == second_child_colids->Size());
	...
	UlongToDoubleMap *column_to_width_map = SPQOS_NEW(mp) UlongToDoubleMap(mp);
	...
	else
	{
		...
		for (ULONG ul = 0; ul < len; ul++)
		{
			...
			SPQOS_ASSERT(NULL != col_width);
			column_to_width_map->Insert(SPQOS_NEW(mp) ULONG(output_colid),
										SPQOS_NEW(mp) CDouble(*col_width));
		}
	...
	CStatistics *unionall_stats = SPQOS_NEW(mp)
