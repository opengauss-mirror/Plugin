...
{
	SPQOS_ASSERT(join_conds_scale_factors != NULL);
	...
	CScaleFactorUtils::OIDPairToScaleFactorArrayMap *scale_factor_hashmap =
		SPQOS_NEW(mp) OIDPairToScaleFactorArrayMap(mp, 7);
	...
	for (ULONG ul = 0; ul < join_conds_scale_factors->Size(); ul++)
	{
		...
		if (oid_pair != NULL && oid_pair->Size() == 2)
		{
			...
			{
				scale_factor_array = SPQOS_NEW(mp) CDoubleArray(mp);
				scale_factor_array->Append(SPQOS_NEW(mp)
											   CDouble(local_scale_factor));
				oid_pair->AddRef();
			...
			else if (both_dist_keys)
			{
				if (!contains_dist_pred)
				{
					...
					for (ULONG i = 0; i < scale_factor_array->Size(); i++)
					{
						...
						new_scale_factor_array->Append(
							SPQOS_NEW(mp) CDouble(scale_factor));
					}
		...
		{
			independent_join_preds->Append(SPQOS_NEW(mp)
											   CDouble(local_scale_factor));
		}
	}
	...
	CDouble limit_for_result_scale_factor)
{
	...
	else
	{
		...
		CDoubleArray *independent_join_preds = SPQOS_NEW(mp) CDoubleArray(mp);
...
{
	SPQOS_ASSERT(NULL != val1 && NULL != val2);
...
{
	SPQOS_ASSERT(NULL != double_val1);
	SPQOS_ASSERT(NULL != double_val2);
	...
	CDouble total_rows)
{
	...
	{
		CDouble local_scale_factor = *(*scale_factors)[ul];
		SPQOS_ASSERT(InvalidScaleFactor < local_scale_factor);
