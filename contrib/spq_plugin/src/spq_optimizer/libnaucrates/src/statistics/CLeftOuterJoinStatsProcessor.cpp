...
{
	SPQOS_ASSERT(NULL != outer_side_stats);
	SPQOS_ASSERT(NULL != inner_side_stats);
	SPQOS_ASSERT(NULL != join_preds_stats);
	...
	CStatistics *result_stats_LOJ = SPQOS_NEW(mp) CStatistics(
	...
	CDouble *result_rows_LASJ)
{
	...
	SPQOS_ASSERT(NULL != inner_side_stats);
	SPQOS_ASSERT(NULL != join_preds_stats);
	SPQOS_ASSERT(NULL != inner_join_stats);

	// build a bitset with all outer child columns contributing to the join
	CBitSet *outer_side_join_cols = SPQOS_NEW(mp) CBitSet(mp);
	...
	UlongToHistogramMap *LOJ_histograms = SPQOS_NEW(mp) UlongToHistogramMap(mp);
	...
	for (ULONG i = 0; i < num_outer_cols; i++)
	{
		...
		SPQOS_ASSERT(NULL != inner_join_histogram);
		...
		if (outer_side_join_cols->Get(colid))
		{
			...
			SPQOS_ASSERT(NULL != LASJ_histogram);
...
{
	SPQOS_ASSERT(NULL != inner_join_stats);
	SPQOS_ASSERT(NULL != inner_colids_with_stats);
	SPQOS_ASSERT(NULL != LOJ_histograms);
	...
	for (ULONG ul = 0; ul < num_inner_cols; ul++)
	{
		...
		CHistogram *null_histogram = SPQOS_NEW(mp) CHistogram(
			mp, SPQOS_NEW(mp) CBucketArray(mp), true /*is_well_defined*/,
