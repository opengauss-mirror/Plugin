...
{
	SPQOS_ASSERT(NULL != projection_colids);
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (NULL == histogram)
		{
			...
			CBucketArray *proj_col_bucket = SPQOS_NEW(mp) CBucketArray(mp);
	...
	CStatistics *projection_stats = SPQOS_NEW(mp) CStatistics(
