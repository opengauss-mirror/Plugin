	...
	BOOL DoIgnoreLASJHistComputation)
{
	...
	SPQOS_ASSERT(NULL != join_pred_stats);
	SPQOS_ASSERT(NULL != result_hist1);
	SPQOS_ASSERT(NULL != result_hist2);
	SPQOS_ASSERT(NULL != scale_factor);
	...
	if (is_input_empty)
	{
		...
		*result_hist1 = SPQOS_NEW(mp) CHistogram(mp);
		*result_hist2 = SPQOS_NEW(mp) CHistogram(mp);
	...
	*scale_factor = CScaleFactorUtils::DefaultJoinPredScaleFactor;
	...
	else if (CHistogram::JoinPredCmpTypeIsSupported(stats_cmp_type))
	{
		...
		SPQOS_ASSERT(join_histogram->IsEmpty());
									  ...
									  CExpression *expr, COperator *pop)
{
	...
	SPQOS_ASSERT(0 < statistics_array->Size());
		...
		case COperator::EopLogicalNAryJoin:
			...
			if (NULL != predIndexes)
			{
				...
				inner_or_simple_2_way_loj_preds =
					(*expr)[SPQOPT_ZERO_INNER_JOIN_PRED_INDEX];
			}
			break;
	...
	for (ULONG i = 1; i < num_stats; i++)
	{
		...
		if (NULL == predIndexes ||
			SPQOPT_ZERO_INNER_JOIN_PRED_INDEX == *(*predIndexes)[i])
		{
	...
	IStatistics::EStatsJoinType join_type, BOOL DoIgnoreLASJHistComputation)
{
	...
	SPQOS_ASSERT(NULL != inner_stats_input);
	SPQOS_ASSERT(NULL != outer_stats_input);

	SPQOS_ASSERT(NULL != join_pred_stats_info);
	...
	CBitSet *join_colids = SPQOS_NEW(mp) CBitSet(mp);
	...
	CScaleFactorUtils::SJoinConditionArray *join_conds_scale_factors =
		SPQOS_NEW(mp) CScaleFactorUtils::SJoinConditionArray(mp);
	...
	for (ULONG i = 0; i < num_join_conds; i++)
	{
		...
		{
			SPQOS_ASSERT(CStatsPred::EstatscmptEqNDV == pred_info->GetCmpType());
		...
		SPQOS_DELETE(outer_histogram_after);
		SPQOS_DELETE(inner_histogram_after);
		...
		SPQOS_ASSERT(colref_outer != NULL);
		SPQOS_ASSERT(colref_inner != NULL);
		...
		if ((mdid_outer != NULL) && (mdid_inner != NULL))
		{
			...
			mdid_pair = SPQOS_NEW(mp) IMdIdArray(mp, 2);
			mdid_outer->AddRef();
			mdid_inner->AddRef();
		...
		join_conds_scale_factors->Append(
			SPQOS_NEW(mp) CScaleFactorUtils::SJoinCondition(
	...
	CStatistics *join_stats = SPQOS_NEW(mp) CStatistics(
	...
	IStatistics::EStatsJoinType join_type)
{
	...
	SPQOS_ASSERT(NULL != join_conds_scale_factors);
	...
	SPQOS_ASSERT(CStatistics::MinRows <= scale_factor);
									   ...
									   IStatistics::EStatsJoinType join_type)
{
	SPQOS_ASSERT(NULL != outer_histogram);
	SPQOS_ASSERT(NULL != inner_histogram);
	SPQOS_ASSERT(NULL != join_histogram);
	BOOL IsLASJ = IStatistics::EsjtLeftAntiSemiJoin == join_type;
									 ...
									 IStatisticsArray *stats_ctxt)
{
	...
	SPQOS_ASSERT(COperator::EopLogicalLeftOuterJoin == op_id ||
