...
CBucket::CBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound,
	...
	: m_bucket_lower_bound(bucket_lower_bound),
	  ...
	  m_distinct(distinct)
{
	SPQOS_ASSERT(NULL != m_bucket_lower_bound);
	SPQOS_ASSERT(NULL != m_bucket_upper_bound);
	SPQOS_ASSERT(0.0 <= m_frequency && 1.0 >= m_frequency);
	SPQOS_ASSERT(0.0 <= m_distinct);

	// singleton bucket lower and upper bound are closed
	SPQOS_ASSERT_IMP(IsSingleton(), is_lower_closed && is_upper_closed);

	// null values should be in null fraction of the histogram
	SPQOS_ASSERT(!m_bucket_lower_bound->GetDatum()->IsNull());
	SPQOS_ASSERT(!m_bucket_upper_bound->GetDatum()->IsNull());
}
...
CBucket::GetOverlapPercentage(const CPoint *point, BOOL include_point) const
{
	...
	if (IsSingleton())
	{
		SPQOS_ASSERT(this->m_bucket_lower_bound->Equals(point));
...
FORCE_GENERATE_DBGSTR(spqnaucrates::CBucket);
...
{
	SPQOS_ASSERT(mp);
	SPQOS_ASSERT(point_upper_new);

	SPQOS_ASSERT(this->Contains(point_upper_new));
	...
	return SPQOS_NEW(mp) CBucket(this->m_bucket_lower_bound, point_upper_new,
...
{
	SPQOS_ASSERT(mp);
	SPQOS_ASSERT(point_lower_new);

	SPQOS_ASSERT(this->Contains(point_lower_new));
...
{
	SPQOS_ASSERT(mp);
	SPQOS_ASSERT(point_singleton);
	SPQOS_ASSERT(this->Contains(point_singleton));
...
{
	SPQOS_ASSERT(this != NULL);
	SPQOS_ASSERT(bucket != NULL);
							 ...
							 CDouble *result_freq_intersect2) const
{
	...
	SPQOS_ASSERT(Intersects(bucket));
	...
	else
	{
		...
		SPQOS_ASSERT(distance_new <= Width());
		SPQOS_ASSERT(distance_new <= bucket->Width());
					...
					CBucket **result_bucket_upper)
{
	...
	SPQOS_ASSERT(NULL == *result_bucket_lower);
	SPQOS_ASSERT(NULL == *result_bucket_upper);
...
{
	// should only be called on intersecting bucket
	SPQOS_ASSERT(Intersects(bucket_other));

	// we shouldn't be overwriting anything important
	SPQOS_ASSERT(NULL == *bucket_new1);
	SPQOS_ASSERT(NULL == *bucket_new2);
		 ...
		 this->IsLowerClosed() != bucket_other->IsLowerClosed()))
	{
		...
		{
			SPQOS_ASSERT(bucket_other->GetLowerBound()->Equals(minLower));
			...
			*result_rows = rows_other;
			...
			maxLower->AddRef();
			lower_third =
				SPQOS_NEW(mp) CBucket(bucket_other->GetLowerBound(), maxLower,
	...
	SPQOS_ASSERT(minLower->Equals(maxLower));
	SPQOS_ASSERT(this->IsLowerClosed() == bucket_other->IsLowerClosed());
	...
	if (!minUpper->Equals(maxUpper))
	{
		...
		else if (this->GetUpperBound()->Equals(maxUpper))
		{
			...
			SPQOS_ASSERT(this_overlap * this->GetFrequency() +
							upper_third->GetFrequency() <=
						this->GetFrequency() + CStatistics::Epsilon);
		}
		else
		{
			SPQOS_ASSERT(bucket_other->GetUpperBound()->Equals(maxUpper));
			upper_third = bucket_other->MakeBucketScaleLower(
				mp, minUpper, true /*include_lower*/);
			bucket_other_overlap = bucket_other->GetOverlapPercentage(
				minUpper, false /*include_point*/);
			SPQOS_ASSERT(bucket_other_overlap * bucket_other->GetFrequency() +
							upper_third->GetFrequency() <=
	...
	else
	{
		...
		{
			SPQOS_ASSERT(this->IsLowerClosed() == bucket_other->IsLowerClosed());
			SPQOS_ASSERT(this->IsUpperClosed() == bucket_other->IsUpperClosed());
		}
	}
	...
	if (is_union_all)
	{
		SPQOS_ASSERT(merged_rows_this + merged_rows_other <= total_rows);
		merged_freq = std::min(
	...
	CDouble max_merged_ndv = (CDouble) spqos::ulong_max;
	...
	maxLower->AddRef();
	minUpper->AddRef();
	middle_third = SPQOS_NEW(mp) CBucket(maxLower, minUpper, isLowerClosed,
	...
	else
	{
		...
		SPQOS_ASSERT(NULL == upper_third);

		/* FIXME: this assert currently triggers for some queries, see SPQQP-74
		SPQOS_ASSERT_IMP(
			is_union_all,
			middle_third->GetFrequency() * total_rows <=
