...
{
	SPQOS_ASSERT(NULL != child_stats);
	SPQOS_ASSERT(NULL != local_scalar_expr);
	SPQOS_ASSERT(NULL != outer_refs_scalar_expr);
	SPQOS_ASSERT(NULL != all_outer_stats);
											  ...
											  CColRefSet *outer_refs)
{
	...
	CColRefSet *used_local_col_refs =
		SPQOS_NEW(mp) CColRefSet(mp, *used_col_refs);
	...
	CColRefSet *dummy_width_set = SPQOS_NEW(mp) CColRefSet(mp);
	...
	if (NULL != expr_with_outer_refs)
	{
		CExpressionArray *outer_ref_exprs =
			CPredicateUtils::PdrspqexprConjuncts(mp, expr_with_outer_refs);
		...
		for (ULONG ul = 0; ul < size; ul++)
		{
					...
					pexpr, outer_refs, &local_col_ref))
			{
				...
				{
					SPQOS_ASSERT(NULL != local_col_ref);
...
CFilterStatsProcessor::MakeStatsFilter(CMemoryPool *mp,
									   ...
									   BOOL do_cap_NDVs)
{
	SPQOS_ASSERT(NULL != base_pred_stats);
	...
	{
		histograms_new = SPQOS_NEW(mp) UlongToHistogramMap(mp);
	...
	else
	{
		...
		SPQOS_ASSERT(CStatistics::MinRows.Get() <= scale_factor.Get());
	...
	SPQOS_ASSERT(rows_filter.Get() <= input_rows.Get());
	...
	CStatistics *filter_stats = SPQOS_NEW(mp)
	...
	CStatsPred *pred_stats, CDouble *scale_factor)
{
	...
	SPQOS_ASSERT(NULL != result_histograms);
	...
	CStatsPredConj *conjunctive_pred_stats, CDouble *scale_factor)
{
	SPQOS_ASSERT(NULL != stats_config);
	SPQOS_ASSERT(NULL != input_histograms);
	SPQOS_ASSERT(NULL != conjunctive_pred_stats);

	conjunctive_pred_stats->Sort();

	CBitSet *filter_colids = SPQOS_NEW(mp) CBitSet(mp);
	CDoubleArray *scale_factors = SPQOS_NEW(mp) CDoubleArray(mp);
	...
	ULONG last_colid = spqos::ulong_max;
	...
	for (ULONG ul = 0; ul < filters; ul++)
	{
		...
		SPQOS_ASSERT(CStatsPred::EsptConj !=
		...
		if (CStatsPredUtils::IsUnsupportedPredOnDefinedCol(child_pred_stats))
		{
			...
			scale_factors->Append(
				SPQOS_NEW(mp) CDouble(unsupported_pred_stats->ScaleFactor()));
		...
		{
			scale_factors->Append(SPQOS_NEW(mp) CDouble(last_scale_factor));
		...
		if (CStatsPred::EsptDisj != child_pred_stats->GetPredStatsType())
		{
			...
			SPQOS_DELETE(hist_before);

			SPQOS_ASSERT(NULL != result_histogram);

			CHistogram *input_histogram = input_histograms->Find(&colid);
			SPQOS_ASSERT(NULL != input_histogram);
	...
	SPQOS_ASSERT(NULL != scale_factors);
	...
	CStatsPredDisj *disjunctive_pred_stats, CDouble *scale_factor)
{
	SPQOS_ASSERT(NULL != stats_config);
	SPQOS_ASSERT(NULL != input_histograms);
	SPQOS_ASSERT(NULL != disjunctive_pred_stats);
	...
	ULONG previous_colid = spqos::ulong_max;
	...
	for (ULONG ul = 0; ul < filters; ul++)
	{
		...
		{
			scale_factors->Append(SPQOS_NEW(mp)
		...
		BOOL is_colid_present = (spqos::ulong_max != colid);
		...
		if (is_colid_present)
		{
			...
			SPQOS_ASSERT(NULL != disjunctive_child_col_histogram);
			...
			else
			{
				...
				SPQOS_DELETE(disjunctive_child_col_histogram);
		...
		else
		{
			...
			SPQOS_ASSERT(NULL != child_histograms);
			SPQOS_ASSERT(NULL == disjunctive_child_col_histogram);
	...
	previous_histogram = NULL;
	scale_factors->Append(SPQOS_NEW(mp) CDouble(
...
{
	SPQOS_ASSERT(NULL != pred_stats);
	SPQOS_ASSERT(NULL != filter_colids);
	SPQOS_ASSERT(NULL != hist_before);

	const ULONG colid = pred_stats->GetColId();
	SPQOS_ASSERT(CHistogram::IsOpSupportedForFilter(pred_stats->GetCmpType()));
	...
	SPQOS_ASSERT(DOUBLE(1.0) <= local_scale_factor.Get());
												 ...
												 ULONG *target_last_colid)
{
	SPQOS_ASSERT(NULL != pred_stats);
	SPQOS_ASSERT(NULL != filter_colids);
	SPQOS_ASSERT(NULL != base_histogram);
	SPQOS_ASSERT(pred_stats->GetCmpType() == CStatsPred::EstatscmptEq);
	...
	CPointArray *deduped_points = SPQOS_NEW(mp) CPointArray(mp);
	...
	for (ULONG ul = 0; ul < points->Size(); ++ul)
	{
		...
		SPQOS_ASSERT(datum->StatsAreComparable(datum));
		if (datum->IsNull())
		{
			continue;
		}
	...
	CHistogram *dummy_histogram = SPQOS_NEW(mp)
	...
	SPQOS_ASSERT(dummy_histogram->IsValid() &&
...
{
	return (spqos::ulong_max == colid || colid != last_colid);
}
