							...
							CPoint *point)
{
		...
		mdtype->GetDatumType() == IMDType::EtiOid)
	{
		...
		return SPQOS_NEW(mp) CPoint(datum_new);
	}
									 ...
									 ULONG num_mcv_values)
{
	...
	CBucketArray *mcv_buckets = SPQOS_NEW(mp) CBucketArray(mp);
	...
	for (ULONG i = 0; i < num_mcv_values; i++)
	{
		...
		CBucket *bucket = SPQOS_NEW(mp)
			CBucket(SPQOS_NEW(mp) CPoint(datum), SPQOS_NEW(mp) CPoint(datum),
					true /* is_lower_closed */, true /* is_upper_closed */,
					bucket_freq, CDouble(1.0));
		mcv_buckets->Append(bucket);
	}
	CHistogram *histogram = SPQOS_NEW(mp) CHistogram(mp, mcv_buckets);
	SPQOS_ASSERT(histogram->IsValid());
	mcv_pairs->Release();
...
{
	SPQOS_ASSERT(NULL != mcv_histogram);
	SPQOS_ASSERT(NULL != histogram);
	SPQOS_ASSERT(mcv_histogram->IsWellDefined());
	SPQOS_ASSERT(histogram->IsWellDefined());
	SPQOS_ASSERT(0 < mcv_histogram->GetNumBuckets());
	SPQOS_ASSERT(0 < histogram->GetNumBuckets());
	...
	SPQOS_ASSERT(mcv_histogram->IsValid());
	SPQOS_ASSERT(histogram->IsValid());

	CBucketArray *merged_buckets =
		MergeMcvHistBucket(mp, mcv_buckets, histogram_buckets);

	CHistogram *merged_histogram = SPQOS_NEW(mp) CHistogram(mp, merged_buckets);
	SPQOS_ASSERT(merged_histogram->IsValid());
...
//---------------------------------------------------------------------------
//	@function:
//		CStatisticsUtils::PdrspqbucketCreateMergedBuckets
//
//	@doc:
...
{
	CBucketArray *merged_buckets = SPQOS_NEW(mp) CBucketArray(mp);
	...
	while (mcv_index < mcv && histogram_index < num_histograms)
	{
		...
		{
			SPQOS_ASSERT(histogram_bucket->Subsumes(mcv_bucket));
...
{
	SPQOS_ASSERT(NULL != histogram_bucket);
	SPQOS_ASSERT(NULL != mcv_buckets);

	CBucketArray *temp_mcv_buckets = SPQOS_NEW(mp) CBucketArray(mp);
	...
	// copy buckets from pdrspqbucketSplitted to pdrgbucketMerged
	...
	const CBucketArray *mcv_buckets)
{
	...
	CBucketArray *buckets_after_split = SPQOS_NEW(mp) CBucketArray(mp);
	const ULONG mcv = mcv_buckets->Size();
	SPQOS_ASSERT(0 < mcv);
	...
	// re-balance distinct and frequency in pdrspqbucketNew
									...
									BOOL is_lower_closed, BOOL is_upper_closed)
{
	...
	return SPQOS_NEW(mp) CBucket(
		bucket_lower_bound, bucket_upper_bound, is_lower_closed,
		is_upper_closed,
		SPQOPT_BUCKET_DEFAULT_FREQ,	   // frequency will be assigned later
		SPQOPT_BUCKET_DEFAULT_DISTINCT  // distinct will be assigned later
	);
}
											 ...
											 const CBucketArray *buckets)
{
	...
	for (ULONG i = 0; i < histogram_buckets->Size(); i++)
	{
		...
		if (!bucket->IsSingleton())
		{
			...
			SPQOS_ASSERT(SPQOPT_BUCKET_DEFAULT_FREQ == bucket->GetFrequency());
			SPQOS_ASSERT(SPQOPT_BUCKET_DEFAULT_DISTINCT ==
...
{
	SPQOS_ASSERT(NULL != colids_bitset);
	SPQOS_ASSERT(NULL != pred_stats);
	SPQOS_ASSERT(NULL != colids);

	if (spqos::ulong_max != pred_stats->GetColId())
	{
		// the predicate is on a single column

		(void) colids_bitset->ExchangeSet(pred_stats->GetColId());
		colids->Append(SPQOS_NEW(mp) ULONG(pred_stats->GetColId()));
	...
	SPQOS_ASSERT(CStatsPred::EsptConj == pred_stats->GetPredStatsType() ||
	...
	SPQOS_ASSERT(NULL != stats_pred_array);
...
{
	SPQOS_ASSERT(NULL != dont_update_stats_bitset);
	SPQOS_ASSERT(NULL != disjunctive_result_histograms);

	if (NULL != previous_histogram && spqos::ulong_max != colid &&
		!dont_update_stats_bitset->Get(colid))
	{
		// 1. the filter is on the same column because spqos::ulong_max != colid
		...
		if (NULL != result_histogram)
		{
			...
			SPQOS_DELETE(previous_histogram);
												 ...
												 CStatsPredDisj *pred_stats)
{
	...
	CBitSet *non_updateable_bitset = SPQOS_NEW(mp) CBitSet(mp);

	const ULONG disj_colid = pred_stats->GetColId();
	if (spqos::ulong_max != disj_colid)
	{
		// disjunction predicate on a single column so all are updatable
		return non_updateable_bitset;
	}

	CBitSet *disj_bitset = SPQOS_NEW(mp) CBitSet(mp);
	ULongPtrArray *disjuncts = SPQOS_NEW(mp) ULongPtrArray(mp);
	...
	for (ULONG child_index = 0; child_index < arity; child_index++)
	{
		...
		CBitSet *child_bitset = SPQOS_NEW(mp) CBitSet(mp);
		ULongPtrArray *child_colids = SPQOS_NEW(mp) ULongPtrArray(mp);
		ExtractUsedColIds(mp, child_bitset, child_pred_stats, child_colids);

		const ULONG length = child_colids->Size();
		SPQOS_ASSERT(length <= num_disj_used_col);
							   ...
							   BOOL replace_old)
{
	...
	if (NULL == col_histogram_mapping->Find(&colid))
	{
		...
		BOOL result =
#endif
			col_histogram_mapping->Insert(SPQOS_NEW(mp) ULONG(colid),
...
CStatisticsUtils::PrintHistogramMap(IOstream &os,
									UlongToHistogramMap *col_histogram_mapping)
{
	SPQOS_ASSERT(NULL != col_histogram_mapping);
...
{
	SPQOS_ASSERT(NULL != non_updatable_cols);
	SPQOS_ASSERT(NULL != hmap1);
	SPQOS_ASSERT(NULL != hmap2);

	CDouble output_rows(CStatistics::MinRows.Get());

	UlongToHistogramMap *merged_hmap = SPQOS_NEW(mp) UlongToHistogramMap(mp);
		...
		while (hmap1_iter.Advance())
		{
			...
			if (NULL != histogram1 && !non_updatable_cols->Get(colid))
			{
				...
				if (NULL != histogram2)
				{
					...
					SPQOS_DELETE(normalized_union_histogram);
				}
				else
				{
								  ...
								  UlongToHistogramMap *col_histogram_mapping)
{
	...
	UlongToHistogramMap *histograms_copy = SPQOS_NEW(mp) UlongToHistogramMap(mp);
...
// 		on a single column else	return spqos::ulong_max
//
//---------------------------------------------------------------------------
ULONG
CStatisticsUtils::GetColId(const CStatsPredPtrArry *pred_stats_array)
{
	SPQOS_ASSERT(NULL != pred_stats_array);

	ULONG result_colid = spqos::ulong_max;
	...
	for (ULONG i = 0; i < length && is_same_col; i++)
	{
		...
		ULONG colid = pred_stats->GetColId();
		if (spqos::ulong_max == result_colid)
		{
			result_colid = colid;
		}
											...
											CPartFilterMap *part_filter_map)
{
	...
	CColRefSetArray *output_colrefs = SPQOS_NEW(mp) CColRefSetArray(mp);
...
CStatisticsUtils::DeriveStatsForBitmapTableGet(CMemoryPool *mp,
											   ...
											   IStatisticsArray *stats_contexts)
{
	SPQOS_ASSERT(CLogical::EopLogicalBitmapTableGet ==
...
//      not a table column then the logical op id is initialized to spqos::ulong_max
//---------------------------------------------------------------------------
...
)
{
	...
	while (col_refset_iter.Advance())
	{
		...
		if (NULL == keys || keys->Get(colid))
		{
			...
			if (NULL == ndv_colid)
			{
				ULongPtrArray *colids_new = SPQOS_NEW(mp) ULongPtrArray(mp);
				colids_new->Append(SPQOS_NEW(mp) ULONG(colid));
#ifdef SPQOS_DEBUG
				BOOL fres =
#endif	// SPQOS_DEBUG
					grp_colid_upper_bound_ndv_idx_map->Insert(
						SPQOS_NEW(mp) ULONG(upper_bound_ndv_idx), colids_new);
				SPQOS_ASSERT(fres);
			}
			else
			{
				(const_cast<ULongPtrArray *>(ndv_colid))
					->Append(SPQOS_NEW(mp) ULONG(colid));
			}
		}
	}
...
{
	SPQOS_ASSERT(NULL != grouping_columns);
	SPQOS_ASSERT(NULL != input_stats);
	SPQOS_ASSERT(NULL != output_ndvs);
	...
	for (ULONG i = 0; i < num_cols; i++)
	{
		...
		output_ndvs->Append(SPQOS_NEW(mp) CDouble(distinct_vals));
	}
}
...
)
{
	...
	CDoubleArray *ndvs = SPQOS_NEW(mp) CDoubleArray(mp);
	...
	while (map_iter.Advance())
	{
		...
		else
		{
			...
			ndvs->Append(SPQOS_NEW(mp) CDouble(max_grps_per_src));
		}
	}
...
{
	SPQOS_ASSERT(NULL != input_stats);
	SPQOS_ASSERT(NULL != src_grouping_cols);
	SPQOS_ASSERT(0 < src_grouping_cols->Size());
...
{
	SPQOS_ASSERT(NULL != stats);
	SPQOS_ASSERT(NULL != stats_config);
	SPQOS_ASSERT(NULL != grouping_cols);

	CColRefSet *computed_groupby_cols = SPQOS_NEW(mp) CColRefSet(mp);
								 ...
								 UlongToDoubleMap *output_col_widths)
{
	SPQOS_ASSERT(NULL != input_stats);
	SPQOS_ASSERT(NULL != grp_cols_refset);
	SPQOS_ASSERT(NULL != output_histograms);
	SPQOS_ASSERT(NULL != output_col_widths);
	...
	while (grp_col_refset_iter.Advance())
	{
		...
		if (NULL != histogram)
		{
			...
			SPQOS_DELETE(result_histogram);
		}

		const CDouble *width = input_stats->GetWidth(grp_colid);
		if (NULL != width)
		{
			output_col_widths->Insert(SPQOS_NEW(mp) ULONG(grp_colid),
...
{
	SPQOS_ASSERT(NULL != grouping_columns);
	SPQOS_ASSERT(NULL != computed_groupby_cols);

	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();

	CColRefSet *grp_col_for_stats = SPQOS_NEW(mp) CColRefSet(mp);
...
{
	SPQOS_ASSERT(NULL != histogram_buckets);
...
CStatisticsUtils::IncreasesRisk(CLogical *logical_op)
{
	...
	for (ULONG i = 0; i < SPQOS_ARRAY_SIZE(grouping_and_semi_join_opid_array);
		 i++)
	{
							   ...
							   UlongToDoubleMap *dest_width)
{
	...
	while (col_width_map_iterator.Advance())
	{
		...
		if (!is_present)
		{
			...
			CDouble *width_copy = SPQOS_NEW(mp) CDouble(*width);
			dest_width->Insert(SPQOS_NEW(mp) ULONG(colid), width_copy);
		}
...
{
	SPQOS_ASSERT(NULL != output_stats);
	SPQOS_ASSERT(CStatistics::EcbmSentinel != card_bounding_method);
