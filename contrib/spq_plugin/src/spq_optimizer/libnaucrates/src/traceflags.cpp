...
)
{
	...
	SPQOS_ASSERT(NULL != ppbsEnabled);
	SPQOS_ASSERT(NULL != ppbsDisabled);
	...
	*ppbsEnabled = SPQOS_NEW(mp) CBitSet(mp, EopttraceSentinel);
	*ppbsDisabled = SPQOS_NEW(mp) CBitSet(mp, EopttraceSentinel);
	CBitSetIter bsiter(*pbsInput);
	while (bsiter.Advance())
	{
		ULONG ulTraceFlag = bsiter.Bit();
		if (SPQOS_FTRACE(ulTraceFlag))
		{
...
ResetTraceflags(CBitSet *pbsEnabled, CBitSet *pbsDisabled)
{
	...
	SPQOS_ASSERT(NULL != pbsEnabled);
	SPQOS_ASSERT(NULL != pbsDisabled);
