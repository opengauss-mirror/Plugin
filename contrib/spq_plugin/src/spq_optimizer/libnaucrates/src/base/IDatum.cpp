...
FORCE_GENERATE_DBGSTR(spqmd::IDatum);
...
IDatum::StatsAreEqual(const IDatum *datum) const
{
	...
	SPQOS_ASSERT(is_double_comparison || is_lint_comparison);
	...
	SPQOS_ASSERT(is_double_comparison);
...
IDatum::StatsAreComparable(const IDatum *datum) const
{
	...
	if (!is_types_match)
	{
		BOOL is_time_comparison =
			CMDTypeGenericSPQDB::IsTimeRelatedType(this->MDId()) &&
			CMDTypeGenericSPQDB::IsTimeRelatedType(datum->MDId());
