...
{
	SPQOS_ASSERT(rel_stats_mdid->IsValid());
...
CDXLRelStats::CreateDXLDummyRelStats(CMemoryPool *mp, IMDId *mdid)
{
	...
	str = SPQOS_NEW(mp) CWStringDynamic(mp, rel_stats_mdid->GetBuffer());
	CAutoP<CMDName> mdname;
	mdname = SPQOS_NEW(mp) CMDName(mp, str.Value());
	CAutoRef<CDXLRelStats> rel_stats_dxl;
	rel_stats_dxl = SPQOS_NEW(mp) CDXLRelStats(
