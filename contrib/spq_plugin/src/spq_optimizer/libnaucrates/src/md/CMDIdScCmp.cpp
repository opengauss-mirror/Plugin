...
//
//---------------------------------------------------------------------------
CMDIdScCmp::CMDIdScCmp(CMDIdSPQDB *left_mdid, CMDIdSPQDB *right_mdid,
...
{
	SPQOS_ASSERT(left_mdid->IsValid());
	SPQOS_ASSERT(right_mdid->IsValid());
	SPQOS_ASSERT(IMDType::EcmptOther != cmp_type);

	SPQOS_ASSERT(left_mdid->Sysid().Equals(right_mdid->Sysid()));
...
CMDIdScCmp::Serialize()
{
	...
	m_str.AppendFormat(SPQOS_WSZ_LIT("%d.%d.%d.%d;%d.%d.%d;%d"), MdidType(),
...
{
	return spqos::CombineHashes(MdidType(),
							   spqos::CombineHashes(m_mdid_left->HashValue(),
