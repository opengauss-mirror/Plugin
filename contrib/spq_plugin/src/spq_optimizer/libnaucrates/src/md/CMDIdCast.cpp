...
//
//---------------------------------------------------------------------------
CMDIdCast::CMDIdCast(CMDIdSPQDB *mdid_src, CMDIdSPQDB *mdid_dest)
	: m_mdid_src(mdid_src),
	  m_mdid_dest(mdid_dest),
	  m_str(m_mdid_buffer, SPQOS_ARRAY_SIZE(m_mdid_buffer))
{
	SPQOS_ASSERT(mdid_src->IsValid());
	SPQOS_ASSERT(mdid_dest->IsValid());
...
CMDIdCast::Serialize()
{
	...
	m_str.AppendFormat(SPQOS_WSZ_LIT("%d.%d.%d.%d;%d.%d.%d"), MdidType(),
