...
#define SPQDXL_SERIALIZE_CFA_FREQUENCY 30
...
CXMLSerializer::StartDocument()
{
	SPQOS_ASSERT(m_strstackElems->IsEmpty());
...
{
	SPQOS_ASSERT(NULL != elem_str);

	m_iteration_since_last_abortcheck++;

	if (SPQDXL_SERIALIZE_CFA_FREQUENCY < m_iteration_since_last_abortcheck)
	{
		SPQOS_CHECK_ABORT;
							 ...
							 const CWStringBase *elem_str)
{
	...
	SPQOS_ASSERT(strOpenElem->Equals(elem_str));
...
{
	SPQOS_ASSERT(NULL != pstrAttr);
	SPQOS_ASSERT(NULL != str_value);

	SPQOS_ASSERT(m_fOpenTag);
