									   ...
									   const CHAR *xsd_file_path)
{
	...
	CDXLMemoryManager *memory_manager = SPQOS_NEW(mp) CDXLMemoryManager(mp);
	...
	CParseHandlerManager *parse_handler_mgr =
		SPQOS_NEW(mp) CParseHandlerManager(memory_manager, sax_2_xml_reader);
	...
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiDXLXercesParseError);
		return NULL;
	}
...
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != plan_id);
	SPQOS_ASSERT(NULL != plan_space_size);
	...
	SPQOS_ASSERT(NULL != parse_handler_dxl_wrapper.Value());
	...
	*plan_space_size = parse_handler_dxl_wrapper->GetPlanSpaceSize();
	...
	SPQOS_ASSERT(NULL != root_dxl_node);
									...
									const CHAR *xsd_file_path)
{
	...
	SPQOS_ASSERT(NULL != parse_handler_dxl->GetOutputColumnsDXLArray());
	...
	SPQOS_ASSERT(NULL != cte_producers);
	cte_producers->AddRef();

	CQueryToDXLResult *ptrOutput = SPQOS_NEW(mp) CQueryToDXLResult(
						  ...
						  const CHAR *xsd_file_path)
{
	...
	SPQOS_ASSERT(1 == mdid_array->Size());
...
CDXLUtils::ParseDXLToOptimizerStatisticObjArray(
	...
	CDXLStatsDerivedRelationArray *dxl_derived_rel_stats_array)
{
	SPQOS_ASSERT(NULL != dxl_derived_rel_stats_array);

	CStatisticsArray *statistics_array = SPQOS_NEW(mp) CStatisticsArray(mp);
	...
	for (ULONG ulIdxRelStat = 0; ulIdxRelStat < ulRelStat; ulIdxRelStat++)
	{
			 ...
			 column_id_idx++)
		{
			...
			column_id_histogram_map->Insert(SPQOS_NEW(mp) ULONG(column_id),
											histogram);
			column_id_width_map->Insert(SPQOS_NEW(mp) ULONG(column_id),
...
CDXLUtils::ParseDXLToBucketsArray(CMemoryPool *mp, CMDAccessor *md_accessor,
								  CDXLStatsDerivedColumn *dxl_derived_col_stats)
{
	CBucketArray *stats_buckets_array = SPQOS_NEW(mp) CBucketArray(mp);
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CPoint *point_lower_bound = SPQOS_NEW(mp) CPoint(datum_lower_bound);

		IDatum *datum_upper_bound =
			GetDatum(mp, md_accessor, dxl_bucket->GetDXLDatumUpper());
		CPoint *point_upper_bound = SPQOS_NEW(mp) CPoint(datum_upper_bound);
...
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != dxl_query_node && NULL != query_output_dxlnode_array);
...
CDXLUtils::SerializeULLONG(CMemoryPool *mp, ULLONG value)
{
	...
	CAutoP<CWStringDynamic> string_var(SPQOS_NEW(mp) CWStringDynamic(mp));
...
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != imd_obj_array);
							 ...
							 BOOL serialize_header_footer, BOOL indentation)
{
	...
	CWStringDynamic *dxl_string = SPQOS_NEW(mp) CWStringDynamic(mp);
...
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != imd_cache_obj);
...
{
	SPQOS_ASSERT(NULL != xml_serializer);
...
//
//	@doc:
//		Create a SPQOS string object from a Xerces XMLCh* string.
...
{
	SPQOS_ASSERT(NULL != memory_manager);
	SPQOS_ASSERT(NULL != xml_string);
...

	{
		...
		CWStringDynamic *dxl_string = SPQOS_NEW(mp) CWStringDynamic(mp);
		dxl_string->AppendFormat(SPQOS_WSZ_LIT("%s"), sz);
...
)
{
	...
	data_in_byte = (XMLByte *) SPQOS_NEW_ARRAY(mp, XMLByte, len + 1);
	for (ULONG ul = 0; ul < len; ul++)
	{
		SPQOS_ASSERT(xml_string[ul] <= 256 &&
...
//
//	@doc:
//		Create a SPQOS string object from a character array.
//		The function allocates the returned string in the provided memory pool
//		and it is the responsibility of the caller to release it.
//
//---------------------------------------------------------------------------
...
CDXLUtils::CreateDynamicStringFromCharArray(CMemoryPool *mp, const CHAR *c)
{
	...
	CAutoP<CWStringDynamic> string_var(SPQOS_NEW(mp) CWStringDynamic(mp));
	string_var->AppendFormat(SPQOS_WSZ_LIT("%s"), c);
	return string_var.Reset();
}
...
CDXLUtils::CreateMDNameFromCharArray(CMemoryPool *mp, const CHAR *c)
{
	...
	CMDName *md_name = SPQOS_NEW(mp) CMDName(mp, dxl_string);

	// CMDName ctor created a copy of the string
	SPQOS_DELETE(dxl_string);
...
//
//	@doc:
//		Create a SPQOS string object from a Xerces character array.
//		The function allocates the returned string in the provided memory pool
//		and it is the responsibility of the caller to release it.
//
//---------------------------------------------------------------------------
								   ...
								   ULONG length)
{
	...
	SPQOS_ASSERT(NULL != xml_byte_buffer.Rgt());

	// assert that last byte is 0
	SPQOS_ASSERT(0 == xml_byte_buffer[output_length]);
	...
	SPQOS_ASSERT(0 == xml_byte_buffer[new_length]);
									 ...
									 ULONG *length)
{
	...
	data_in_byte = (XMLByte *) SPQOS_NEW_ARRAY(mp, XMLByte, input_length + 1);

	for (XMLSize_t i = 0; i < input_length; i++)
	{
		SPQOS_ASSERT(wc[i] <= 256);
...
CDXLUtils::Serialize(CMemoryPool *mp, const ULongPtr2dArray *array_2D)
{
	...
	CWStringDynamic *keys_buffer = SPQOS_NEW(mp) CWStringDynamic(mp);
	for (ULONG ul = 0; ul < len; ul++)
	{
		ULongPtrArray *pdrspqul = (*array_2D)[ul];
		CWStringDynamic *key_set_string = CDXLUtils::Serialize(mp, pdrspqul);

		keys_buffer->Append(key_set_string);

		if (ul < len - 1)
		{
			keys_buffer->AppendFormat(SPQOS_WSZ_LIT("%ls"), SPQOS_WSZ_LIT(";"));
		}

		SPQOS_DELETE(key_set_string);
	}
										   ...
										   const CharPtrArray *char_ptr_array)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (ul == length - 1)
		{
			...
			dxl_string->AppendFormat(SPQOS_WSZ_LIT("%c"), value);
		}
		else
		{
			dxl_string->AppendFormat(
				SPQOS_WSZ_LIT("%c%ls"), value,
...
CDXLUtils::CreateMultiByteCharStringFromWCString(CMemoryPool *mp,
												 const WCHAR *wc)
{
	SPQOS_ASSERT(NULL != wc);

	ULONG max_length = SPQOS_WSZ_LENGTH(wc) * SPQOS_SIZEOF(WCHAR) + 1;
	CHAR *c = SPQOS_NEW_ARRAY(mp, CHAR, max_length);
...
{
	SPQOS_TRACE_FORMAT("opening file %s", filename);

	CFileReader fr;
	fr.Open(filename);

	ULONG_PTR file_size = (ULONG_PTR) fr.FileSize();
	CAutoRg<CHAR> read_buffer(SPQOS_NEW_ARRAY(mp, CHAR, file_size + 1));

	ULONG_PTR read_bytes =
		fr.ReadBytesToBuffer((BYTE *) read_buffer.Rgt(), file_size);
	fr.Close();

	SPQOS_ASSERT(read_bytes == file_size);
