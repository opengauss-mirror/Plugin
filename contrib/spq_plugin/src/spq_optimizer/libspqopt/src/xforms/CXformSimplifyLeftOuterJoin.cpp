									   ...
									   CExpression *pexpr) const
{
	...
	SPQOS_ASSERT(CUtils::FScalarConstFalse(pexprScalar));

	// extract output columns of inner child
	CColRefArray *colref_array = pexprInner->DeriveOutputColumns()->Pdrspqcr(mp);

	// generate empty constant table with the same columns
	COperator *popCTG = SPQOS_NEW(mp)
		CLogicalConstTableGet(mp, colref_array, SPQOS_NEW(mp) IDatum2dArray(mp));
	pexprResult = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprOuter,
					SPQOS_NEW(mp) CExpression(mp, popCTG), pexprScalar);
