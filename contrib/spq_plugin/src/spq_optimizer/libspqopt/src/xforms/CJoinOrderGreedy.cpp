...
CJoinOrderGreedy::CJoinOrderGreedy(CMemoryPool *pmp,
	...
	: CJoinOrder(pmp, pdrspqexprComponents, pdrspqexprConjuncts,
...
CJoinOrderGreedy::GetStartingJoins()
{
	...
	CJoinOrder::SComponent *pcompBest =
		SPQOS_NEW(m_mp) SComponent(m_mp, NULL /*pexpr*/);

	for (ULONG ul1 = 0; ul1 < m_ulComps; ul1++)
	{
		for (ULONG ul2 = ul1 + 1; ul2 < m_ulComps; ul2++)
		{
			SComponent *comp1 = m_rspqcomp[ul1];
			SComponent *comp2 = m_rspqcomp[ul2];
	...
	SComponent *comp1 = m_rspqcomp[ul1Counter];
	comp1->m_fUsed = true;
	SComponent *comp2 = m_rspqcomp[ul2Counter];
	comp2->m_fUsed = true;
...
CJoinOrderGreedy::PexprExpand()
{
	SPQOS_ASSERT(NULL == m_pcompResult && "join order is already expanded");
	...
	else
	{
		...
		m_pcompResult = SPQOS_NEW(m_mp) SComponent(m_mp, NULL /*pexpr*/);
	}

	// create a bitset for all the unused components
	CBitSet *unused_components_set = SPQOS_NEW(m_mp) CBitSet(m_mp);
	for (ULONG ul = 0; ul < m_ulComps; ul++)
	{
		if (!m_rspqcomp[ul]->m_fUsed)
		{
	...
	while (unused_components_set->Size() > 0)
	{
		...
		ULONG best_comp_idx = spqos::ulong_max;
		...
		if (candidate_comp_set->Size() == 0 || spqos::ulong_max == best_comp_idx)
		{
		...
		if (spqos::ulong_max == best_comp_idx)
		{
		...
		candidate_comp_set->Release();
		SPQOS_ASSERT(spqos::ulong_max != best_comp_idx);
	}
	unused_components_set->Release();
	SPQOS_ASSERT(NULL != m_pcompResult->m_pexpr);
...
CJoinOrderGreedy::PickBestJoin(CBitSet *candidate_comp_set)
{
	...
	while (iter.Advance())
	{
		SComponent *pcompCurrent = m_rspqcomp[iter.Bit()];
	...
	SPQOS_ASSERT(spqos::ulong_max != best_comp_idx);
	SPQOS_ASSERT(NULL != pcompBest);
	SPQOS_ASSERT(!pcompBestComponent->m_fUsed);
...
{
	// iterator over index of edges in m_rspqedge array associated with this component
	CBitSetIter edges_iter(*(m_pcompResult->m_edge_set));
	CBitSet *candidate_component_set = SPQOS_NEW(m_mp) CBitSet(m_mp);

	while (edges_iter.Advance())
	{
		SEdge *edge = m_rspqedge[edges_iter.Bit()];
		if (!edge->m_fUsed)
		{
