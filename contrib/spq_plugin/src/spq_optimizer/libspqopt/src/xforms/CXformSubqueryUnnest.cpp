										  ...
										  BOOL fEnforceCorrelatedApply)
{
	...
	if (SPQOS_FTRACE(EopttraceEnforceCorrelatedExecution) &&
	...
	if (COperator::EopScalarProjectList == pexprScalar->Pop()->Eopid())
	{
		...
		switch (op_id)
		{
			...
			case COperator::EopLogicalGbAgg:
				...
				popGbAgg->Pdrspqcr()->AddRef();
				pexprResult = CUtils::PexprLogicalGbAgg(
					mp, popGbAgg->Pdrspqcr(), pexprNewOuter, pexprResidualScalar,
					popGbAgg->Egbaggtype());
				break;
			...
			case COperator::EopLogicalSequenceProject:
				...
				popSeqPrj->Pdrspqos()->AddRef();
				popSeqPrj->Pdrspqwf()->AddRef();
				pexprResult = CUtils::PexprLogicalSequenceProject(
					mp, popSeqPrj->Pds(), popSeqPrj->Pdrspqos(),
					popSeqPrj->Pdrspqwf(), pexprNewOuter, pexprResidualScalar);
				break;

			default:
				SPQOS_ASSERT(!"Unnesting subqueries for an invalid operator");
				break;
		}
	}
	else
