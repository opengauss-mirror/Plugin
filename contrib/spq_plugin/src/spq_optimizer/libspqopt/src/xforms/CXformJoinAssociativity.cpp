...
#define SPQOPT_MAX_JOIN_DEPTH_FOR_ASSOCIATIVITY 20
#define SPQOPT_MAX_JOIN_RIGHT_CHILD_DEPTH_FOR_ASSOCIATIVITY 1
...
void
CXformJoinAssociativity::CreatePredicates(
	CMemoryPool *mp, CExpression *pexpr, CExpressionArray *pdrspqexprLower,
	CExpressionArray *pdrspqexprUpper) const
{
	...
	CExpressionArray *pdrspqexprJoins = SPQOS_NEW(mp) CExpressionArray(mp);

	pexprLeft->AddRef();
	pdrspqexprJoins->Append(pexprLeft);

	pexpr->AddRef();
	pdrspqexprJoins->Append(pexpr);

	// columns for new lower join
	CColRefSet *pcrsLower = SPQOS_NEW(mp) CColRefSet(mp);
	...
	CExpressionArray *pdrspqexprOrig = SPQOS_NEW(mp) CExpressionArray(mp);

	for (ULONG ul = 0; ul < 2; ul++)
	{
		CExpressionArray *pdrspqexprPreds = CPredicateUtils::PdrspqexprConjuncts(
			mp, (*(*pdrspqexprJoins)[ul])[2]);
		ULONG length = pdrspqexprPreds->Size();
		for (ULONG ulConj = 0; ulConj < length; ulConj++)
		{
			CExpression *pexprConj = (*pdrspqexprPreds)[ulConj];
			pexprConj->AddRef();

			pdrspqexprOrig->Append(pexprConj);
		}
		pdrspqexprPreds->Release();
	}

	// divvy up conjuncts for upper and lower join
	ULONG ulConj = pdrspqexprOrig->Size();
	for (ULONG ul = 0; ul < ulConj; ul++)
	{
		CExpression *pexprPred = (*pdrspqexprOrig)[ul];
		CColRefSet *pcrs = pexprPred->DeriveUsedColumns();

		pexprPred->AddRef();
		if (pcrsLower->ContainsAll(pcrs))
		{
			pdrspqexprLower->Append(pexprPred);
		}
		else
		{
			pdrspqexprUpper->Append(pexprPred);
		}
	}

	// No predicates indicate a cross join. And for that, ORCA expects
	// predicate to be a scalar const "true".
	if (pdrspqexprLower->Size() == 0)
	{
		CExpression *pexprCrossLowerJoinPred =
			CUtils::PexprScalarConstBool(mp, true, false);
		pdrspqexprLower->Append(pexprCrossLowerJoinPred);
	}

	// Same for upper predicates
	if (pdrspqexprUpper->Size() == 0)
	{
		CExpression *pexprCrossUpperJoinPred =
			CUtils::PexprScalarConstBool(mp, true, false);
		pdrspqexprUpper->Append(pexprCrossUpperJoinPred);
	}

	// clean up
	pcrsLower->Release();
	pdrspqexprOrig->Release();

	pdrspqexprJoins->Release();
}
...
{
	if (SPQOPT_MAX_JOIN_DEPTH_FOR_ASSOCIATIVITY <
			exprhdl
				.DeriveJoinDepth() ||  // disallow xform beyond max join depth
		SPQOPT_MAX_JOIN_RIGHT_CHILD_DEPTH_FOR_ASSOCIATIVITY <
								   ...
								   CExpression *pexpr) const
{
	...
	CExpressionArray *pdrspqexprLower = SPQOS_NEW(mp) CExpressionArray(mp);
	CExpressionArray *pdrspqexprUpper = SPQOS_NEW(mp) CExpressionArray(mp);
	CreatePredicates(mp, pexpr, pdrspqexprLower, pdrspqexprUpper);

	SPQOS_ASSERT(pdrspqexprLower->Size() > 0);
	...
	BOOL fOutputLeftIsCrossJoin =
		(1 == pdrspqexprLower->Size() &&
		 CUtils::FScalarConstTrue((*pdrspqexprLower)[0]));
	...
	if (!fOutputLeftIsCrossJoin || fInputLeftIsCrossJoin)
	{
		...
		CExpression *pexprBottomJoin =
			CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
				...
				CPredicateUtils::PexprConjunction(mp, pdrspqexprLower));

		CExpression *pexprResult = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
			mp, pexprBottomJoin, pexprLeftRight,
			CPredicateUtils::PexprConjunction(mp, pdrspqexprUpper));
	...
	{
		pdrspqexprLower->Release();
		pdrspqexprUpper->Release();
	}
}
