									 ...
									 CExpressionArray *pdrspqexprCorrelations)
{
	...
	SPQOS_ASSERT(NULL != pdrspqexprChildren);
	SPQOS_ASSERT(NULL != pdrspqexprCorrelations);
	...
	SPQOS_ASSERT_IMP(semi_join, 2 == pdrspqexprChildren->Size());

	if (semi_join)
	{
		return CPredicateUtils::FValidSemiJoinCorrelations(
			mp, (*pdrspqexprChildren)[0], (*pdrspqexprChildren)[1],
			pdrspqexprCorrelations);
	}
						...
						CColRefSet *outerRefsToRemove)
{
	...
	SPQOS_ASSERT_IMP(CUtils::FApply(pexpr->Pop()),
					CUtils::FCorrelatedApply(pexpr->Pop()) &&
	...
	BOOL fSuccess =
		FProcessOperator(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
						 pdrspqexprCorrelations, outerRefsToRemove);

	// in case of success make sure there are no outer references left
	SPQOS_ASSERT_IMP(fSuccess,
...
CDecorrelator::FProcessPredicate(
	...
	CExpression *pexprLogical,	// logical parent of predicate tree
	CExpression *pexprScalar, BOOL fEqualityOnly,
	CExpression **ppexprDecorrelated, CExpressionArray *pdrspqexprCorrelations,
	CColRefSet *outerRefsToRemove)
{
	SPQOS_ASSERT(pexprLogical->Pop()->FLogical());
	SPQOS_ASSERT(pexprScalar->Pop()->FScalar());
	...
	*ppexprDecorrelated = NULL;
	...
	CExpressionArray *pdrspqexprConj =
		CPredicateUtils::PdrspqexprConjuncts(mp, pexprScalar);
	CExpressionArray *pdrspqexprResiduals = SPQOS_NEW(mp) CExpressionArray(mp);
	BOOL fSuccess = true;

	// divvy up the predicates in residuals (w/ no outer ref) and correlations (w/ outer refs)
	ULONG length = pdrspqexprConj->Size();
	for (ULONG ul = 0; ul < length && fSuccess; ul++)
	{
		CExpression *pexprConj = (*pdrspqexprConj)[ul];
		...
		if (outerRefsToRemove->IsDisjoint(pcrsUsed))
		{
			// no outer ref
			pexprConj->AddRef();
			pdrspqexprResiduals->Append(pexprConj);
		...
		{
			pexprConj->AddRef();
			pdrspqexprCorrelations->Append(pexprConj);
		}
	}

	pdrspqexprConj->Release();

	if (!fSuccess || 0 == pdrspqexprResiduals->Size())
	{
		// clean up
		pdrspqexprResiduals->Release();
	}
	else
	{
		// residuals become new predicate
		*ppexprDecorrelated =
			CPredicateUtils::PexprConjunction(mp, pdrspqexprResiduals);
	}
							  ...
							  CColRefSet *outerRefsToRemove)
{
	...
	BOOL fSuccess = FProcessPredicate(mp, pexpr, (*pexpr)[1], fEqualityOnly,
									  &pexprPredicate, pdrspqexprCorrelations,
									  outerRefsToRemove);
	...
	if (fSuccess)
	{
		if (NULL != pexprPredicate)
		{
			...
			*ppexprDecorrelated = SPQOS_NEW(mp)
							 ...
							 CColRefSet *outerRefsToRemove)
{
	...
	pdrspqexprCorrelations->AddRef();
	CExpression *pexprTemp =
		CPredicateUtils::PexprConjunction(mp, pdrspqexprCorrelations);
	CColRefSet *pcrs =
		SPQOS_NEW(mp) CColRefSet(mp, *(pexprTemp->DeriveUsedColumns()));

	// Get the columns from pexprRelational that are referenced in the pulled-up correlated
	// "=" predicates in pdrspqexprCorrelations and add them to the grouping columns.
	...
	pcrs->Include(popAggOriginal->Pdrspqcr());
	...
	CLogicalGbAgg *popAgg = SPQOS_NEW(mp)
		CLogicalGbAgg(mp, colref_array, popAggOriginal->Egbaggtype());
	*ppexprDecorrelated =
		SPQOS_NEW(mp) CExpression(mp, popAgg, pexprRelational, pexprProjList);
...
CDecorrelator::FProcessJoin(CMemoryPool *mp, CExpression *pexpr,
							...
							CColRefSet *outerRefsToRemove)
{
	SPQOS_ASSERT(CUtils::FLogicalJoin(pexpr->Pop()) ||
				CUtils::FApply(pexpr->Pop()));

	ULONG arity = pexpr->Arity();
	CExpressionArray *pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp, arity);
	...
	if (!FPullableCorrelations(mp, pexpr, pdrspqexpr, pdrspqexprCorrelations))
	{
		pdrspqexpr->Release();
	...
	if (fSuccess)
	{
		...
		pdrspqexpr->Append(pexprPredicate);

		COperator *pop = pexpr->Pop();
		pop->AddRef();
		*ppexprDecorrelated = SPQOS_NEW(mp) CExpression(mp, pop, pdrspqexpr);
	}
	else
	{
		pdrspqexpr->Release();
							  ...
							  CColRefSet *outerRefsToRemove)
{
	...
	COperator *pop = pexpr->Pop();
	SPQOS_ASSERT(COperator::EopLogicalAssert == pop->Eopid());
	...
	pexprScalar->AddRef();
	*ppexprDecorrelated =
		SPQOS_NEW(mp) CExpression(mp, pop, pexprRelational, pexprScalar);
								 ...
								 CColRefSet *outerRefsToRemove)
{
	...
	COperator *pop = pexpr->Pop();
	SPQOS_ASSERT(COperator::EopLogicalMaxOneRow == pop->Eopid());
	...
	// assemble new project
	pop->AddRef();
	*ppexprDecorrelated = SPQOS_NEW(mp) CExpression(mp, pop, pexprRelational);
							   ...
							   CColRefSet *outerRefsToRemove)
{
	...
	SPQOS_ASSERT(COperator::EopLogicalProject == op_id ||
	...
	*ppexprDecorrelated =
		SPQOS_NEW(mp) CExpression(mp, pop, pexprRelational, pexprPrjList);
