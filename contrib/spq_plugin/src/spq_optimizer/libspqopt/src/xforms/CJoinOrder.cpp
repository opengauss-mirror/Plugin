...
CJoinOrder::SComponent::SComponent(CMemoryPool *mp, CExpression *pexpr,
	...
	: m_pbs(NULL),
	  ...
	  m_position(position)
{
	m_pbs = SPQOS_NEW(mp) CBitSet(mp);
	m_edge_set = SPQOS_NEW(mp) CBitSet(mp);
	SPQOS_ASSERT_IMP(EpSentinel != m_position,
...
CJoinOrder::SComponent::OsPrint(IOstream &os) const
{
	...
	{
		SPQOS_ASSERT(m_position != EpSentinel);
		os << "Parent LOJ id: ";
	  ...
	  m_include_loj_childs(include_loj_childs)
{
	...
	// +--CScalarCmp (=)
	//
	// In above case the pdrspqexpr comes with two elements in it:
	//  - CLogicalGet "t1"
	//  - CLogicalLeftOuterJoin
	...
	m_rspqcomp = SPQOS_NEW_ARRAY(mp, Pcomp, m_ulComps);

	m_ulEdges = inner_join_conjuncts->Size() + num_of_lojs;
	m_rspqedge = SPQOS_NEW_ARRAY(mp, Pedge, m_ulEdges);
	...
	for (ULONG ul = 0; ul < inner_join_conjuncts->Size(); ul++)
	{
		...
		pexprEdge->AddRef();
		m_rspqedge[ul] = SPQOS_NEW(mp) SEdge(mp, pexprEdge, 0 /* not an LOJ */);
	}
	...
	for (ULONG ul = 0; ul < num_of_nary_children; ul++)
	{
			...
			COperator::EopLogicalLeftOuterJoin == expr->Pop()->Eopid())
		{
			...
			m_rspqedge[edge_idx] =
				SPQOS_NEW(mp) SEdge(mp, scalar_expr, 1 /* is an LOJ */);

			m_rspqcomp[comp_num]->m_edge_set->ExchangeSet(edge_idx);
			m_rspqedge[edge_idx]->m_pbs->ExchangeSet(comp_num);

			m_rspqcomp[comp_num + 1]->m_edge_set->ExchangeSet(edge_idx);
			m_rspqedge[edge_idx]->m_pbs->ExchangeSet(comp_num + 1);
	  ...
	  m_include_loj_childs(false)  // not used by CXformExpandNAryJoinDPv2
{
	...
	for (ULONG ul2 = 0; ul2 < onPreds->Size(); ul2++)
	{
		...
		SPQOS_ASSERT(0 < logicalChildIndex);

		pexprEdge->AddRef();
		m_rspqedge[ul2 + innerJoinEdges] =
			SPQOS_NEW(mp) SEdge(mp, pexprEdge, lojId);
		...
		m_rspqedge[ul2 + innerJoinEdges]->m_pbs->ExchangeSet(logicalChildIndex);
	}
...
CJoinOrder::~CJoinOrder()
{
	...
	{
		m_rspqcomp[ul]->Release();
	}
	SPQOS_DELETE_ARRAY(m_rspqcomp);

	for (ULONG ul = 0; ul < m_ulEdges; ul++)
	{
		m_rspqedge[ul]->Release();
	}
	SPQOS_DELETE_ARRAY(m_rspqedge);
}
...
CJoinOrder::ComputeEdgeCover()
{
	...
	{
		CExpression *pexpr = m_rspqedge[ulEdge]->m_pexpr;
		CColRefSet *pcrsUsed = pexpr->DeriveUsedColumns();

		for (ULONG ulComp = 0; ulComp < m_ulComps; ulComp++)
		{
			CExpression *pexprComp = m_rspqcomp[ulComp]->m_pexpr;
			CColRefSet *pcrsOutput = pexprComp->DeriveOutputColumns();

			if (!pcrsUsed->IsDisjoint(pcrsOutput))
			{
				(void) m_rspqcomp[ulComp]->m_edge_set->ExchangeSet(ulEdge);
				(void) m_rspqedge[ulEdge]->m_pbs->ExchangeSet(ulComp);
			}
		}
	}
}
...
{
	SPQOS_ASSERT(IsValidJoinCombination(comp1, comp2));
	CBitSet *pbs = SPQOS_NEW(m_mp) CBitSet(m_mp);
	CBitSet *edge_set = SPQOS_NEW(m_mp) CBitSet(m_mp);
	...
	CExpressionArray *loj_conjuncts = SPQOS_NEW(m_mp) CExpressionArray(m_mp);
	CExpressionArray *other_conjuncts = SPQOS_NEW(m_mp) CExpressionArray(m_mp);
	...
	if (NULL == pexprChild1)
	{
		...
		SPQOS_ASSERT(loj_conjuncts->Size() == 0);
	...
	else
	{
		...
		else
		{
				...
				comp2->ParentLojId() > NON_LOJ_DEFAULT_ID)
			{
				...
				SPQOS_ASSERT_IMP(comp1->ParentLojId() > NON_LOJ_DEFAULT_ID,
								comp2->ParentLojId() == 0);
				SPQOS_ASSERT_IMP(comp2->ParentLojId() > NON_LOJ_DEFAULT_ID,
								comp1->ParentLojId() == 0);
				...
				SPQOS_ASSERT(CJoinOrder::EpLeft == position);
				...
				SPQOS_ASSERT(NON_LOJ_DEFAULT_ID < parent_loj_id);
			}
	...
	SPQOS_ASSERT_IMP(NON_LOJ_DEFAULT_ID < parent_loj_id, EpLeft == position);
	SComponent *join_comp = SPQOS_NEW(m_mp)
...
CJoinOrder::OsPrint(IOstream &os) const
{
	...
	for (ULONG ul = 0; ul < m_ulComps; ul++)
	{
		os << (void *) m_rspqcomp[ul] << " - " << std::endl;
		m_rspqcomp[ul]->OsPrint(os);
	}
...
CJoinOrder::IsValidJoinCombination(SComponent *comp1, SComponent *comp2) const
{
		...
		NON_LOJ_DEFAULT_ID < comp2_parent_loj_id)
	{
		...
		if (comp1_parent_loj_id == comp2_parent_loj_id)
		{
			...
			SPQOS_ASSERT(comp1_position != EpSentinel &&
...
CJoinOrder::MarkUsedEdges(SComponent *pcomponent)
{
	...
	{
		SEdge *pedge = m_rspqedge[edges_iter.Bit()];
		if (pedge->m_fUsed)
		{
			continue;
		}
...
{
	expr->AddRef();
	SComponent *comp = SPQOS_NEW(mp) SComponent(mp, expr, loj_id, position);
	m_rspqcomp[comp_num] = comp;
	// component always covers itself
	(void) m_rspqcomp[comp_num]->m_pbs->ExchangeSet(comp_num);
}
// EOF
