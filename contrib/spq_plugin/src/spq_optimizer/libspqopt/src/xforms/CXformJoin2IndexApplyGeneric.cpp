	...
	CTableDescriptor *ptabDesc, const CColRefSet *pcrsDist) const
{
	...
	CColRefSet *pcrsInnerRefs = SPQOS_NEW(mp) CColRefSet(mp, *pcrsScalarExpr);
		...
		switch (pexprCurrInnerChild->Pop()->Eopid())
		{
			...
			case COperator::EopLogicalProject:
				...
				// be used for index predicates.
				{
					...
					CColRefSet *joinPredUsedCols = SPQOS_NEW(mp)
						...
						pexprCurrInnerChild->Pop()->Eopid())
					{
						...
						SPQOS_ASSERT(NULL != grbyAggOp);
						if (NULL != grbyAggOp->Pdrspqcr() &&
							0 < grbyAggOp->Pdrspqcr()->Size())
						{
							...
							CColRefSet *groupingCols = SPQOS_NEW(mp)
								CColRefSet(mp, grbyAggOp->Pdrspqcr());
