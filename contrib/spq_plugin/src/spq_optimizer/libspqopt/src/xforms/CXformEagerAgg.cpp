...
{
	if (SPQOS_FTRACE(EopttraceEnableEagerAgg) &&
						  ...
						  CExpression *agg_expr) const
{
	...
	SPQOS_ASSERT(FPromising(pxfctxt->Pmp(), this, agg_expr));
	SPQOS_ASSERT(FCheckPattern(agg_expr));
	...
	//		WHERE t1.j1 = t2.j2
	//		GROUP BY t1.g1;
	CColRefSet *push_down_gb_cols = SPQOS_NEW(mp)
	...
	CColRefArray *push_down_gb_col_array = push_down_gb_cols->Pdrspqcr(mp);
	join_outer_child_expr->AddRef();
	CExpression *lower_agg_expr = SPQOS_NEW(mp)
		CExpression(mp,
					SPQOS_NEW(mp) CLogicalGbAgg(mp, push_down_gb_col_array,
	...
	CExpression *new_join_expr =
		SPQOS_NEW(mp) CExpression(mp, join_op, lower_agg_expr,
								 join_inner_child_expr, join_condition_expr);

	// upper expression as a global aggregate
	CColRefArray *grouping_col_array = grouping_cols->Pdrspqcr(mp);
	CExpression *upper_agg_expr = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp)
...
) const
{
	...
	CExpressionArray *lower_proj_elem_array = SPQOS_NEW(mp) CExpressionArray(mp);
	CExpressionArray *upper_proj_elem_array = SPQOS_NEW(mp) CExpressionArray(mp);
	...
	*lower_proj_list = SPQOS_NEW(mp) CExpression(
		mp, SPQOS_NEW(mp) CScalarProjectList(mp), lower_proj_elem_array);

	*upper_proj_list = SPQOS_NEW(mp) CExpression(
		mp, SPQOS_NEW(mp) CScalarProjectList(mp), upper_proj_elem_array);
}
...
) const
{
	...
	CExpression *lower_agg_expr =
		SPQOS_NEW(mp) CExpression(mp, lower_agg_func, agg_arg_array);
...
) const
{
	...
	CExpressionArray *args = SPQOS_NEW(mp) CExpressionArray(mp);
	args->Append(CUtils::PexprScalarIdent(mp, lower_colref));

	CExpression *upper_agg_expr = SPQOS_NEW(mp)
