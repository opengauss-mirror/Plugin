...
//		expression is generated instead, which results in a correlated join (subplan in SPQDB)
//
									   ...
									   CExpression **ppexprResidualScalar)
{
	...
	SPQOS_ASSERT(pexprOuter->Pop()->FLogical());
	SPQOS_ASSERT(NULL != pexprScalar);
	SPQOS_ASSERT(pexprScalar->Pop()->FScalar());
	SPQOS_ASSERT(NULL != ppexprNewOuter);
	SPQOS_ASSERT(NULL != ppexprResidualScalar);
}
#endif	// SPQOS_DEBUG
									...
									CSubqueryHandler::ESubqueryCtxt esqctxt)
{
	...
	SPQOS_ASSERT(NULL != pexprNewScalar);
...
CSubqueryHandler::FProjectCountSubquery(CExpression *pexprSubquery,
										CColRef *pcrCount)
{
	SPQOS_ASSERT(NULL != pexprSubquery);
	SPQOS_ASSERT(COperator::EopScalarSubquery == pexprSubquery->Pop()->Eopid());
	SPQOS_ASSERT(COperator::EopLogicalProject ==
				(*pexprSubquery)[0]->Pop()->Eopid());
	SPQOS_ASSERT(NULL != pcrCount);
#ifdef SPQOS_DEBUG
	CColRef *colref = NULL;
	SPQOS_ASSERT(CUtils::FHasCountAgg((*pexprSubquery)[0], &colref));
	SPQOS_ASSERT(colref == pcrCount);
#endif	// SPQOS_DEBUG
...
{
	SPQOS_ASSERT(NULL != pexprSubquery);
	SPQOS_ASSERT(CUtils::FSubquery(pexprSubquery->Pop()));
	SPQOS_ASSERT(NULL != pexprOuter);
	...
	SSubqueryDesc *psd = SPQOS_NEW(mp) SSubqueryDesc();
										...
										CExpression **ppexprResidualScalar)
{
	...
	if (psd->m_fProjectCount && !psd->m_fCorrelatedExecution)
	{
		...
		CScalarSubquery *popInnerSubq = SPQOS_NEW(m_mp) CScalarSubquery(
			m_mp, psd->m_pcrCountAgg, false /*fGeneratedByExist*/,
			false /*fGeneratedByQuantified*/);
		pexprGbAgg->AddRef();
		CExpression *pexprNewSubq =
			SPQOS_NEW(m_mp) CExpression(m_mp, popInnerSubq, pexprGbAgg);
		...
		if (fSuccess)
		{
			...
			*ppexprNewOuter = SPQOS_NEW(m_mp) CExpression(
	...
	CExpression **ppexprNewOuter, CExpression **ppexprResidualScalar)
{
	...
	AssertValidArguments(mp, pexprOuter, pexprSubquery, ppexprNewOuter,
						 ppexprResidualScalar);
#endif	// SPQOS_DEBUG
	SPQOS_ASSERT(NULL != psd);
	SPQOS_ASSERT(psd->m_fCorrelatedExecution || fEnforceCorrelatedApply);
	...
	BOOL fUseMaxOneRow = !fEnforceCorrelatedApply && !psd->m_fHasOuterRefs &&
						 SPQOPT_FENABLED_XFORM(CXform::ExfMaxOneRow2Assert);
	...
	if (psd->m_fValueSubquery)
	{
		if (fUseMaxOneRow)
		{
			...
			CExpression *pexprMaxOneRow = SPQOS_NEW(mp)
				CExpression(mp, SPQOS_NEW(mp) CLogicalMaxOneRow(mp), pexprInner);
	...
	SPQOS_ASSERT(EsqctxtFilter == esqctxt);
								   ...
								   BOOL *useNotNullableInnerOpt)
{
	...
	*useNotNullableInnerOpt = false;
	...
	{
		predToUse = SPQOS_NEW(mp) CExpression(
			mp,
			SPQOS_NEW(mp)
	...
	CExpression **ppexprNewOuter, CExpression **ppexprResidualScalar)
{
	...
	*ppexprNewOuter = pexprPrj;
	...
	if (fGeneratedByQuantified ||
		(fHasCountAggMatchingColumn && 0 == pgbAgg->Pdrspqcr()->Size()))
	{
		CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
		const IMDTypeInt8 *pmdtypeint8 = md_accessor->PtMDType<IMDTypeInt8>();
		IMDId *pmdidInt8 = pmdtypeint8->MDId();
		pmdidInt8->AddRef();
		CExpression *pexprCoalesce = SPQOS_NEW(mp)
			CExpression(mp, SPQOS_NEW(mp) CScalarCoalesce(mp, pmdidInt8),
		...
		if (fGeneratedByQuantified)
		{
			...
			pmdidInt8->AddRef();
			*ppexprResidualScalar = SPQOS_NEW(mp) CExpression(
				mp, SPQOS_NEW(mp) CScalarIf(mp, pmdidInt8),
...
CSubqueryHandler::FCreateGrpCols(
	...
	CColRefArray **ppdrspqcr,  // output: constructed grouping columns
	BOOL *pfGbOnInner		  // output: is Gb created on inner expression
)
{
	SPQOS_ASSERT(NULL != pexprOuter);
	SPQOS_ASSERT(NULL != pexprInner);
	SPQOS_ASSERT(NULL != ppdrspqcr);
	SPQOS_ASSERT(NULL != pfGbOnInner);
	...
	{
		SPQOS_ASSERT(COperator::EopLogicalSelect == pexprInner->Pop()->Eopid() &&
	...
	if (fGbOnInner)
	{
		...
		colref_array = pcrsGb->Pdrspqcr(mp);
		pcrsGb->Release();
	}
	else
	{
		...
		CColRefArray *pdrspqcrSystemCols =
			COptCtxt::PoctxtFromTLS()->PdrspqcrSystemCols();
		if (NULL != pdrspqcrSystemCols && 0 < pdrspqcrSystemCols->Size())
		{
			CColRefSet *pcrsSystemCols =
				SPQOS_NEW(mp) CColRefSet(mp, pdrspqcrSystemCols);
		...
		colref_array = CUtils::PdrspqcrGroupingKey(mp, pexprOuter, &pdrspqcrKey);
		pdrspqcrKey->Release();	// key is not used here
	}

	*ppdrspqcr = colref_array;
									...
									CColRef **pcrCount, CColRef **pcrSum)
{
	...
	CExpression *pexprGb = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp) CLogicalGbAgg(mp, colref_array,
...
//
//		+-Select
//			+--Gb[T.i, T.ctid, T.spq_segment_id], C3 = count(C0), C4 = sum(C1)
//			|	+---Project
//			|		|---LOApply
//			|		|	|---T
	...
	CExpression **ppexprResidualScalar)
{
	...
	SPQOS_ASSERT(COperator::EopScalarSubqueryAny == eopidSubq ||
	...
	pdrspqcrInner->Append(pcrBool);
	pdrspqcrInner->Append(colref);

	*ppexprNewOuter =
		CUtils::PexprLogicalApply<CLogicalLeftOuterCorrelatedApply>(
			mp, pexprOuter, pexprProjectConstTrue, pdrspqcrInner, eopidSubq,
			pexprPredicate);
...
{
	COperator::EOperatorId eopidSubq = pexprSubquery->Pop()->Eopid();
	SPQOS_ASSERT(COperator::EopScalarSubqueryExists == eopidSubq ||
...
#endif	// SPQOS_DEBUG
	SPQOS_ASSERT(fExistential || fQuantified);
								...
								BOOL useNotNullableInnerOpt)
{
	...
	SPQOS_ASSERT(fExistential || fQuantified);
	SPQOS_ASSERT_IMP(fExistential, NULL != pcrBool);
	SPQOS_ASSERT_IMP(fQuantified, NULL != pcrSum && NULL != pcrCount);
	SPQOS_ASSERT_IMP(useNotNullableInnerOpt, !fExistential);
	...
	if (fExistential)
	{
		...
		mdid->AddRef();
		return SPQOS_NEW(mp)
			CExpression(mp, SPQOS_NEW(mp) CScalarIf(mp, mdid), pexprIsNotNull,
	...
	CExpression *pexprScalarIf = SPQOS_NEW(mp) CExpression(
		mp, SPQOS_NEW(mp) CScalarIf(mp, mdid), pexprEquality,
	...
	mdid->AddRef();
	CExpression *result = SPQOS_NEW(mp) CExpression(
		mp, SPQOS_NEW(mp) CScalarIf(mp, mdid), pexprRowCountGreaterZero,
	...
	if (useNotNullableInnerOpt)
	{
		...
		pexprScalar->AddRef();
		mdid->AddRef();
		result = SPQOS_NEW(mp) CExpression(
			mp, SPQOS_NEW(mp) CScalarIf(mp, mdid),
	...
	CExpression **ppexprNewOuter, CExpression **ppexprResidualScalar)
{
	...
	AssertValidArguments(mp, pexprOuter, pexprSubquery, ppexprNewOuter,
						 ppexprResidualScalar);
	SPQOS_ASSERT(COperator::EopScalarSubqueryExists == op_id ||
				COperator::EopScalarSubqueryNotExists == op_id);
	SPQOS_ASSERT(op_id == pexprSubquery->Pop()->Eopid());
#endif	// SPQOS_DEBUG
									...
									CExpression **ppexprResidualScalar)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (pexprScalarChild->DeriveHasSubquery())
		{
			...
			SPQOS_ASSERT(NULL != pexprNewLogical);

			// update current logical expression based on recursion results
			pexprCurrentOuter = pexprNewLogical;
		}
		pdrspqexpr->Append(pexprNewScalar);
	}

	*ppexprNewOuter = pexprCurrentOuter;
	COperator *pop = pexprScalar->Pop();
	pop->AddRef();
	*ppexprResidualScalar = SPQOS_NEW(mp) CExpression(mp, pop, pdrspqexpr);
	...
	switch (op_id)
	{
		...
		default:
			SPQOS_RAISE(spqopt::ExmaSPQOPT, spqopt::ExmiUnexpectedOp,
					   SPQOS_WSZ_LIT("Subquery in unexpected context"));
	}
	...
	if (fSuccess)
	{
		...
		*ppexprResidualScalar = pexprPruned;
		...
		CExpressionArray *pdrspqexpr =
			CPredicateUtils::PdrspqexprConjuncts(mp, *ppexprResidualScalar);
		(*ppexprResidualScalar)->Release();
		*ppexprResidualScalar =
