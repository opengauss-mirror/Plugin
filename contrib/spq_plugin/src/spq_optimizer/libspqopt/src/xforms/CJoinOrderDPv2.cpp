...
#define SPQOPT_DPV2_JOIN_ORDERING_TOPK 10
...
#define SPQOPT_DPV2_CROSS_JOIN_DEFAULT_PENALTY 1024
// prohibitively high penalty for cross products when in GreedyAvoidXProd
#define SPQOPT_DPV2_CROSS_JOIN_GREEDY_PENALTY 1e9

// from cost model used during optimization in CCostModelParamsSPQDB.cpp
...
CJoinOrderDPv2::CJoinOrderDPv2(CMemoryPool *mp,
	...
	: CJoinOrder(mp, pdrspqexprAtoms, innerJoinConjuncts, onPredConjuncts,
	  ...
	  m_cross_prod_penalty(SPQOPT_DPV2_CROSS_JOIN_DEFAULT_PENALTY),
	  m_outer_refs(outerRefs)
{
	m_join_levels = SPQOS_NEW(mp) DPv2Levels(mp, m_ulComps + 1);
	...
	for (ULONG l = 0; l <= m_ulComps; l++)
	{
		m_join_levels->Append(
			SPQOS_NEW(mp) SLevelInfo(l, SPQOS_NEW(mp) SGroupInfoArray(mp)));
	}

	m_bitset_to_group_info_map = SPQOS_NEW(mp) BitSetToGroupInfoMap(mp);

	// Contains top k expressions for a general DP algorithm, without considering cost of motions/PS
	m_top_k_expressions =
		SPQOS_NEW(mp) CKHeap<SExpressionInfoArray, SExpressionInfo>(
			mp, SPQOPT_DPV2_JOIN_ORDERING_TOPK);
	...
	if (0 < m_on_pred_conjuncts->Size())
	{
		...
		m_non_inner_join_dependencies =
			SPQOS_NEW(mp) CBitSetArray(mp, numNonInnerJoins);
		for (ULONG ul = 0; ul < numNonInnerJoins; ul++)
		{
			m_non_inner_join_dependencies->Append(SPQOS_NEW(mp) CBitSet(mp));
		}
		...
		for (ULONG en = 0; en < m_ulEdges; en++)
		{
			...
			if (0 < pedge->m_loj_num)
			{
				...
				SPQOS_ASSERT(0 < logicalChildNum);
							...
							CDouble join_cardinality)
{
	...
	{
		SPQOS_ASSERT(expr_info->m_right_child_expr.IsValid());
							...
							SExpressionProperties &result_properties)
{
	...
	else
	{
		...
		SPQOS_ASSERT(NULL != scalar_expr && NULL != required_on_left);
	...
	return SPQOS_NEW(m_mp) SExpressionInfo(m_mp, join_expr, left_child_expr,
										 ...
										 SExpressionProperties &props)
{
	...
	for (ULONG ul = 0; ul < group_info->m_best_expr_info_array->Size(); ul++)
	{
		...
		{
			if (spqos::ulong_max == best_ix || expr_info->GetCost() < best_cost)
			{
...
CJoinOrderDPv2::PopulateExpressionToEdgeMapIfNeeded()
{
	...
	if (!populate && NULL != m_child_pred_indexes)
	{
		...
		CBitSet *loj_right_children = SPQOS_NEW(m_mp) CBitSet(m_mp);
	...
	if (populate)
	{
		m_expression_to_edge_map = SPQOS_NEW(m_mp) ExpressionToEdgeMap(m_mp);

		for (ULONG en2 = 0; en2 < m_ulEdges; en2++)
		{
			SEdge *pedge = m_rspqedge[en2];
...
CJoinOrderDPv2::AddSelectNodeForRemainingEdges(CExpression *join_expr)
{
	...
	CExpressionArray *exprArray = SPQOS_NEW(m_mp) CExpressionArray(m_mp);
	...
	if (0 < exprArray->Size())
	{
		...
		return SPQOS_NEW(m_mp) CExpression(
			m_mp, SPQOS_NEW(m_mp) CLogicalSelect(m_mp), join_expr, conj);
	}
...
CJoinOrderDPv2::RecursivelyMarkEdgesAsUsed(CExpression *expr)
{
	...
	else
	{
		...
		SPQOS_ASSERT(0 < expr->Arity() || CUtils::FScalarConstTrue(expr));
								...
								SGroupInfo *part_selector_group_info)
{
	...
	{
		SPQOS_ASSERT(NULL != pt_atom);
		SPQOS_ASSERT(NULL != partition_keys && partition_keys->Size() > 0);
...
{
	SPQOS_ASSERT(left_level > 0 && right_level > 0 &&
	...
	for (ULONG left_ix = 0; left_ix < left_size; left_ix++)
	{
		...
		for (; right_ix < right_size; right_ix++)
		{
			...
			if (NULL != join_expr_info)
			{
				...
				CBitSet *join_bitset =
					SPQOS_NEW(m_mp) CBitSet(m_mp, *left_bitset);
...
{
	ULONG right_level = 1;
	SPQOS_ASSERT(left_level > 0 && left_level + right_level <= m_ulComps);
	...
	if (left_ix >= left_size)
	{
		...
		SPQOS_ASSERT(!"No greedy solution found for the left side");
		return;
	}
	...
	for (; right_ix < right_size; right_ix++)
	{
		...
		{
			// we have a valid join
			CBitSet *join_bitset = SPQOS_NEW(m_mp) CBitSet(m_mp, *left_bitset);
	...
	else
	{
		...
		SPQOS_ASSERT(!"We should always find a greedy solution");
	}
}
										...
										SExpressionInfo *stats_expr_info)
{
	...
	if (NULL == group_info)
	{
		...
		group_info = SPQOS_NEW(m_mp) SGroupInfo(m_mp, atoms);
		...
		{
			SPQOS_ASSERT(NULL != real_expr_info_for_stats->m_expr->Pstats());
...
CJoinOrderDPv2::PexprExpand()
{
	...
	{
		CBitSet *atom_bitset = SPQOS_NEW(m_mp) CBitSet(m_mp);
		atom_bitset->ExchangeSet(atom_id);
		CExpression *pexpr_atom = m_rspqcomp[atom_id]->m_pexpr;
		pexpr_atom->AddRef();
		SExpressionInfo *atom_expr_info =
			SPQOS_NEW(m_mp) SExpressionInfo(m_mp, pexpr_atom, atom_props);
		if (0 == atom_id)
		{
		...
		if (table_desc != NULL)
		{
			...
			CMDIdRelStats *rel_stats_mdid =
				SPQOS_NEW(m_mp) CMDIdRelStats(CMDIdSPQDB::CastMdid(rel_mdid));
...
CJoinOrderDPv2::EnumerateDP()
{
	if (SPQOS_FTRACE(EopttraceGreedyOnlyInDPv2) ||
		SPQOS_FTRACE(EopttraceMinCardOnlyInDPv2) ||
		SPQOS_FTRACE(EopttraceQueryOnlyInDPv2))
	{
		return;
	}
	...
	if (join_order_exhaustive_limit < m_ulComps)
	{
		for (ULONG l = 2; l <= m_ulComps; l++)
		{
			...
			Level(l)->m_top_k_groups =
				SPQOS_NEW(m_mp) CKHeap<SGroupInfoArray, SGroupInfo>(
...
{
	if (SPQOS_FTRACE(EopttraceQueryOnlyInDPv2))
	{
		return;
	}
...
{
	if (SPQOS_FTRACE(EopttraceQueryOnlyInDPv2) ||
		SPQOS_FTRACE(EopttraceGreedyOnlyInDPv2))
	{
		return;
	}
...
CJoinOrderDPv2::EnumerateGreedyAvoidXProd()
{
	...
	m_cross_prod_penalty = SPQOPT_DPV2_CROSS_JOIN_GREEDY_PENALTY;
...
{
	SPQOS_ASSERT(NULL == onPredToUse || NULL == *onPredToUse);
	SPQOS_ASSERT(NULL == requiredBitsOnLeft || NULL == *requiredBitsOnLeft);
	...
	if (SPQOPT_ZERO_INNER_JOIN_PRED_INDEX != childPredIndex)
	{
...
{
	SPQOS_ASSERT(NULL != m_child_pred_indexes);
...
FORCE_GENERATE_DBGSTR(spqopt::CJoinOrderDPv2);
