									...
									const CDXLScalarConstValue *dxl_op)
{
	...
	return SPQOS_NEW(mp) CScalarConst(mp, datum);
}
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorDXLToExprUtils::Pdrspqdatum
//
//	@doc:
...
//---------------------------------------------------------------------------
IDatumArray *
CTranslatorDXLToExprUtils::Pdrspqdatum(CMemoryPool *mp, CMDAccessor *md_accessor,
									  const CDXLDatumArray *pdrspqdxldatum)
{
	SPQOS_ASSERT(NULL != pdrspqdxldatum);

	IDatumArray *pdrgdatum = SPQOS_NEW(mp) IDatumArray(mp);
	const ULONG length = pdrspqdxldatum->Size();
	for (ULONG ul = 0; ul < length; ul++)
	{
		CDXLDatum *dxl_datum = (*pdrspqdxldatum)[ul];
									  ...
									  UlongToUlongMap *phmululColMapping)
{
	...
	{
		CBitSet *pbs = SPQOS_NEW(mp) CBitSet(mp, ptabdesc->ColumnCount());
		const ULongPtrArray *pdrspqulKeys = pmdrel->KeySetAt(ul);
		const ULONG ulKeys = pdrspqulKeys->Size();

		for (ULONG ulKey = 0; ulKey < ulKeys; ulKey++)
		{
			// populate current keyset
			ULONG ulOriginalKey = *((*pdrspqulKeys)[ulKey]);
			ULONG *pulRemappedKey = phmululColMapping->Find(&ulOriginalKey);
			SPQOS_ASSERT(NULL != pulRemappedKey);
...
CTranslatorDXLToExprUtils::EBoolOperator(EdxlBoolExprType edxlbooltype)
{
	...
	SPQOS_ASSERT(CScalarBoolOp::EboolopSentinel > eboolop);
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorDXLToExprUtils::Pdrspqcr
//
//	@doc:
...
//---------------------------------------------------------------------------
CColRefArray *
CTranslatorDXLToExprUtils::Pdrspqcr(CMemoryPool *mp,
