	  ...
	  m_window_oids(pwindowoids)
{
	...
	SPQOS_ASSERT(NULL != pcteconf);
	SPQOS_ASSERT(NULL != m_cost_model);
	SPQOS_ASSERT(NULL != phint);
	SPQOS_ASSERT(NULL != m_window_oids);
}
...
COptimizerConfig::PoconfDefault(CMemoryPool *mp)
{
	return SPQOS_NEW(mp) COptimizerConfig(
		SPQOS_NEW(mp) CEnumeratorConfig(mp, 0 /*plan_id*/, 0 /*ullSamples*/),
							...
							CBitSet *pbsTrace) const
{
	...
	xml_serializer->AddAttribute(
		CDXLTokens::GetDXLTokenStr(spqdxl::EdxltokenSkewFactor),
		m_hint->UlSkewFactor());
	xml_serializer->CloseElement(
		CDXLTokens::GetDXLTokenStr(EdxltokenNamespacePrefix),
		CDXLTokens::GetDXLTokenStr(EdxltokenHint));
	...
	spqos::CBitSetIter bsi(*pbsTrace);
	...
	for (ULONG ul = 0; bsi.Advance(); ul++)
	{
		...
		wsTraceFlags.AppendFormat(SPQOS_WSZ_LIT("%d"), bsi.Bit());
	}
