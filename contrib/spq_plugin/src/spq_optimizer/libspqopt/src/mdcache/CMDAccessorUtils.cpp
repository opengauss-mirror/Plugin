							 ...
							 IMDId *right_mdid, IMDType::ECmpType cmp_type)
{
	...
	SPQOS_CATCH_EX(ex)
	{
		if (SPQOS_MATCH_EX(ex, spqdxl::ExmaMD, spqdxl::ExmiMDCacheEntryNotFound))
		{
			SPQOS_RESET_EX;
			return false;
		}
							   ...
							   IMDId *right_mdid, IMDType::ECmpType cmp_type)
{
	...
	SPQOS_ASSERT(IMDId::IsValid(sc_cmp_mdid));
									 ...
									 CExpression *&pexprRight, IMDId *op_mdid)
{
	...
	if (!op_left_mdid->Equals(left_mdid))
	{
		...
		else
		{
			...
			SPQOS_ASSERT(CMDAccessorUtils::FCastExists(md_accessor, left_mdid,
													  right_mdid));
			...
			if (!right_mdid->Equals(op_left_mdid))
			{
				...
				SPQOS_ASSERT(CMDAccessorUtils::FCastExists(
	...
	if (!op_right_mdid->Equals(right_mdid))
	{
		...
		{
			// validate proposition B
			SPQOS_ASSERT(CMDAccessorUtils::FCastExists(md_accessor, right_mdid,
													  left_mdid));
