...
#define SPQOPT_CACHEACC_HT_NUM_OF_BUCKETS 128
...
{
	SPQOS_ASSERT(m_pmdidInvalid != mdid);
...
{
	SPQOS_ASSERT(!Equals(mdpelem, m_mdpelemInvalid));
	...
	: m_mp(mp), m_pcache(pcache), m_dLookupTime(0.0), m_dFetchTime(0.0)
{
	...
	SPQOS_ASSERT(NULL != m_pcache);

	m_pmdpGeneric = SPQOS_NEW(mp) CMDProviderGeneric(mp);
...
//---------------------------------------------------------------------------
CMDAccessor::CMDAccessor(CMemoryPool *mp, MDCache *pcache,
						 const CSystemIdArray *pdrspqsysid,
						 const CMDProviderArray *pdrspqmdp)
	: m_mp(mp), m_pcache(pcache), m_dLookupTime(0.0), m_dFetchTime(0.0)
{
	SPQOS_ASSERT(NULL != m_mp);
	SPQOS_ASSERT(NULL != m_pcache);
	...
	RegisterProviders(pdrspqsysid, pdrspqmdp);
}
...
CMDAccessor::InitHashtables(CMemoryPool *mp)
{
	...
	m_shtCacheAccessors.Init(mp, SPQOPT_CACHEACC_HT_NUM_OF_BUCKETS,
							 SPQOS_OFFSET(SMDAccessorElem, m_link),
							 SPQOS_OFFSET(SMDAccessorElem, m_mdid),
							 &(SMDAccessorElem::m_pmdidInvalid),
							 SMDAccessorElem::HashValue,
							 SMDAccessorElem::Equals);

	// initialize MD providers hash table
	m_shtProviders.Init(mp, SPQOPT_CACHEACC_HT_NUM_OF_BUCKETS,
						SPQOS_OFFSET(SMDProviderElem, m_link),
...
CMDAccessor::RegisterProvider(CSystemId sysid, IMDProvider *pmdp)
{
	...
	a_pmdpelem = SPQOS_NEW(m_mp) SMDProviderElem(sysid, pmdp);
...
//---------------------------------------------------------------------------
void
CMDAccessor::RegisterProviders(const CSystemIdArray *pdrspqsysid,
							   const CMDProviderArray *pdrspqmdp)
{
	SPQOS_ASSERT(NULL != pdrspqmdp);
	SPQOS_ASSERT(NULL != pdrspqsysid);
	SPQOS_ASSERT(pdrspqmdp->Size() == pdrspqsysid->Size());
	SPQOS_ASSERT(0 < pdrspqmdp->Size());

	const ULONG ulProviders = pdrspqmdp->Size();
	for (ULONG ul = 0; ul < ulProviders; ul++)
	{
		IMDProvider *pmdp = (*pdrspqmdp)[ul];
		pmdp->AddRef();
		RegisterProvider(*((*pdrspqsysid)[ul]), pmdp);
	}
}
...
CMDAccessor::Pmdp(CSystemId sysid)
{
	...
	SPQOS_ASSERT(NULL != pmdpelem && "Could not find MD provider");
...
CMDAccessor::GetImdObj(IMDId *mdid, IMDCacheObject::Emdtype mdtype)
{
	...
	if (NULL == pimdobj)
	{
		...
		a_pmdcacc = SPQOS_NEW(m_mp) CacheAccessorMD(m_pcache);
		...
		if (NULL == pmdobjNew)
		{
			...
			SPQOS_ASSERT(NULL != a_pstr.Value());
			CMemoryPool *mp = m_mp;

			if (IMDId::EmdidSPQDBCtas != mdid->MdidType())
			{
				// create the accessor memory pool
				mp = a_pmdcacc->Pmp();
			}

			pmdobjNew = spqdxl::CDXLUtils::ParseDXLToIMDIdCacheObj(
				mp, a_pstr.Value(), NULL /* XSD path */);
			SPQOS_ASSERT(NULL != pmdobjNew);

			if (fPrintOptStats)
			{
				// add fetch time in msec
				CDouble dFetch(timerFetch.ElapsedUS() /
							   CDouble(SPQOS_USEC_IN_MSEC));
			...
			if (IMDId::EmdidSPQDBCtas != mdid->MdidType())
			{
				...
				a_pmdkeyCache = SPQOS_NEW(mp) CMDKey(pmdobjNew->MDId());
				...
				SPQOS_ASSERT(NULL != pmdobjInserted);
...

		{
			...
			a_pmdaccelem = SPQOS_NEW(m_mp) SMDAccessorElem(pmdobjNew, pmdidNew);
	...
	pimdobj = pmdaccelem->GetImdObj();
	SPQOS_ASSERT(NULL != pimdobj);

	if (fPrintOptStats)
	{
		// add lookup time in msec
		CDouble dLookup(timerLookup.ElapsedUS() / CDouble(SPQOS_USEC_IN_MSEC));
...
CMDAccessor::Pmdcast(IMDId *mdid_src, IMDId *mdid_dest)
{
	...
	a_pmdidCast = SPQOS_NEW(m_mp) CMDIdCast(CMDIdSPQDB::CastMdid(mdid_src),
										   CMDIdSPQDB::CastMdid(mdid_dest));
					  ...
					  IMDType::ECmpType cmp_type)
{
	...
	a_pmdidScCmp =
		SPQOS_NEW(m_mp) CMDIdScCmp(CMDIdSPQDB::CastMdid(left_mdid),
								  CMDIdSPQDB::CastMdid(right_mdid), cmp_type);
...
{
	SPQOS_ASSERT(NULL != rel_mdid);
	SPQOS_ASSERT(NULL != col_histogram_mapping);
	SPQOS_ASSERT(NULL != colid_width_mapping);

	// get the column statistics
	const IMDColStats *pmdcolstats = Pmdcolstats(mp, rel_mdid, ulPos);
	SPQOS_ASSERT(NULL != pmdcolstats);

	// fetch the column width and insert it into the hashmap
	CDouble *width = SPQOS_NEW(mp) CDouble(pmdcolstats->Width());
	colid_width_mapping->Insert(SPQOS_NEW(mp) ULONG(colid), width);
	...
	col_histogram_mapping->Insert(SPQOS_NEW(mp) ULONG(colid), histogram);

	BOOL fGuc = SPQOS_FTRACE(EopttracePrintColsWithMissingStats);
	...
	if (fRecordMissingStats)
	{
		...
		CMDIdColStats *pmdidCol =
			SPQOS_NEW(mp) CMDIdColStats(CMDIdSPQDB::CastMdid(rel_mdid), ulPos);
...
{
	SPQOS_ASSERT(NULL != rel_mdid);
	SPQOS_ASSERT(NULL != pcrsHist);
	SPQOS_ASSERT(NULL != pcrsWidth);

	// retrieve MD relation and MD relation stats objects
	rel_mdid->AddRef();
	CMDIdRelStats *rel_stats_mdid =
		SPQOS_NEW(mp) CMDIdRelStats(CMDIdSPQDB::CastMdid(rel_mdid));
	...
	while (crsiWidth.Advance())
	{
		...
		CDouble *width = SPQOS_NEW(mp) CDouble(pmdrel->ColWidth(ulPos));
		colid_width_mapping->Insert(SPQOS_NEW(mp) ULONG(colid), width);
	}

	CDouble rows = std::max(DOUBLE(1.0), pmdRelStats->Rows().Get());

	return SPQOS_NEW(mp) CStatistics(
					 ...
					 const CDXLBucket *dxl_bucket)
{
	...
	CPoint *bucket_lower_bound = SPQOS_NEW(mp) CPoint(pdatumLower);
	CPoint *bucket_upper_bound = SPQOS_NEW(mp) CPoint(pdatumUpper);
...
CMDAccessor::Serialize(COstream &oos)
{
	...
	// while iterating.
	cacheEntries = SPQOS_NEW_ARRAY(m_mp, IMDCacheObject *, nentries);
...
CMDAccessor::SerializeSysid(COstream &oos)
{
	...
	while (mdhtit.Advance())
	{
		...
		WCHAR wszSysId[SPQDXL_MDID_LENGTH];
		CWStringStatic str(wszSysId, SPQOS_ARRAY_SIZE(wszSysId));

		if (0 < ul)
		{
			str.AppendFormat(SPQOS_WSZ_LIT("%s"), ",");
		}

		str.AppendFormat(SPQOS_WSZ_LIT("%d.%ls"), sysid.MdidType(),
						 sysid.GetBuffer());
