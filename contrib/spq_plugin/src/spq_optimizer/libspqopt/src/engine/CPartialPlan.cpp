	  ...
	  m_ulChildIndex(child_index)
{
	...
	SPQOS_ASSERT_IMP(NULL != pccChild, child_index < pgexpr->Arity());
}
...
{
	SPQOS_ASSERT(m_pgexpr == exprhdl.Pgexpr());
	SPQOS_ASSERT(NULL != pci);
	SPQOS_ASSERT_IMP(NULL != m_pccChild, m_ulChildIndex < exprhdl.Arity());
...
CPartialPlan::CostCompute(CMemoryPool *mp)
{
	...
	ICostModel::SCostingInfo ci(mp, exprhdl.UlNonScalarChildren(),
								SPQOS_NEW(mp) ICostModel::CCostingStats(stats));
...
{
	SPQOS_ASSERT(NULL != pppFst);
	SPQOS_ASSERT(NULL != pppSnd);
