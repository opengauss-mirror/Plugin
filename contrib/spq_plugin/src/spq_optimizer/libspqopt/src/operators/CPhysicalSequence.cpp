	...
	: CPhysical(mp), m_pcrsEmpty(NULL)
{
	...
	m_pcrsEmpty = SPQOS_NEW(mp) CColRefSet(mp);
}
...
)
{
	...
	m_pcrsEmpty->AddRef();
	SPQOS_ASSERT(0 == m_pcrsEmpty->Size());
...
) const
{
	...
	CCTEReq *pcterUnresolved =
		pcter->PcterUnresolvedSequence(mp, pcmCombined, pdrspqdpCtxt);
	pcmCombined->Release();
							   ...
							   ULONG ulOptReq) const
{
	...
	if (0 == ulOptReq)
	{
			...
			CDistributionSpec::EdtStrictSingleton == pdsRequired->Edt())
		{
			...
			return SPQOS_NEW(mp) CDistributionSpecSingleton(pdss->Est());
		}
