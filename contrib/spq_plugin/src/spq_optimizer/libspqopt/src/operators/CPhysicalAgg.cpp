...
#include "spqopt/base/CDistributionSpecStrictSingleton.h"
	  ...
	  m_should_enforce_distribution(should_enforce_distribution)
{
	...
	SPQOS_ASSERT_IMP(EgbaggtypeGlobal != egbaggtype, fMultiStage);

	ULONG ulDistrReqs = 1;
	if (pdrspqcrMinimal == NULL || 0 == pdrspqcrMinimal->Size())
	{
		colref_array->AddRef();
		m_pdrspqcrMinimal = colref_array;
	}
	else
	{
	...
	else if (COperator::EgbaggtypeIntermediate == egbaggtype)
	{
		...
		SPQOS_ASSERT(pdrspqcrArgDQA->Size() <= colref_array->Size());
		...
		if (pdrspqcrArgDQA->Size() == colref_array->Size() ||
			SPQOS_FTRACE(EopttraceForceAggSkewAvoidance))
		{
...
{
	return PcrsRequiredAgg(mp, exprhdl, pcrsRequired, child_index, m_pdrspqcr);
}
...
CPhysicalAgg::PdsRequiredAgg(CMemoryPool *mp, CExpressionHandle &exprhdl,
							 ...
							 ULONG ulOptReq, CColRefArray *pdrspqcgGrp,
							 CColRefArray *pdrspqcrGrpMinimal) const
{
	...
	if (FGlobal())
	{
		return PdsRequiredGlobalAgg(mp, exprhdl, pdsInput, child_index,
									pdrspqcgGrp, pdrspqcrGrpMinimal, ulOptReq);
	}
	...
	if (COperator::EgbaggtypeLocal == m_egbaggtype && m_pdrspqcrArgDQA != NULL &&
		0 != m_pdrspqcrArgDQA->Size())
	{
		if (ulOptReq == 0)
		{
			return PdsMaximalHashed(mp, m_pdrspqcrArgDQA);
		}
		else
		{
			SPQOS_ASSERT(1 == ulOptReq);
			SPQOS_ASSERT(0 < m_pdrspqcr->Size());
			CColRefArray *grpAndDistinctCols = SPQOS_NEW(mp) CColRefArray(mp);
			grpAndDistinctCols->AppendArray(m_pdrspqcr);
			grpAndDistinctCols->AppendArray(m_pdrspqcrArgDQA);
	...
	SPQOS_ASSERT(0 == ulOptReq || 1 == ulOptReq);
...
CPhysicalAgg::PdsRequiredGlobalAgg(CMemoryPool *mp, CExpressionHandle &exprhdl,
								   ...
								   ULONG child_index, CColRefArray *pdrspqcrGrp,
								   CColRefArray *pdrspqcrGrpMinimal,
								   ULONG ulOptReq) const
{
	SPQOS_ASSERT(FGlobal());
	...
	return PdsMaximalHashed(mp, pdrspqcrGrpMinimal);
}
...
{
	SPQOS_ASSERT(COperator::EgbaggtypeIntermediate == m_egbaggtype);

	if (0 == ulOptReq)
	{
		return PdsMaximalHashed(mp, m_pdrspqcr);
	}

	CColRefArray *colref_array = SPQOS_NEW(mp) CColRefArray(mp);
	const ULONG length = m_pdrspqcr->Size() - m_pdrspqcrArgDQA->Size();
...
) const
{
	...
	// include grouping columns
	pcrs->Include(PdrspqcrGroupingCols());
...
CPhysicalAgg::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	{
		return SPQOS_NEW(mp) CDistributionSpecStrictSingleton(
...
CPhysicalAgg::HashValue() const
{
	...
	const ULONG arity = m_pdrspqcr->Size();
	...
	ulHash = spqos::CombineHashes(ulHash, spqos::HashValue<ULONG>(&ulaggstage));
...
CPhysicalAgg::Matches(COperator *pop) const
{
	...
	if (popAgg->Egbaggtype() == m_egbaggtype &&
		m_pdrspqcr->Equals(popAgg->m_pdrspqcr))
	{
		if (CColRef::Equals(m_pdrspqcrMinimal, popAgg->m_pdrspqcrMinimal))
		{
			return (m_pdrspqcrArgDQA == NULL || 0 == m_pdrspqcrArgDQA->Size()) ||
