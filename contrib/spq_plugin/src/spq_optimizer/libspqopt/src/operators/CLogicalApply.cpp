...
//
//---------------------------------------------------------------------------
CLogicalApply::CLogicalApply(CMemoryPool *mp, CColRefArray *pdrspqcrInner,
...
CLogicalApply::~CLogicalApply()
{
	CRefCount::SafeRelease(m_pdrspqcrInner);
}
...
CLogicalApply::Matches(COperator *pop) const
{
	if (pop->Eopid() == Eopid())
	{
		...
		if (NULL == m_pdrspqcrInner || NULL == pdrspqcrInner)
		{
			return (NULL == m_pdrspqcrInner && NULL == pdrspqcrInner);
		}

		return m_pdrspqcrInner->Equals(pdrspqcrInner);
	}
