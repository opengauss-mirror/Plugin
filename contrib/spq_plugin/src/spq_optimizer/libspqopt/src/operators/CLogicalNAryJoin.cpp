										   ...
										   CExpressionHandle &exprhdl) const
{
	...
	CColRefSetArray *equivalenceClasses = SPQOS_NEW(mp) CColRefSetArray(mp);
	CConstraintArray *constraints = SPQOS_NEW(mp) CConstraintArray(mp);
	...
	for (ULONG ul = 0; ul < arity - 1; ul++)
	{
		if (IsInnerJoinChild(ul))
		{
			...
			CColRefSetArray *pdrspqcrsMerged = CUtils::PdrspqcrsMergeEquivClasses(
				mp, equivalenceClasses, pdrspqcrsChild);
			equivalenceClasses->Release();
			equivalenceClasses = pdrspqcrsMerged;
	...
	return SPQOS_NEW(mp) CPropConstraint(mp, equivalenceClasses, pcnstrNew);
}
										...
										CExpressionHandle &exprhdl) const
{
	...
	predArray = CPredicateUtils::PdrspqexprConjuncts(mp, innerJoinPreds);
	...
	CExpression *new_inner_join_preds)
{
	...
	if (EopScalarNAryJoinPredList == pop->Eopid())
	{
		...
		return SPQOS_NEW(mp) CExpression(mp, pop, new_children);
	}
