...
CWindowPreprocessor::SplitPrjList(
	...
	CExpressionArray **
		ppdrspqexprDistinctAggsPrEl,	 // output: list of project elements with Distinct Aggs
	CExpressionArray **
		ppdrspqexprOtherPrEl,  // output: list of project elements with Other window functions
	COrderSpecArray **
		ppdrspqosOther,	// output: array of order specs of window functions used in Others list
	CWindowFrameArray **
		ppdrspqwfOther  // output: array of frame specs of window functions used in Others list
)
{
	SPQOS_ASSERT(NULL != pexprSeqPrj);
	SPQOS_ASSERT(NULL != ppdrspqexprDistinctAggsPrEl);
	SPQOS_ASSERT(NULL != ppdrspqexprOtherPrEl);
	SPQOS_ASSERT(NULL != ppdrspqosOther);
	SPQOS_ASSERT(NULL != ppdrspqwfOther);
	...
	CWindowFrameArray *pdrspqwf = popSeqPrj->Pdrspqwf();
	BOOL fHasFrameSpecs = popSeqPrj->FHasFrameSpecs();

	CExpressionArray *pdrspqexprDistinctAggsPrEl =
		SPQOS_NEW(mp) CExpressionArray(mp);

	CExpressionArray *pdrspqexprOtherPrEl = SPQOS_NEW(mp) CExpressionArray(mp);
	COrderSpecArray *pdrspqosOther = SPQOS_NEW(mp) COrderSpecArray(mp);
	CWindowFrameArray *pdrspqwfOther = SPQOS_NEW(mp) CWindowFrameArray(mp);
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (popScWinFunc->IsDistinct() && popScWinFunc->FAgg())
		{
			...
			pdrspqexprDistinctAggsPrEl->Append(pexprNewPrjElem);
		}
		else
		{
			if (fHasOrderSpecs)
			{
				(*pdrspqos)[ul]->AddRef();
				pdrspqosOther->Append((*pdrspqos)[ul]);
			}

			if (fHasFrameSpecs)
			{
				(*pdrspqwf)[ul]->AddRef();
				pdrspqwfOther->Append((*pdrspqwf)[ul]);
			}
	...
	*ppdrspqexprDistinctAggsPrEl = pdrspqexprDistinctAggsPrEl;
	*ppdrspqexprOtherPrEl = pdrspqexprOtherPrEl;
	*ppdrspqosOther = pdrspqosOther;
	*ppdrspqwfOther = pdrspqwfOther;
}
...
{
	SPQOS_ASSERT(NULL != pexprSeqPrj);
	SPQOS_ASSERT(NULL != ppexprGbAgg);
	SPQOS_ASSERT(NULL != ppexprOutputSeqPrj);

	// split project elements between Distinct Aggs list, and Others list
	CExpressionArray *pdrspqexprDistinctAggsPrEl = NULL;
	CExpressionArray *pdrspqexprOtherPrEl = NULL;
	COrderSpecArray *pdrspqosOther = NULL;
	CWindowFrameArray *pdrspqwfOther = NULL;
	SplitPrjList(mp, pexprSeqPrj, &pdrspqexprDistinctAggsPrEl,
				 &pdrspqexprOtherPrEl, &pdrspqosOther, &pdrspqwfOther);
	...
	if (CDistributionSpec::EdtHashed == pds->Edt())
	{
		...
		pdrspqcrGrpCols = pcrs->Pdrspqcr(mp);
		pcrs->Release();
	}
	else
	{
	...
	*ppexprGbAgg = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp)
	...
	if (0 == pdrspqexprOtherPrEl->Size())
	{
		...
		pdrspqosOther->Release();
		pdrspqwfOther->Release();
	...
	pds->AddRef();
	*ppexprOutputSeqPrj = SPQOS_NEW(mp) CExpression(
		mp,
		SPQOS_NEW(mp)
			CLogicalSequenceProject(mp, pds, pdrspqosOther, pdrspqwfOther),
		pexprSeqPrjChild,
		SPQOS_NEW(mp) CExpression(mp, SPQOS_NEW(mp) CScalarProjectList(mp),
								 pdrspqexprOtherPrEl));
}
...
{
	SPQOS_ASSERT(NULL != pexprSeqPrj);
	SPQOS_ASSERT(COperator::EopLogicalSequenceProject ==
...
//---------------------------------------------------------------------------
//	@function:
//		CWindowPreprocessor::PdrspqcrGrpCols
//
//	@doc:
...
//---------------------------------------------------------------------------
CColRefArray *
CWindowPreprocessor::PdrspqcrGrpCols(CExpression *pexpr)
{
	...
	if (COperator::EopLogicalGbAgg == pop->Eopid())
	{
		...
		return CLogicalGbAgg::PopConvert(pop)->Pdrspqcr();
	}
	...
	if (CUtils::FLogicalJoin(pop))
	{
		...
		{
			return CLogicalGbAgg::PopConvert(popInner)->Pdrspqcr();
		}
	}
...
CWindowPreprocessor::PexprSeqPrj2Join(CMemoryPool *mp, CExpression *pexprSeqPrj)
{
	...
	SPQOS_ASSERT(COperator::EopLogicalSequenceProject ==
				pexprSeqPrj->Pop()->Eopid());
	SPQOS_ASSERT(0 < (*pexprSeqPrj)[1]->DeriveTotalDistinctAggs());
	...
	CColRefArray *pdrspqcrChildOutput =
		pexprChild->DeriveOutputColumns()->Pdrspqcr(mp);
	...
	CColRefArray *pdrspqcrConsumerOutput =
		CLogicalCTEConsumer::PopConvert(pexprGbAggConsumer->Pop())->Pdrspqcr();
	UlongToColRefMap *colref_mapping =
		CUtils::PhmulcrMapping(mp, pdrspqcrChildOutput, pdrspqcrConsumerOutput);
	...
	CExpression *pexprGbAggWithConsumer =
		SPQOS_NEW(mp) CExpression(mp, pexprGbAggRemapped->Pop(),
	...
	if (COperator::EopLogicalSequenceProject == pexprWindow->Pop()->Eopid())
	{
		...
		pexprWindowFinal = SPQOS_NEW(mp) CExpression(
	...
	CColRefArray *pdrspqcrGrpCols = PdrspqcrGrpCols(pexprJoinDQAs);

	// create final join condition
	CExpression *pexprJoinCondition = NULL;

	if (NULL != pdrspqcrGrpCols && 0 < pdrspqcrGrpCols->Size())
	{
		...
		CColRefSet *pcrs = CUtils::PcrsExtractColumns(
			mp, CDistributionSpecHashed::PdsConvert(pds)->Pdrspqexpr());
		CColRefArray *pdrspqcrPartitionBy = pcrs->Pdrspqcr(mp);
		pcrs->Release();
		SPQOS_ASSERT(
			pdrspqcrGrpCols->Size() == pdrspqcrPartitionBy->Size() &&
		...
		pexprJoinCondition = CPredicateUtils::PexprINDFConjunction(
			mp, pdrspqcrGrpCols, pdrspqcrPartitionBy);
	...
	return SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEId), pexprJoin);
}
