...
CExpression::CExpression(CMemoryPool *mp, COperator *pop,
	...
	: m_mp(mp),
	  ...
	  m_ulOriginGrpId(spqos::ulong_max),
	  m_ulOriginGrpExprId(spqos::ulong_max)
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != pop);

	m_pdprel = SPQOS_NEW(m_mp) CDrvdPropRelational(m_mp);
	m_pdpscalar = SPQOS_NEW(m_mp) CDrvdPropScalar(m_mp);
	  ...
	  m_ulOriginGrpExprId(spqos::ulong_max)
{
	...
	m_pdprel = SPQOS_NEW(m_mp) CDrvdPropRelational(m_mp);
	m_pdpscalar = SPQOS_NEW(m_mp) CDrvdPropScalar(m_mp);
	m_pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp, 1);
	m_pdrspqexpr->Append(pexpr);

	SPQOS_ASSERT(m_pdrspqexpr->Size() == 1);
}
	  ...
	  m_ulOriginGrpExprId(spqos::ulong_max)
{
	...
	SPQOS_ASSERT(NULL != pexprChildFirst);
	SPQOS_ASSERT(NULL != pexprChildSecond);

	m_pdprel = SPQOS_NEW(m_mp) CDrvdPropRelational(m_mp);
	m_pdpscalar = SPQOS_NEW(m_mp) CDrvdPropScalar(m_mp);
	m_pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp, 2);
	m_pdrspqexpr->Append(pexprChildFirst);
	m_pdrspqexpr->Append(pexprChildSecond);

	SPQOS_ASSERT(m_pdrspqexpr->Size() == 2);
}
	  ...
	  m_ulOriginGrpExprId(spqos::ulong_max)
{
	...
	SPQOS_ASSERT(NULL != pexprChildFirst);
	SPQOS_ASSERT(NULL != pexprChildSecond);
	SPQOS_ASSERT(NULL != pexprChildThird);

	m_pdprel = SPQOS_NEW(m_mp) CDrvdPropRelational(m_mp);
	m_pdpscalar = SPQOS_NEW(m_mp) CDrvdPropScalar(m_mp);
	m_pdrspqexpr = SPQOS_NEW(mp) CExpressionArray(mp, 3);
	m_pdrspqexpr->Append(pexprChildFirst);
	m_pdrspqexpr->Append(pexprChildSecond);
	m_pdrspqexpr->Append(pexprChildThird);

	SPQOS_ASSERT(m_pdrspqexpr->Size() == 3);
}
...
//---------------------------------------------------------------------------
CExpression::CExpression(CMemoryPool *mp, COperator *pop,
						 CGroupExpression *pgexpr, CExpressionArray *pdrspqexpr,
	  ...
	  m_ulOriginGrpExprId(spqos::ulong_max)
{
	SPQOS_ASSERT(NULL != mp);
	SPQOS_ASSERT(NULL != pop);
	SPQOS_ASSERT(pgexpr->Arity() == (pdrspqexpr == NULL ? 0 : pdrspqexpr->Size()));
...
{
	{
		...
		CRefCount::SafeRelease(m_pdrspqexpr);
...
CExpression::Pdp(const CDrvdProp::EPropType ept) const
{
	...
	SPQOS_ASSERT(!"Invalid property type");
...
{
	SPQOS_RTL_ASSERT(m_pdprel->IsComplete());
	return m_pdprel;
}
...
{
	SPQOS_RTL_ASSERT(m_pdpplan->IsComplete());
	return m_pdpplan;
}
...
{
	SPQOS_RTL_ASSERT(m_pdpscalar->IsComplete());
	return m_pdpscalar;
}
...
CExpression::AssertValidPropDerivation(const CDrvdProp::EPropType ept)
{
	...
	SPQOS_ASSERT_IMP(pop->FScalar(), CDrvdProp::EptScalar == ept);
	SPQOS_ASSERT_IMP(pop->FLogical(), CDrvdProp::EptRelational == ept);
	SPQOS_ASSERT_IMP(pop->FPattern(), CDrvdProp::EptRelational == ept ||
										 CDrvdProp::EptScalar == ept);

	SPQOS_ASSERT_IMP(pop->FPhysical(), CDrvdProp::EptRelational == ept ||
...
CExpression::Ept() const
{
	...
	SPQOS_ASSERT(!"Unexpected operator type");
...
)
{
	...
	if (NULL == Pdp(ept))
	{
		SPQOS_ASSERT(CDrvdProp::EptRelational != ept);
		SPQOS_ASSERT(CDrvdProp::EptScalar != ept);

		const ULONG arity = Arity();
		for (ULONG ul = 0; ul < arity; ul++)
		{
			CExpression *pexprChild = (*m_pdrspqexpr)[ul];
		...
		{
			case CDrvdProp::EptPlan:
				m_pdpplan = SPQOS_NEW(m_mp) CDrvdPropPlan();
				break;
			default:
				break;
		}
	...
	SPQOS_ASSERT(Pdp(ept)->IsComplete());
	return Pdp(ept);
}
						  ...
						  IStatisticsArray *stats_ctxt)
{
	...
	SPQOS_ASSERT(prprel->FRelational());
	SPQOS_CHECK_ABORT;
	...
	if (NULL == stats_ctxt)
	{
		...
		pdrspqstatCtxtNew = SPQOS_NEW(m_mp) IStatisticsArray(m_mp);
	}
	else
	{
	...
	CDrvdPropCtxtRelational *pdpctxtrel =
		SPQOS_NEW(m_mp) CDrvdPropCtxtRelational(m_mp);
	...
	exprhdl.DeriveStats(pdrspqstatCtxtNew);
...
CExpression::ResetDerivedProperties()
{
	...
	for (ULONG i = 0; i < SPQOS_ARRAY_SIZE(rgept); i++)
	{
		// reset self
...
{
	SPQOS_ASSERT(NULL != m_pop);
	// this is only valid for logical and scalar expressions
	SPQOS_ASSERT(m_pop->FLogical() || m_pop->FScalar());
	...
	{
		CExpression *pexprChild = (*m_pdrspqexpr)[ul];
		pdrspqexpr->Append(pexprChild->PexprCopyWithRemappedColumns(
	...
	{
		pdrspqexpr->Release();
		return SPQOS_NEW(mp) CExpression(mp, pop);
	}
...
CExpression::FMatchDebug(CExpression *pexpr) const
{
	...
	if (!Pop()->Matches(pexpr->Pop()))
	{
		SPQOS_ASSERT(Pop()->HashValue() == pexpr->Pop()->HashValue());
		return false;
	}

	// operator match must be commutative
	SPQOS_ASSERT(pexpr->Pop()->Matches(Pop()));

	// scalar operators must agree on return type
	SPQOS_ASSERT_IMP(Pop()->FScalar() &&
	...
	SPQOS_ASSERT_IMP(0 < arity, Pop()->FInputOrderSensitive() ==
...
FORCE_GENERATE_DBGSTR(spqopt::CExpression);
							   ...
							   BOOL fLast) const
{
	...
	if (spqos::ulong_max != m_ulOriginGrpId)
	{
...
#ifdef SPQOS_DEBUG
	if (SPQOS_FTRACE(EopttracePrintExpressionProperties))
	{
...
// sensitive. This hash function specifically used in CUtils::PdrspqexprDedup
							...
							CDrvdPropCtxtPlan *pdpctxtplan)
{
	...
	if (pop->FPhysical())
	{
		...
		CCostArray *pdrspqcost = SPQOS_NEW(mp) CCostArray(mp);
		for (ULONG ul = 0; ul < arity; ul++)
		{
			CExpression *pexprChild = (*pdrspqexpr)[ul];
			CCost costChild = pexprChild->Cost();
			pdrspqcost->Append(SPQOS_NEW(mp) CCost(costChild));
		}
		cost = pcc->CostCompute(mp, pdrspqcost);
		pdrspqcost->Release();
	}
	CExpression *pexpr = SPQOS_NEW(mp)
		CExpression(mp, pop, pgexpr, pdrspqexpr, pcc->Pstats(), CCost(cost));
...
#endif	// SPQOS_DEBUG
		SPQOS_RAISE(spqopt::ExmaSPQOPT, spqopt::ExmiUnsatisfiedRequiredProperties);
	}
	return pexpr;
}
