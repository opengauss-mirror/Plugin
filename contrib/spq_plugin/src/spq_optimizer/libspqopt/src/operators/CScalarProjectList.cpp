...
CScalarProjectList::FHasMultipleDistinctAggs(CExpressionHandle &exprhdl)
{
	...
	CXformUtils::MapPrjElemsWithDistinctAggs(
		amp.Pmp(), pexprPrjList, &phmexprdrspqexpr, &ulDifferentDQAs);
	...
	CExpressionHandle &exprhdl)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		OID safe_oid = CMDIdSPQDB::CastMdid(popScAggFunc->MDId())->Oid();
		...
		if (!(safe_oid == SPQDB_INT4_AGG_MIN || safe_oid == SPQDB_INT4_AGG_MAX ||
			  safe_oid == SPQDB_INT4_AGG_AVG || safe_oid == SPQDB_INT4_AGG_SUM ||
			  safe_oid == SPQDB_INT4_AGG_COUNT || safe_oid == SPQDB_COUNT_STAR))
		{
			return false;
		}
	}
