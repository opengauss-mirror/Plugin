...
CExpressionHandle::~CExpressionHandle()
{
	...
	CRefCount::SafeRelease(m_pdrspqstat);
	CRefCount::SafeRelease(m_pdrspqrp);
}
...
CExpressionHandle::CopyStats()
{
	...
	// attach child stats
	SPQOS_ASSERT(NULL == m_pdrspqstat);
	m_pdrspqstat = SPQOS_NEW(m_mp) IStatisticsArray(m_mp);
	const ULONG arity = Arity();
	for (ULONG ul = 0; ul < arity; ul++)
	{
		IStatistics *child_stats = NULL;
...
//---------------------------------------------------------------------------
//	@function:
//		CExpressionHandle::PdrspqstatOuterRefs
//
//	@doc:
...
//---------------------------------------------------------------------------
IStatisticsArray *
CExpressionHandle::PdrspqstatOuterRefs(IStatisticsArray *statistics_array,
									  ULONG child_index)
{
	SPQOS_ASSERT(NULL != statistics_array);
	SPQOS_ASSERT(child_index < Arity());

	if (FScalarChild(child_index) || !HasOuterRefs(child_index))
	{
		// if child is scalar or has no outer references, return empty array
		return SPQOS_NEW(m_mp) IStatisticsArray(m_mp);
	}

	IStatisticsArray *pdrspqstatResult = SPQOS_NEW(m_mp) IStatisticsArray(m_mp);
	CColRefSet *outer_refs = DeriveOuterReferences(child_index);
	SPQOS_ASSERT(0 < outer_refs->Size());

	const ULONG size = statistics_array->Size();
	ULONG ulStartIndex = spqos::ulong_max;
	...
	if (spqos::ulong_max != ulStartIndex)
	{
		...
		CUtils::AddRefAppend<IStatistics, CleanupStats>(
			pdrspqstatResult, statistics_array, ulStartIndex);
	}

	return pdrspqstatResult;
}
...
CExpressionHandle::DeriveRootStats(IStatisticsArray *stats_ctxt)
{
	...
	if (FAttachedToLeafPattern())
	{
		...
		SPQOS_ASSERT(NULL != m_pexpr);
		SPQOS_ASSERT(NULL != m_pexpr->Pgexpr());
							   ...
							   BOOL fComputeRootStats)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		IStatisticsArray *pdrspqstatChildCtxt =
			PdrspqstatOuterRefs(pdrspqstatCurrentCtxt, ul);
		...
		else
		{
			...
			stats = (*Pgexpr())[ul]->PstatsRecursiveDerive(
				m_mp, m_mp, GetReqdRelationalProps(ul), pdrspqstatChildCtxt);
		}
		SPQOS_ASSERT(NULL != stats);

		// add child stat to current context
		stats->AddRef();
		pdrspqstatCurrentCtxt->Append(stats);
		pdrspqstatChildCtxt->Release();

		// add child stat to children stat array
		stats->AddRef();
		m_pdrspqstat->Append(stats);
...
{
	SPQOS_ASSERT(NULL != m_pcc);
	SPQOS_ASSERT(NULL == m_pcc->Pstats());
	...
	if (NULL != m_pstats && !m_pcc->FNeedsNewStats())
	{
		...
		// (note that m_pdrspqstat may not contain the correct DPE
		...
		!pepp->PpfmDerived()->IsEmpty())
	{
		...
		IStatistics *pstatsDS = popScan->PstatsDerive(
			m_mp, *this, m_pcc->Poc()->Prpp(), m_pcc->Poc()->Pdrspqstat());
	...
	m_pdrspqstat = SPQOS_NEW(m_mp) IStatisticsArray(m_mp);
	const ULONG arity = m_pcc->Pdrspqoc()->Size();
	for (ULONG ul = 0; ul < arity; ul++)
	{
		COptimizationContext *pocChild = (*m_pcc->Pdrspqoc())[ul];
		CCostContext *pccChild = pocChild->PccBest();
		SPQOS_ASSERT(NULL != pccChild);
		SPQOS_ASSERT(NULL != pccChild->Pstats());

		pccChild->Pstats()->AddRef();
		m_pdrspqstat->Append(pccChild->Pstats());
	}

	if (CPhysical::PopConvert(m_pgexpr->Pop())->FPassThruStats())
	{
		SPQOS_ASSERT(1 == m_pdrspqstat->Size());

		// copy stats from first child
		(*m_pdrspqstat)[0]->AddRef();
		m_pstats = (*m_pdrspqstat)[0];
	...
	SPQOS_ASSERT(NULL != pgexprForStats);

	CExpressionHandle exprhdl(m_mp);
	exprhdl.Attach(pgexprForStats);
	exprhdl.DeriveProps(NULL /*pdpctxt*/);
	m_pdrspqstat->AddRef();
	exprhdl.m_pdrspqstat = m_pdrspqstat;
	exprhdl.ComputeReqdProps(m_pcc->Poc()->GetReqdRelationalProps(),
							 0 /*ulOptReq*/);

	SPQOS_ASSERT(NULL == exprhdl.m_pstats);
							   ...
							   IStatisticsArray *stats_ctxt)
{
	...
	if (NULL == prprelNew)
	{
		...
		CColRefSet *pcrs = SPQOS_NEW(pmpGlobal) CColRefSet(pmpGlobal);
		prprelNew = SPQOS_NEW(pmpGlobal) CReqdPropRelational(pcrs);
	}
	else
	{
		prprelNew->AddRef();
	...
	IStatisticsArray *pdrspqstatCtxtNew = stats_ctxt;
	if (NULL == stats_ctxt)
	{
		// create empty context
		pdrspqstatCtxtNew = SPQOS_NEW(pmpGlobal) IStatisticsArray(pmpGlobal);
	}
	else
	{
		pdrspqstatCtxtNew->AddRef();
	}

	if (NULL != Pgexpr())
	{
		(void) Pgexpr()->Pgroup()->PstatsRecursiveDerive(
			pmpLocal, pmpGlobal, prprelNew, pdrspqstatCtxtNew);
	}
	else
	{
		SPQOS_ASSERT(NULL != Pexpr());

		(void) Pexpr()->PstatsDerive(prprelNew, pdrspqstatCtxtNew);
	}

	prprelNew->Release();
	pdrspqstatCtxtNew->Release();
}
...
CExpressionHandle::DerivePlanPropsForCostContext()
{
	...
	CDrvdPropCtxtPlan *pdpctxtplan = SPQOS_NEW(m_mp) CDrvdPropCtxtPlan(m_mp);
	CopyStats();
...
{
	SPQOS_ASSERT(NULL != prpInput);
	SPQOS_ASSERT(NULL == m_prp);
	SPQOS_ASSERT(NULL == m_pdrspqrp);
	...
	m_pdrspqrp = SPQOS_NEW(m_mp) CReqdPropArray(m_mp);
										 ...
										 ULONG ulOptReq)
{
	SPQOS_ASSERT(NULL != m_prp);
	SPQOS_ASSERT(NULL != m_pdrspqrp);
	SPQOS_ASSERT(m_pdrspqrp->Size() == Arity());
	SPQOS_ASSERT(child_index < m_pdrspqrp->Size() &&
	...
	else
	{
		...
		prp->Compute(m_mp, *this, m_prp, child_index, pdrspqdpCtxt, ulOptReq);
	}

	// replace required properties of given child
	m_pdrspqrp->Replace(child_index, prp);
}
...
CExpressionHandle::ComputeReqdProps(CReqdProp *prpInput, ULONG ulOptReq)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		ComputeChildReqdProps(ul, NULL /*pdrspqdpCtxt*/, ulOptReq);
	}
}
...
CExpressionHandle::GetRelationalProperties(ULONG child_index) const
{
	...
	SPQOS_ASSERT(NULL != m_pcc || NULL != m_pgexpr);
	...
	SPQOS_ASSERT(drvdProps->IsComplete());
...
{
	SPQOS_ASSERT(child_index < m_pdrspqstat->Size());

	return (*m_pdrspqstat)[child_index];
}
...
CExpressionHandle::Pdpplan(ULONG child_index) const
{
	...
	COptimizationContext *pocChild = (*m_pcc->Pdrspqoc())[child_index];
...
CExpressionHandle::GetReqdRelationalProps(ULONG child_index) const
{
	...
	CReqdProp *prp = (*m_pdrspqrp)[child_index];
	SPQOS_ASSERT(prp->FRelational() && "Unexpected property type");
...
CExpressionHandle::Prpp(ULONG child_index) const
{
	...
	CReqdProp *prp = (*m_pdrspqrp)[child_index];
	SPQOS_ASSERT(prp->FPlan() && "Unexpected property type");
...
CExpressionHandle::Pop() const
{
	...
	SPQOS_ASSERT(!"Handle was not attached properly");
	return NULL;
}
								 ...
								 CCostContext **grandchildContext) const
{
	...
	if (NULL != m_pcc)
	{
		...
		COptimizationContext *pocGrandchild =
			(*pccChild->Pdrspqoc())[grandchild_index];
...
CExpressionHandle::PexprScalarRepChild(ULONG child_index) const
{
	...
	if (NULL != m_pgexpr)
	{
		...
		SPQOS_ASSERT((*m_pgexpr)[child_index]->FScalar());
										 ...
										 BOOL error_on_null_return) const
{
	...
	if (NULL == result_expr && error_on_null_return)
	{
		SPQOS_RAISE(CException::ExmaInvalid, CException::ExmiInvalid,
				   SPQOS_WSZ_LIT("Generated invalid plan with subquery"));
	}
	return result_expr;
}
...
CExpressionHandle::UlFirstOptimizedChildIndex() const
{
	...
	SPQOS_ASSERT(CPhysical::EceoLeftToRight == eceo);
...
//		return spqos::ulong_max if there is no next child index
//
//
//---------------------------------------------------------------------------
ULONG
CExpressionHandle::UlNextOptimizedChildIndex(ULONG child_index) const
{
	...
	ULONG ulNextChildIndex = spqos::ulong_max;
	if (CPhysical::EceoRightToLeft == eceo)
	{
		if (0 < child_index)
		{
...
//		return spqos::ulong_max if there is no previous child index
//
//
//---------------------------------------------------------------------------
ULONG
CExpressionHandle::UlPreviousOptimizedChildIndex(ULONG child_index) const
{
	...
	ULONG ulPrevChildIndex = spqos::ulong_max;
	if (CPhysical::EceoRightToLeft == eceo)
	{
		if (Arity() - 1 > child_index)
		{
...
{
	SPQOS_ASSERT(NULL != pulChildIndex);
	...
	if (spqos::ulong_max == ulNextChildIndex)
	{
		return false;
	}
...
CExpressionHandle::Pdrspqfd(ULONG child_index)
{
	if (NULL != Pexpr())
	{
