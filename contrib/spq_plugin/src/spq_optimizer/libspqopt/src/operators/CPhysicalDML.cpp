...
{
	SPQOS_ASSERT(CLogicalDML::EdmlSentinel != edmlop);
	...
	m_pds =
		CPhysical::PdsCompute(m_mp, m_ptabdesc, pdrspqcrSource, pcrSegmentId);
		...
		ptabdesc->ConvertHashToRandom())
	{
		...
		// originally but then we either entered phase 1 of a spqexpand or we altered some of the partitions
		// to be randomly distributed (works on SPQDB 5X only).
		...
		//         based on the spq_segment_id of the row, which will work for both hash and random partitions
		...
		if (CLogicalDML::EdmlUpdate == edmlop)
		{
			...
			CColRefSet *updatedCols = SPQOS_NEW(mp) CColRefSet(mp);
			CColRefSet *distributionCols = hashDistSpec->PcrsUsed(mp);

			// compute a ColRefSet of the updated columns
			for (ULONG c = 0; c < pdrspqcrSource->Size(); c++)
			{
				if (pbsModified->Get(c))
				{
					updatedCols->Include((*pdrspqcrSource)[c]);
				}
			}
...
CPhysicalDML::PosComputeRequired(CMemoryPool *mp, CTableDescriptor *ptabdesc)
{
	...
	const CBitSetArray *pdrspqbsKeys = ptabdesc->PdrspqbsKeys();
	if (1 < pdrspqbsKeys->Size() && CLogicalDML::EdmlUpdate == m_edmlop)
	{
		// if this is an update on the target table's keys, enforce order on
		// the action column, see explanation in function's comment
		const ULONG ulKeySets = pdrspqbsKeys->Size();
...
CPhysicalDML::ComputeRequiredLocalColumns(CMemoryPool *mp)
{
	...
	m_pcrsRequiredLocal->Include(m_pdrspqcrSource);
