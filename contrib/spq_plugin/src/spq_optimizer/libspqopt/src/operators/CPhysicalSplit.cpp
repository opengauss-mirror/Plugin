...
//
//---------------------------------------------------------------------------
CPhysicalSplit::CPhysicalSplit(CMemoryPool *mp, CColRefArray *pdrspqcrDelete,
	  ...
	  m_pcrsRequiredLocal(NULL)
{
	...
	m_pcrsRequiredLocal->Include(m_pdrspqcrDelete);
	m_pcrsRequiredLocal->Include(m_pdrspqcrInsert);
...
CPhysicalSplit::~CPhysicalSplit()
{
	m_pdrspqcrDelete->Release();
	m_pdrspqcrInsert->Release();
...
CPhysicalSplit::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	CColRefSet *pcrsModified = SPQOS_NEW(mp) CColRefSet(mp);
	CColRefSet *pcrsDelete = SPQOS_NEW(mp) CColRefSet(mp);
	const ULONG num_cols = m_pdrspqcrDelete->Size();

	for (ULONG ul = 0; ul < num_cols; ul++)
	{
		CColRef *pcrOld = (*m_pdrspqcrDelete)[ul];
	...
	{
		CColRef *pcrOld = (*m_pdrspqcrDelete)[ul];
		CColRef *new_colref = (*m_pdrspqcrInsert)[ul];
		SPQOS_ASSERT(m_pdrspqcrDelete->Size() == m_pdrspqcrInsert->Size());
	...
	do
	{
		CColRefSet *pcrsHashed =
			CUtils::PcrsExtractColumns(mp, pdsHashed->Pdrspqexpr());
