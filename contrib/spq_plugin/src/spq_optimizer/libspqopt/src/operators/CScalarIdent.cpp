...
{
	SPQOS_ASSERT(!"Unexpected call of function FInputOrderSensitive");
	return false;
}
										 ...
										 BOOL must_exist)
{
	...
	return SPQOS_NEW(mp) CScalarIdent(mp, colref);
}
...
CScalarIdent::FAllowedFuncScId(CExpression *pexpr)
{
		...
		COperator::EopScalarIdent == (*pexpr)[0]->Pop()->Eopid())
	{
		...
		CMDIdSPQDB *funcmdid = CMDIdSPQDB::CastMdid(func->FuncMdId());
