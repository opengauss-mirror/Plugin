...
)
{
	...
	for (ULONG ul = 0; ul < ulPartIndexIds; ul++)
	{
		...
		CPartKeysArray *pdrspqpartkeys = ppimReqd->Pdrspqpartkeys(part_idx_id);
...
CPhysicalFilter::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
		...
		exprhdl.HasOuterRefs())
	{
			...
			!pdshashedOriginal->HasCompleteEquivSpec(mp))
		{
			...
			{
				SPQOS_ASSERT(!pdshashedOriginal->HasCompleteEquivSpec(mp));
			...
			CExpressionArray *pdrspqexprOriginal =
				pdshashedOriginal->Pdrspqexpr();
			pdrspqexprOriginal->AddRef();
			...
			if (NULL == pdshashedComplete)
			{
				...
				pdsResult = SPQOS_NEW(mp) CDistributionSpecHashed(
					pdrspqexprOriginal, pdshashedOriginal->FNullsColocated(),
					opfamiliesOriginal);
			}
			else
			{
