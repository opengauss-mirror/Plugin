	  ...
	  m_inner_merge_clauses(inner_merge_clauses)
{
	...
	SPQOS_ASSERT(NULL != outer_merge_clauses);
	SPQOS_ASSERT(NULL != inner_merge_clauses);
	SPQOS_ASSERT(outer_merge_clauses->Size() == inner_merge_clauses->Size());
	...
	ULONG num_hash_reqs = std::min((ULONG) SPQOPT_MAX_HASH_DIST_REQUESTS,
							...
							CDrvdPropArray * /*pdrspqdpCtxt*/, ULONG ulOptReq)
{
	...
	ULONG num_hash_reqs =
		std::min((ULONG) SPQOPT_MAX_HASH_DIST_REQUESTS, clauses->Size());
	if (ulOptReq < num_hash_reqs)
	{
		CExpressionArray *pdrspqexprCurrent = SPQOS_NEW(mp) CExpressionArray(mp);
		CExpression *expr = (*clauses)[ulOptReq];
		expr->AddRef();
		pdrspqexprCurrent->Append(expr);

		CDistributionSpecHashed *pds = SPQOS_NEW(mp)
			CDistributionSpecHashed(pdrspqexprCurrent, nulls_collocated);
		return SPQOS_NEW(mp) CEnfdDistribution(pds, CEnfdDistribution::EdmExact);
	}
	else if (ulOptReq == num_hash_reqs)
	{
		clauses->AddRef();
		CDistributionSpecHashed *pds =
			SPQOS_NEW(mp) CDistributionSpecHashed(clauses, nulls_collocated);
		return SPQOS_NEW(mp) CEnfdDistribution(pds, CEnfdDistribution::EdmExact);
	}
	else
	{
		SPQOS_ASSERT(ulOptReq == (num_hash_reqs + 1));
		return SPQOS_NEW(mp) CEnfdDistribution(
			PdsRequireSingleton(mp, exprhdl, pdsRequired, child_index),
			CEnfdDistribution::EdmExact);
	}
...
) const
{
	...
	COrderSpec *os = SPQOS_NEW(mp) COrderSpec(mp);
	...
	for (ULONG ul = 0; ul < clauses->Size(); ++ul)
	{
		...
		SPQOS_ASSERT(CUtils::FScalarIdent(expr));
								  ...
								  CExpressionHandle &exprhdl) const
{
	...
	SPQOS_ASSERT(CDistributionSpec::EdtSingleton == pdsOuter->Edt() ||
