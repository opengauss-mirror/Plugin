...
{
	m_phmPexprPartPred = SPQOS_NEW(mp) ExprPredToExprPredPartMap(mp);
}
...
) const
{
	...
	CPartKeysArray *pdrspqpartkeys = ppartinfo->Pdrspqpartkeys(0 /*ulPos*/);
	...
	for (ULONG ul = 0; NULL == pexprPredOnPartKey && ul < ulKeySets; ul++)
	{
		pexprPredOnPartKey = CPredicateUtils::PexprExtractPredicatesOnPartKeys(
			mp, pexprScalar, (*pdrspqpartkeys)[ul]->Pdrspqdrspqcr(),
