...
{
	SPQOS_ASSERT(CDistributionSpecSingleton::EstSentinel != est);

	m_pdssSingeton = SPQOS_NEW(mp) CDistributionSpecSingleton(est);
	m_pos = SPQOS_NEW(m_mp) COrderSpec(m_mp);
...
CPhysicalMotionGather::PopConvert(COperator *pop)
{
	...
	SPQOS_ASSERT(EopPhysicalMotionGather == pop->Eopid());
