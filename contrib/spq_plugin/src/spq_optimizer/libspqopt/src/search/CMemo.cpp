...
#define SPQOPT_MEMO_HT_BUCKETS 50000
	...
	: m_mp(mp), m_aul(0), m_pgroupRoot(NULL), m_ulpGrps(0), m_pmemotmap(NULL)
{
	...
	m_sht.Init(
		mp, SPQOPT_MEMO_HT_BUCKETS, SPQOS_OFFSET(CGroupExpression, m_linkMemo),
	...
	m_listGroups.Init(SPQOS_OFFSET(CGroup, m_link));
}
...
{
	SPQOS_ASSERT(NULL == m_pgroupRoot);
...
{
	SPQOS_ASSERT(NULL != pgroup);
	SPQOS_ASSERT(NULL != pexprOrigin);
	SPQOS_ASSERT(!pexprOrigin->Pop()->FPhysical() &&
...
{
	SPQOS_ASSERT(NULL != pgroupTarget);
	SPQOS_ASSERT(NULL != pgexpr);
...
{
	SPQOS_ASSERT(NULL != ppgroupTarget);

	if (NULL == *ppgroupTarget && NULL == pgexpr)
	{
		*ppgroupTarget = SPQOS_NEW(m_mp) CGroup(m_mp, fScalar);
					...
					CGroupExpression *pgexpr)
{
	...
	SPQOS_CHECK_ABORT;
	SPQOS_ASSERT(NULL != pexprOrigin);
	SPQOS_ASSERT(pgexpr->Pop() == pexprOrigin->Pop());
	SPQOS_ASSERT(pgexpr->Arity() == pexprOrigin->Arity());
						...
						CReqdPropPlan *prppInput, ULONG ulSearchStages)
{
	...
	CCost cost = SPQOPT_INVALID_COST;
	...
	if (pgroupRoot->FScalar())
	{
		...
		CGroupProxy spq(pgroupRoot);
		pgexprBest = spq.PgexprFirst();
	}
	else
	{
	...
	for (ULONG i = 0; i < arity; i++)
	{
		...
		if (!pgroupChild->FScalar())
		{
			...
			COptimizationContext *pocChild = (*poc->PccBest()->Pdrspqoc())[i];
	...
	CExpression *pexpr = SPQOS_NEW(mp)
		CExpression(mp, pgexprBest->Pop(), pgexprBest, pdrspqexpr, stats, cost);
...
CMemo::FRehash()
{
	SPQOS_ASSERT(m_pgroupRoot->FExplored());
	SPQOS_ASSERT(!m_pgroupRoot->FImplemented());

	// dump memo hash table into a local list
	CList<CGroupExpression> listGExprs;
	listGExprs.Init(SPQOS_OFFSET(CGroupExpression, m_linkMemo));
	...
	while (!listGExprs.IsEmpty())
	{
		...
		SPQOS_ASSERT(pgexprFound != pgexpr);
...
FORCE_GENERATE_DBGSTR(spqopt::CMemo);
...
{
	SPQOS_ASSERT(NULL != poc);
	SPQOS_ASSERT(NULL == m_pmemotmap && "tree map is already built");

	m_pmemotmap = SPQOS_NEW(m_mp) MemoTreeMap(m_mp, CExpression::PexprRehydrate);
	m_pgroupRoot->BuildTreeMap(m_mp, poc, NULL /*pccParent*/,
							   spqos::ulong_max /*child_index*/, m_pmemotmap);
}
