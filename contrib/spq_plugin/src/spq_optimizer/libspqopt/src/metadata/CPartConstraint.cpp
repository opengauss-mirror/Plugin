...
CPartConstraint::CPartConstraint(CMemoryPool *mp,
	...
	: m_phmulcnstr(phmulcnstr),
	  ...
	  m_pdrspqdrspqcr(pdrspqdrspqcr)
{
	SPQOS_ASSERT(NULL != phmulcnstr);
	SPQOS_ASSERT(NULL != pbsDefaultParts);
	SPQOS_ASSERT(NULL != pdrspqdrspqcr);
	m_num_of_part_levels = pdrspqdrspqcr->Size();
	SPQOS_ASSERT_IMP(is_unbounded, FAllDefaultPartsIncluded());
	  ...
	  m_fUninterpreted(false)
{
	...
	SPQOS_ASSERT_IMP(is_unbounded, fDefaultPartition);

	m_phmulcnstr = SPQOS_NEW(mp) UlongToConstraintMap(mp);
#ifdef SPQOS_DEBUG
	BOOL result =
#endif	// SPQOS_DEBUG
		m_phmulcnstr->Insert(SPQOS_NEW(mp) ULONG(0 /*ulLevel*/), pcnstr);
	SPQOS_ASSERT(result);
	...
	m_pdrspqdrspqcr = SPQOS_NEW(mp) CColRef2dArray(mp);
	m_pdrspqdrspqcr->Append(colref_array);

	m_num_of_part_levels = 1;
	m_pbsDefaultParts = SPQOS_NEW(mp) CBitSet(mp);
...
CPartConstraint::CPartConstraint(BOOL fUninterpreted)
	: m_phmulcnstr(NULL),
	  ...
	  m_pdrspqdrspqcr(NULL),
	  m_pcnstrCombined(NULL)
{
	SPQOS_ASSERT(fUninterpreted);
}
...
CPartConstraint::~CPartConstraint()
{
	...
	CRefCount::SafeRelease(m_pdrspqdrspqcr);
...
{
	SPQOS_ASSERT(NULL != ppartcnstr);
	SPQOS_ASSERT(!IsConstraintUnbounded());
	SPQOS_ASSERT(!ppartcnstr->IsConstraintUnbounded());

	CConstraintArray *pdrspqcnstr = SPQOS_NEW(mp) CConstraintArray(mp);
	CConstraint *pcnstrCurrent = Pcnstr(ulLevel);
	CConstraint *pcnstrOther = ppartcnstr->Pcnstr(ulLevel);
	SPQOS_ASSERT(NULL != pcnstrCurrent);
	SPQOS_ASSERT(NULL != pcnstrOther);

	pcnstrCurrent->AddRef();
	pcnstrOther->AddRef();
	pdrspqcnstr->Append(pcnstrCurrent);
	pdrspqcnstr->Append(pcnstrOther);

	CConstraint *pcnstrIntersect =
		CConstraint::PcnstrConjunction(mp, pdrspqcnstr);
									 ...
									 CPartConstraint *ppartcnstr)
{
	...
	CBitSet *pbsDefaultParts = SPQOS_NEW(mp) CBitSet(mp);
	...
	BOOL result =
#endif	// SPQOS_DEBUG
		phmulcnstr->Insert(SPQOS_NEW(mp) ULONG(0), pcnstrRemaining);
	SPQOS_ASSERT(result);
	...
	m_pdrspqdrspqcr->AddRef();
	return SPQOS_NEW(mp) CPartConstraint(mp, phmulcnstr, pbsDefaultParts,
										false /*is_unbounded*/, m_pdrspqdrspqcr);
}
...
{
	SPQOS_ASSERT(NULL != pcnstrSnd);

	pcnstrSnd->AddRef();
	CConstraint *pcnstrNegation =
		SPQOS_NEW(mp) CConstraintNegation(mp, pcnstrSnd);
	...
	CConstraintArray *pdrspqcnstr = SPQOS_NEW(mp) CConstraintArray(mp);
	pcnstrFst->AddRef();
	pdrspqcnstr->Append(pcnstrFst);
	pdrspqcnstr->Append(pcnstrNegation);
	...
	CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	if (m_fUninterpreted)
	{
		return SPQOS_NEW(mp) CPartConstraint(true /*m_fUninterpreted*/);
	}
	...
	for (ULONG ul = 0; ul < m_num_of_part_levels; ul++)
	{
		...
		CColRefArray *pdrspqcrMapped =
			CUtils::PdrspqcrRemap(mp, colref_array, colref_mapping, must_exist);
		pdrspqdrspqcr->Append(pdrspqcrMapped);
		...
		if (NULL != pcnstr)
		{
			...
			BOOL result =
#endif	// SPQOS_DEBUG
				phmulcnstr->Insert(SPQOS_NEW(mp) ULONG(ul), pcnstrRemapped);
			SPQOS_ASSERT(result);
		}
	}

	m_pbsDefaultParts->AddRef();
	return SPQOS_NEW(mp) CPartConstraint(mp, phmulcnstr, m_pbsDefaultParts,
										m_is_unbounded, pdrspqdrspqcr);
}
...
{
	SPQOS_ASSERT(NULL != ppartcnstrFst);
	SPQOS_ASSERT(NULL != ppartcnstrSnd);
	SPQOS_ASSERT(ppartcnstrFst->m_num_of_part_levels ==
									   ...
									   CPartConstraint *ppartcnstrSnd)
{
	...
	UlongToConstraintMap *phmulcnstr = SPQOS_NEW(mp) UlongToConstraintMap(mp);
	CBitSet *pbsCombined = SPQOS_NEW(mp) CBitSet(mp);
	...
	for (ULONG ul = 0; ul < ulLevels - 1; ul++)
	{
		...
		BOOL result =
#endif	// SPQOS_DEBUG
			phmulcnstr->Insert(SPQOS_NEW(mp) ULONG(ul), pcnstrFst);
		SPQOS_ASSERT(result);
	...
	pcnstrFst->AddRef();
	pcnstrSnd->AddRef();
	CConstraintArray *pdrspqcnstrCombined = SPQOS_NEW(mp) CConstraintArray(mp);

	pdrspqcnstrCombined->Append(pcnstrFst);
	pdrspqcnstrCombined->Append(pcnstrSnd);

	CConstraint *pcnstrDisj =
		CConstraint::PcnstrDisjunction(mp, pdrspqcnstrCombined);
	SPQOS_ASSERT(NULL != pcnstrDisj);
#ifdef SPQOS_DEBUG
	BOOL result =
#endif	// SPQOS_DEBUG
		phmulcnstr->Insert(SPQOS_NEW(mp) ULONG(ulLevels - 1), pcnstrDisj);
	SPQOS_ASSERT(result);
	...
	CColRef2dArray *pdrspqdrspqcr = ppartcnstrFst->Pdrspqdrspqcr();
	pdrspqdrspqcr->AddRef();
	return SPQOS_NEW(mp) CPartConstraint(mp, phmulcnstr, pbsCombined,
										false /*is_unbounded*/, pdrspqdrspqcr);
}
...
{
	SPQOS_ASSERT(NULL != ppartcnstrmapDest);
	SPQOS_ASSERT(NULL != ppartcnstrmapSource);
	...
	while (pcmi.Advance())
	{
		...
		SPQOS_ASSERT_IMP(NULL != ppartcnstrDest,
		...
		if (NULL == ppartcnstrDest)
		{
			...
			BOOL result =
#endif	// SPQOS_DEBUG
				ppartcnstrmapDest->Insert(SPQOS_NEW(mp) ULONG(ulKey),
										  ppartcnstrSource);

			SPQOS_ASSERT(result && "Duplicate part constraints");
		}
	}
}
	...
	UlongToPartConstraintMap *ppartcnstrmapSnd)
{
	...
	SPQOS_ASSERT(NULL != ppartcnstrmapFst);
	SPQOS_ASSERT(NULL != ppartcnstrmapSnd);
