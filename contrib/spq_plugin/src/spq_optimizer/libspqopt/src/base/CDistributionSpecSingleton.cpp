											...
											CExpression *pexpr)
{
	...
	if (SPQOS_FTRACE(EopttraceDisableMotionGather))
	{
	...
	CExpression *pexprMotion = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CPhysicalMotionGather(mp, m_est), pexpr);
	...
	if (!prpp->Peo()->PosRequired()->IsEmpty())
	{
		...
		CExpression *pexprGatherMerge = SPQOS_NEW(mp) CExpression(
			mp, SPQOS_NEW(mp) CPhysicalMotionGather(mp, m_est, pos), pexpr);
		pdrspqexpr->Append(pexprGatherMerge);
	}
}
