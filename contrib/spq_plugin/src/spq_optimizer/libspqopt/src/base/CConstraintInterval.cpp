...
{
	return (!m_fIncludesNull && 0 == m_pdrspqrng->Size());
}
...
{
	return (m_fIncludesNull && 1 == m_pdrspqrng->Size() &&
			(*m_pdrspqrng)[0]->IsConstraintUnbounded());
}
											   ...
											   BOOL infer_nulls_as)
{
	...
	switch (pexpr->Pop()->Eopid())
	{
		...
		case COperator::EopScalarConst:
		{
			...
			{
				pci = SPQOS_NEW(mp) CConstraintInterval(
					mp, colref, SPQOS_NEW(mp) CRangeArray(mp),
					false /*fIncludesNull*/);
			}
		}
		break;
													  ...
													  BOOL infer_nulls_as)
{
	...
	spqos::CAutoRef<CDatumSortedSet> apdatumsortedset(
		SPQOS_NEW(mp) CDatumSortedSet(mp, pexprArray, pcomp));
	// construct ranges representing IN or NOT IN
	CRangeArray *prgrng = SPQOS_NEW(mp) CRangeArray(mp);
	...
	return SPQOS_NEW(mp) CConstraintInterval(mp, colref, prgrng, infer_nulls_as);
}
											   ...
											   CColRef *colref)
{
	...
	SPQOS_ASSERT_IMP(NULL != colref, pcrFirst == colref);
...
{
	SPQOS_ASSERT(NULL != pexpr);
	SPQOS_ASSERT(CUtils::FScalarNullTest(pexpr));
	...
	if (CUtils::FScalarIdent(pexprChild))
	{
		...
		return SPQOS_NEW(mp) CConstraintInterval(
			mp, colref, SPQOS_NEW(mp) CRangeArray(mp), true /*fIncludesNull*/);
	}
												...
												BOOL infer_nulls_as)
{
	...
	CRangeArray *pdrnspqrng = PciRangeFromColConstCmp(mp, cmp_type, popScConst);
	...
	if (NULL != pdrnspqrng)
	{
		...
		pcri = SPQOS_NEW(mp)
			CConstraintInterval(mp, colref, pdrnspqrng, infer_nulls_as);
	}
	return pcri;
}
...
{
	SPQOS_ASSERT(NULL != pexpr);
	SPQOS_ASSERT(CUtils::FScalarCmp(pexpr) || CUtils::FScalarArrayCmp(pexpr));
	...
	if (CPredicateUtils::FCompareIdentToConst(pexpr))
	{
		...
		{
			SPQOS_ASSERT(CCastUtils::FBinaryCoercibleCastedConst(pexprRight));
...
{
	SPQOS_ASSERT(NULL != pexpr);
	SPQOS_ASSERT(CPredicateUtils::FIDF(pexpr));
	...
	if (CPredicateUtils::FIdentIDFConst(pexpr))
	{
		...
		SPQOS_ASSERT(CScalar::EopScalarConst == popScConst->Eopid());
		SPQOS_ASSERT(IMDType::EcmptIDF == popScCmp->ParseCmpType());
		...
		if (datum->IsNull())
		{
			...
			CConstraintInterval *pcriChild = SPQOS_NEW(mp)
				CConstraintInterval(mp, colref, SPQOS_NEW(mp) CRangeArray(mp),
									true /*fIncludesNull*/);
		...
		else
		{
			...
			CRangeArray *pdrspqrng = PciRangeFromColConstCmp(
				mp, popScCmp->ParseCmpType(), popScConst);
			if (NULL != pdrspqrng)
			{
				pcri = SPQOS_NEW(mp) CConstraintInterval(mp, colref, pdrspqrng,
														true /*fIncludesNull*/);
			}
		}
												 ...
												 BOOL infer_nulls_as)
{
	SPQOS_ASSERT(NULL != pexpr);
	SPQOS_ASSERT(CUtils::FScalarBoolOp(pexpr));
	SPQOS_ASSERT(CScalarBoolOp::EboolopOr ==
	...
	CConstraintIntervalArray *child_constraints =
		SPQOS_NEW(mp) CConstraintIntervalArray(mp);
	...
	{
		constraints = SPQOS_NEW(mp) CConstraintIntervalArray(mp);
	...
	SPQOS_ASSERT(child_constraints->Size() == 1);
												  ...
												  BOOL infer_nulls_as)
{
	SPQOS_ASSERT(NULL != pexpr);
	SPQOS_ASSERT(CUtils::FScalarBoolOp(pexpr));
	SPQOS_ASSERT(CScalarBoolOp::EboolopAnd ==
				CScalarBoolOp::PopConvert(pexpr->Pop())->Eboolop());
...
CConstraintInterval::PexprConstructDisjunctionScalar(CMemoryPool *mp) const
{
	...
	const ULONG length = m_pdrspqrng->Size();
	for (ULONG ul = 0; ul < length; ul++)
	{
		CRange *prange = (*m_pdrspqrng)[ul];
		CExpression *pexprChild = prange->PexprScalar(mp, m_pcr);
		pdrspqexpr->Append(pexprChild);
	}

	if (1 == pdrspqexpr->Size() && CUtils::FScalarConstTrue((*pdrspqexpr)[0]))
	{
...
CConstraintInterval::FConvertsToNotIn() const
{
	...
	if ((*m_pdrspqrng)[0]->PdatumLeft() != NULL ||
		(*m_pdrspqrng)[m_pdrspqrng->Size() - 1]->PdatumRight() != NULL)
	{
		return false;
	}
...
{
	SPQOS_ASSERT(FConvertsToIn() || FConvertsToNotIn());

	ULONG ulRngs = m_pdrspqrng->Size();
	...
	CExpressionArray *prngexpr = SPQOS_NEW(mp) CExpressionArray(mp);
	...
	{
		IDatum *datum = (*m_pdrspqrng)[ul]->PdatumRight();
		datum->AddRef();
		CScalarConst *popScConst = SPQOS_NEW(mp) CScalarConst(mp, datum);
		CExpression *pexpr = SPQOS_NEW(mp) CExpression(mp, popScConst);
										  ...
										  CColRef *colref) const
{
	...
	m_pdrspqrng->AddRef();
	return SPQOS_NEW(mp)
		CConstraintInterval(mp, colref, m_pdrspqrng, m_fIncludesNull);
}
...
CConstraintInterval::PciIntersect(CMemoryPool *mp, CConstraintInterval *pci)
{
	...
	SPQOS_ASSERT(m_pcr == pci->Pcr());

	CRangeArray *pdrspqrngOther = pci->Pdrspqrng();

	CRangeArray *pdrspqrngNew = SPQOS_NEW(mp) CRangeArray(mp);

	ULONG ulFst = 0;
	ULONG ulSnd = 0;
	const ULONG ulNumRangesFst = m_pdrspqrng->Size();
	const ULONG ulNumRangesSnd = pdrspqrngOther->Size();
	while (ulFst < ulNumRangesFst && ulSnd < ulNumRangesSnd)
	{
		CRange *prangeThis = (*m_pdrspqrng)[ulFst];
		CRange *prangeOther = (*pdrspqrngOther)[ulSnd];
		...
		if (NULL != prangeNew)
		{
			pdrspqrngNew->Append(prangeNew);
		}
	}

	return SPQOS_NEW(mp) CConstraintInterval(
		mp, m_pcr, pdrspqrngNew, m_fIncludesNull && pci->FIncludesNull());
}
...
CConstraintInterval::PciUnion(CMemoryPool *mp, CConstraintInterval *pci)
{
	...
	while (ulFst < ulNumRangesFst && ulSnd < ulNumRangesSnd)
	{
		...
		AppendOrExtend(mp, pdrspqrngNew, prangeNew);
	}

	AddRemainingRanges(mp, m_pdrspqrng, ulFst, pdrspqrngNew);
	AddRemainingRanges(mp, pdrspqrngOther, ulSnd, pdrspqrngNew);
...
CConstraintInterval::PciDifference(CMemoryPool *mp, CConstraintInterval *pci)
{
	...
	ULONG ulFst = 0;
	ULONG ulSnd = 0;
	CRangeArray *pdrspqrngResidual = SPQOS_NEW(mp) CRangeArray(mp);
	...
	while (ulFst < ulNumRangesFst && ulSnd < ulNumRangesSnd)
	{
		...
		CRange *prangeThis =
			(NULL == prangeResidual ? (*m_pdrspqrng)[ulFst] : prangeResidual);
		CRange *prangeOther = (*pdrspqrngOther)[ulSnd];
		...
		else
		{
			prangeNew = PrangeDiffWithRightResidual(
				mp, prangeThis, prangeOther, &prangeResidual, pdrspqrngResidual);
			ulSnd++;
		}
	...
	AppendOrExtend(mp, pdrspqrngNew, prangeResidual);
	pdrspqrngResidual->Release();
	AddRemainingRanges(mp, m_pdrspqrng, ulFst, pdrspqrngNew);
								  ...
								  BOOL fIncludesNull)
{
	...
	return SPQOS_NEW(mp)
		CConstraintInterval(mp, colref, pdrspqrng, fIncludesNull);
}
...
CConstraintInterval::MdidType()
{
	...
	if (0 < m_pdrspqrng->Size())
	{
		CRange *prange = (*m_pdrspqrng)[0];
		return prange->MDId();
	}
												 ...
												 CRangeArray *pdrspqrngResidual)
{
	...
	*pprangeResidual = prangeFirst->PrngDifferenceRight(mp, prangeSecond);
	...
	{
		pdrspqrngResidual->Append(*pprangeResidual);
	}
...
CConstraintInterval::AddRemainingRanges(CMemoryPool *mp,
										CRangeArray *pdrspqrngSrc, ULONG ulStart,
										CRangeArray *pdrspqrngDest)
{
	const ULONG length = pdrspqrngSrc->Size();
	for (ULONG ul = ulStart; ul < length; ul++)
	{
		CRange *prange = (*pdrspqrngSrc)[ul];
		prange->AddRef();
		AppendOrExtend(mp, pdrspqrngDest, prange);
	}
}
...
//---------------------------------------------------------------------------
void
CConstraintInterval::AppendOrExtend(CMemoryPool *mp, CRangeArray *pdrspqrng,
									CRange *prange)
{
	if (NULL == prange)
	{
	...
	const ULONG length = pdrspqrng->Size();
	if (0 == length)
	{
		pdrspqrng->Append(prange);
		return;
	...
	CRange *prangeLast = (*pdrspqrng)[length - 1];
	...
	{
		pdrspqrng->Replace(length - 1, prangeNew);
		prange->Release();
	}
}
...
{
	SPQOS_ASSERT(CScalar::EopScalarConst == popsccnst->Eopid());
	...
	if (IMDType::EcmptNEq == cmp_type || IMDType::EcmptIDF == cmp_type)
	{
		...
		datum->AddRef();
		pdrspqrng->Append(SPQOS_NEW(mp) CRange(pcomp, IMDType::EcmptL, datum));
		datum->AddRef();
		pdrspqrng->Append(SPQOS_NEW(mp) CRange(pcomp, IMDType::EcmptG, datum));
	}
	else
	{
		datum->AddRef();
		pdrspqrng->Append(SPQOS_NEW(mp) CRange(pcomp, cmp_type, datum));
	}
