...
#define SPQOPT_DISTR_SPEC_HASHED_EXPRESSIONS (ULONG(5))
...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed::CDistributionSpecHashed(CExpressionArray *pdrspqexpr,
												 BOOL fNullsColocated,
												 IMdIdArray *opfamilies)
	: m_pdrspqexpr(pdrspqexpr),
	  ...
	  m_equiv_hash_exprs(NULL)
{
	...
	SPQOS_ASSERT(m_opfamilies == NULL ||
				m_opfamilies->Size() == m_pdrspqexpr->Size());
}
...
CDistributionSpecHashed::PopulateDefaultOpfamilies()
{
	...
	m_opfamilies = SPQOS_NEW(mp) IMdIdArray(mp);
	for (ULONG ul = 0; ul < m_pdrspqexpr->Size(); ++ul)
	{
		CExpression *expr = (*m_pdrspqexpr)[ul];
		IMDId *mdid_type = CScalar::PopConvert(expr->Pop())->MdidType();
		IMDId *mdid_opfamily =
			mda->RetrieveType(mdid_type)->GetDistrOpfamilyMdid();
		SPQOS_ASSERT(NULL != mdid_opfamily && mdid_opfamily->IsValid());
	...
	CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	...
	const ULONG length = m_pdrspqexpr->Size();
	for (ULONG ul = 0; ul < length; ul++)
	{
		CExpression *pexpr = (*m_pdrspqexpr)[ul];
		pdrspqexpr->Append(pexpr->PexprCopyWithRemappedColumns(
	...
	{
		return SPQOS_NEW(mp)
			CDistributionSpecHashed(pdrspqexpr, m_fNullsColocated);
	}
	...
	return SPQOS_NEW(mp) CDistributionSpecHashed(pdrspqexpr, m_fNullsColocated,
...
CDistributionSpecHashed::StripEquivColumns(CMemoryPool *mp)
{
	...
	return SPQOS_NEW(mp)
		CDistributionSpecHashed(m_pdrspqexpr, m_fNullsColocated, m_opfamilies);
}
...
{
	const ULONG length = m_pdrspqexpr->Size();
	COperator *pop = (*(m_pdrspqexpr))[0]->Pop();
...
CDistributionSpecHashed::FSatisfies(const CDistributionSpec *pds) const
{
	...
	SPQOS_ASSERT(EdtHashed == pds->Edt());
	...
	if (pdsHashed->FDistributionSpecHashedOnlyOnGpSegmentId())
	{
		...
		//
		// SELECT * FROM t t1, t t2 WHERE t1.spq_segment_id=t2.id;
...
{
	const ULONG ulOwnExprs = m_pdrspqexpr->Size();
	const ULONG ulOtherExprs = pdsHashed->m_pdrspqexpr->Size();
		...
		for (ULONG ulInner = 0; ulInner < ulOtherExprs; ulInner++)
		{
			CExpression *pexprOther =
				CCastUtils::PexprWithoutBinaryCoercibleCasts(
					(*(pdsHashed->m_pdrspqexpr))[ulInner]);
												 ...
												 CColRefSet *pcrs)
{
	...
	const ULONG ulExprs = m_pdrspqexpr->Size();
	for (ULONG ul = 0; ul < ulExprs; ul++)
	{
		...
		pexpr->AddRef();
		pdrspqexprNew->Append(pexpr);
	}

	if (0 == pdrspqexprNew->Size())
	{
	...
	return SPQOS_NEW(mp)
		CDistributionSpecHashed(pdrspqexprNew, m_fNullsColocated);
}
										 ...
										 CExpression *pexpr)
{
	...
	if (SPQOS_FTRACE(EopttraceDisableMotionHashDistribute))
	{
	...
	CExpression *pexprMotion = SPQOS_NEW(mp) CExpression(
		mp, SPQOS_NEW(mp) CPhysicalMotionHashDistribute(mp, this_copy), pexpr);
...
CDistributionSpecHashed::HashValue() const
{
	...
	ULONG ulHashedExpressions =
		std::min(m_pdrspqexpr->Size(), SPQOPT_DISTR_SPEC_HASHED_EXPRESSIONS);

	for (ULONG ul = 0; ul < ulHashedExpressions; ul++)
	{
		CExpression *pexpr = (*m_pdrspqexpr)[ul];
		ulHash = spqos::CombineHashes(ulHash, CExpression::HashValue(pexpr));
	}
	...
	if (NULL != m_opfamilies && m_opfamilies->Size() > 0)
	{
		for (ULONG ul = 0; ul < m_opfamilies->Size(); ul++)
		{
			...
			ulHash = spqos::CombineHashes(ulHash, mdid->HashValue());
		}
	}
	...
	if (NULL != m_equiv_hash_exprs && m_equiv_hash_exprs->Size() > 0)
	{
		for (ULONG ul = 0; ul < m_equiv_hash_exprs->Size(); ul++)
		{
			...
			for (ULONG id = 0; id < equiv_distribution_exprs->Size(); id++)
			{
				...
				ulHash =
					spqos::CombineHashes(ulHash, CExpression::HashValue(pexpr));
			}
		}
	}
	return ulHash;
...
{
	return CUtils::PcrsExtractColumns(mp, m_pdrspqexpr);
}
	...
	const CDistributionSpecHashed *pdshashed) const
{
	...
	if (m_pdrspqexpr->Size() != pdshashed->m_pdrspqexpr->Size() ||
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CExpression *pexprLeft = (*(pdshashed->m_pdrspqexpr))[ul];
		CExpression *pexprRight = (*m_pdrspqexpr)[ul];
...
CDistributionSpecHashed::Equals(const CDistributionSpec *input_spec) const
{
	...
	BOOL matches =
		...
		CUtils::Equals(m_pdrspqexpr, other_spec->m_pdrspqexpr);
										  ...
										  BOOL fNullsColocated)
{
	...
	CColRefArray *pdrspqcrHashable =
		CUtils::PdrspqcrRedistributableSubset(mp, colref_array);
	CDistributionSpecHashed *pdshashed = NULL;
	if (0 < pdrspqcrHashable->Size())
	{
		CExpressionArray *pdrspqexpr =
			CUtils::PdrspqexprScalarIdents(mp, pdrspqcrHashable);
		pdshashed =
			SPQOS_NEW(mp) CDistributionSpecHashed(pdrspqexpr, fNullsColocated);
	}
	pdrspqcrHashable->Release();
	...
	const CExpressionArray *dist_cols_expr_array) const
{
	...
	while (pds && !covers)
	{
		covers = CUtils::Contains(dist_cols_expr_array, pds->Pdrspqexpr());
...
{
	CExpressionArray *distribution_exprs = m_pdrspqexpr;
	...
	if (NULL == equiv_distribution_all_exprs)
	{
		...
		equiv_distribution_all_exprs = SPQOS_NEW(mp) CExpressionArrays(mp);
			 ...
			 distribution_key_idx++)
		{
			...
			SPQOS_ASSERT(NULL != distribution_colref);
	...
	SPQOS_ASSERT(equiv_distribution_all_exprs->Size() == m_pdrspqexpr->Size());
...
CDistributionSpecHashed::Copy(CMemoryPool *mp)
{
	...
	CDistributionSpecHashed *result = SPQOS_NEW(mp)
		CDistributionSpecHashed(m_pdrspqexpr, m_fNullsColocated, m_opfamilies);
...
CDistributionSpecHashed::Copy(CMemoryPool *mp, BOOL fNullsColocated)
{
	...
	CDistributionSpecHashed *result = SPQOS_NEW(mp)
		CDistributionSpecHashed(m_pdrspqexpr, fNullsColocated, m_opfamilies);
										   ...
										   const char *prefix) const
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		CExpression *hash_expr = (*m_pdrspqexpr)[ul];

		if (!SPQOS_FTRACE(EopttracePrintEquivDistrSpecs) && 1 == length &&
	...
	if (NULL != m_pdshashedEquiv && SPQOS_FTRACE(EopttracePrintEquivDistrSpecs))
	{
		os << ", equiv. dist: ";
	...
	if (NULL != m_equiv_hash_exprs && m_equiv_hash_exprs->Size() > 0 &&
		SPQOS_FTRACE(EopttracePrintEquivDistrSpecs))
	{
		os << "," << std::endl;
		for (ULONG ul = 0; ul < m_equiv_hash_exprs->Size(); ul++)
		{
...
CDistributionSpecHashed::GetAllDistributionExprs(CMemoryPool *mp)
{
	...
	while (spec)
	{
		CExpressionArray *distribution_exprs = spec->Pdrspqexpr();
	...
	CColRefSet *outerRefs)
{
	...
	const ULONG size = pdrspqexprHashed->Size();
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		CExpression *pexprMatching =
			CUtils::PexprMatchEqualityOrINDF(pexpr, pdrspqexprPred);
			...
			outerRefs->FIntersects(pexprMatching->DeriveUsedColumns()))
		{
			...
			pdrspqexprResult->Append(pexprMatching);
		}
		else
		{
			++num_orig_exprs_used;
	...
	SPQOS_ASSERT(pdrspqexprResult->Size() == pdrspqexprHashed->Size());
	...
	CDistributionSpecHashed *pdshashedEquiv =
		SPQOS_NEW(mp) CDistributionSpecHashed(pdrspqexprResult, fNullsColocated);
...
CDistributionSpecHashed::MakeHashedDistrSpec(
	CMemoryPool *mp, CExpressionArray *pdrspqexpr, BOOL fNullsColocated,
	CDistributionSpecHashed *pdshashedEquiv, IMdIdArray *opfamilies)
{
	if (SPQOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution) &&
		NULL == opfamilies)
	{
		...
		{
			CExpression *expr = (*pdrspqexpr)[ul];
			IMDId *mdid_type = CScalar::PopConvert(expr->Pop())->MdidType();
			IMDId *mdid_opfamily =
				mda->RetrieveType(mdid_type)->GetDistrOpfamilyMdid();
	...
	return SPQOS_NEW(mp) CDistributionSpecHashed(pdrspqexpr, fNullsColocated,
