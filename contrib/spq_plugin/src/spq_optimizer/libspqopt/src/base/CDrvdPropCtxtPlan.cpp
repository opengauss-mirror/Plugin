...
{
	m_phmulpdpCTEs = SPQOS_NEW(m_mp) UlongToDrvdPropPlanMap(m_mp);
}
...
CDrvdPropCtxtPlan::PdpctxtCopy(CMemoryPool *mp) const
{
	...
	while (hmulpdpiter.Advance())
	{
		...
		BOOL fInserted =
#endif	// SPQOS_DEBUG
			pdpctxtplan->m_phmulpdpCTEs->Insert(SPQOS_NEW(m_mp) ULONG(id),
												pdpplan);
...
CDrvdPropCtxtPlan::AddProps(CDrvdProp *pdp)
{
	...
	if (m_fUpdateCTEMap)
	{
		...
		BOOL fInserted =
#endif	// SPQOS_DEBUG
			m_phmulpdpCTEs->Insert(SPQOS_NEW(m_mp) ULONG(ulProducerId),
								   pdpplanProducer);
		SPQOS_ASSERT(fInserted);
	}
}
...
{
	SPQOS_ASSERT(NULL != m_phmulpdpCTEs);
...
CDrvdPropCtxtPlan::CopyCTEProducerProps(CDrvdPropPlan *pdpplan, ULONG ulCTEId)
{
	...
	BOOL fInserted =
#endif	// SPQOS_DEBUG
		m_phmulpdpCTEs->Insert(SPQOS_NEW(m_mp) ULONG(ulCTEId), pdpplan);
	SPQOS_ASSERT(fInserted);
}
