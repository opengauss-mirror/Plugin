...
#include "spqopt/base/CCTEInfo.h"
	  ...
	  m_fUsed(true)
{
	...
	m_phmcrulConsumers = SPQOS_NEW(mp) ColRefToUlongMap(mp);
}
...
CCTEInfo::CCTEInfoEntry::AddConsumerCols(CColRefArray *colref_array)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (NULL == m_phmcrulConsumers->Find(colref))
		{
			...
			BOOL fSuccess =
#endif	// SPQOS_DEBUG
				m_phmcrulConsumers->Insert(colref, SPQOS_NEW(m_mp) ULONG(ul));
...
//		return spqos::ulong_max if column is not found in local map
//
//---------------------------------------------------------------------------
ULONG
...
{
	SPQOS_ASSERT(NULL != mp);
	m_phmulcteinfoentry = SPQOS_NEW(m_mp) UlongToCTEInfoEntryMap(m_mp);
	m_phmulprodconsmap = SPQOS_NEW(m_mp) UlongToProducerConsumerMap(m_mp);
}
...
CCTEInfo::PexprPreprocessCTEProducer(const CExpression *pexprCTEProducer)
{
	...
	CExpression *pexprProducerPreprocessed =
		SPQOS_NEW(m_mp) CExpression(m_mp, pop, pexprChildPreprocessed);
...
CCTEInfo::AddCTEProducer(CExpression *pexprCTEProducer)
{
	...
	BOOL fInserted =
		...
		m_phmulcteinfoentry->Insert(
			SPQOS_NEW(m_mp) ULONG(ulCTEId),
			SPQOS_NEW(m_mp) CCTEInfoEntry(m_mp, pexprProducerToAdd));
...
CCTEInfo::ReplaceCTEProducer(CExpression *pexprCTEProducer)
{
	...
	BOOL fReplaced =
		...
		m_phmulcteinfoentry->Replace(
			&ulCTEId, SPQOS_NEW(m_mp) CCTEInfoEntry(m_mp, pexprCTEProducerNew,
												   pcteinfoentry->FUsed()));
							  ...
							  CColRefSet *pcrsStat)
{
	...
	SPQOS_ASSERT(pcrsStat->Size() == pcrsCTEProducer->Size());

	CReqdPropRelational *prprel =
		SPQOS_NEW(m_mp) CReqdPropRelational(pcrsCTEProducer);
	(void) pexprCTEProducer->PstatsDerive(prprel, NULL /* stats_ctxt */);
...
CCTEInfo::UlConsumers(ULONG ulCTEId) const
{
	...
	ULONG ulConsumers = UlConsumersInParent(ulCTEId, spqos::ulong_max);
...
CCTEInfo::IncrementConsumers(ULONG ulConsumerId, ULONG ulParentCTEId)
{
	...
	if (NULL == phmulconsumermap)
	{
		phmulconsumermap = SPQOS_NEW(m_mp) UlongToConsumerCounterMap(m_mp);
#ifdef SPQOS_DEBUG
		BOOL fInserted =
#endif
			m_phmulprodconsmap->Insert(SPQOS_NEW(m_mp) ULONG(ulParentCTEId),
									   phmulconsumermap);
		SPQOS_ASSERT(fInserted);
	}
	...
	if (NULL == pconsumercounter)
	{
		...
		BOOL fInserted =
#endif
			phmulconsumermap->Insert(SPQOS_NEW(m_mp) ULONG(ulConsumerId),
									 SPQOS_NEW(m_mp)
								...
								CStack<ULONG> *pstack)
{
	...
	while (hmulci.Advance())
	{
		...
		{
			pstack->Push(SPQOS_NEW(m_mp) ULONG(ulConsumerId));
...
CCTEInfo::MarkUnusedCTEs()
{
	CBitSet *pbsUnusedConsumers = SPQOS_NEW(m_mp) CBitSet(m_mp);
	...
	FindConsumersInParent(spqos::ulong_max, pbsUnusedConsumers, &stack);
...
CCTEInfo::PhmulcrConsumerToProducer(
	...
	CColRefArray *pdrspqcrProducer  // producer columns
)
{
	SPQOS_ASSERT(NULL != pcrs);
	SPQOS_ASSERT(NULL != pdrspqcrProducer);
	...
	while (crsi.Advance())
	{
		...
		if (spqos::ulong_max != ulPos)
		{
			SPQOS_ASSERT(ulPos < pdrspqcrProducer->Size());

			CColRef *pcrProducer = (*pdrspqcrProducer)[ulPos];
#ifdef SPQOS_DEBUG
			BOOL fSuccess =
#endif	// SPQOS_DEBUG
