...
CDrvdPropRelational::CDrvdPropRelational(CMemoryPool *mp)
	: m_mp(mp),
	  ...
	  m_is_complete(false)
{
	m_is_prop_derived = SPQOS_NEW(mp) CBitSet(mp, EdptSentinel);
}
...
{
	{
		...
		CRefCount::SafeRelease(m_pdrspqfd);
...
CDrvdPropRelational::FSatisfies(const CReqdPropPlan *prpp) const
{
	...
	SPQOS_ASSERT(NULL != prpp->PcrsRequired());
...
CDrvdPropRelational::GetRelationalProperties(CDrvdProp *pdp)
{
	...
	SPQOS_ASSERT(EptRelational == pdp->Ept() &&
...
//---------------------------------------------------------------------------
//	@function:
//		CDrvdPropRelational::PdrspqfdChild
//
//	@doc:
...
{
	SPQOS_ASSERT(child_index < exprhdl.Arity());
	SPQOS_ASSERT(!exprhdl.FScalarChild(child_index));

	// get FD's of the child
	CFunctionalDependencyArray *pdrspqfdChild = exprhdl.Pdrspqfd(child_index);
	...
	const ULONG size = pdrspqfdChild->Size();
	for (ULONG ul = 0; ul < size; ul++)
	{
		CFunctionalDependency *pfd = (*pdrspqfdChild)[ul];

		// check applicability of FD's LHS
		if (pcrsOutput->ContainsAll(pfd->PcrsKey()))
		{
			// decompose FD's RHS to extract the applicable part
			CColRefSet *pcrsDetermined = SPQOS_NEW(mp) CColRefSet(mp);
			...
			if (0 < pcrsDetermined->Size())
			{
				...
				CFunctionalDependency *pfdNew = SPQOS_NEW(mp)
					CFunctionalDependency(pfd->PcrsKey(), pcrsDetermined);
				pdrspqfd->Append(pfdNew);
			}
...
//---------------------------------------------------------------------------
//	@function:
//		CDrvdPropRelational::PdrspqfdLocal
//
//	@doc:
	...
	for (ULONG ul = 0; ul < ulKeys; ul++)
	{
		...
		if (0 < pcrsDetermined->Size())
		{
			...
			CFunctionalDependency *pfdLocal =
				SPQOS_NEW(mp) CFunctionalDependency(pcrsKey, pcrsDetermined);
			pdrspqfd->Append(pfdLocal);
		}

		pcrsKey->Release();
		pcrsDetermined->Release();
		CRefCount::SafeRelease(pdrspqcrKey);
	}
...
CDrvdPropRelational::DeriveKeyCollection(CExpressionHandle &exprhdl)
{
	if (!m_is_prop_derived->ExchangeSet(EdptPkc))
	{
		...
		if (NULL == m_pkc && 1 == DeriveMaxCard(exprhdl))
		{
			...
			{
				m_pcrsOutput->AddRef();
				m_pkc = SPQOS_NEW(m_mp) CKeyCollection(m_mp, m_pcrsOutput);
			}
		}
	}
...
{
	if (!m_is_prop_derived->ExchangeSet(EdptPdrspqfd))
	{
		CFunctionalDependencyArray *pdrspqfd =
			SPQOS_NEW(m_mp) CFunctionalDependencyArray(m_mp);
		...
		for (ULONG ul = 0; ul < arity; ul++)
		{
			...
			{
				CFunctionalDependencyArray *pdrspqfdChild =
					DeriveChildFunctionalDependencies(m_mp, ul, exprhdl);
				CUtils::AddRefAppend<CFunctionalDependency, CleanupRelease>(
					pdrspqfd, pdrspqfdChild);
				pdrspqfdChild->Release();
			}
		}
		// add local FD's
		CFunctionalDependencyArray *pdrspqfdLocal =
			DeriveLocalFunctionalDependencies(m_mp, exprhdl);
		CUtils::AddRefAppend<CFunctionalDependency, CleanupRelease>(
			pdrspqfd, pdrspqfdLocal);
		pdrspqfdLocal->Release();
