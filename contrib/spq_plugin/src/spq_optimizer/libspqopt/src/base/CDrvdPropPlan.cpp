					  ...
					  CDrvdPropCtxt *pdpctxt)
{
	...
	else
	{
		...
		SPQOS_ASSERT(NULL != m_ppim);
		SPQOS_ASSERT(CDistributionSpec::EdtAny != m_pds->Edt() &&
										...
										COperator *pop)
{
	...
	if (NULL != pdpplan)
	{
		...
		m_ppim = SPQOS_NEW(mp) CPartIndexMap(mp);
...
CDrvdPropPlan::FSatisfies(const CReqdPropPlan *prpp) const
{
	...
	SPQOS_ASSERT(NULL != prpp->Peo());
	SPQOS_ASSERT(NULL != prpp->Ped());
	SPQOS_ASSERT(NULL != prpp->Per());
	SPQOS_ASSERT(NULL != prpp->Pepp());
	SPQOS_ASSERT(NULL != prpp->Pcter());
...
{
	ULONG ulHash = spqos::CombineHashes(m_pos->HashValue(), m_pds->HashValue());
	ulHash = spqos::CombineHashes(ulHash, m_prs->HashValue());
	ulHash = spqos::CombineHashes(ulHash, m_ppim->HashValue());
	ulHash = spqos::CombineHashes(ulHash, m_pcm->HashValue());
