...
{
	SPQOS_ASSERT(m_pceeval->FCanEvalExpressions());

	IDatum *pdatum1Copy = datum1->MakeCopy(mp);
	CExpression *pexpr1 = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CScalarConst(mp, pdatum1Copy));
	IDatum *pdatum2Copy = datum2->MakeCopy(mp);
	CExpression *pexpr2 = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CScalarConst(mp, pdatum2Copy));
	...
	SPQOS_ASSERT(IMDType::EtiBool == datum->GetDatumType());
