	...
	CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	...
	return SPQOS_NEW(mp) CConstraintNegation(mp, pcnstr);
}
...
CConstraintNegation::Pcnstr(CMemoryPool *mp, const CColRef *colref)
{
	...
	return SPQOS_NEW(mp) CConstraintNegation(mp, m_pcnstr->Pcnstr(mp, colref));
}
...
CConstraintNegation::Pcnstr(CMemoryPool *mp, CColRefSet *pcrs)
{
	...
	return SPQOS_NEW(mp) CConstraintNegation(mp, m_pcnstr->Pcnstr(mp, pcrs));
}
...
{
	SPQOS_ASSERT(1 == m_pcrsUsed->Size());
