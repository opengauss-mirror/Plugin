	  ...
	  m_eriRight(eriRight)
{
	...
	SPQOS_ASSERT(NULL != pcomp);
	SPQOS_ASSERT(CUtils::FConstrainableType(mdid));
	SPQOS_ASSERT_IMP(NULL != pdatumLeft && NULL != pdatumRight,
	  ...
	  m_eriRight(EriExcluded)
{
	...
	SPQOS_ASSERT(NULL != pcomp);
	SPQOS_ASSERT(CUtils::FConstrainableType(m_mdid));
	m_mdid->AddRef();
...
CRange::FStartsBefore(CRange *prange)
{
	...
	SPQOS_ASSERT(m_pcomp->Equals(m_pdatumLeft, pdatumLeft));
...
CRange::FEndsAfter(CRange *prange)
{
	...
	SPQOS_ASSERT(m_pcomp->Equals(m_pdatumRight, pdatumRight));
...
CRange::PexprEquality(CMemoryPool *mp, const CColRef *colref)
{
	...
	m_pdatumLeft->AddRef();
	CExpression *pexprVal = SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) CScalarConst(mp, m_pdatumLeft));
...
CRange::PrngIntersect(CMemoryPool *mp, CRange *prange)
{
	...
	if (FOverlapsRight(prange))
	{
		...
		return SPQOS_NEW(mp) CRange(m_mdid, m_pcomp, m_pdatumLeft, m_eriLeft,
...
CRange::PrngDifferenceRight(CMemoryPool *mp, CRange *prange)
{
	...
	if (NULL != prange->PdatumRight() && FEndsAfter(prange))
	{
		...
		return SPQOS_NEW(mp) CRange(m_mdid, m_pcomp, pdatumRight,
