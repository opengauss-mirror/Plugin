...
CPartIndexMap::CPartTableInfo::CPartTableInfo(
	...
	EPartIndexManipulator epim, IMDId *mdid, CPartKeysArray *pdrspqpartkeys,
...
{
	SPQOS_ASSERT(EpimSentinel > epim && "Invalid manipulator type");
	...
	SPQOS_ASSERT_IMP(CPartIndexMap::EpimConsumer != epim, 0 == ulPropagators);
...
{
	SPQOS_ASSERT(NULL != m_ppartcnstrmap);
#ifdef SPQOS_DEBUG
	BOOL result =
#endif	// SPQOS_DEBUG
		m_ppartcnstrmap->Insert(SPQOS_NEW(mp) ULONG(scan_id), ppartcnstr);

	SPQOS_ASSERT(result && "Part constraint already exists in map");
}
...
{
	SPQOS_ASSERT(NULL != ppartcnstrmap);
...
{
	SPQOS_ASSERT(EpimSentinel > epim);
	...
	: m_mp(mp), m_pim(NULL), m_ulUnresolved(0), m_ulUnresolvedZeroPropagators(0)
{
	...
	m_pim = SPQOS_NEW(m_mp) ScanIdToPartTableInfoMap(m_mp);
}
...
CPartIndexMap::Insert(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap,
					  ...
					  IMDId *mdid, CPartKeysArray *pdrspqpartkeys,
					  CPartConstraint *ppartcnstrRel)
{
	...
	if (NULL == ppti)
	{
		...
		ppti = SPQOS_NEW(m_mp)
			CPartTableInfo(scan_id, ppartcnstrmap, epim, mdid, pdrspqpartkeys,
						   ppartcnstrRel, ulExpectedPropagators);
#ifdef SPQOS_DEBUG
		BOOL fSuccess =
#endif	// SPQOS_DEBUG
			m_pim->Insert(SPQOS_NEW(m_mp) ULONG(scan_id), ppti);
		SPQOS_ASSERT(fSuccess && "failed to insert partition index map entry");
...
//---------------------------------------------------------------------------
CPartKeysArray *
CPartIndexMap::Pdrspqpartkeys(ULONG scan_id) const
{
	CPartTableInfo *ppti = m_pim->Find(&scan_id);
	SPQOS_ASSERT(NULL != ppti);

	return ppti->Pdrspqpartkeys();
}
...
//		Returns spqos::ulong_max if no entry with the given scan id is found
//
//---------------------------------------------------------------------------
ULONG
							 ...
							 CPartIndexMap *ppimResult)
{
	...
	while (pimiFst.Advance())
	{
		...
		if (NULL != pptiSnd)
		{
			...
			SPQOS_ASSERT_IMP(epimFst == EpimConsumer && epimSnd == EpimConsumer,
		...
		CPartKeysArray *pdrspqpartkeys = pptiFst->Pdrspqpartkeys();
		...
		ppimResult->Insert(scan_id, ppartcnstrmap, epimResult,
						   ulPropagatorsResult, mdid, pdrspqpartkeys,
						   ppartcnstrRel);
	}
}
...
{
	SPQOS_ASSERT(NULL != pepimResult);
	SPQOS_ASSERT(NULL != pulExpectedPropagatorsResult);
	...
	if (EpimPropagator == epimFst && EpimConsumer == epimSnd)
	{
		...
		{
			*pulExpectedPropagatorsResult = spqos::ulong_max;
...
//---------------------------------------------------------------------------
ULongPtrArray *
CPartIndexMap::PdrspqulScanIds(CMemoryPool *mp, BOOL fConsumersOnly) const
{
	...
	while (pimi.Advance())
	{
		...
		pdrspqul->Append(SPQOS_NEW(mp) ULONG(ppti->ScanId()));
	}
...
CPartIndexMap::HashValue() const
{
	...
	while (pimi.Advance() && ul < ulMaxScanIds)
	{
		...
		ulHash = spqos::CombineHashes(ulHash, spqos::HashValue<ULONG>(&scan_id));
		ul++;
	}
...
{
	SPQOS_ASSERT(NULL != pptiReqd);
	SPQOS_ASSERT(NULL != pptiDrvd);
	SPQOS_ASSERT(EpimConsumer == pptiReqd->Epim());
	...
	return spqos::ulong_max == ulExpectedPropagators ||
...
{
	SPQOS_ASSERT(NULL != ppimSource);
	SPQOS_ASSERT(EppraSentinel > eppra);
	...
	if (NULL == pdrspqpartkeys)
	{
		...
		pdrspqpartkeys = ppti->Pdrspqpartkeys();
		pdrspqpartkeys->AddRef();
	}
	...
	this->Insert(ppti->ScanId(), ppti->Ppartcnstrmap(), ppti->Epim(),
				 ulExpectedPropagators, ppti->MDId(), pdrspqpartkeys,
				 ppti->PpartcnstrRel());
}
									 ...
									 ULONG ulExpectedFromReq) const
{
	...
	while (pimi.Advance())
	{
		...
		CPartKeysArray *pdrspqpartkeys = ppti->Pdrspqpartkeys();
		...
		ppimResult->Insert(ppti->ScanId(), ppartcnstrmap, epim,
						   ulExpectedPropagators, mdid, pdrspqpartkeys,
						   ppartcnstrRel);
	}
