...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CTableDescriptor_H
#define SPQOPT_CTableDescriptor_H
...
private:
	...
	CColumnDescriptorArray *m_pdrspqcoldescDist;
	...
	ULongPtrArray *m_pdrspqulPart;

	// key sets
	CBitSetArray *m_pdrspqbsKeys;
...
public:
	...
	PdrspqcoldescDist() const
	{
		return m_pdrspqcoldescDist;
	}
	...
	IsPartitioned() const
	{
		return 0 < m_pdrspqulPart->Size();
	}

	// true iff a hash distributed table needs to be considered as random;
	// this happens for when we are in phase 1 of a spqexpand or (for SPQDB 5X)
...
#endif	// !SPQOPT_CTableDescriptor_H
