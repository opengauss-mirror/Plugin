...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CUtils_H
#define SPQOPT_CUtils_H
...
private:
	...
	static void AppendArrayExpr(CExpressionArray *pdrspqexprSrc,
...
public:
	...
	static IOstream &OsPrintDrgPcoldesc(
		IOstream &os, CColumnDescriptorArray *pdrspqcoldescIncludedCols,
		ULONG length);
	...
	static CExpressionArray *PexprAggFuncArgs(CMemoryPool *mp,
											  CExpressionArray *pdrspqexprArgs);
	...
	static CExpression *PexprScalarProjListConst(
		CMemoryPool *mp, CColRefArray *colref_array, IDatumArray *pdrspqdatum,
	...
	static CExpression *PexprLogicalApply(
		CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight,
		CColRefArray *pdrspqcrInner, COperator::EOperatorId eopidOriginSubq,
		CExpression *pexprPred = NULL);
	...
	static CColRef *PcrExtractPartKey(CColRef2dArray *pdrspqdrspqcr,
									  ULONG ulLevel);
	...
	static CExpressionArray *PdrspqexprDedup(CMemoryPool *mp,
											CExpressionArray *pdrspqexpr);
	...
	static BOOL Equals(const CExpressionArray *pdrspqexprLeft,
					   const CExpressionArray *pdrspqexprRight);

	// check if first expression array contains all expressions in second array
	static BOOL Contains(const CExpressionArray *pdrspqexprFst,
	...
	static CColRefArray *PdrspqcrGroupingKey(CMemoryPool *mp, CExpression *pexpr,
											CColRefArray **ppdrspqcrKey);
	...
	static CColRefSetArray *PdrspqcrsMergeEquivClasses(
		CMemoryPool *mp, CColRefSetArray *pdrspqcrsFst,
		CColRefSetArray *pdrspqcrsSnd);

	// intersect 2 arrays of equivalence classes
	static CColRefSetArray *PdrspqcrsIntersectEquivClasses(
		CMemoryPool *mp, CColRefSetArray *pdrspqcrsFst,
		CColRefSetArray *pdrspqcrsSnd);

	// return a copy of equivalence classes from all children
	static CColRefSetArray *PdrspqcrsCopyChildEquivClasses(
		CMemoryPool *mp, CExpressionHandle &exprhdl);

	// return a copy of the given array of columns, excluding the columns
	// in the given colrefset
	static CColRefArray *PdrspqcrExcludeColumns(CMemoryPool *mp,
											   CColRefArray *pdrspqcrOriginal,
	...
	static void AddRefAppend(CDynamicPtrArray<T, CleanupFn> *pdrspqtOutput,
							 CDynamicPtrArray<T, CleanupFn> *pdrspqtInput,
							 ULONG ulStart = 0);
	...
	static CColRefArray *PdrspqcrRedistributableSubset(
	...
	static ULongPtrArray *Pdrspqul(CMemoryPool *mp,
								  const CColRefArray *colref_array);
	...
	// and based on the given mapping
	static CColRefArray *PdrspqcrRemap(CMemoryPool *mp,
									  CColRefArray *colref_array,
									  UlongToColRefMap *colref_mapping,
									  BOOL must_exist);
	...
	static CColRefArray *PdrspqcrExactCopy(CMemoryPool *mp,
										  CColRefArray *colref_array);
	...
	static CColRefArray *PdrspqcrCopy(CMemoryPool *mp,
									 CColRefArray *colref_array,
									 BOOL fAllComputed = false,
									 UlongToColRefMap *colref_mapping = NULL);

	// equality check between two arrays of column refs. Inputs can be NULL
	static BOOL Equals(CColRefArray *pdrspqcrFst, CColRefArray *pdrspqcrSnd);
	...
	static CExpressionArray *PdrspqexprScalarIdents(CMemoryPool *mp,
												   CColRefArray *colref_array);
	...
	static CBitSet *Pbs(CMemoryPool *mp, ULongPtrArray *pdrspqul);
	...
	static UlongToConstraintMap *PhmulcnstrBoolConstOnPartKeys(
		CMemoryPool *mp, CColRef2dArray *pdrspqdrspqcrPartKey, BOOL value);
	...
	static CPartConstraint *PpartcnstrFromMDPartCnstr(
		...
		const IMDPartConstraint *mdpart_constraint, CColRefArray *pdrspqcrOutput,
	...
	static CColRef *PcrMap(CColRef *pcrSource, CColRefArray *pdrspqcrSource,
	...
	static ULONG UlPcrIndexContainingSet(CColRefSetArray *pdrspqcrs,
										 const CColRef *colref);
						 ...
						 CXform::EXformId origin_xform)
{
	...
	return SPQOS_NEW(mp) CExpression(mp, SPQOS_NEW(mp) T(mp, origin_xform),
						  ...
						  CExpression *pexprPred)
{
	...
	return SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) T(mp, colref_array, eopidOriginSubq),
...
CExpression *
CUtils::PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft,
						  CExpression *pexprRight, CColRefArray *pdrspqcrInner,
						  COperator::EOperatorId eopidOriginSubq,
						  CExpression *pexprPred)
{
	SPQOS_ASSERT(NULL != pexprLeft);
	...
	return SPQOS_NEW(mp)
		CExpression(mp, SPQOS_NEW(mp) T(mp, pdrspqcrInner, eopidOriginSubq),
					pexprLeft, pexprRight, pexprScalar);
}
...
template <class T, void (*CleanupFn)(T *)>
void
CUtils::AddRefAppend(CDynamicPtrArray<T, CleanupFn> *pdrspqtOutput,
					 CDynamicPtrArray<T, CleanupFn> *pdrspqtInput, ULONG ulStart)
{
	SPQOS_ASSERT(NULL != pdrspqtOutput);
	SPQOS_ASSERT(NULL != pdrspqtInput);

	const ULONG size = pdrspqtInput->Size();
	SPQOS_ASSERT_IMP(0 < size, ulStart < size);

	for (ULONG ul = ulStart; ul < size; ul++)
	{
		T *pt = (*pdrspqtInput)[ul];
		CRefCount *prc = dynamic_cast<CRefCount *>(pt);
		prc->AddRef();
		pdrspqtOutput->Append(pt);
	}
}
...
CUtils::FScalarConstInt(CExpression *pexpr)
{
	...
	SPQOS_ASSERT(IMDType::EtiInt2 == type_info ||
...
CUtils::FMatchIndex(T *pop1, COperator *pop2)
{
	...
	return pop1->UlOriginOpId() == popIndex->UlOriginOpId() &&
		   ...
		   pop1->PdrspqcrOutput()->Equals(popIndex->PdrspqcrOutput());
}
...
CUtils::FMatchDynamicIndex(T *pop1, COperator *pop2)
{
	...
	return pop1->UlOriginOpId() == popIndex2->UlOriginOpId() &&
		   ...
		   pop1->PdrspqcrOutput()->Equals(popIndex2->PdrspqcrOutput()) &&
		   (!pop1->IsPartial() ||
...
CUtils::FMatchDynamicScan(T *pop1, COperator *pop2)
{
	...
	return pop1->ScanId() == popScan2->ScanId() &&
		   ...
		   pop1->PdrspqcrOutput()->Equals(popScan2->PdrspqcrOutput()) &&
