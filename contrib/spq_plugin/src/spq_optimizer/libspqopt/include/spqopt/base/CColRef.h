...
//---------------------------------------------------------------------------
#ifndef SPQOS_CColRef_H
#define SPQOS_CColRef_H
...
typedef CHashMap<ULONG, CColRef, spqos::HashValue<ULONG>, spqos::Equals<ULONG>,
				 CleanupDelete<ULONG>, CleanupNULL<CColRef> >
	UlongToColRefMap;
// iterator
typedef CHashMapIter<ULONG, CColRef, spqos::HashValue<ULONG>,
...
//
//---------------------------------------------------------------------------
class CColRef : public spqos::DbgPrintMixin<CColRef>
{
public:
	enum EUsedStatus
	{
...
public:
	...
	static ULongPtrArray *Pdrspqul(CMemoryPool *mp, CColRefArray *colref_array);

	// check if the the array of column references are equal
	static BOOL Equals(const CColRefArray *pdrspqcr1,
					   const CColRefArray *pdrspqcr2);

	// check if the the array of column reference arrays are equal
	static BOOL Equals(const CColRef2dArray *pdrgdrspqcr1,
	...
	{
		if (SPQOS_FTRACE(EopttraceTranslateUnusedColrefs) ||
