...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CConstraintInterval_H
#define SPQOPT_CConstraintInterval_H
...
private:
	...
	void AddRemainingRanges(CMemoryPool *mp, CRangeArray *pdrspqrngSrc,
							ULONG ulStart, CRangeArray *pdrspqrngDest);

	// append the given range to the array or extend the last element
	void AppendOrExtend(CMemoryPool *mp, CRangeArray *pdrspqrng, CRange *prange);
...
public:
	...
	CConstraintInterval(CMemoryPool *mp, const CColRef *colref,
						CRangeArray *pdrspqrng, BOOL is_null);
	...
	// check if constraint is on the spq_segment_id column
	virtual BOOL
...
#endif	// !SPQOPT_CConstraintInterval_H
