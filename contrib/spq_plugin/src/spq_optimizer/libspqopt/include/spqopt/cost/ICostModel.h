...
#ifndef SPQOPT_ICostModel_H
#define SPQOPT_ICostModel_H
...
#define SPQOPT_DEFAULT_REBINDS 1
...
public:
	...
	{
		EcmtSPQDBLegacy = 0,
		EcmtSPQDBCalibrated = 1,
		EcmtSPQDBExperimental = 2,
		EcmtSentinel = 3
	};
	...
	public:
		...
		SCostingInfo(CMemoryPool *mp, ULONG ulChildren, CCostingStats *pcstats)
			: m_ulChildren(ulChildren),
			  ...
			  m_rows(0),
			  m_width(0),
			  m_num_rebinds(SPQOPT_DEFAULT_REBINDS),
		...
		{
			SPQOS_ASSERT(NULL != pcstats);
			if (0 < ulChildren)
			{
				m_pdRowsChildren = SPQOS_NEW_ARRAY(mp, DOUBLE, ulChildren);
				m_pdWidthChildren = SPQOS_NEW_ARRAY(mp, DOUBLE, ulChildren);
				m_pdRebindsChildren = SPQOS_NEW_ARRAY(mp, DOUBLE, ulChildren);
				m_pdCostChildren = SPQOS_NEW_ARRAY(mp, DOUBLE, ulChildren);
				m_pdrgstatsChildren =
					SPQOS_NEW_ARRAY(mp, CCostingStats *, ulChildren);
		...
		~SCostingInfo()
		{
			SPQOS_DELETE_ARRAY(m_pdRowsChildren);
			SPQOS_DELETE_ARRAY(m_pdWidthChildren);
			SPQOS_DELETE_ARRAY(m_pdRebindsChildren);
			SPQOS_DELETE_ARRAY(m_pdCostChildren);
			...
			SPQOS_DELETE_ARRAY(m_pdrgstatsChildren);
			m_pcstats->Release();
		}
		...
		{
			SPQOS_ASSERT(SPQOPT_DEFAULT_REBINDS <= num_rebinds);
		...
		{
			SPQOS_ASSERT(0 <= dRowsChild);
			SPQOS_ASSERT(ulPos < m_ulChildren);
		...
		{
			SPQOS_ASSERT(SPQOPT_DEFAULT_REBINDS <= dRebindsChild);
			SPQOS_ASSERT(ulPos < m_ulChildren);
	...
	void SetParams(ICostModelParamsArray *pdrspqcp);
