...
public:
	// ctor
	CGroupExpression(CMemoryPool *mp, COperator *pop, CGroupArray *pdrspqgroup,
	...
	{
		SPQOS_ASSERT(NULL != m_pdrspqgroup);

		CGroup *pgroup = (*m_pdrspqgroup)[ulPos];
		...
		{
			SPQOS_ASSERT(pgroup->FDuplicateGroup());
	...
	HashValue() const
	{
		return HashValue(m_pop, m_pdrspqgroup);
	}

	// static hash function for operator and group references
	static ULONG HashValue(COperator *pop, CGroupArray *drspqgroup);
	...
	CCostContextArray *PdrspqccLookupAll(CMemoryPool *mp,
...
#endif	// !SPQOPT_CGroupExpression_H
