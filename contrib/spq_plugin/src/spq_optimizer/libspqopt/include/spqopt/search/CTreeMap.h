...
//			ACM SIGMOD, 2000
//---------------------------------------------------------------------------
#ifndef SPQOPT_CTreeMap_H
#define SPQOPT_CTreeMap_H
...
private:
	...
	public:
		...
		STreeLink(const T *ptParent, ULONG child_index, const T *ptChild)
			: m_ptParent(ptParent),
			  ...
			  m_ptChild(ptChild)
		{
			SPQOS_ASSERT(NULL != ptParent);
			SPQOS_ASSERT(NULL != ptChild);
		}
		...
		HashValue(const STreeLink *ptlink)
		{
			...
			ULONG ulHashChildIndex =
				spqos::HashValue<ULONG>(&ptlink->m_ulChildIndex);
	...
	private:
		...
		CTreeNode2dArray *m_pdrgdrspqtn;
		...
		UllCount(ULONG ulChild)
		{
			...
			ULONG ulCandidates = (*m_pdrgdrspqtn)[ulChild]->Size();
			for (ULONG ulAlt = 0; ulAlt < ulCandidates; ulAlt++)
			{
				CTreeNode *ptn = (*(*m_pdrgdrspqtn)[ulChild])[ulAlt];
				ULLONG ullCount = ptn->UllCount();
				ull = spqos::Add(ull, ullCount);
			}
		...
		{
			SPQOS_CHECK_STACK_SIZE;
			SPQOS_ASSERT(ullRank < UllCount(ulChild));

			CTreeNodeArray *pdrspqtn = (*m_pdrgdrspqtn)[ulChild];
			ULONG ulCandidates = pdrspqtn->Size();
	...
	public:
		...
		CTreeNode(CMemoryPool *mp, ULONG ul, const T *value)
			: m_mp(mp),
			  ...
			  m_value(value),
			  m_pdrgdrspqtn(NULL),
			  m_ullCount(spqos::ullong_max),
			  m_ulIncoming(0),
			  m_ens(EnsUncounted)
		{
			m_pdrgdrspqtn = SPQOS_NEW(mp) CTreeNode2dArray(mp);
		}

		// dtor
		~CTreeNode()
		{
			m_pdrgdrspqtn->Release();
		}
		...
		Add(ULONG ulPos, CTreeNode *ptn)
		{
			...
			ULONG length = m_pdrgdrspqtn->Size();
			for (ULONG ul = length; ul <= ulPos; ul++)
			{
				CTreeNodeArray *pdrg = SPQOS_NEW(m_mp) CTreeNodeArray(m_mp);
				m_pdrgdrspqtn->Append(pdrg);
			}
			...
			CTreeNodeArray *pdrg = (*m_pdrgdrspqtn)[ulPos];
			SPQOS_ASSERT(NULL != pdrg);
			pdrg->Append(ptn);
		}
		...
		UllCount()
		{
			...
			SPQOS_ASSERT(EnsCounting != m_ens && "cycle in graph detected");
			...
			if (!FCounted())
			{
				...
				ULONG arity = m_pdrgdrspqtn->Size();
				for (ULONG ulChild = 0; ulChild < arity; ulChild++)
				{
					...
					ullCount = spqos::Multiply(ullCount, ull);
				}
		...
		OsPrint(IOstream &os)
		{
			...
			for (ULONG ul = 0; ul < ulChildren; ul++)
			{
				...
				ULONG ulAlt = (*m_pdrgdrspqtn)[ul]->Size();

				for (ULONG ulChild = 0; ulChild < ulAlt; ulChild++)
				{
					CTreeNode *ptn = (*(*m_pdrgdrspqtn)[ul])[ulChild];
	...
	// map of all nodes
	typedef spqos::CHashMap<T, CTreeNode, HashFn, EqFn, CleanupNULL,
						   CleanupDelete<CTreeNode> >
		TMap;
	typedef spqos::CHashMapIter<T, CTreeNode, HashFn, EqFn, CleanupNULL,
							   CleanupDelete<CTreeNode> >
		TMapIter;
	...
	Ptn(const T *value)
	{
		...
		if (NULL == ptn)
		{
			ptn = SPQOS_NEW(m_mp) CTreeNode(m_mp, ++m_ulCountNodes, value);
...
public:
		  ...
		  m_plinkmap(NULL)
	{
		...
		SPQOS_ASSERT(NULL != prfn);

		m_ptmap = SPQOS_NEW(mp) TMap(mp);
		m_plinkmap = SPQOS_NEW(mp) LinkMap(mp);

		// insert dummy node as global root -- the only node with NULL payload
		m_ptnRoot =
			SPQOS_NEW(mp) CTreeNode(mp, 0 /* ulCounter */, NULL /* value */);
	}
	...
	{
		SPQOS_ASSERT(ptParent != ptChild);

		// exit function if link already exists
		STreeLink *ptlink = SPQOS_NEW(m_mp) STreeLink(ptParent, ulPos, ptChild);
		...
		BOOL fInserted =
#endif	// SPQOS_DEBUG
			m_plinkmap->Insert(ptlink, SPQOS_NEW(m_mp) BOOL(true));
