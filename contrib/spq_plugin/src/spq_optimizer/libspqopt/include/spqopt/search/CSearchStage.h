...
//		Search stage
//---------------------------------------------------------------------------
#ifndef SPQOPT_CSearchStage_H
#define SPQOPT_CSearchStage_H
...
public:
	// ctor
	CSearchStage(CXformSet *xform_set, ULONG ulTimeThreshold = spqos::ulong_max,
	...
	RestartTimer()
	{
		if (m_time_threshold != spqos::ulong_max)
			m_timer.Restart();
	}

	// is search stage timed-out?
	// if threshold is spqos::ulong_max, its the default and we need not time out
	...
	static CSearchStageArray *PdrspqssDefault(CMemoryPool *mp);
};
