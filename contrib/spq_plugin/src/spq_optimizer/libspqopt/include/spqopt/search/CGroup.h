...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CGroup_H
#define SPQOPT_CGroup_H
...
#define SPQOPT_INVALID_GROUP_ID spqos::ulong_max
...
private:
	...
	CExpressionArray *m_pdrspqexprJoinKeysOuter;

	// join keys for inner child (only for scalar groups) (used by hash & merge joins)
	CExpressionArray *m_pdrspqexprJoinKeysInner;
	...
	// set hash join keys
	void SetJoinKeys(CExpressionArray *pdrspqexprOuter,
...
public:
	...
	CExpressionArray *
	PdrspqexprJoinKeysOuter() const
	{
		return m_pdrspqexprJoinKeysOuter;
	}

	// join keys of inner child
	CExpressionArray *
	PdrspqexprJoinKeysInner() const
	{
		return m_pdrspqexprJoinKeysInner;
	}
	...
	{
		return (spqos::ulong_max != m_ulCTEProducerId);
	}
	...
	static BOOL FMatchGroups(CGroupArray *pdrspqgroupFst,
							 CGroupArray *pdrspqgroupSnd);

	// matching of pairs of arrays of groups while skipping scalar groups
	static BOOL FMatchNonScalarGroups(CGroupArray *pdrspqgroupFst,
									  CGroupArray *pdrspqgroupSnd);
    ...
    void PrintNewOptContext(const COptimizationContext *poc) const;
    void PrintUpdateBestCost(const COptimizationContext *poc, const CCostContext *pcc) const;
    /* SPQ end: print cost*/
