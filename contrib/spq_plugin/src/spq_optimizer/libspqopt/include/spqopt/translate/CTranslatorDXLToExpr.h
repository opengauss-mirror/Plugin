...
#ifndef SPQOPT_CTranslatorDXLToExpr_H
#define SPQOPT_CTranslatorDXLToExpr_H
...
#include "spqopt/translate/CTranslatorDXLToExprUtils.h"
...
// hash maps
typedef CHashMap<ULONG, CExpressionArray, spqos::HashValue<ULONG>,
				 spqos::Equals<ULONG>, CleanupDelete<ULONG>, CleanupNULL>
	UlongToExprArrayMap;

// iterator
typedef CHashMapIter<ULONG, CExpressionArray, spqos::HashValue<ULONG>,
					 spqos::Equals<ULONG>, CleanupDelete<ULONG>, CleanupNULL>
...
private:
	...
	ULongPtrArray *m_pdrspqulOutputColRefs;

	// array of output column names
	CMDNameArray *m_pdrspqmdname;
	...
	// id of CTE that we are currently processing (spqos::ulong_max for main query)
	ULONG m_ulCTEId;

	// DXL operator translators indexed by the operator id
	PfPexpr m_rspqfTranslators[EdxlopSentinel];
	...
	CColRefArray *PdrspqcrPartitionByCol(
	...
	CExpressionArray *PdrspqexprPreprocessSetOpInputs(
	...
	CExpressionArray *PdrspqexprChildren(const CDXLNode *dxlnode);
	...
	CColRefArray *Pdrspqcr(const CDXLColDescrArray *dxl_col_descr_array);
...
public:
	...
	ULongPtrArray *PdrspqulOutputColRefs();

	// return the array of output column names
	CMDNameArray *
	Pdrspqmdname()
	{
		SPQOS_ASSERT(NULL != m_pdrspqmdname);
		return m_pdrspqmdname;
	}
};
}  // namespace spqopt

#endif	// !SPQOPT_CTranslatorDXLToExpr_H
