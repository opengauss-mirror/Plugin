...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CJoinOrderDPv2_H
#define SPQOPT_CJoinOrderDPv2_H
...
class CJoinOrderDPv2 : public CJoinOrder,
					   public spqos::DbgPrintMixin<CJoinOrderDPv2>
{
private:
	...
	struct SGroupAndExpression
	{
		...
		SGroupAndExpression()
			: m_group_info(NULL), m_expr_index(spqos::ulong_max)
		{
		}
		...
		GetExprInfo() const
		{
			return m_expr_index == spqos::ulong_max
					   ? NULL
		...
		IsValid()
		{
			return NULL != m_group_info && spqos::ulong_max != m_expr_index;
		}
		BOOL
	...
	struct SExpressionInfo : public CRefCount
	{
		...
		{
			m_contain_PS = SPQOS_NEW(mp) CBitSet(mp);
	...
	struct SGroupInfo : public CRefCount
	{
		...
		{
			m_best_expr_info_array = SPQOS_NEW(mp) SExpressionInfoArray(mp);
		}
...
public:
	// ctor
	CJoinOrderDPv2(CMemoryPool *mp, CExpressionArray *pdrspqexprAtoms,
