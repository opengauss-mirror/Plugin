...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CPhysical_H
#define SPQOPT_CPhysical_H
...
private:
	...
	ULONG m_rgulOptReqs[SPQOPT_PLAN_PROPS];

	// array of expanded requests
	UlongPtrArray *m_pdrspqulpOptReqsExpanded;
...
protected:
	...
	static CCTEMap *PcmCombine(CMemoryPool *mp, CDrvdPropArray *pdrspqdpCtxt);
...
public:
	...
	FPhysical() const
	{
		SPQOS_ASSERT(!FLogical() && !FScalar() && !FPattern());
		return true;
	}
	...
	virtual CColRefSet *PcrsRequired(
		...
		ULONG child_index, CDrvdPropArray *pdrspqdpCtxt, ULONG ulOptReq) = 0;
	...
	virtual CPartitionPropagationSpec *PppsRequired(
		CMemoryPool *mp, CExpressionHandle &exprhdl,
		CPartitionPropagationSpec *pppsRequired, ULONG child_index,
		CDrvdPropArray *pdrspqdpCtxt, ULONG ulOptReq) = 0;
