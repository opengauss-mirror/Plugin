...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CScalar_H
#define SPQOPT_CScalar_H
...
protected:
	...
	static EBoolEvalResult EberConjunction(ULongPtrArray *pdrspqulChildren);

	// perform disjunction of child boolean evaluation results
	static EBoolEvalResult EberDisjunction(ULongPtrArray *pdrspqulChildren);

	// return Null if any child is Null
	static EBoolEvalResult EberNullOnAnyNullChild(
		ULongPtrArray *pdrspqulChildren);
...
public:
	...
	FScalar() const
	{
		SPQOS_ASSERT(!FPhysical() && !FLogical() && !FPattern());
		return true;
	}
	...
	Eber(ULongPtrArray *  // pdrspqulChildren
	) const
	{
