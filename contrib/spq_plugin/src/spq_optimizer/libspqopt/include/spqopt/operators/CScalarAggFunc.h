...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CScalarAggFunc_H
#define SPQOPT_CScalarAggFunc_H
...
private:
	...
	// parsing if the actual return type of Agg is ambiguous (e.g., AnyElement in SPQDB)
	...
	// corresponding spq_agg mdid for supported ordered aggs
	IMDId *m_spq_agg_mdid;
...
public:
	...
	CScalarAggFunc(CMemoryPool *mp, IMDId *pmdidAggFunc,
				   ...
				   EAggfuncKind aggkind, IMDId *spq_agg_mdid = NULL);
	...
	virtual ~CScalarAggFunc()
	{
		...
		CRefCount::SafeRelease(m_spq_agg_mdid);
		SPQOS_DELETE(m_pstrAggFunc);
	}
	...
	PopConvert(COperator *pop)
	{
		...
		SPQOS_ASSERT(EopScalarAggFunc == pop->Eopid());
	...
	// return spq_agg MDId. Valid only for supported ordered aggs, else NULL
	IMDId *
	GetGpAggMDId() const
	{
		return m_spq_agg_mdid;
	}
