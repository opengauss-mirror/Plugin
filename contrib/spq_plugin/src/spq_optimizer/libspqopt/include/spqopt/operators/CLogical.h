...
//---------------------------------------------------------------------------
#ifndef SPQOPT_CLogical_H
#define SPQOPT_CLogical_H
...
protected:
	...
	CColRefArray *PdrspqcrCreateMapping(
		CMemoryPool *mp, const CColumnDescriptorArray *pdrspqcoldesc,
		ULONG ulOpSourceId, IMDId *mdid_table = NULL) const;

	// initialize the array of partition columns
	CColRef2dArray *PdrspqdrspqcrCreatePartCols(CMemoryPool *mp,
	...
	static CKeyCollection *PkcKeysBaseTable(CMemoryPool *mp,
											const CBitSetArray *pdrspqbsKeys,
...
public:
	...
	FLogical() const
	{
		SPQOS_ASSERT(!FPhysical() && !FScalar() && !FPattern());
		return true;
	}
	...
	static CColRefArray *PdrspqcrOutputFromLogicalGet(CLogical *pop);
