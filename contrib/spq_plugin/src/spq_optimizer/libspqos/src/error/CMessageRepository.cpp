...
{
	SPQOS_ASSERT(exc != CException::m_invalid_exception &&
...
CMessageRepository::Init()
{
	...
	CMessageRepository *repository = SPQOS_NEW(mp) CMessageRepository(mp);
...
{
	m_hash_table.Init(mp, 128, SPQOS_OFFSET(CMessageTable, m_link),
					  SPQOS_OFFSET(CMessageTable, m_locale),
...
CMessageRepository::AddMessage(ELocale locale, CMessage *msg)
{
	...
	SPQOS_ASSERT(!"Adding message table on demand failed");
}
...
CMessageRepository::AddMessageTable(ELocale locale)
{
	CMessageTable *new_mt =
		SPQOS_NEW(m_mp) CMessageTable(m_mp, SPQOS_MSGTAB_SIZE, locale);
