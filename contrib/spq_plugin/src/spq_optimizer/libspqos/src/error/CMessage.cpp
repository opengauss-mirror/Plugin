...
{
	SPQOS_ASSERT(index < CException::ExmiSentinel);
	...
	static CMessage msg[CException::ExmiSentinel] = {
		...
		CMessage(
			...
			CException::ExsevError,
			SPQOS_WSZ_WSZLEN("Last check for aborts before %d ms, at:\n%ls"),
		...
		CMessage(
			...
			CException::ExsevError,
			SPQOS_WSZ_WSZLEN("Error during I/O operation, error code: %d"),
			1,	// # params
			SPQOS_WSZ_WSZLEN(
		...
		CMessage(CException(CException::ExmaSystem, CException::ExmiOverflow),
				 CException::ExsevError, SPQOS_WSZ_WSZLEN("Arithmetic Overflow"),
				 0,	 // # params
				 SPQOS_WSZ_WSZLEN("Arithmetic Overflow")),

		CMessage(
			CException(CException::ExmaSystem, CException::ExmiInvalidDeletion),
			CException::ExsevError,
			SPQOS_WSZ_WSZLEN("Error during delete operation, error code: %d"),
			1,	// # params
			SPQOS_WSZ_WSZLEN(
		...
		CMessage(
			...
			CException::ExsevError,
			SPQOS_WSZ_WSZLEN("Unexpected out of memory during fault simulation"),
			0,	// # params
			SPQOS_WSZ_WSZLEN(
		...
		CMessage(
			...
			SPQOS_WSZ_WSZLEN("This is a dummy warning"),
			1,	// # params
			SPQOS_WSZ_WSZLEN("Used to test warning logging")),

		CMessage(CException(CException::ExmaSQL, CException::ExmiSQLDefault),
				 CException::ExsevError, SPQOS_WSZ_WSZLEN("Internal error"), 0,
				 SPQOS_WSZ_WSZLEN("Internal error")),
		...
		CMessage(
			...
			SPQOS_WSZ_WSZLEN("Not null constraint for table was violated")),

		CMessage(
			CException(CException::ExmaSQL,
					   CException::ExmiSQLCheckConstraintViolation),
			CException::ExsevError,
			SPQOS_WSZ_WSZLEN("Check constraint %ls for table %ls was violated"),
			2,	// constraint name, table name
			SPQOS_WSZ_WSZLEN("Check constraint for table was violated")),
		...
		CMessage(CException(CException::ExmaSQL, CException::ExmiSQLTest),
				 CException::ExsevError,
				 SPQOS_WSZ_WSZLEN("Test sql error message: %ls"),
				 1,	 // error message
				 SPQOS_WSZ_WSZLEN("Test sql error message")),

		CMessage(
			CException(CException::ExmaUnhandled, CException::ExmiUnhandled),
			CException::ExsevError, SPQOS_WSZ_WSZLEN("Unhandled exception"), 0,
			SPQOS_WSZ_WSZLEN("Unhandled exception")),
