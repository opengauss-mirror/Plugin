...
#include "spqos/string/CWString.h"
#include "spqos/task/IWorker.h"
#include "spqos/utils.h"

#define SPQOS_STACK_DESCR_TRACE_BUF (4096)
...
CStackDescriptor::BackTrace(ULONG top_frames_to_skip)
{
	...
	SPQOS_GET_FRAME_POINTER(current_frame);
	...
	// consider the first SPQOS_STACK_TRACE_DEPTH frames below worker object
	for (ULONG frame_counter = 0; frame_counter < SPQOS_STACK_TRACE_DEPTH;
		 frame_counter++)
	{
								   ...
								   ULONG index) const
{
	...
	if (symbol_info.dli_sname)
	{
		...
		SPQOS_ASSERT(size <= SPQOS_STACK_SYMBOL_SIZE);
	...
	ws->AppendFormat(
		SPQOS_WSZ_LIT("%-4d 0x%016lx %s + %lu\n"),
		index + 1,	// frame no.
		(long unsigned int)
...
{
	SPQOS_ASSERT(SPQOS_STACK_TRACE_DEPTH >= m_depth &&
	...
	CHAR demangling_symbol_buffer[SPQOS_STACK_SYMBOL_SIZE];
	...
	for (ULONG i = 0; i < m_depth && i < depth; i++)
	{
		...
		AppendSymbolInfo(ws, demangling_symbol_buffer, SPQOS_STACK_SYMBOL_SIZE,
						 symbol_info, i);
	}
}
...
{
	WCHAR wsz[SPQOS_STACK_DESCR_TRACE_BUF];
...
{
	SPQOS_ASSERT(0 < m_depth && "No stack to hash");
	SPQOS_ASSERT(SPQOS_STACK_TRACE_DEPTH >= m_depth &&
				"Stack exceeds maximum depth");

	return spqos::HashByteArray((BYTE *) m_array_of_addresses,
							   m_depth * SPQOS_SIZEOF(m_array_of_addresses[0]));
}
