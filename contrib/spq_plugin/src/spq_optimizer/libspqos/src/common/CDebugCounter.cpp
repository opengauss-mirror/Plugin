...
CDebugCounter::CDebugCounter(CMemoryPool *mp)
	: m_mp(mp),
	  ...
	  m_hashmap(NULL)
{
	m_hashmap = SPQOS_NEW(mp) CounterKeyToValueMap(mp);
}
...
CDebugCounter::Init()
{
	...
	SPQOS_RTL_ASSERT(NULL == m_instance);
	m_instance = SPQOS_NEW(mp) CDebugCounter(mp);
...
CDebugCounter::NextQry(const char *next_qry_name)
{
	...
	if (0 < m_instance->m_hashmap->Size())
	{
		if (!m_instance->m_is_name_constant_get)
		{
			...
			{
				SPQOS_TRACE(SPQOS_WSZ_LIT("CDebugCounterEventLoggingStart"));
			...
			while (iter.Advance())
			{
				...
				switch (val->m_type)
				{
					...
					case ECounterTypeCpuTime:
						...
						break;
					default:
						SPQOS_RTL_ASSERT(!"Corrupted debug counter type");
						break;
				}
		...
		m_instance->m_hashmap =
			SPQOS_NEW(m_instance->m_mp) CounterKeyToValueMap(m_instance->m_mp);
	}
}
...
{
	SPQOS_RTL_ASSERT(NULL != key && NULL == *key);
	SPQOS_RTL_ASSERT(NULL != val && NULL == *val);

	// search with a newly made key
	*key = SPQOS_NEW(m_mp) SDebugCounterKey(counter_name);
	*val = m_hashmap->Find(*key);

	if (NULL != *val)
	{
		// make sure we don't mix counter types
		SPQOS_RTL_ASSERT((*val)->m_type == typ);
	...
	*val = SPQOS_NEW(m_mp) SDebugCounterValue(typ);
	return false;
}
...
CDebugCounter::StartCpuTime(const char *counter_name)
{
	if (OkToProceed())
	{
		...
		{
			val->m_cpu_timer = SPQOS_NEW(m_instance->m_mp) CTimerUser();
		}
		else
		{
...
CDebugCounter::StopCpuTime(const char *counter_name)
{
	if (OkToProceed())
	{
		...
		SPQOS_RTL_ASSERT(NULL != val->m_cpu_timer && val->m_timer_is_running);
