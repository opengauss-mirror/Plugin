...
#define BYTES_PER_UNIT SPQOS_SIZEOF(ULLONG)
	...
	: m_nbits(nbits), m_len(0), m_vec(NULL)
{
	...
	SPQOS_ASSERT(m_len * BITS_PER_UNIT >= m_nbits &&
				"Bit vector sized incorrectly");

	// allocate and clear
	m_vec = SPQOS_NEW_ARRAY(mp, ULLONG, m_len);
...
{
	SPQOS_ASSERT(pos < m_nbits && "Bit index out of bounds.");
...
{
	SPQOS_ASSERT(ulBit < m_nbits && "Bit index out of bounds.");
...
{
	SPQOS_ASSERT(m_nbits == vec->m_nbits && m_len == vec->m_len &&
...
CBitVector::Equals(const CBitVector *vec) const
{
	...
	{
		SPQOS_ASSERT(this->ContainsAll(vec) && vec->ContainsAll(this));
		return true;
	}
...
{
	return spqos::HashByteArray((BYTE *) &m_vec[0],
							   SPQOS_SIZEOF(m_vec[0]) * m_len);
}
