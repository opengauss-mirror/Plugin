...
CBitSet::CBitSetLink::CBitSetLink(CMemoryPool *mp, ULONG offset,
	...
	: m_offset(offset)
{
	m_vec = SPQOS_NEW(mp) CBitVector(mp, vector_size);
}
...
{
	m_vec = SPQOS_NEW(mp) CBitVector(mp, *bsl.GetVec());
}
...
CBitSet::FindLinkByOffset(ULONG offset, CBitSetLink *bsl) const
{
	...
	{
		SPQOS_ASSERT(bsl->GetOffset() <= offset && "invalid start cursor");
		found = bsl;
	}

	SPQOS_ASSERT_IMP(NULL != cursor,
					SPQOS_OK == m_bsllist.Find(cursor) && "cursor not in list");
	...
	SPQOS_ASSERT_IMP(found, found->GetOffset() <= offset);
	return found;
}
...
{
	m_bsllist.Init(SPQOS_OFFSET(CBitSetLink, m_link));
}
...
CBitSet::ExchangeSet(ULONG pos)
{
	...
	if (NULL == bsl || bsl->GetOffset() != offset)
	{
		CBitSetLink *pbsl_new =
			SPQOS_NEW(m_mp) CBitSetLink(m_mp, offset, m_vector_size);
		if (NULL == bsl)
		{
	...
	SPQOS_ASSERT(bsl->GetOffset() == offset);
...
CBitSet::Union(const CBitSet *pbsOther)
{
	...
	CAutoRef<CBitSetLinkArray> a_drspqbsl;
	a_drspqbsl = SPQOS_NEW(m_mp) CBitSetLinkArray(m_mp);
		 ...
		 bsl_other = pbsOther->m_bsllist.Next(bsl_other))
	{
		...
		if (NULL == bsl || bsl->GetOffset() != bsl_other->GetOffset())
		{
			...
			a_pbsl = SPQOS_NEW(m_mp) CBitSetLink(m_mp, *bsl_other);
			a_drspqbsl->Append(a_pbsl.Value());
	...
	bsl = NULL;
	for (ULONG i = 0; i < a_drspqbsl->Size(); i++)
	{
		CBitSetLink *pbslInsert = (*a_drspqbsl)[i];
		bsl = FindLinkByOffset(pbslInsert->GetOffset(), bsl);

		SPQOS_ASSERT_IMP(NULL != bsl,
...
CBitSet::HashValue() const
{
	...
	while (NULL != bsl)
	{
		ulHash = spqos::CombineHashes(ulHash, bsl->GetVec()->HashValue());
		bsl = m_bsllist.Next(bsl);
	}
