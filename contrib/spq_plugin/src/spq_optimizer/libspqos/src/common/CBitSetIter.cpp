//---------------------------------------------------------------------------
//	Greenplum Database
//	Copyright (C) 2009 Greenplum, Inc.
//
//	@filename:
//		CBitSetIter.cpp
//
//	@doc:
//		Implementation of bitset iterator
//---------------------------------------------------------------------------

#include "spqos/common/CBitSetIter.h"

#include "spqos/base.h"
#include "spqos/common/CAutoRef.h"

using namespace spqos;


//---------------------------------------------------------------------------
//	@function:
//		CBitSetIter::CBitSetIter
//
//	@doc:
//		ctor
//
//---------------------------------------------------------------------------
CBitSetIter::CBitSetIter(const CBitSet &bs)
	: m_bs(bs), m_cursor((ULONG) -1), m_bsl(NULL), m_active(true)
{
}


//---------------------------------------------------------------------------
//	@function:
//		CBitSetIter::Advance
//
//	@doc:
//		Move to next bit
//
//---------------------------------------------------------------------------
BOOL
CBitSetIter::Advance()
{
	SPQOS_ASSERT(m_active && "called advance on exhausted iterator");

	if (NULL == m_bsl)
	{
		m_bsl = m_bs.m_bsllist.First();
	}

	while (NULL != m_bsl)
	{
		if (m_cursor + 1 <= m_bs.m_vector_size &&
			m_bsl->GetVec()->GetNextSetBit(m_cursor + 1, m_cursor))
		{
			break;
		}

		m_bsl = m_bs.m_bsllist.Next(m_bsl);
		m_cursor = (ULONG) -1;
	}

	m_active = (NULL != m_bsl);
	return m_active;
}


//---------------------------------------------------------------------------
//	@function:
//		CBitSetIter::UlBit
//
//	@doc:
//		Return current position of cursor
//
//---------------------------------------------------------------------------
ULONG
CBitSetIter::Bit() const
{
	SPQOS_ASSERT(m_active && NULL != m_bsl && "iterator uninitialized");
	SPQOS_ASSERT(m_bsl->GetVec()->Get(m_cursor));

	return m_bsl->GetOffset() + m_cursor;
}

// EOF
