...
//---------------------------------------------------------------------------
COstream::COstream()
	: m_static_string_buffer(m_string_format_buffer, SPQOS_OSTREAM_CONVBUF_SIZE),
...
{
	return AppendFormat(SPQOS_WSZ_LIT("%s"), input_char);
}
...
{
	return AppendFormat(SPQOS_WSZ_LIT("%lc"), input_char);
}
...
COstream::operator<<(const CHAR input_char)
{
	return AppendFormat(SPQOS_WSZ_LIT("%c"), input_char);
}
...
COstream::operator<<(ULONG input_ulong)
{
	...
	{
		case EsmDec:
			return AppendFormat(SPQOS_WSZ_LIT("%u"), input_ulong);

		case EsmHex:
			return AppendFormat(SPQOS_WSZ_LIT("%x"), input_ulong);

		default:
			SPQOS_ASSERT(!"Unexpected stream mode");
	}
...
COstream::operator<<(ULLONG input_ullong)
{
	...
	{
		case EsmDec:
			return AppendFormat(SPQOS_WSZ_LIT("%llu"), input_ullong);

		case EsmHex:
			return AppendFormat(SPQOS_WSZ_LIT("%llx"), input_ullong);
...
COstream::operator<<(INT input_int)
{
	...
	{
		case EsmDec:
			return AppendFormat(SPQOS_WSZ_LIT("%d"), input_int);

		case EsmHex:
			return AppendFormat(SPQOS_WSZ_LIT("%x"), input_int);
...
COstream::operator<<(LINT input_long_int)
{
	...
	{
		case EsmDec:
			return AppendFormat(SPQOS_WSZ_LIT("%lld"), input_long_int);

		case EsmHex:
			return AppendFormat(SPQOS_WSZ_LIT("%llx"), input_long_int);
...
{
	return AppendFormat(SPQOS_WSZ_LIT("%f"), input_double);
}
...
{
	return AppendFormat(SPQOS_WSZ_LIT("%p"), input_pointer);
}
...
// It is enabled only for GNU libstdc++, where it is known to work.
	...
	TManip tmf = func_ptr;
	SPQOS_ASSERT(tmf == static_cast<TManip>(std::endl) &&
