...
{
	m_list.Init(SPQOS_OFFSET(CTask, m_proxy_link));
...
CAutoTaskProxy::Create(void *(*pfunc)(void *), void *arg, BOOL *cancel)
{
	...
	if (NULL == task_parent)
	{
		...
		task_ctxt = SPQOS_NEW(mp) CTaskContext(mp);
	}
	else
	{
		// clone parent task's context
		task_ctxt = SPQOS_NEW(mp) CTaskContext(mp, *task_parent->GetTaskCtxt());
	}

	// auto pointer to hold error context
	CAutoP<CErrorContext> err_ctxt;
	err_ctxt = SPQOS_NEW(mp) CErrorContext();
	...
	new_task =
		SPQOS_NEW(m_mp) CTask(mp, task_ctxt.Value(), err_ctxt.Value(), cancel);
...
{
	SPQOS_ASSERT(CTask::EtsInit == task->m_status && "Task already scheduled");
...
CAutoTaskProxy::FindFinished(CTask **task)
{
	*task = NULL;
	...
	SPQOS_ASSERT(scheduled && "No task scheduled yet");
	SPQOS_ASSERT(!reported_all && "All tasks have been reported as finished");

	return SPQOS_NOT_FOUND;
}
...
CAutoTaskProxy::Execute(CTask *task)
{
	...
	if (task->HasPendingExceptions())
	{
		if (m_propagate_error)
		{
			SPQOS_RETHROW(task->GetErrCtxt()->GetException());
		}
		else
		{
...
CAutoTaskProxy::CheckError(CTask *sub_task)
{
	...
	if (sub_task->HasPendingExceptions())
	{
		...
		SPQOS_ASSERT(ITask::EtsError == sub_task->GetStatus());
	...
	else if (ITask::EtsError == sub_task->GetStatus())
	{
		...
		SPQOS_ASSERT(!sub_task->HasPendingExceptions() &&
					sub_task->IsCanceled());
	}
#endif	// SPQOS_DEBUG
}
...
{
	SPQOS_ASSERT(m_propagate_error);
	...
	SPQOS_ASSERT(NULL != current_task && !current_task->HasPendingExceptions());
