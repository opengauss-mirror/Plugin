...
//---------------------------------------------------------------------------
#ifndef SPQOS_CList_H
#define SPQOS_CList_H
...
private:
	...
	{
		SPQOS_ASSERT(MAX_ALIGNED(pv));
		SPQOS_ASSERT(MAX_ALIGNED(m_offset));

		SPQOS_ASSERT(spqos::ulong_max != m_offset &&
...
public:
	// ctor
	CList() : m_offset(spqos::ulong_max), m_size(0), m_head(NULL), m_tail(NULL)
	{
	}
	...
	Init(ULONG offset)
	{
		SPQOS_ASSERT(MAX_ALIGNED(offset));
		m_offset = offset;
	}
	...
	Prepend(T *elem)
	{
		...
		if (NULL == m_head)
		{
			SPQOS_ASSERT(NULL == m_tail);
			SPQOS_ASSERT(0 == m_size);
