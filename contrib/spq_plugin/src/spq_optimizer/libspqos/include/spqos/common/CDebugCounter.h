...
#ifndef SPQOS_CDebugCounter_h
#define SPQOS_CDebugCounter_h

/*--------------------------------------------------------------------------
                              How to use this class
   --------------------------------------------------------------------------

 1. Ensure the SPQOS_DEBUG_COUNTERS define is enabled:

    Use a debug build or change the code below to enable SPQOS_DEBUG_COUNTERS
 ...
 3. Add one or more of the following lines to your code:
      ...
      SPQOS_DEBUG_COUNTER_BUMP("my counter name 1");

      // to sum up things to be counted for each query:
      SPQOS_DEBUG_COUNTER_ADD("my counter name 2", num_widgets_allocated);

      // to sum up a floating point number
      SPQOS_DEBUG_COUNTER_ADD_DOUBLE("my counter name 3", scan_cost);

      // to count the total time spent to do something
      SPQOS_DEBUG_COUNTER_START_CPU_TIME("my counter name 4");
      // do something
      SPQOS_DEBUG_COUNTER_STOP_CPU_TIME("my counter name 4");
 ...
 5. Extract the log information into a CSV format, using the
    script spqorca/scripts/get_debug_event_counters.py
 ...
 6. Clean up the code before submitting a PR
    ...
    - in rare cases, if you really do want to keep some lines with
      SPQOS_DEBUG_COUNTER_... macros in the code, surround them
      ...
      // #define SPQOS_DEBUG_COUNTER_XFORMS

      // in the instrumented code
      #ifdef SPQOS_DEBUG_COUNTER_XFORMS
      SPQOS_DEBUG_COUNTER_BUMP(xform_name);
      #endif
...
#define SPQOS_DEBUG_COUNTER_BUMP(c) spqos::CDebugCounter::Bump(c)
#define SPQOS_DEBUG_COUNTER_ADD(c, d) spqos::CDebugCounter::Add(c, d)
#define SPQOS_DEBUG_COUNTER_ADD_DOUBLE(c, d) spqos::CDebugCounter::AddDouble(c, d)
#define SPQOS_DEBUG_COUNTER_START_CPU_TIME(c) \
	spqos::CDebugCounter::StartCpuTime(c)
#define SPQOS_DEBUG_COUNTER_STOP_CPU_TIME(c) spqos::CDebugCounter::StopCpuTime(c)
#else
// empty definitions otherwise
#define SPQOS_DEBUG_COUNTER_BUMP(c)
#define SPQOS_DEBUG_COUNTER_ADD(c, d)
#define SPQOS_DEBUG_COUNTER_ADD_DOUBLE(c, d)
#define SPQOS_DEBUG_COUNTER_START_CPU_TIME(c)
#define SPQOS_DEBUG_COUNTER_STOP_CPU_TIME(c)
#endif
...
public:
	...
	// The static CDebugCounter lives from spqos_init to spqos_terminate
	static void Init();
...
#endif /* SPQOS_DEBUG_COUNTERS */
}  // namespace spqos
#endif /* SPQOS_CDebugCounter_h */
