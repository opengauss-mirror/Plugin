...
//---------------------------------------------------------------------------
#ifndef SPQOS_CDouble_H
#define SPQOS_CDouble_H

#include <math.h>

#include "spqos/base.h"

#define SPQOS_FP_ABS_MIN (1e-250)  // min value: 4.94066e-324
#define SPQOS_FP_ABS_MAX (1e+250)  // max value: 8.98847e+307
...
private:
	...
	CheckValidity()
	{
		SPQOS_ASSERT(0.0 < SPQOS_FP_ABS_MIN);
		SPQOS_ASSERT(SPQOS_FP_ABS_MIN < SPQOS_FP_ABS_MAX);

		double abs_value = fabs(m_d);

		if (SPQOS_FP_ABS_MAX < abs_value)
		{
			SetSignedVal(SPQOS_FP_ABS_MAX);
		}

		if (SPQOS_FP_ABS_MIN > abs_value)
		{
			SetSignedVal(SPQOS_FP_ABS_MIN);
		}
	}
...
public:
	...
	operator==(const CDouble &left, const CDouble &right)
	{
		...
		return (fabs(fpCompare.m_d) == SPQOS_FP_ABS_MIN);
	}
	...
	operator>(const CDouble &left, const CDouble &right)
	{
		...
		return (fpCompare.m_d > SPQOS_FP_ABS_MIN);
	}
	...
	inline static BOOL
	Equals(DOUBLE dLeft, DOUBLE dRight, DOUBLE dPrecision = SPQOS_FP_ABS_MIN)
	{
