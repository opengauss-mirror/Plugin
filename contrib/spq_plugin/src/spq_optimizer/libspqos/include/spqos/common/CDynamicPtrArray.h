...
//---------------------------------------------------------------------------
#ifndef SPQOS_CDynamicPtrArray_H
#define SPQOS_CDynamicPtrArray_H
...
private:
	...
	{
		SPQOS_ASSERT(new_size > m_capacity &&
					"Invalid call to Resize, cannot shrink array");

		// get new target array
		T **new_elems = SPQOS_NEW_ARRAY(m_mp, T *, new_size);

		if (m_size > 0)
		{
			SPQOS_ASSERT(NULL != m_elems);
			clib::Memcpy(new_elems, m_elems, sizeof(T *) * m_size);
			SPQOS_DELETE_ARRAY(m_elems);
		}
...
public:
	...
	explicit CDynamicPtrArray<T, CleanupFn>(CMemoryPool *mp, ULONG min_size = 4,
		...
		: m_mp(mp),
		  ...
		  m_elems(NULL)
	{
		SPQOS_ASSERT(NULL != CleanupFn && "No valid destroy function specified");
	...
	Append(T *elem)
	{
		...
		SPQOS_ASSERT(m_size < m_capacity);
	...
	{
		SPQOS_ASSERT(NULL != arr);
		SPQOS_ASSERT(this != arr && "Cannot append array to itself");
		...
		SPQOS_ASSERT(m_size <= m_capacity);
		SPQOS_ASSERT_IMP(m_size == m_capacity, 0 == arr->m_size);

		SPQOS_ASSERT(total_size <= m_capacity);

		// at this point old memory is no longer accessible, hence, no self-copy
		if (arr->m_size > 0)
		{
			SPQOS_ASSERT(NULL != arr->m_elems);
	...
	{
		SPQOS_ASSERT(pos < m_size && "Out of bounds access");
	...
	{
		SPQOS_ASSERT(pos1 < m_size && pos2 < m_size &&
	...
	IndexesOfSubsequence(CDynamicPtrArray<T, CleanupFn> *subsequence)
	{
		...
		for (ULONG ul1 = 0; ul1 < subsequence_length; ul1++)
		{
			...
			indexes->Append(SPQOS_NEW(m_mp) ULONG(index));
		}
		return indexes;
	}
	...
	CreateReducedArray(ULongPtrArray *indexes_to_choose)
	{
		CDynamicPtrArray<T, CleanupFn> *result =
			SPQOS_NEW(m_mp) CDynamicPtrArray<T, CleanupFn>(m_mp, m_min_size,
														  m_expansion_factor);
...
#endif	// !SPQOS_CDynamicPtrArray_H
