...
//
//---------------------------------------------------------------------------
#ifndef SPQOS_CHashSet_H
#define SPQOS_CHashSet_H
...
private:
	...
	Lookup(const T *value) const
	{
		...
		if (NULL != *chain)
		{
			...
			SPQOS_ASSERT_IMP(NULL != found_hse, *found_hse == hse);
		}
...
public:
	...
	CHashSet<T, HashFn, EqFn, CleanupFn>(CMemoryPool *mp, ULONG size = 127)
		: m_mp(mp),
		  m_num_chains(size),
		  m_size(0),
		  m_chains(SPQOS_NEW_ARRAY(m_mp, HashSetElemArray *, m_num_chains)),
		  m_elements(SPQOS_NEW(m_mp) Elements(m_mp)),
	...
	Insert(T *value)
	{
		...
		if (NULL == *chain)
		{
			*chain = SPQOS_NEW(m_mp) HashSetElemArray(m_mp);
			INT chain_idx = HashFn(value) % m_num_chains;
			m_filled_chains->Append(SPQOS_NEW(m_mp) INT(chain_idx));
		}

		CHashSetElem *elem = SPQOS_NEW(m_mp) CHashSetElem(value, true /*fOwn*/);
		(*chain)->Append(elem);
