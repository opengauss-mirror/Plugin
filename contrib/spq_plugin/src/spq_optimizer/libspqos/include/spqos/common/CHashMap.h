...
//---------------------------------------------------------------------------
#ifndef SPQOS_CHashMap_H
#define SPQOS_CHashMap_H
...
private:
	...
	Lookup(const K *key) const
	{
		...
		if (NULL != *chain)
		{
			...
			SPQOS_ASSERT_IMP(NULL != found_hme, *found_hme == hme);
		}
...
public:
	...
	CHashMap<K, T, HashFn, EqFn, DestroyKFn, DestroyTFn>(CMemoryPool *mp,
		...
		: m_mp(mp),
		  ...
		  m_size(0),
		  m_chains(SPQOS_NEW_ARRAY(m_mp, CHashSetElemArray *, m_num_chains)),
		  m_keys(SPQOS_NEW(m_mp) Keys(m_mp)),
		  m_filled_chains(SPQOS_NEW(mp) IntPtrArray(mp))
	{
		SPQOS_ASSERT(m_num_chains > 0);
	...
	Insert(K *key, T *value)
	{
		...
		if (NULL == *chain)
		{
			*chain = SPQOS_NEW(m_mp) CHashSetElemArray(m_mp);
			INT chain_idx = HashFn(key) % m_num_chains;
			m_filled_chains->Append(SPQOS_NEW(m_mp) INT(chain_idx));
		}

		CHashMapElem *elem =
			SPQOS_NEW(m_mp) CHashMapElem(key, value, true /*fOwn*/);
		(*chain)->Append(elem);
