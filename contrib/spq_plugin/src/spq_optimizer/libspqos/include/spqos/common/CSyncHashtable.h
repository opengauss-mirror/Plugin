...
//---------------------------------------------------------------------------
#ifndef SPQOS_CSyncHashtable_H
#define SPQOS_CSyncHashtable_H
...
private:
	...
	{
		SPQOS_ASSERT(IsValid(key) && "Invalid key is inaccessible");
	...
	{
		SPQOS_ASSERT(index < m_nbuckets && "Invalid bucket index");
	...
	Key(T *value) const
	{
		SPQOS_ASSERT(spqos::ulong_max != m_key_offset &&
...
public:
	...
	{
		SPQOS_ASSERT(NULL == m_buckets);
		SPQOS_ASSERT(0 == m_nbuckets);
		SPQOS_ASSERT(NULL != invalid_key);
		SPQOS_ASSERT(NULL != func_hash);
		SPQOS_ASSERT(NULL != func_equal);
		...
		m_buckets = SPQOS_NEW_ARRAY(mp, SBucket, m_nbuckets);
