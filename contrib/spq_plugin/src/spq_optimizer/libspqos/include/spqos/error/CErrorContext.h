...
//---------------------------------------------------------------------------
#ifndef SPQOS_CErrorContext_H
#define SPQOS_CErrorContext_H
...
#define SPQOS_ERROR_MESSAGE_BUFFER_SIZE (4 * 1024)
...
private:
	...
	WCHAR m_error_msg[SPQOS_ERROR_MESSAGE_BUFFER_SIZE];

	// system error message buffer
	CHAR m_system_error_msg[SPQOS_ERROR_MESSAGE_BUFFER_SIZE];
...
public:
	...
	{
		SPQOS_ASSERT(NULL == m_mini_dumper_handle);
	...
	{
		SPQOS_ASSERT(mini_dumper_handle == m_mini_dumper_handle);
	...
	AppendStackTrace()
	{
		m_static_buffer.AppendFormat(SPQOS_WSZ_LIT("\nStack trace:\n"));
