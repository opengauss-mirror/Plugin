...
//---------------------------------------------------------------------------
#ifndef SPQOS_CException_H
#define SPQOS_CException_H

#include "spqos/types.h"

// SQL state code length
#define SPQOS_SQLSTATE_LENGTH 5

// standard way to raise an exception
#define SPQOS_RAISE(...) spqos::CException::Raise(__FILE__, __LINE__, __VA_ARGS__)

// raises SPQOS exception,
// these exceptions can later be translated to SPQDB log severity levels
// so that they can be written in SPQDB with appropriate severity level.
#define SPQOS_THROW_EXCEPTION(...) \
	spqos::CException::Raise(__FILE__, __LINE__, __VA_ARGS__)

// helper to match a caught exception
#define SPQOS_MATCH_EX(ex, major, minor) \
...
#define SPQOS_CATCH_EX(exc)               \
	}                                    \
	catch (spqos::CException & exc)       \
	{                                    \
		{                                \
			if (NULL != err_hdl__)       \
...
#define SPQOS_CATCH_END \
	}                  \
	}                  \
	while (0)

// to be used inside a catch block
#define SPQOS_RESET_EX ITask::Self()->GetErrCtxt()->Reset()
#define SPQOS_RETHROW(exc) spqos::CException::Reraise(exc)

// short hands for frequently used exceptions
#define SPQOS_ABORT SPQOS_RAISE(CException::ExmaSystem, CException::ExmiAbort)
#define SPQOS_OOM_CHECK(x)                                            \
	do                                                               \
	{                                                                \
		if (NULL == (void *) x)                                      \
		{                                                            \
...
private:
	...
	// severity level mapped to SPQDB log severity level
