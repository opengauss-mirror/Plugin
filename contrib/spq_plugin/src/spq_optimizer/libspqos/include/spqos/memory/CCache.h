...
//
//---------------------------------------------------------------------------
#ifndef SPQOS_CCACHE_H_
#define SPQOS_CCACHE_H_
...
#include "spqos/memory/CCacheEntry.h"
...
private:
	...
	ReleaseEntry(CCacheHashTableEntry *entry)
	{
		...
		SPQOS_ASSERT(EXPECTED_REF_COUNT_FOR_DELETE < entry->RefCount() &&
	...
	Next(CCacheHashTableEntry *entry)
	{
		...
		SPQOS_ASSERT_IMP(AllowsDuplicateKeys(), NULL == next);
		...
		if (m_cache_size > m_cache_quota)
		{
				 ...
				 retry_count++)
			{
				...
				if (total_freed >= num_to_free)
				{
					...
					SPQOS_ASSERT(m_clock_hand_advanced);
					// no need to retry
					break;
				}
...
public:
		  ...
		  m_equal_func(equal_func)
	{
		...
		SPQOS_ASSERT(0 != g_clock_init_counter);

		// initialize hashtable
		m_hash_table.Init(m_mp, CACHE_HT_NUM_OF_BUCKETS,
						  SPQOS_OFFSET(CCacheHashTableEntry, m_link_hash),
						  SPQOS_OFFSET(CCacheHashTableEntry, m_key),
						  (&CCacheHashTableEntry::m_invalid_key), m_hash_func,
						  m_equal_func);

		m_clock_hand = SPQOS_NEW(mp) CCacheHashtableIter(m_hash_table);
	}
