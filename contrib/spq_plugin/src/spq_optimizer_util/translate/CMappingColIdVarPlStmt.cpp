...
//
//	@doc:
//		Translates a DXL scalar identifier operator into a SPQDB Param node
//
//---------------------------------------------------------------------------
Param *
CMappingColIdVarPlStmt::ParamFromDXLNodeScId(const CDXLScalarIdent *dxlop)
{
	SPQOS_ASSERT(NULL != m_output_context);
	...
	if (NULL != elem)
	{
		...
		param->paramtype = CMDIdSPQDB::CastMdid(elem->MdidType())->Oid();
...
//
//	@doc:
//		Translates a DXL scalar identifier operator into a SPQDB Var node
//
//---------------------------------------------------------------------------
Var *
CMappingColIdVarPlStmt::VarFromDXLNodeScId(const CDXLScalarIdent *dxlop)
{
	...
	{
		SPQOS_ASSERT(0 != m_child_contexts->Size());

		const CDXLTranslateContext *left_context = (*m_child_contexts)[0];

		// not a base table
		SPQOS_ASSERT(NULL != left_context);
		...
		else
		{
			...
			SPQOS_ASSERT(NULL != right_context);
	...
	Var *var = spqdb::MakeVar(varno, attno,
