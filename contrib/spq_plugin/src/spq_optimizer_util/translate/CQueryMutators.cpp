...
//
//	@doc:
//		Implementation of methods used during translating a SPQDB Query object into a
//		DXL Tree
//
//	@test:
//
...
CQueryMutators::ShouldFallback(Node *node, SContextTLWalker *context)
{
	...
	TargetEntry *entry = spqdb::FindFirstMatchingMemberInTargetList(
...
CQueryMutators::NormalizeGroupByProjList(CMemoryPool *mp,
										 ...
										 const Query *query)
{
	Query *query_copy = (Query *) spqdb::CopyObject(const_cast<Query *>(query));
	...
	spqdb::SPQDBFree(query_copy);

	SPQOS_ASSERT(1 == spqdb::ListLength(new_query->rtable));
	Query *derived_table_query =
		(Query *) ((RangeTblEntry *) spqdb::ListNth(new_query->rtable, 0))
			->subquery;
	SContextGrpbyPlMutator context(mp, md_accessor, derived_table_query, NULL);
	List *target_list_copy =
		(List *) spqdb::CopyObject(derived_table_query->targetList);
	ListCell *lc = NULL;
	...
	ForEach(lc, target_list_copy)
	{
		...
		if (!is_grouping_col)
		{
			...
			SPQOS_ASSERT(!IsA(target_entry->expr, Aggref) &&
						!IsA(target_entry->expr, GroupingFunc) &&
									   ...
									   SContextIncLevelsupMutator *context)
{
	...
	if (IsA(node, Var))
	{
		Var *var = (Var *) spqdb::CopyObject(node);
	...
	{
		return (Node *) spqdb::CopyObject(node);
	}
	...
	if (IsA(node, Query))
	{
		...
		Query *query = spqdb::MutateQueryTree((Query *)node, (MutatorWalkerFn)CQueryMutators::RunIncrLevelsUpMutator, context,
            0 // flags
            );
		context->m_current_query_level--;
	...
	return spqdb::MutateExpressionTree(node, (MutatorWalkerFn)CQueryMutators::RunIncrLevelsUpMutator, context);
}
										...
										SContextIncLevelsupMutator *context)
{
	...
	if (IsA(node, RangeTblEntry))
	{
			...
			rte->ctelevelsup >= context->m_current_query_level)
		{
			...
			// the walker in SPQDB does not walk range table entries of type CTE
			rte->ctelevelsup++;
		}
									  ...
									  SContextGrpbyPlMutator *context)
{
	...
	if (IsA(node, Var))
	{
		Var *var_copy = (Var *) spqdb::CopyObject(node);
	...
	if (IsA(node, Aggref))
	{
		...
		{
			Node *arg = (Node *) spqdb::CopyObject((Node *) lfirst(lc));
			SPQOS_ASSERT(NULL != arg);
			// traverse each argument and fix levels up when needed
			new_args = spqdb::LAppend(
				new_args,
				spqdb::MutateQueryOrExpressionTree(
					arg,
	...
	if (IsA(node, SubLink))
	{
		...
		new_sublink->operName =
			(List *) spqdb::CopyObject(old_sublink->operName);

		new_sublink->testexpr = spqdb::MutateQueryOrExpressionTree(
		...
		SPQOS_ASSERT(IsA(old_sublink->subselect, Query));

		new_sublink->subselect = spqdb::MutateQueryOrExpressionTree(
			old_sublink->subselect,
			(MutatorWalkerFn) CQueryMutators::RunGroupingColMutator, context,
			0  // flags -- mutate into cte-lists
		);
		new_sublink->location = -1;
	...
	{
		CommonTableExpr *cte = (CommonTableExpr *) spqdb::CopyObject(node);
		context->m_current_query_level++;

		SPQOS_ASSERT(IsA(cte->ctequery, Query));

		cte->ctequery = spqdb::MutateQueryOrExpressionTree(
			cte->ctequery,
			(MutatorWalkerFn) CQueryMutators::RunGroupingColMutator,
			(void *) context,
			0  // flags --- mutate into cte-lists
	...
	if (IsA(node, Query))
	{
		Query *query = spqdb::MutateQueryTree(
			(Query *) node,
			(MutatorWalkerFn) CQueryMutators::RunGroupingColMutator, context,
	...
	return spqdb::MutateExpressionTree(
								...
								SContextGrpbyPlMutator *context)
{
	...
	ULONG arity = spqdb::ListLength(context->m_lower_table_tlist) + 1;
	...
	CHAR *name = CQueryMutators::GetTargetEntryColName(orginal_target_entry,
													   context->m_query);
	TargetEntry *new_target_entry = spqdb::MakeTargetEntry(
	...
	context->m_lower_table_tlist =
		spqdb::LAppend(context->m_lower_table_tlist, new_target_entry);

	Var *new_var = spqdb::MakeVar(
		1,	// varno
		(AttrNumber) arity, spqdb::ExprType((Node *) orginal_target_entry->expr),
		spqdb::ExprTypeMod((Node *) orginal_target_entry->expr),
		0  // query levelsup
	);
...
CQueryMutators::GetTargetEntryForAggExpr(CMemoryPool *mp,
										 ...
										 ULONG attno)
{
	SPQOS_ASSERT(IsA(node, Aggref) || IsA(node, GroupingFunc));
	...
	else
	{
		...
		CMDIdSPQDB *agg_mdid =
			SPQOS_NEW(mp) CMDIdSPQDB(IMDId::EmdidGeneral, aggref->aggfnoid);
	...
	SPQOS_ASSERT(NULL != name);

	return spqdb::MakeTargetEntry((Expr *) node, (AttrNumber) attno, name,
								 false);
}
											...
											SContextGrpbyPlMutator *context)
{
	...
	if (0 == context->m_current_query_level)
	{
		...
		{
			// create a new entry in the derived table and return its corresponding var
			Node *node_copy = (Node *) spqdb::CopyObject(node);
	...
	if (IsA(node, Var))
	{
		...
		if (var->varlevelsup == context->m_current_query_level)
		{
			...
			var->varlevelsup = 0;
			TargetEntry *found_tle = spqdb::FindFirstMatchingMemberInTargetList(
									  ...
									  SContextGrpbyPlMutator *context)
{
	...
	SPQOS_ASSERT(IsA(node, Aggref) || IsA(node, GroupingFunc) || IsA(node, Var));

	// Append a new target entry for the node to the derived target list ...
	const ULONG attno = spqdb::ListLength(context->m_lower_table_tlist) + 1;
	TargetEntry *tle = NULL;
	if (IsA(node, Aggref) || IsA(node, GroupingFunc))
		tle = GetTargetEntryForAggExpr(context->m_mp, context->m_mda, node,
									   attno);
	else if (IsA(node, Var))
		tle = spqdb::MakeTargetEntry((Expr *) node, (AttrNumber) attno, NULL,
									false);

	context->m_lower_table_tlist =
		spqdb::LAppend(context->m_lower_table_tlist, tle);
	...
	Var *new_var =
		spqdb::MakeVar(1 /* varno */, attno, spqdb::ExprType((Node *) node),
					  spqdb::ExprTypeMod((Node *) node),
											...
											SContextGrpbyPlMutator *context)
{
	...
	if (NULL != found_tle)
	{
		...
		Var *new_var =
			spqdb::MakeVar(1 /* varno */, found_tle->resno,
						  spqdb::ExprType((Node *) found_tle->expr),
						  spqdb::ExprTypeMod((Node *) found_tle->expr),
						  context->m_current_query_level /* varlevelsup */);
...
CQueryMutators::IncrLevelsUpIfOuterRef(Var *var)
{
	...
	Var *var_copy = (Var *) spqdb::CopyObject(var);
								...
								const Query *query)
{
	...
	RangeTblEntry *rte =
		((RangeTblEntry *) spqdb::ListNth(new_query->rtable, 0));
	...
	ForEach(lc, derived_table_query->targetList)
	{
			...
			!target_entry->resjunk)
		{
			...
			new_query->targetList =
				spqdb::LAppend(new_query->targetList, new_target_entry);
	...
	if (!rte->subquery->hasAggs && NIL == rte->subquery->groupClause)
	{
		...
		ForEach(lc, rte->subquery->targetList)
		{
			...
			SPQOS_ASSERT(!target_entry->resjunk);

			new_subquery->targetList =
				spqdb::LAppend(new_subquery->targetList,
							  (TargetEntry *) spqdb::CopyObject(
								  const_cast<TargetEntry *>(target_entry)));
		}

		spqdb::SPQDBFree(rte->subquery);
							   ...
							   const Query *query, ULONG query_level)
{
	...
	Query *pqueryResolveJoinVarReferences =
		spqdb::FlattenJoinAliasVar(const_cast<Query *>(query), query_level);
	...
	SPQOS_ASSERT(NULL == pqueryEliminateDistinct->distinctClause);
	spqdb::SPQDBFree(pqueryResolveJoinVarReferences);
	...
	spqdb::SPQDBFree(pqueryEliminateDistinct);
	...
	SPQOS_ASSERT(NULL == pqueryHavingNormalized->havingQual);
	spqdb::SPQDBFree(pqueryWindowPlNormalized);
	...
	spqdb::SPQDBFree(pqueryHavingNormalized);
...
{
	Var *new_var = spqdb::MakeVar(
		1, (AttrNumber) attno, spqdb::ExprType((Node *) old_target_entry->expr),
		spqdb::ExprTypeMod((Node *) old_target_entry->expr),
		0  // query levelsup
	);
...
CQueryMutators::GetTargetEntryColName(TargetEntry *target_entry, Query *query)
{
	...
	CWStringConst dummy_colname(SPQOS_WSZ_LIT("?column?"));
									  ...
									  BOOL should_fix_having_qual)
{
	...
	Query *query_copy =
		(Query *) spqdb::CopyObject(const_cast<Query *>(original_query));
	...
	struct GpPolicy *into_policy = (GpPolicy*)query_copy->intoPolicy;
	...
	Query *lower_query;
	{
		...
		lower_query = spqdb::MutateQueryTree(
	...
	// derived table.
	{
		...
		(void) spqdb::WalkQueryOrExpressionTree(
	...
	upper_query->rtable = spqdb::LAppend(upper_query->rtable, rte);
	...
	fromexpr->quals = NULL;
	fromexpr->fromlist = spqdb::LAppend(fromexpr->fromlist, rtref);

	upper_query->jointree = fromexpr;
	upper_query->commandType = CMD_SELECT;

	SPQOS_ASSERT(1 == spqdb::ListLength(upper_query->rtable));
	SPQOS_ASSERT(false == upper_query->hasWindowFuncs);
...
{
	if (0 == spqdb::ListLength(query->distinctClause))
	{
		return (Query *) spqdb::CopyObject(const_cast<Query *>(query));
	}
	...
	{
		ULONG resno = spqdb::ListLength(new_query->targetList) + 1;
		TargetEntry *target_entry = (TargetEntry *) lfirst(lc);
		SPQOS_ASSERT(NULL != target_entry);
		...
		if (!target_entry->resjunk)
		{
			...
			Var *new_var =
				spqdb::MakeVar(1, target_entry->resno,
							  spqdb::ExprType((Node *) target_entry->expr),
							  spqdb::ExprTypeMod((Node *) target_entry->expr),
							  0	 // query levels up
				);
			TargetEntry *new_target_entry =
				spqdb::MakeTargetEntry((Expr *) new_var, (AttrNumber) resno,
	...
	if (spqdb::ListLength(new_query->targetList) !=
		spqdb::ListLength(query->distinctClause))
	{
		SPQOS_RAISE(
	...
	ForEach(pl, query->distinctClause)
	{
		...
		SPQOS_ASSERT(NULL != sort_group_clause);
		...
		new_query->groupClause =
			spqdb::LAppend(new_query->groupClause, new_sort_group_clause);
	}
										...
										const Query *original_query)
{
	...
	SPQOS_ASSERT(NULL == original_query->distinctClause);
	SPQOS_ASSERT(NULL == original_query->groupClause);
	...
	SPQOS_ASSERT(1 == spqdb::ListLength(upper_query->rtable));
	Query *lower_query =
		(Query *) ((RangeTblEntry *) spqdb::ListNth(upper_query->rtable, 0))
			->subquery;
	...
	ForEach(lc, target_entries)
	{
		...
		TargetEntry *target_entry = (TargetEntry *) lfirst(lc);
		const ULONG ulResNoNew = spqdb::ListLength(upper_query->targetList) + 1;
				...
				target_entry, original_query->windowClause))
		{
			...
			TargetEntry *lower_target_entry =
				(TargetEntry *) spqdb::MutateExpressionTree(
					(Node *) target_entry,
					(MutatorWalkerFn) RunIncrLevelsUpMutator, &level_context);

			lower_target_entry->resno =
				spqdb::ListLength(projlist_context.m_lower_table_tlist) + 1;
			projlist_context.m_lower_table_tlist = spqdb::LAppend(
			...
			if (!target_entry->resjunk || is_sorting_col)
			{
				...
				TargetEntry *upper_target_entry = spqdb::MakeTargetEntry(
				...
				upper_query->targetList =
					spqdb::LAppend(upper_query->targetList, upper_target_entry);
			}
		}
		else
		{
	...
	SPQOS_ASSERT(spqdb::ListLength(upper_query->targetList) <=
				spqdb::ListLength(original_query->targetList));
										 ...
										 SContextGrpbyPlMutator *context)
{
	...
	const ULONG resno = spqdb::ListLength(context->m_lower_table_tlist) + 1;
	...
	if (IsA(node, WindowFunc) && 0 == context->m_current_query_level)
	{
		...
		WindowFunc *window_func = 
			(WindowFunc *)spqdb::MutateExpressionTree(node, (MutatorWalkerFn)RunIncrLevelsUpMutator, &levelsUpContext);
		SPQOS_ASSERT(IsA(window_func, WindowFunc));

		// get the function name and create a new target entry for window_func
		CMDIdSPQDB *mdid_func = SPQOS_NEW(context->m_mp)
			CMDIdSPQDB(IMDId::EmdidGeneral, window_func->winfnoid);
		...
		context->m_lower_table_tlist =
			spqdb::LAppend(context->m_lower_table_tlist, target_entry);
		...
		Var *new_var = spqdb::MakeVar(
			...
			(AttrNumber) resno, spqdb::ExprType(node), spqdb::ExprTypeMod(node),
			0  // query levelsup
		);
		...
		((Var *) node)->varlevelsup == context->m_current_query_level)
	{
		...
		if (NULL == found_tle)
		{
			...
			Node *var_copy = (Node *) spqdb::CopyObject(var);
	...
	if (IsA(node, Query))
	{
		...
		context->m_current_query_level++;
		Query *result = spqdb::MutateQueryTree((Query *)node, (MutatorWalkerFn)RunWindowProjListMutator, context, 0);
	...
	return spqdb::MutateExpressionTree(node, (MutatorWalkerFn)CQueryMutators::RunWindowProjListMutator, context);
}
