...
#include "spq_optimizer_util/translate/CQueryMutators.h"
	  ...
	  m_cteid_at_current_query_level_map(NULL)
{
	...
	// SPQDB_94_MERGE_FIXME: WITH CHECK OPTION views are not supported yet.
	...
	/*if (query->withCheckOptions)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("View with WITH CHECK OPTION"));
	}*/

	// Initialize the map that stores spqdb att to optimizer col mapping.
	...
	if (var_colid_mapping)
		...
		m_var_to_colid_map = SPQOS_NEW(m_mp) CMappingVarColId(m_mp);

	m_query_level_to_cte_map = SPQOS_NEW(m_mp) HMUlCTEListEntry(m_mp);
	m_dxl_cte_producers = SPQOS_NEW(m_mp) CDXLNodeArray(m_mp);
	m_cteid_at_current_query_level_map = SPQOS_NEW(m_mp) UlongBoolHashMap(m_mp);
			...
			if (cte_query_level < query_level && NULL != cte_list_entry)
			{
				...
				BOOL is_res =
					...
					m_query_level_to_cte_map->Insert(
						SPQOS_NEW(m_mp) ULONG(cte_query_level), cte_list_entry);
				SPQOS_ASSERT(is_res);
			}
		}
	}
	...
	m_scalar_translator = SPQOS_NEW(m_mp)
...
CTranslatorQueryToDXL::QueryToDXLInstance(CMemoryPool *mp,
										  ...
										  Query *query)
{
	CContextQueryToDXL *context = SPQOS_NEW(mp) CContextQueryToDXL(mp);

	return SPQOS_NEW(context->m_mp)
...
CTranslatorQueryToDXL::~CTranslatorQueryToDXL()
{
	SPQOS_DELETE(m_scalar_translator);
	SPQOS_DELETE(m_var_to_colid_map);
...
CTranslatorQueryToDXL::CheckUnsupportedNodeTypes(Query *query)
{
	static const SUnsupportedFeature unsupported_features[] = {
		{T_RowExpr, SPQOS_WSZ_LIT("ROW EXPRESSION")},
		{T_RowCompareExpr, SPQOS_WSZ_LIT("ROW COMPARE")},
		{T_FieldSelect, SPQOS_WSZ_LIT("FIELDSELECT")},
		{T_FieldStore, SPQOS_WSZ_LIT("FIELDSTORE")},
		{T_CoerceToDomainValue, SPQOS_WSZ_LIT("COERCETODOMAINVALUE")},
		//{T_GroupId, SPQOS_WSZ_LIT("GROUPID")},
		{T_CurrentOfExpr, SPQOS_WSZ_LIT("CURRENT OF")},
	};

	List *unsupported_list = NIL;
	for (ULONG ul = 0; ul < SPQOS_ARRAY_SIZE(unsupported_features); ul++)
	{
		unsupported_list = spqdb::LAppendInt(unsupported_list,
											unsupported_features[ul].node_tag);
	}

	INT unsupported_node = spqdb::FindNodes((Node *) query, unsupported_list);
	...
	INT non_default_collation = spqdb::CheckCollation((Node *) query);

	if (0 < non_default_collation)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("Non-default collation"));
	}
}
...
CTranslatorQueryToDXL::CheckSirvFuncsWithoutFromClause(Query *query)
{
	...
	if ((NULL != query->jointree &&
		 0 < spqdb::ListLength(query->jointree->fromlist)) ||
...
CTranslatorQueryToDXL::HasSirvFunctions(Node *node) const
{
	...
	List *function_list = spqdb::ExtractNodesExpression(
...
CTranslatorQueryToDXL::CheckSupportedCmdType(Query *query)
{
	...
	if (CMD_SELECT == query->commandType)
	{
		...
		if (query->parentStmtType == PARENTSTMTTYPE_COPY)
		{
			SPQOS_RAISE(
				spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				SPQOS_WSZ_LIT(
					"COPY. Copy select statement to file on segment is not supported with SPQORCA"));
		}
		if (query->parentStmtType == PARENTSTMTTYPE_REFRESH_MATVIEW)
		{
			SPQOS_RAISE(
				spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				SPQOS_WSZ_LIT("Refresh matview is not supported with SPQORCA"));
		}
	...
	static const SCmdNameElem unsupported_commands[] = {
		{CMD_UTILITY, SPQOS_WSZ_LIT("UTILITY command")}};

	const ULONG length = SPQOS_ARRAY_SIZE(unsupported_commands);
...
CTranslatorQueryToDXL::CheckRangeTable(Query *query)
{
	...
	ForEach(lc, query->rtable)
	{
		...
		{
			SPQOS_ASSERT(RTE_SUBQUERY == rte->rtekind);
			// otherwise ORCA most likely pushes potentially leaky filters down
			SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
					   SPQOS_WSZ_LIT("views with security_barrier ON"));
		}
	}
}
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	SPQOS_ASSERT_IMP(NULL != m_query->setOperations,
					0 == spqdb::ListLength(m_query->windowClause));
	...
	else
	{
		child_dxlnode = TranslateGroupingSets(
			m_query->jointree, m_query->targetList, m_query->groupClause, m_query->groupingSets,
	...
	{
		m_dxl_query_output_cols = SPQOS_NEW(m_mp) CDXLNodeArray(m_mp);
	}
	else
	{
	...
	{
		SPQOS_ASSERT(NULL != dxl_cte_anchor_bottom);
...
CTranslatorQueryToDXL::TranslateQueryToDXL()
{
	...
	switch (m_query->commandType)
	{
		...
		default:
			SPQOS_ASSERT(!"Statement type not supported");
			return NULL;
	}
}
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	SPQOS_ASSERT(CMD_INSERT == m_query->commandType);
	SPQOS_ASSERT(0 < m_query->resultRelation);

	if (!u_sess->attr.attr_spq.spq_optimizer_enable_dml)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("DML not enabled"));
	}

	CDXLNode *query_dxlnode = TranslateSelectQueryToDXL();
	const RangeTblEntry *rte = (RangeTblEntry *) spqdb::ListNth(
	...
	if (!u_sess->attr.attr_spq.spq_optimizer_enable_dml_triggers &&
		CTranslatorUtils::RelHasTriggers(m_mp, m_md_accessor, md_rel,
										 Edxldmlinsert))
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("INSERT with triggers"));
	}

	BOOL rel_has_constraints = CTranslatorUtils::RelHasConstraints(md_rel);
	if (!u_sess->attr.attr_spq.spq_optimizer_enable_dml_constraints && rel_has_constraints)
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("INSERT with constraints"));
	}
	...
	const ULONG target_list_length = spqdb::ListLength(m_query->targetList);
	SPQOS_ASSERT(num_table_columns >= target_list_length);
	SPQOS_ASSERT(target_list_length == m_dxl_query_output_cols->Size());
	...
	for (ULONG ul = 0; ul < num_table_columns; ul++)
	{
		...
		if (target_list_pos < target_list_length)
		{
			...
			TargetEntry *target_entry = (TargetEntry *) spqdb::ListNth(
			...
			if (attno == resno)
			{
				...
				source_array->Append(
					SPQOS_NEW(m_mp) ULONG(dxl_ident->GetDXLColRef()->Id()));
				target_list_pos++;
				continue;
			}
		}
		...
		source_array->Append(SPQOS_NEW(m_mp) ULONG(colid));
	}

	CDXLLogicalInsert *insert_dxlnode =
		SPQOS_NEW(m_mp) CDXLLogicalInsert(m_mp, table_descr, source_array);

	if (NULL != project_list_dxlnode)
	{
		SPQOS_ASSERT(0 < project_list_dxlnode->Arity());
	...
	return SPQOS_NEW(m_mp) CDXLNode(m_mp, insert_dxlnode, query_dxlnode);
}
...
CTranslatorQueryToDXL::TranslateCTASToDXL()
{
	SPQOS_ASSERT(CMD_SELECT == m_query->commandType);
	//SPQOS_ASSERT(NULL != m_query->intoClause);

	m_is_ctas_query = true;
	CDXLNode *query_dxlnode = TranslateSelectQueryToDXL();

	// SPQDB_92_MERGE_FIXME: we should plumb through the intoClause
	...
	const ULONG num_columns = spqdb::ListLength(m_query->targetList);

	ULongPtrArray *source_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	IntPtrArray *var_typmods = SPQOS_NEW(m_mp) IntPtrArray(m_mp);
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		TargetEntry *target_entry =
			(TargetEntry *) spqdb::ListNth(m_query->targetList, ul);
		if (target_entry->resjunk)
		{
			continue;
		}
		AttrNumber resno = target_entry->resno;
		int var_typmod = spqdb::ExprTypeMod((Node *) target_entry->expr);
		var_typmods->Append(SPQOS_NEW(m_mp) INT(var_typmod));

		CDXLNode *dxl_column = (*m_dxl_query_output_cols)[ul];
		CDXLScalarIdent *dxl_ident =
			CDXLScalarIdent::Cast(dxl_column->GetOperator());
		source_array->Append(SPQOS_NEW(m_mp)
								 ULONG(dxl_ident->GetDXLColRef()->Id()));

		CMDName *md_colname = NULL;
		if (NULL != col_names && ul < spqdb::ListLength(col_names))
		{
			ColumnDef *col_def = (ColumnDef *) spqdb::ListNth(col_names, ul);
			md_colname =
				CDXLUtils::CreateMDNameFromCharArray(m_mp, col_def->colname);
		}
		else
		{
			md_colname = SPQOS_NEW(m_mp)
		...
		mdid->AddRef();
		CDXLColDescr *dxl_col_descr = SPQOS_NEW(m_mp) CDXLColDescr(
	...
	IMdIdArray *distr_opfamilies = SPQOS_NEW(m_mp) IMdIdArray(m_mp);
	IMdIdArray *distr_opclasses = SPQOS_NEW(m_mp) IMdIdArray(m_mp);

	if (NULL != m_query->intoPolicy)
	{
		rel_distr_policy =
			CTranslatorRelcacheToDXL::GetRelDistribution((GpPolicy*)m_query->intoPolicy);

		if (IMDRelation::EreldistrHash == rel_distr_policy)
		{
			distribution_colids = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
			GpPolicy* gp = (GpPolicy*)m_query->intoPolicy;
			for (ULONG ul = 0; ul < (ULONG) gp->nattrs; ul++)
			{
				AttrNumber attno = gp->attrs[ul];
				SPQOS_ASSERT(0 < attno);
				distribution_colids->Append(SPQOS_NEW(m_mp) ULONG(attno - 1));

				Oid opfamily =
					spqdb::GetOpclassFamily(gp->opclasses[ul]);
				SPQOS_ASSERT(InvalidOid != opfamily);
				...
				distr_opfamilies->Append(
					SPQOS_NEW(m_mp) CMDIdSPQDB(IMDId::EmdidGeneral, opfamily));
				distr_opclasses->Append(SPQOS_NEW(m_mp) CMDIdSPQDB(
					IMDId::EmdidGeneral, gp->opclasses[ul]));
			}
		}
	}
	else
	...
	SPQOS_ASSERT(IMDRelation::EreldistrMasterOnly != rel_distr_policy);
	m_context->m_has_distributed_tables = true;

	// TODO: Mar 5, 2014; reserve an OID
	OID oid = 1;
	CMDIdSPQDB *mdid = SPQOS_NEW(m_mp) CMDIdSPQDBCtas(oid);
	...
	//	BOOL has_oids = spqdb::InterpretOidsOption(into_clause->options);
	BOOL has_oids = false;
	BOOL fTempTable = true;
	CDXLLogicalCTAS *ctas_dxlop = SPQOS_NEW(m_mp) CDXLLogicalCTAS(
		m_mp, mdid, md_schema_name, md_relname, dxl_col_descr_array,
		SPQOS_NEW(m_mp) CDXLCtasStorageOptions(
	...
	return SPQOS_NEW(m_mp) CDXLNode(m_mp, ctas_dxlop, query_dxlnode);
}
...
)
{
	...
	SPQOS_ASSERT(NULL != storage_type);

	CDXLCtasStorageOptions::CDXLCtasOptionArray *ctas_storage_options =
		SPQOS_NEW(m_mp) CDXLCtasStorageOptions::CDXLCtasOptionArray(m_mp);
	ListCell *lc = NULL;
	BOOL is_ao_table = false;
	BOOL is_AOCO = false;
	BOOL is_parquet = false;

	CWStringConst str_append_only(SPQOS_WSZ_LIT("appendonly"));
	CWStringConst str_orientation(SPQOS_WSZ_LIT("orientation"));
	CWStringConst str_orientation_parquet(SPQOS_WSZ_LIT("parquet"));
	CWStringConst str_orientation_column(SPQOS_WSZ_LIT("column"));
	...
	ForEach(lc, options)
	{
		...
		if (is_null_arg)
		{
			...
			value_str = SPQOS_NEW(m_mp) CWStringDynamic(m_mp);
		}
		else
		{
		...
		CDXLCtasStorageOptions::CDXLCtasOption *dxl_ctas_storage_option =
			SPQOS_NEW(m_mp) CDXLCtasStorageOptions::CDXLCtasOption(
...
{
	SPQOS_ASSERT(NULL != def_elem);

	CHAR *value = spqdb::DefGetString(def_elem);
...
CTranslatorQueryToDXL::TranslateDeleteQueryToDXL()
{
	SPQOS_ASSERT(CMD_DELETE == m_query->commandType);
	SPQOS_ASSERT(0 < m_query->resultRelation);
	...
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("DELETE with triggers"));
	}
	...
	ULongPtrArray *delete_colid_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		delete_colid_array->Append(SPQOS_NEW(m_mp) ULONG(colid));
	}

	CDXLLogicalDelete *delete_dxlop = SPQOS_NEW(m_mp) CDXLLogicalDelete(
		m_mp, table_descr, ctid_colid, segid_colid, delete_colid_array);

	return SPQOS_NEW(m_mp) CDXLNode(m_mp, delete_dxlop, query_dxlnode);
}
...
CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
{
	SPQOS_ASSERT(CMD_UPDATE == m_query->commandType);
	SPQOS_ASSERT(0 < m_query->resultRelation);
	...
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("UPDATE with triggers"));
	}

	if (!u_sess->attr.attr_spq.spq_optimizer_enable_dml_constraints &&
		CTranslatorUtils::RelHasConstraints(md_rel))
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("UPDATE with constraints"));
	}
	...
	const ULONG num_of_non_sys_cols = md_rel->ColumnCount();
	ULongPtrArray *insert_colid_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	ULongPtrArray *delete_colid_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		{
			insert_colid_array->Append(SPQOS_NEW(m_mp) ULONG(*updated_colid));
		}
		else
		{
			insert_colid_array->Append(SPQOS_NEW(m_mp) ULONG(colid));
		}
	...
	CDXLLogicalUpdate *pdxlopupdate = SPQOS_NEW(m_mp) CDXLLogicalUpdate(
		m_mp, table_descr, ctid_colid, segmentid_colid, delete_colid_array,
		insert_colid_array, has_oids, tuple_oid_colid);

	return SPQOS_NEW(m_mp) CDXLNode(m_mp, pdxlopupdate, query_dxlnode);
}
...
CTranslatorQueryToDXL::UpdatedColumnMapping()
{
	IntToUlongMap *update_column_map = SPQOS_NEW(m_mp) IntToUlongMap(m_mp);
	...
	{
		TargetEntry *target_entry = (TargetEntry *) lfirst(lc);
		SPQOS_ASSERT(IsA(target_entry, TargetEntry));
		ULONG resno = target_entry->resno;
		SPQOS_ASSERT(0 < resno);

		// resjunk true columns may be now existing in the query tree, for instance
		// ctid column in case of relations, see rewriteTargetListUD in SPQDB.
	...
	SPQOS_ASSERT(output_columns == m_dxl_query_output_cols->Size());
...
CTranslatorQueryToDXL::IsLeadWindowFunc(CDXLOperator *dxlop)
{
	...
	if (EdxlopScalarWindowRef == dxlop->GetDXLOperator())
	{
		...
		const CMDIdSPQDB *mdid_spqdb =
			CMDIdSPQDB::CastMdid(winref_dxlop->FuncMdId());
		OID oid = mdid_spqdb->Oid();
		is_lead_func =
			OIDFound(oid, lead_func_oids, SPQOS_ARRAY_SIZE(lead_func_oids));
	}
...
CTranslatorQueryToDXL::IsLagWindowFunc(CDXLOperator *dxlop)
{
	...
	if (EdxlopScalarWindowRef == dxlop->GetDXLOperator())
	{
		...
		const CMDIdSPQDB *mdid_spqdb =
			CMDIdSPQDB::CastMdid(winref_dxlop->FuncMdId());
		OID oid = mdid_spqdb->Oid();
		is_lag = OIDFound(oid, lag_func_oids, SPQOS_ARRAY_SIZE(lag_func_oids));
	}
												  ...
												  CDXLNode *dxl_offset) const
{
	...
	return SPQOS_NEW(m_mp) CDXLWindowFrame(
		m_mp,
		EdxlfsRow,	   // frame specification
		EdxlfesNulls,  // frame exclusion strategy is set to exclude NULLs in SPQDB
...
) const
{
	...
	SPQOS_ASSERT(NULL != window_spec_array);
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (is_lead_func || is_lag)
		{
			...
			CDXLWindowSpec *pdxlwsNew = SPQOS_NEW(m_mp) CDXLWindowSpec(
...
{
	SPQOS_ASSERT(NULL != window_clause);
	SPQOS_ASSERT(NULL != sort_col_attno_to_colid_mapping);
	SPQOS_ASSERT(NULL != project_list_dxlnode_node);

	CDXLWindowSpecArray *window_spec_array =
		SPQOS_NEW(m_mp) CDXLWindowSpecArray(m_mp);
	...
	ForEach(lc, window_clause)
	{
		...
		if (NULL != wc->name)
		{
			...
			mdname = SPQOS_NEW(m_mp) CMDName(m_mp, alias_str);
			SPQOS_DELETE(alias_str);
		}

		if (0 < spqdb::ListLength(wc->orderClause))
		{
			// create a sorting col list
			sort_col_list_dxl = SPQOS_NEW(m_mp)
		...
		CDXLWindowSpec *window_spec_dxlnode = SPQOS_NEW(m_mp) CDXLWindowSpec(
...
CTranslatorQueryToDXL::TranslateWindowToDXL(
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
	if (0 == spqdb::ListLength(window_clause))
	{
	...
	ForEach(lc, target_list)
	{
		...
		if (!target_entry->resjunk)
		{
			...
			else
			{
				...
				omitted_target_entries =
					spqdb::LAppend(omitted_target_entries, target_entry);
				resno_list = spqdb::LAppendInt(resno_list, resno);
		...
		{
			SPQOS_ASSERT(CTranslatorUtils::IsReferencedInWindowSpec(
	...
	{
		spqdb::SPQDBFree(omitted_target_entries);
	}
	...
	if (0 < new_child_project_list_dxlnode->Arity())
	{
		...
		new_child_dxlnode = SPQOS_NEW(m_mp)
			CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalProject(m_mp));
	...
	CDXLLogicalWindow *window_dxlop =
		SPQOS_NEW(m_mp) CDXLLogicalWindow(m_mp, window_spec_array);
	CDXLNode *dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, window_dxlop);
...
{
	ULongPtrArray *part_cols = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);

	ListCell *lc = NULL;
	ForEach(lc, partition_by_clause)
	{
		Node *partition_clause = (Node *) lfirst(lc);
		SPQOS_ASSERT(NULL != partition_clause);

		SPQOS_ASSERT(IsA(partition_clause, SortGroupClause));
		...
		part_cols->Append(SPQOS_NEW(m_mp) ULONG(colid));
	}
...
{
	CDXLNodeArray *dxlnodes = SPQOS_NEW(m_mp) CDXLNodeArray(m_mp);

	ListCell *lc = NULL;
	ForEach(lc, sort_clause)
	{
		Node *node_sort_clause = (Node *) lfirst(lc);
		SPQOS_ASSERT(NULL != node_sort_clause);

		SPQOS_ASSERT(IsA(node_sort_clause, SortGroupClause));
		...
		// get operator name
		CMDIdSPQDB *op_mdid = SPQOS_NEW(m_mp) CMDIdSPQDB(IMDId::EmdidGeneral, oid);
		...
		CDXLScalarSortCol *sc_sort_col_dxlop = SPQOS_NEW(m_mp)
		...
		CDXLNode *sort_col_dxlnode =
			SPQOS_NEW(m_mp) CDXLNode(m_mp, sc_sort_col_dxlop);
...
{
	if (0 == spqdb::ListLength(sort_clause) && NULL == limit_count &&
		NULL == limit_offset_node)
	{
		return child_dxlnode;
	}

	// do not remove limit if it is immediately under a DML (JIRA: SPQSQL-2669)
	...
	CDXLNode *limit_dxlnode = SPQOS_NEW(m_mp) CDXLNode(
		m_mp, SPQOS_NEW(m_mp) CDXLLogicalLimit(m_mp, is_limit_top_level));

	// create a sorting col list
	CDXLNode *sort_col_list_dxl = SPQOS_NEW(m_mp)
		CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLScalarSortColList(m_mp));
	...
	ULONG colid) const
{
	if (0 < target_entry->ressortgroupref)
	{
		INT *key = SPQOS_NEW(m_mp) INT(target_entry->ressortgroupref);
		ULONG *value = SPQOS_NEW(m_mp) ULONG(colid);
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
			...
			while (mi.Advance())
			{
				...
				BOOL result =
					...
					output_attno_to_colid_mapping->Insert(
						SPQOS_NEW(m_mp) INT(*(mi.Key())),
						SPQOS_NEW(m_mp) ULONG(*(mi.Value())));
				SPQOS_ASSERT(result);
			}
		}
		// else:
	...
	ForEach(lc, target_list)
	{
		...
		else if (IsA(target_entry->expr, Aggref))
		{
			...
			{
				dqa_list = spqdb::LAppend(dqa_list,
										 spqdb::CopyObject(target_entry->expr));
				num_dqa++;
			}
	...
	if (1 < num_dqa && !u_sess->attr.attr_spq.spq_optimizer_enable_multiple_distinct_aggs)
	{
		SPQOS_RAISE(
			spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
			SPQOS_WSZ_LIT(
	...
	if (NIL != dqa_list)
	{
		spqdb::ListFree(dqa_list);
	}

	return SPQOS_NEW(m_mp)
		CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalGroupBy(m_mp, grouping_cols),
...
{
	SPQOS_ASSERT(NULL != aggref);

	if (NIL == dqa_list || 0 == spqdb::ListLength(dqa_list))
	{
		return false;
	}
	...
	ForEach(lc, dqa_list)
	{
		...
		SPQOS_ASSERT(IsA(node, Aggref));

		if (spqdb::Equals(aggref->args, ((Aggref *) node)->args))
		{
			return true;
		}
	}
...
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingSets(
	FromExpr *from_expr, List *target_list, List *group_clause, List *grouping_set, 
	BOOL has_aggs, IntToUlongMap *sort_grpref_to_colid_mapping,
	IntToUlongMap *output_attno_to_colid_mapping)
{
	const ULONG num_of_cols = spqdb::ListLength(target_list) + 1;
	...
	if (NULL == group_clause)
	{
		...
		if (has_aggs)
		{
			bitset = SPQOS_NEW(m_mp) CBitSet(m_mp);
		}
	...
	UlongToUlongMap *grpcol_index_to_colid_mapping =
		SPQOS_NEW(m_mp) UlongToUlongMap(m_mp);
	CBitSet *unique_grp_cols_bitset = SPQOS_NEW(m_mp) CBitSet(m_mp, num_of_cols);
	CBitSetArray *bitset_array = CTranslatorUtils::GetColumnAttnosForGroupBy(
		m_mp, group_clause,grouping_set, num_of_cols, grpcol_index_to_colid_mapping,
...
{
	SPQOS_ASSERT(NULL != bitset_array);
	SPQOS_ASSERT(1 < bitset_array->Size());
	...
	SPQOS_ASSERT(NULL != m_dxl_cte_producers);

	CDXLLogicalCTEProducer *cte_prod_dxlop = SPQOS_NEW(m_mp)
		CDXLLogicalCTEProducer(m_mp, cte_id, op_colid_array_cte_producer);
	CDXLNode *cte_producer_dxlnode = SPQOS_NEW(m_mp)
	...
	for (ULONG ul = 0; ul < num_of_grouping_sets; ul++)
	{
		...
		CDXLNode *cte_consumer_dxlnode = SPQOS_NEW(m_mp)
			CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalCTEConsumer(
							   m_mp, cte_id, colid_array_cte_consumer));

		List *target_list_copy = (List *) spqdb::CopyObject(target_list);
		...
		{
			SPQOS_ASSERT(NULL != colid_array_inner);
			CDXLColDescrArray *dxl_col_descr_array =
	...
	SPQOS_DELETE(var_colid_orig_mapping);
	...
	return SPQOS_NEW(m_mp)
		CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalCTEAnchor(m_mp, cte_id),
				 unionall_dxlnode);
}
...
CTranslatorQueryToDXL::DXLDummyConstTableGet() const
{
	...
	const CMDTypeBoolSPQDB *md_type_bool = dynamic_cast<const CMDTypeBoolSPQDB *>(
		m_md_accessor->PtMDType<IMDTypeBool>(m_sysid));
	const CMDIdSPQDB *mdid = CMDIdSPQDB::CastMdid(md_type_bool->MDId());

	// empty column name
	CWStringConst str_unnamed_col(SPQOS_WSZ_LIT(""));
	CMDName *mdname = SPQOS_NEW(m_mp) CMDName(m_mp, &str_unnamed_col);
	CDXLColDescr *dxl_col_descr = SPQOS_NEW(m_mp) CDXLColDescr(
		m_mp, mdname, m_context->m_colid_counter->next_id(), 1 /* attno */,
		SPQOS_NEW(m_mp) CMDIdSPQDB(IMDId::EmdidGeneral, mdid->Oid()),
	...
	CDXLDatum2dArray *dispatch_identifier_datum_arrays =
		SPQOS_NEW(m_mp) CDXLDatum2dArray(m_mp);

	// create a datum array
	CDXLDatumArray *dxl_datum_array = SPQOS_NEW(m_mp) CDXLDatumArray(m_mp);

	Const *const_expr =
		(Const *) spqdb::MakeBoolConst(true /*value*/, false /*isnull*/);
	CDXLDatum *datum_dxl = m_scalar_translator->TranslateConstToDXL(const_expr);
	spqdb::SPQDBFree(const_expr);

	dxl_datum_array->Append(datum_dxl);
	dispatch_identifier_datum_arrays->Append(dxl_datum_array);

	CDXLLogicalConstTable *dxlop = SPQOS_NEW(m_mp) CDXLLogicalConstTable(
...
CTranslatorQueryToDXL::TranslateSetOpToDXL(
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
	SPQOS_ASSERT(IsA(setop_node, SetOperationStmt));
	SetOperationStmt *psetopstmt = (SetOperationStmt *) setop_node;
	SPQOS_ASSERT(SETOP_NONE != psetopstmt->op);

	EdxlSetOpType setop_type =
		CTranslatorUtils::GetSetOpType(psetopstmt->op, psetopstmt->all);

	// translate the left and right child
	ULongPtrArray *leftchild_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	ULongPtrArray *rightchild_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	IMdIdArray *mdid_array_leftchild = SPQOS_NEW(m_mp) IMdIdArray(m_mp);
	IMdIdArray *mdid_array_rightchild = SPQOS_NEW(m_mp) IMdIdArray(m_mp);
	...
	ULONG *colid = SPQOS_NEW_ARRAY(m_mp, ULONG, leftchild_array->Size());
	BOOL *outer_ref_array = SPQOS_NEW_ARRAY(m_mp, BOOL, leftchild_array->Size());
	...
	ULongPtr2dArray *input_colids = SPQOS_NEW(m_mp) ULongPtr2dArray(m_mp);
	...
	SPQOS_ASSERT(output_colids->Size() == leftchild_array->Size());

	SPQOS_DELETE_ARRAY(colid);
	SPQOS_DELETE_ARRAY(outer_ref_array);

	BOOL is_cast_across_input =
		SetOpNeedsCast(target_list, mdid_array_leftchild) ||
		SetOpNeedsCast(target_list, mdid_array_rightchild);

	CDXLNodeArray *children_dxlnodes = SPQOS_NEW(m_mp) CDXLNodeArray(m_mp);
	...
	BOOL keep_res_junked) const
{
	...
	SPQOS_ASSERT(NULL != output_colids);
	SPQOS_ASSERT(NULL != input_colids);
	SPQOS_ASSERT(NULL != children_dxlnodes);
	SPQOS_ASSERT(1 < input_colids->Size());
	SPQOS_ASSERT(1 < children_dxlnodes->Size());
	...
	SPQOS_ASSERT(num_of_cols == input_first_child_array->Size());
	SPQOS_ASSERT(num_of_cols == output_colids->Size());

	CBitSet *bitset = SPQOS_NEW(m_mp) CBitSet(m_mp);
	...
	ULongPtrArray *input_first_child_new_array =
		SPQOS_NEW(m_mp) ULongPtrArray(m_mp);
	...
	for (ULONG ul = 0; ul < num_of_cols; ul++)
	{
		...
		TargetEntry *target_entry =
			(TargetEntry *) spqdb::ListNth(output_target_list, target_list_pos);
		...
		if (!is_col_exists)
		{
			...
			input_first_child_new_array->Append(SPQOS_NEW(m_mp)
													ULONG(colid_input));
	...
	if (0 < new_child_project_list_dxlnode->Arity())
	{
		...
		CDXLNode *new_child_dxlnode = SPQOS_NEW(m_mp)
			CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalProject(m_mp));
		new_child_dxlnode->AddChild(new_child_project_list_dxlnode);
	...
	CDXLLogicalSetOp *dxlop =
		SPQOS_NEW(m_mp) CDXLLogicalSetOp(m_mp, setop_type, output_col_descrs,
										input_colids, is_cast_across_input);
	CDXLNode *dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxlop, children_dxlnodes);
...
{
	SPQOS_ASSERT(NULL != input_col_mdids);
	SPQOS_ASSERT(NULL != target_list);
	SPQOS_ASSERT(
		input_col_mdids->Size() <=
		spqdb::ListLength(target_list));	 // there may be resjunked columns
										   ...
										   List *target_list)
{
	...
	//
	// INFO:  SPQORCA failed to produce a plan, falling back to planner
	// DETAIL:  CKeyCollection.cpp:84: Failed assertion: __null != colref_array && 0 < colref_array->Size()
	// Stack trace:
	// 1    0x000055c239243b8a spqos::CException::Raise + 278
	// 2    0x000055c2393ab075 spqopt::CKeyCollection::CKeyCollection + 221
	// 3    0x000055c239449ab6 spqopt::CLogicalSetOp::DeriveKeyCollection + 98
	// 4    0x000055c2393a5a67 spqopt::CDrvdPropRelational::DeriveKeyCollection + 135
	// 5    0x000055c2393a4937 spqopt::CDrvdPropRelational::Derive + 197
	// 6    0x000055c239405d9f spqopt::CExpression::PdpDerive + 703
	// 7    0x000055c2394d1e14 spqopt::CMemo::PgroupInsert + 512
	// 8    0x000055c2393dd734 spqopt::CEngine::PgroupInsert + 632
	// 9    0x000055c2393dcd73 spqopt::CEngine::InitLogicalExpression + 225
	// 10   0x000055c2393dd106 spqopt::CEngine::Init + 884
	// 11   0x000055c23949da9f spqopt::COptimizer::PexprOptimize + 103
	// 12   0x000055c23949d3d8 spqopt::COptimizer::PdxlnOptimize + 1414
	// 13   0x000055c23960e55e COptTasks::OptimizeTask + 1530
	// 14   0x000055c2392572b6 spqos::CTask::Execute + 196
	// 15   0x000055c239259dbf spqos::CWorker::Execute + 191
	// 16   0x000055c2392556b5 spqos::CAutoTaskProxy::Execute + 221
	// 17   0x000055c23925c0c0 spqos_exec + 876
	//
	...
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("Empty target list"));
	}
	...
	if (IsA(child_node, RangeTblRef))
	{
		...
		if (RTE_SUBQUERY == rte->rtekind)
		{
			...
			SPQOS_ASSERT(NULL != query_derived_tbl);
			...
			SPQOS_ASSERT(NULL != query_dxlnode);
	...
	else if (IsA(child_node, SetOperationStmt))
	{
		...
		for (ULONG ul = 0; ul < length; ul++)
		{
			...
			ULONG *colid = SPQOS_NEW(m_mp) ULONG(dxl_col_descr->Id());
			colids->Append(colid);
	...
	CHAR *temp_str =
		(CHAR *) spqdb::NodeToString(const_cast<Node *>(child_node));
	CWStringDynamic *str =
...
//
//	@doc:
//		Translate the FromExpr on a SPQDB query into either a CDXLLogicalJoin
//		or a CDXLLogicalGet
//
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateFromExprToDXL(FromExpr *from_expr)
{
	...
	if (0 == spqdb::ListLength(from_expr->fromlist))
	{
		dxlnode = DXLDummyConstTableGet();
	}
	else
	{
		if (1 == spqdb::ListLength(from_expr->fromlist))
		{
			Node *node = (Node *) spqdb::ListNth(from_expr->fromlist, 0);
			SPQOS_ASSERT(NULL != node);
		...
		else
		{
			...
			dxlnode = SPQOS_NEW(m_mp) CDXLNode(
				m_mp, SPQOS_NEW(m_mp) CDXLLogicalJoin(m_mp, EdxljtInner));
	...
	if (1 >= spqdb::ListLength(from_expr->fromlist))
	{
		...
		{
			CDXLNode *select_dxlnode = SPQOS_NEW(m_mp)
				CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalSelect(m_mp));
...
CTranslatorQueryToDXL::TranslateFromClauseToDXL(Node *node)
{
	...
	if (IsA(node, RangeTblRef))
	{
		...
		const ULONG num_of_translators =
			SPQOS_ARRAY_SIZE(dxlop_translator_func_mapping_array);
	...
	CHAR *sz = (CHAR *) spqdb::NodeToString(const_cast<Node *>(node));
	CWStringDynamic *str =
...
{
	SPQOS_ASSERT(!(RTE_RELATION == rtekind || RTE_CTE == rtekind ||
				  RTE_FUNCTION == rtekind || RTE_SUBQUERY == rtekind ||
				  RTE_VALUES == rtekind));

	static const SRTENameElem rte_name_map[] = {
		{RTE_JOIN, SPQOS_WSZ_LIT("RangeTableEntry of type Join")},
		{RTE_VOID, SPQOS_WSZ_LIT("RangeTableEntry of type Void")},
		{RTE_TABLEFUNCTION,
		 SPQOS_WSZ_LIT("RangeTableEntry of type Table Function")}};

	const ULONG length = SPQOS_ARRAY_SIZE(rte_name_map);
	for (ULONG ul = 0; ul < length; ul++)
	{
	...
	SPQOS_ASSERT(!"Unrecognized RTE kind");
}
...
)
{
	if (false == rte->inh)
	{
		SPQOS_ASSERT(RTE_RELATION == rte->rtekind);
		...
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("ONLY in the FROM clause"));
	}
	...
	{
		dxl_op = SPQOS_NEW(m_mp) CDXLLogicalExternalGet(m_mp, dxl_table_descr);
	}
	else
	{
		dxl_op = SPQOS_NEW(m_mp) CDXLLogicalGet(m_mp, dxl_table_descr);
	}

	CDXLNode *dxl_node = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
...
CTranslatorQueryToDXL::NoteDistributionPolicyOpclasses(const RangeTblEntry *rte)
{
	...
	{
		Relation rel = spqdb::GetRelation(rte->relid);
		GpPolicy *policy = nullptr; //rel->rd_cdbpolicy; TODO SPQ
		...
		if (!u_sess->attr.attr_spq.spq_optimizer_enable_replicated_table &&
		...
		for (int i = 0; i < policy_nattrs; i++)
		{
			...
			opfamily = spqdb::GetOpclassFamily(opclasses[i]);
			hashfunc = spqdb::GetHashProcInOpfamily(opfamily, typeoid);

			if (spqdb::IsLegacyCdbHashFunction(hashfunc))
			{
			...
			{
				Oid default_opclass =
					spqdb::GetDefaultDistributionOpclassForType(typeoid);
												  ...
												  ULONG current_query_level)
{
	...
	SPQOS_ASSERT(NULL != tuples_list);

	const ULONG num_of_tuples = spqdb::ListLength(tuples_list);
	SPQOS_ASSERT(0 < num_of_tuples);
	...
	ForEach(lc_tuple, tuples_list)
	{
		...
		SPQOS_ASSERT(IsA(tuple_list, List));

		// array of column colids
		ULongPtrArray *colid_array = SPQOS_NEW(m_mp) ULongPtrArray(m_mp);

		// array of project elements (for expression elements)
		CDXLNodeArray *project_elem_dxlnode_array =
			SPQOS_NEW(m_mp) CDXLNodeArray(m_mp);
		...
		SPQOS_ASSERT(NULL != col_names);
		SPQOS_ASSERT(spqdb::ListLength(tuple_list) ==
					spqdb::ListLength(col_names));
		...
		ForEach(lc_column, tuple_list)
		{
			...
			CHAR *col_name_char_array =
				(CHAR *) strVal(spqdb::ListNth(col_names, col_pos_idx));
			...
			if (IsA(expr, Const))
			{
				...
				CMDName *mdname = SPQOS_NEW(m_mp) CMDName(m_mp, alias_str);
				SPQOS_DELETE(alias_str);

				CDXLColDescr *dxl_col_descr = SPQOS_NEW(m_mp) CDXLColDescr(
					m_mp, mdname, colid, col_pos_idx + 1 /* attno */,
					SPQOS_NEW(m_mp)
						CMDIdSPQDB(IMDId::EmdidGeneral, const_expr->consttype),
			...
			else
			{
				...
				if (0 == tuple_pos)
				{
					...
					CDXLColDescr *dxl_col_descr = SPQOS_NEW(m_mp) CDXLColDescr(
						m_mp, mdname, colid, col_pos_idx + 1 /* attno */,
						SPQOS_NEW(m_mp) CMDIdSPQDB(IMDId::EmdidGeneral,
												 spqdb::ExprType((Node *) expr)),
						spqdb::ExprTypeMod((Node *) expr), false /* is_dropped */
					);
					dxl_col_descr_array->Append(dxl_col_descr);
				}
			}
			...
			colid_array->Append(SPQOS_NEW(m_mp) ULONG(colid));
			col_pos_idx++;
		}
	...
	else if (1 < num_of_tuples)
	{
		...
		CDXLLogicalSetOp *dxlop = SPQOS_NEW(m_mp) CDXLLogicalSetOp(
			m_mp, EdxlsetopUnionAll, dxl_col_descr_array, input_colids, false);
		CDXLNode *dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxlop, dxlnodes);
...
{
	SPQOS_ASSERT(NULL != dxl_datum_array_const_tbl_get);
	SPQOS_ASSERT(NULL != project_elem_dxlnode_array);
	...
	else
	{
		...
		const_tbl_get_dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxlop);
	}
	...
	CDXLNode *project_dxlnode =
		SPQOS_NEW(m_mp) CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalProject(m_mp),
										 ...
										 ULONG current_query_level)
{
	...
	SPQOS_ASSERT(NULL != cte_producer_dxlnode);
	...
	CDXLLogicalCTEConsumer *cte_consumer_dxlop = SPQOS_NEW(m_mp)
		CDXLLogicalCTEConsumer(m_mp, cte_id, colid_array_cte_consumer);
	CDXLNode *cte_dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, cte_consumer_dxlop);
												   ...
												   ULONG current_query_level)
{
	...
	SPQOS_ASSERT(NULL != derived_tbl_dxlnode &&
...
CTranslatorQueryToDXL::TranslateExprToDXL(Expr *expr)
{
	...
	SPQOS_ASSERT(NULL != scalar_dxlnode);
...
//
//	@doc:
//		Translate the JoinExpr on a SPQDB query into a CDXLLogicalJoin node
//
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateJoinExprInFromToDXL(JoinExpr *join_expr)
{
	SPQOS_ASSERT(NULL != join_expr);
	...
	CDXLNode *join_dxlnode = SPQOS_NEW(m_mp)
		CDXLNode(m_mp, SPQOS_NEW(m_mp) CDXLLogicalJoin(m_mp, join_type));

	SPQOS_ASSERT(NULL != left_child_dxlnode && NULL != right_child_dxlnode);
	...
	RangeTblEntry *rte =
		(RangeTblEntry *) spqdb::ListNth(m_query->rtable, rtindex - 1);
	SPQOS_ASSERT(NULL != rte);

	Alias *alias = rte->eref;
	SPQOS_ASSERT(NULL != alias);
	SPQOS_ASSERT(NULL != alias->colnames &&
				0 < spqdb::ListLength(alias->colnames));
	SPQOS_ASSERT(spqdb::ListLength(rte->joinaliasvars) ==
				spqdb::ListLength(alias->colnames));
	...
	ForBoth(lc_node, rte->joinaliasvars, lc_col_name, alias->colnames)
	{
		...
		if (!join_alias_node)
			continue;
		SPQOS_ASSERT(IsA(join_alias_node, Var) ||
...
{
	BOOL is_groupby =
		(0 != spqdb::ListLength(m_query->groupClause) || m_query->hasAggs);
	...
	ForEach(lc, target_list)
	{
		...
		{
			vars_list = spqdb::ListConcat(
				vars_list,
				spqdb::ExtractNodesExpression((Node *) target_entry->expr, T_Var,
											 false /*descendIntoSubqueries*/));
		}
		else if (!IsA(target_entry->expr, Aggref))
		{
			omitted_te_list = spqdb::LAppend(omitted_te_list, target_entry);
		}
	}
	...
	{
		spqdb::SPQDBFree(omitted_te_list);
	}

	SPQOS_ASSERT_IMP(!is_expand_aggref_expr, NULL == vars_list);
	...
	if (0 < project_list_dxlnode->Arity())
	{
		...
		SPQOS_ASSERT(NULL != project_dxlnode);
		return project_dxlnode;
	}
...
) const
{
	...
	ForEach(lc, target_list)
	{
		...
		if (IsA(target_entry->expr, GroupingFunc))
		{
			...
			{
				CWStringConst str_unnamed_col(SPQOS_WSZ_LIT("grouping"));
				mdname_alias = SPQOS_NEW(m_mp) CMDName(m_mp, &str_unnamed_col);
			}
			else
			{
				CWStringDynamic *alias_str =
				...
				mdname_alias = SPQOS_NEW(m_mp) CMDName(m_mp, alias_str);
				SPQOS_DELETE(alias_str);
			}
			CDXLNode *project_elem_dxlnode = SPQOS_NEW(m_mp) CDXLNode(
				m_mp,
				SPQOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname_alias),
		...
		{
			OID oid_type = spqdb::ExprType((Node *) target_entry->expr);

			colid = m_context->m_colid_counter->next_id();

			CMDIdSPQDB *mdid =
				SPQOS_NEW(m_mp) CMDIdSPQDB(IMDId::EmdidGeneral, oid_type);
			CDXLNode *project_elem_dxlnode =
				CTranslatorUtils::CreateDXLProjElemConstNULL(
...
{
	SPQOS_ASSERT(NULL != attno_to_colid_mapping);

	INT *key = SPQOS_NEW(m_mp) INT(attno);
	ULONG *value = SPQOS_NEW(m_mp) ULONG(colid);
	...
	List *target_list, IntToUlongMap *attno_to_colid_mapping) const
{
	...
	ForEach(lc, target_list)
	{
		...
		if (NULL == target_entry->resname)
		{
			CWStringConst str_unnamed_col(SPQOS_WSZ_LIT("?column?"));
			mdname = SPQOS_NEW(m_mp) CMDName(m_mp, &str_unnamed_col);
		}
		else
		{
			CWStringDynamic *alias_str =
		...
		IMDId *mdid_type = SPQOS_NEW(m_mp) CMDIdSPQDB(
			IMDId::EmdidGeneral, spqdb::ExprType((Node *) target_entry->expr));
		...
		CDXLNode *dxlnode = SPQOS_NEW(m_mp) CDXLNode(m_mp, dxl_ident);
												 ...
												 BOOL insist_new_colids)
{
	...
	CDXLNode *project_elem_dxlnode = SPQOS_NEW(m_mp) CDXLNode(
		m_mp,
		SPQOS_NEW(m_mp) CDXLScalarProjElem(m_mp, project_elem_id, mdname_alias));
...
{
	SPQOS_ASSERT(IsA(expr, GroupingFunc));
	SPQOS_ASSERT(NULL != grpcol_index_to_colid_mapping);

	const GroupingFunc *grouping_func = (GroupingFunc *) expr;

	if (1 < spqdb::ListLength(grouping_func->args))
	{
		SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
				   SPQOS_WSZ_LIT("Grouping function with multiple arguments"));
	}

	Node *node = (Node *) spqdb::ListNth(grouping_func->args, 0);
	ULONG grouping_idx = spqdb::GetIntFromValue(node);
	...
	SPQOS_ASSERT(NULL != sort_group_ref);
	...
	CMDIdSPQDB *mdid_cast = CMDIdSPQDB::CastMdid(md_type->MDId());
	CMDIdSPQDB *mdid = SPQOS_NEW(m_mp) CMDIdSPQDB(*mdid_cast);

	CDXLDatum *datum_dxl =
		SPQOS_NEW(m_mp) CDXLDatumInt8(m_mp, mdid, false /* is_null */, l_value);
	CDXLScalarConstValue *dxlop =
		SPQOS_NEW(m_mp) CDXLScalarConstValue(m_mp, datum_dxl);
	return SPQOS_NEW(m_mp) CDXLNode(m_mp, dxlop);
}
												...
												ULONG cte_query_level)
{
	...
	ForEach(lc, cte_list)
	{
		...
		SPQOS_ASSERT(IsA(cte->ctequery, Query));

		if (cte->cterecursive)
		{
			SPQOS_RAISE(spqdxl::ExmaDXL, spqdxl::ExmiQuery2DXLUnsupportedFeature,
					   SPQOS_WSZ_LIT("WITH RECURSIVE"));
		}
		...
		SPQOS_ASSERT(NULL != cte_child_dxlnode &&
		...
		for (ULONG ul = 0; ul < output_columns; ul++)
		{
			...
			colid_array->Append(
				SPQOS_NEW(m_mp) ULONG(dxl_scalar_ident->GetDXLColRef()->Id()));
		}

		CDXLLogicalCTEProducer *lg_cte_prod_dxlop =
			SPQOS_NEW(m_mp) CDXLLogicalCTEProducer(
				m_mp, m_context->m_cte_id_counter->next_id(), colid_array);
		CDXLNode *cte_producer_dxlnode =
			SPQOS_NEW(m_mp) CDXLNode(m_mp, lg_cte_prod_dxlop, cte_child_dxlnode);
		...
		BOOL result =
			...
			m_cteid_at_current_query_level_map->Insert(
				SPQOS_NEW(m_mp) ULONG(lg_cte_prod_dxlop->Id()),
				SPQOS_NEW(m_mp) BOOL(true));
		SPQOS_ASSERT(result);
...
{
	SPQOS_ASSERT(NULL == *dxl_cte_anchor_top);
	SPQOS_ASSERT(NULL == *dxl_cte_anchor_bottom);
	...
	for (ULONG ul = num_of_ctes; ul > 0; ul--)
	{
		...
		CDXLNode *cte_anchor_new_dxlnode = SPQOS_NEW(m_mp) CDXLNode(
			m_mp, SPQOS_NEW(m_mp) CDXLLogicalCTEAnchor(m_mp, cte_producer_id));
...
CTranslatorQueryToDXL::GenerateColIds(CMemoryPool *mp, ULONG size) const
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		colid_array->Append(SPQOS_NEW(mp)
	...
	CMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping) const
{
	...
	while (att_iter.Advance())
	{
		...
		{
			colid_array->Append(SPQOS_NEW(m_mp) ULONG(colid));
			old_new_col_mapping->Insert(SPQOS_NEW(m_mp) ULONG(colid),
...
{
	SPQOS_ASSERT(NULL != attno_to_colid_mapping);
	SPQOS_ASSERT(NULL != from_list_colids && NULL != to_list_colids);
	SPQOS_ASSERT(from_list_colids->Size() == to_list_colids->Size());
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		BOOL result =
			...
			old_new_col_mapping->Insert(
				SPQOS_NEW(mp) ULONG(*((*from_list_colids)[ul])),
				SPQOS_NEW(mp) ULONG(*((*to_list_colids)[ul])));
		SPQOS_ASSERT(result);
	}

	IntToUlongMap *result_attno_to_colid_mapping =
		SPQOS_NEW(mp) IntToUlongMap(mp);
	IntUlongHashmapIter mi(attno_to_colid_mapping);
	while (mi.Advance())
	{
		INT *key = SPQOS_NEW(mp) INT(*(mi.Key()));
		const ULONG *value = mi.Value();
		SPQOS_ASSERT(NULL != value);

		ULONG *remapped_value =
			SPQOS_NEW(mp) ULONG(*(old_new_col_mapping->Find(value)));
