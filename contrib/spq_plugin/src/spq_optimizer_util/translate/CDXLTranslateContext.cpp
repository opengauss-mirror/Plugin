	...
	: m_mp(mp), m_is_child_agg_node(is_child_agg_node)
{
	...
	m_colid_to_target_entry_map = SPQOS_NEW(m_mp) ULongToTargetEntryMap(m_mp);
	m_colid_to_paramid_map = SPQOS_NEW(m_mp) ULongToColParamMap(m_mp);
}
...
CDXLTranslateContext::CopyParamHashmap(ULongToColParamMap *original)
{
	...
	while (hashmapiter.Advance())
	{
		...
		ULONG *key = SPQOS_NEW(m_mp) ULONG(colid);
		colidparamid->AddRef();
