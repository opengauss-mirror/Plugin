...
//
//	@doc:
//		Class for translating SPQDB's relcache entries into DXL MD objects
//
//	@test:
//
//
...
#ifndef SPQDXL_CTranslatorRelcacheToDXL_H
#define SPQDXL_CTranslatorRelcacheToDXL_H
...
private:
	...
	static CMDFunctionSPQDB::EFuncStbl GetFuncStability(CHAR c);

	// get function data access property from the SPQDB character representation
	static CMDFunctionSPQDB::EFuncDataAcc GetEFuncDataAccess(CHAR c);
	...
	// retrieve a SPQDB metadata object from the relcache
	static IMDCacheObject *RetrieveObjectSPQDB(CMemoryPool *mp, IMDId *mdid,
	...
	// transform SPQDB's MCV information to optimizer's histogram structure
	...
	// transform SPQDB's hist information to optimizer's histogram structure
	...
	// construct a mapping SPQDB attnos -> position in the column array
	...
	static CMDPartConstraintSPQDB *RetrievePartConstraintForIndex(
	...
	static CMDPartConstraintSPQDB *RetrievePartConstraintForRel(
		CMemoryPool *mp, CMDAccessor *md_accessor, OID rel_oid,
		CMDColumnArray *mdcol_array, BOOL construct_full_expr);

	// retrieve part constraint from a SPQDB node
	static CMDPartConstraintSPQDB *RetrievePartConstraintFromNode(
	...
	static CMDIndexInfoArray *RetrieveRelIndexInfoForMultiLevelPartTable(CMemoryPool *mp, Relation root_rel);
	...
	// translate Optimizer comparison types to SPQDB
...
public:
	...
	static CMDCheckConstraintSPQDB *RetrieveCheckConstraints(
	...
	static CMDScalarOpSPQDB *RetrieveScOp(CMemoryPool *mp, IMDId *mdid);

	// retrieve a function from the relcache
	static CMDFunctionSPQDB *RetrieveFunc(CMemoryPool *mp, IMDId *mdid);

	// retrieve an aggregate from the relcache
	static CMDAggregateSPQDB *RetrieveAgg(CMemoryPool *mp, IMDId *mdid);

	// retrieve a trigger from the relcache
	static CMDTriggerSPQDB *RetrieveTrigger(CMemoryPool *mp, IMDId *mdid);

	// translate SPQDB comparison type
	static IMDType::ECmpType ParseCmpType(ULONG cmpt);

	// get the distribution policy of the relation
	static IMDRelation::Ereldistrpolicy GetRelDistribution(GpPolicy *spq_policy);
};
}  // namespace spqdxl



#endif	// !SPQDXL_CTranslatorRelcacheToDXL_H
