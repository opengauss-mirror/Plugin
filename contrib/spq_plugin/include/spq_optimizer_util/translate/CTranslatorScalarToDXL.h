...
//
//	@doc:
//		Class providing methods for translating a SPQDB Scalar Operation (in a Query / PlStmt object)
//		into a DXL tree.
//
//	@test:
//
...
#ifndef SPQDXL_CTranslatorScalarToDXL_H
#define SPQDXL_CTranslatorScalarToDXL_H
...
{
	// shorthand for functions for translating SPQDB expressions into DXL nodes
	typedef CDXLNode *(CTranslatorScalarToDXL::*ExprToDXLFn)(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// shorthand for functions for translating DXL nodes to SPQDB expressions
...
private:
	...
	EdxlBoolExprType EdxlbooltypeFromSPQDBBoolType(BoolExprType) const;
	...
	// create a DXL scalar distinct comparison node from a SPQDB DistinctExpr
	CDXLNode *TranslateDistinctExprToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar boolean expression node from a SPQDB qual list
	CDXLNode *CreateScalarCondFromQual(
		List *quals, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar comparison node from a SPQDB op expression
	CDXLNode *CreateScalarCmpFromOpExpr(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar opexpr node from a SPQDB expression
	...
	// create a DXL scalar array comparison node from a SPQDB expression
	CDXLNode *CreateScalarArrayCompFromExpr(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar Const node from a SPQDB expression
	...
	// create a DXL scalar boolexpr node from a SPQDB expression
	CDXLNode *TranslateBoolExprToDXL(const Expr *expr,
									 const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar boolean test node from a SPQDB expression
	CDXLNode *TranslateBooleanTestToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar nulltest node from a SPQDB expression
	CDXLNode *TranslateNullTestToDXL(const Expr *expr,
									 const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar case statement node from a SPQDB expression
	CDXLNode *TranslateCaseExprToDXL(const Expr *expr,
									 const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar if statement node from a SPQDB case expression
	CDXLNode *CreateScalarIfStmtFromCaseExpr(
		const CaseExpr *case_expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar switch node from a SPQDB case expression
	...
	// create a DXL scalar coalesce node from a SPQDB expression
	CDXLNode *TranslateCoalesceExprToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar minmax node from a SPQDB expression
	CDXLNode *TranslateMinMaxExprToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar relabeltype node from a SPQDB expression
	CDXLNode *TranslateRelabelTypeToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar coerce node from a SPQDB expression
	CDXLNode *TranslateCoerceToDomainToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar coerceviaio node from a SPQDB expression
	CDXLNode *TranslateCoerceViaIOToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar array coerce expression node from a SPQDB expression
	CDXLNode *TranslateArrayCoerceExprToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar funcexpr node from a SPQDB expression
	CDXLNode *TranslateFuncExprToDXL(const Expr *expr,
									 const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar WindowFunc node from a SPQDB expression
	CDXLNode *TranslateWindowFuncToDXL(
		const Expr *expr, const CMappingVarColId *var_colid_mapping);

	// create a DXL scalar Aggref node from a SPQDB expression
	...
	// create a DXL SubPlan node for a from a SPQDB SubPlan
...
public:
	...
	// create a DXL scalar operator node from a SPQDB expression
	...
	// create a DXL WindowFrame node from a SPQDB expression
...
#endif	// SPQDXL_CTranslatorScalarToDXL_H
