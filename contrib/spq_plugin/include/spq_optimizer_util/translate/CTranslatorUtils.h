...
//
//	@doc:
//		Class providing utility methods for translating SPQDB's PlannedStmt/Query
//		into DXL Tree
//
//	@test:
//
...
//
//	@doc:
//		Class providing methods for translating SPQDB's PlannedStmt/Query
//		into DXL Tree
//
//---------------------------------------------------------------------------
...
private:
	...
	static CBitSetArray *CreateGroupingSetsForRollup(
		CMemoryPool *mp, GroupingSet *grouping_clause, ULONG num_cols,
...
public:
	...
	// get the SPQDB scan direction from its corresponding DXL representation
	...
	static CMDIdSPQDB *GetSystemColType(CMemoryPool *mp, AttrNumber attno);
	...
	// translate the join type from its SPQDB representation into the DXL one
	static EdxlJoinType ConvertToDXLJoinType(JoinType jt);

	// translate the index scan direction from its SPQDB representation into the DXL one
	...
	static CBitSetArray *GetColumnAttnosForGroupBy(
		CMemoryPool *mp, List *group_clause, List *grouping_set_list, ULONG num_cols,
	...
	static List *ConvertColidToAttnos(ULongPtrArray *pdrspqul,
	...
	// check required permissions for the range table with return value
	static bool CheckRTEPermissionsWithRet(List *range_table_list);
...
#endif	// !SPQDXL_CTranslatorUtils_H
