...
//
//	@doc:
//		Class providing methods for translating from DXL tree to SPQDB PlannedStmt
//
//	@test:
//
//
...
#ifndef SPQDXL_CTranslatorDxlToPlStmt_H
#define SPQDXL_CTranslatorDxlToPlStmt_H
...
public:
	...
	// translate the join types from its DXL representation to the SPQDB one
	static JoinType GetSPQDBJoinTypeFromDXLJoinType(EdxlJoinType join_type);
...
private:
	...
	// translate DXL window node into SPQDB window node
	...
	// translate a CTE producer into a SPQDB share input scan
	...
	// translate a CTE consumer into a SPQDB share input scan
	...
	// translate a DXL PartitionSelector into a SPQDB PartitionSelector
	...
	// translate a DXL Value Scan into SPQDB Value Scan
	...
	// hash a DXL datum with SPQDB's hash function
	ULONG GetDXLDatumSPQDBHash(CDXLDatumArray *dxl_datum_array);

	// translate nest loop colrefs to SPQDB nestparams
    ...
    /* SPQ: for ShareScan */
    Plan *TranslateDXLTblShareScan(
                const CDXLNode *tbl_scan_dxlnode,
                CDXLTranslateContext *output_context,
                CDXLTranslationContextArray *ctxt_translation_prev_siblings);
    /* SPQ: for shareindexscan */
    Plan *TranslateDXLShareIndexScan(
                const CDXLNode *index_scan_dxlnode,
                CDXLTranslateContext *output_context,
                CDXLTranslationContextArray *ctxt_translation_prev_siblings);
};
...
#endif	// !SPQDXL_CTranslatorDxlToPlStmt_H
