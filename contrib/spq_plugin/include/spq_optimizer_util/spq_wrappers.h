...
//
//	@doc:
//		Definition of SPQDB function wrappers
//
//	@test:
//
//
//---------------------------------------------------------------------------
#ifndef SPQDB_spq_wrappers_H
#define SPQDB_spq_wrappers_H
...
// expression collation - SPQDB_91_MERGE_FIXME
...
Query *FlattenJoinAliasVar(Query *query, spqos::ULONG query_level);
...
void *SPQDBAlloc(Size size);
void SPQDBFree(void *ptr);
...
#define GpdbEreport(xerrcode, severitylevel, xerrmsg, xerrhint)       \
	spqdb::GpdbEreportImpl(xerrcode, severitylevel, xerrmsg, xerrhint, \
...
spqos::BOOL IsChildPartDistributionMismatched(Relation rel);

// return true if the table is partitioned and any of the child partitions
// have a trigger of the given type
spqos::BOOL ChildPartHasTriggers(Oid oid, int trigger_type);
...
double SpqEstimatePartitionedNumTuples(Relation rel);
...
// check if two spqdb objects are equal
...
unsigned int SpqHashRandomSeg(int num_segments);

// check permissions on range table
void CheckRTPermissions(List *rtable);

// check permissions on range table with return value
bool CheckRTPermissionsWithRet(List *rtable);
...
// simple fault injector used by COptTasks.cpp to inject SPQDB fault
// spq FaultInjector support
//FaultInjectorType_e InjectFaultInOptTasks(const char *fault_name);

// return the number of leaf partition for a given table oid
spqos::ULONG CountLeafPartTables(Oid oidRelation);
...
void *SPQDBMemoryContextAlloc(MemoryContext context, Size size);

MemoryContext SPQDBAllocSetContextCreate();

void SPQDBMemoryContextDelete(MemoryContext context);

}  //namespace spqdb

#define ForEach(cell, l) \
	for ((cell) = spqdb::ListHead(l); (cell) != NULL; (cell) = lnext(cell))

#define ForBoth(cell1, list1, cell2, list2)                                \
	for ((cell1) = spqdb::ListHead(list1), (cell2) = spqdb::ListHead(list2); \
		 (cell1) != NULL && (cell2) != NULL;                               \
		 (cell1) = lnext(cell1), (cell2) = lnext(cell2))

#define ForThree(cell1, list1, cell2, list2, cell3, list3)                 \
	for ((cell1) = spqdb::ListHead(list1), (cell2) = spqdb::ListHead(list2), \
		(cell3) = spqdb::ListHead(list3);                                   \
		 (cell1) != NULL && (cell2) != NULL && (cell3) != NULL;            \
		 (cell1) = lnext(cell1), (cell2) = lnext(cell2),                   \
		(cell3) = lnext(cell3))

#define ForEachWithCount(cell, list, counter)                          \
	for ((cell) = spqdb::ListHead(list), (counter) = 0; (cell) != NULL; \
		 (cell) = lnext(cell), ++(counter))

#define ListMake1(x1) spqdb::LPrepend(x1, NIL)

#define ListMake2(x1, x2) spqdb::LPrepend(x1, ListMake1(x2))

#define ListMake1Int(x1) spqdb::LPrependInt(x1, NIL)

#define ListMake1Oid(x1) spqdb::LPrependOid(x1, NIL)
#define ListMake2Oid(x1, x2) spqdb::LPrependOid(x1, ListMake1Oid(x2))

#define LInitial(l) lfirst(spqdb::ListHead(l))

#define LInitialOID(l) lfirst_oid(spqdb::ListHead(l))

#define Palloc0Fast(sz)                                              \
	(MemSetTest(0, (sz))                                             \
		 ? spqdb::MemCtxtAllocZeroAligned(CurrentMemoryContext, (sz)) \
		 : spqdb::MemCtxtAllocZero(CurrentMemoryContext, (sz)))
...
#define PStrDup(str) spqdb::MemCtxtStrdup(CurrentMemoryContext, (str))

#endif	// !SPQDB_spq_wrappers_H
