...
BEGIN t.x := $1; t.y := $2; t.v := $3;
  ...
  PERFORM gms_pipe.pack_message(t);
END;
$$ LANGUAGE plpgsql;
...
BEGIN t.x := 1; t.y := 2; t.v := 'Pavel Stehule';
  RAISE NOTICE 'SEND';
  PERFORM gms_pipe.pack_message(t);
  PERFORM gms_pipe.send_message('boo',4,10);
  RAISE NOTICE 'RECEIVE';
--  PERFORM gms_pipe.receive_message('boo',4);
--  SELECT INTO o * from gms_pipe.unpack_message_record() as (x integer, y integer, v varchar);
...
BEGIN
  FOR i IN 1..100000 LOOP
    PERFORM gms_pipe.pack_message('Prvni '||i);
    PERFORM gms_pipe.pack_message('Druhy '||i);
    RAISE NOTICE 'SEND';
    IF gms_pipe.send_message('pipe_name',4,10) = 1 THEN
      RAISE NOTICE 'Timeout';
      PERFORM pg_sleep(5);
      PERFORM gms_pipe.send_message('pipe_name',4,10);
    END IF;
...
BEGIN
  FOR i IN 1..100000 LOOP
    IF gms_pipe.receive_message('pipe_name',4) = 1 THEN
      RAISE NOTICE 'Timeout';
      PERFORM pg_sleep(5);
      CONTINUE;
    END IF;
