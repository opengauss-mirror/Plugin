...
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_rvrs);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_normalize);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_is_prefix_text);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_is_prefix_int);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_is_prefix_int64);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_lpart);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_rpart);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_lstrip);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_rstrip);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_left);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_right);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_substr2);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_substr3);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_instr2);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_instr3);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_instr4);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_betwn_i);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_betwn_c);
PG_FUNCTION_INFO_V1_PUBLIC(plvstr_swap);

PG_FUNCTION_INFO_V1_PUBLIC(plvchr_nth);
PG_FUNCTION_INFO_V1_PUBLIC(plvchr_first);
PG_FUNCTION_INFO_V1_PUBLIC(plvchr_last);
PG_FUNCTION_INFO_V1_PUBLIC(plvchr_is_kind_i);
PG_FUNCTION_INFO_V1_PUBLIC(plvchr_is_kind_a);
PG_FUNCTION_INFO_V1_PUBLIC(plvchr_char_name);

PG_FUNCTION_INFO_V1_PUBLIC(orafce_substr2);
PG_FUNCTION_INFO_V1_PUBLIC(orafce_substr3);

static text *ora_substr(Datum str, int start, int len);

#define ora_substr_text(str, start, len) ora_substr(PointerGetDatum((str)), (start), (len))

static const char *char_names[] = {"NULL", "SOH", "STX", "ETX", "EOT", "ENQ", "ACK", "DEL", "BS",  "HT",  "NL",
...
#define PARAMETER_ERROR(detail) \
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid parameter"), errdetail(detail)));

#ifndef _pg_mblen
#define _pg_mblen pg_mblen
#endif

typedef enum { POSITION, FIRST, LAST } position_mode;
...
static text *ora_substr(Datum str, int start, int len)
{
    ...
    if (len < 0)
        return DatumGetTextP(DirectFunctionCall2(text_substr_no_len, str, Int32GetDatum(start)));
    else
        return DatumGetTextP(DirectFunctionCall3(text_substr, str, Int32GetDatum(start), Int32GetDatum(len)));
}
...
Datum plvstr_normalize(PG_FUNCTION_ARGS)
{
    ...
    sret = memcpy_s(VARDATA(result), l, aux, l);
    securec_check(sret, "", "");
...
static bool is_kind(char c, int kind)
{
    switch (kind) {
        ...
        case 3:
            return c == '\'';
        case 4:
            return (32 <= c && c <= 47) || (58 <= c && c <= 64) || (91 <= c && c <= 96) || (123 <= c && c <= 126);
        case 5:
 ...
 *
 * Purpouse:
 *   Returns len chars from start_in position, compatible with Orafce
 *
 ****************************************************************/

Datum orafce_substr3(PG_FUNCTION_ARGS)
{
...
Datum orafce_substr2(PG_FUNCTION_ARGS)
{
    PG_RETURN_TEXT_P(ora_substr(PG_GETARG_DATUM(0), PG_GETARG_INT32(1), -1));
}
...
static text *ora_concat2(text *str1, text *str2)
{
    ...
    sret = memcpy_s(VARDATA(result), l1, VARDATA_ANY(str1), l1);
    securec_check(sret, "", "");
    sret = memcpy_s(VARDATA(result) + l1, l2, VARDATA_ANY(str2), l2);
    securec_check(sret, "", "");
...
static text *ora_concat3(text *str1, text *str2, text *str3)
{
    ...
    sret = memcpy_s(VARDATA(result) + l1, l2, VARDATA_ANY(str2), l2);
    securec_check(sret, "", "");
    sret = memcpy_s(VARDATA(result) + l1 + l2, l3, VARDATA_ANY(str3), l3);
    securec_check(sret, "", "");
...
Datum plvstr_swap(PG_FUNCTION_ARGS)
{
    ...
    if (start_in == 0 || start_in > v_len)
        ...
        PG_RETURN_TEXT_P(ora_concat2(replace_in, ora_substr_text(string_in, oldlen_in + 1, -1)));
    else
        PG_RETURN_TEXT_P(ora_concat3(ora_substr_text(string_in, 1, start_in - 1), replace_in,
...
Datum plvstr_betwn_i(PG_FUNCTION_ARGS)
{
    ...
    if ((start_in < 0 && end_in > 0) || (start_in > 0 && end_in < 0) || (start_in > end_in))
    ...
    PG_RETURN_TEXT_P(ora_substr_text(string_in, start_in, end_in - start_in + 1));
}
...
Datum plvstr_betwn_c(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0) || PG_ARGISNULL(1) || PG_ARGISNULL(3) || PG_ARGISNULL(4) || PG_ARGISNULL(5) || PG_ARGISNULL(6))
        PG_RETURN_NULL();
    ...
    if (((v_start > v_end) && (v_end > 0)) || (v_end <= 0 && !gotoend))
        PG_RETURN_NULL();

    if (v_end <= 0)
        v_end = ora_mb_strlen1(string_in);

    PG_RETURN_TEXT_P(ora_substr_text(string_in, v_start, v_end - v_start + 1));
}
