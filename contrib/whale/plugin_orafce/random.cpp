 ...
 * number generator used in orafce. So the results of these
 * functions should be different then native Orafce functions!
...
static const double b[] = {-5.447609879822406e+01, 1.615858368580409e+02, -1.556989798598866e+02, 6.680131188771972e+01,
 ...
 */
static text *random_string(const char *charset, size_t chrset_size, int len)
{
    StringInfo str;
    int i;
    unsigned int seed = GetSessionContext()->seed;
...
Datum dbms_random_string(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0) || PG_ARGISNULL(1))
        ereport(ERROR, (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED), errmsg("an argument is NULL")));
    ...
    switch (option[0]) {
        ...
        default:
            ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("unknown option '%s'", option),
