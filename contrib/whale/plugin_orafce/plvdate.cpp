...
ora_add_bizdays(DateADT day, int days)
{
	...
	while (days != 0)
	{
		...
		if (NULL != bsearch(&day, GetSessionContext()->exceptions, GetSessionContext()->exceptions_c,
		...
		if (NULL != bsearch(&hd, GetSessionContext()->holidays, GetSessionContext()->holidays_c,
...
ora_diff_bizdays(DateADT day1, DateADT day2)
{
	...
	while (day1 <= day2)
	{
		...
		if (NULL != bsearch(&day1, GetSessionContext()->exceptions, GetSessionContext()->exceptions_c,
							sizeof(DateADT), dateadt_comp))
			continue;
...
plvdate_set_nonbizday_day (PG_FUNCTION_ARGS)
{
	...
	if (arg2)
	{
		...
		if (NULL != bsearch(&hd, GetSessionContext()->holidays, GetSessionContext()->holidays_c, sizeof(holiday_desc), holiday_desc_comp))
			ereport(ERROR,
	...
	else
	{
		...
		if (NULL != bsearch(&arg1, GetSessionContext()->exceptions, GetSessionContext()->exceptions_c, sizeof(DateADT), dateadt_comp))
			ereport(ERROR,
				    (errcode(ERRCODE_DUPLICATE_OBJECT),
				     errmsg("nonbizday registration error"),
				     errdetail("Date is registered.")));
...
plvdate_default_holidays (PG_FUNCTION_ARGS)
{
	...
	sret = memcpy_s(GetSessionContext()->holidays, GetSessionContext()->holidays_c*sizeof(holiday_desc),
			defaults_ci[GetSessionContext()->country_id].holidays, GetSessionContext()->holidays_c*sizeof(holiday_desc));
	securec_check(sret, "", "");
