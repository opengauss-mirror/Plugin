...
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fopen);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_is_open);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_get_line);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_get_nextline);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_put);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_put_line);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_new_line);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_putf);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fflush);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fclose);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fclose_all);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fremove);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_frename);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fcopy);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_fgetattr);
PG_FUNCTION_INFO_V1_PUBLIC(utl_file_tmpdir);

#define CUSTOM_EXCEPTION(msg, detail) \
    ereport(ERROR, (errcode(ERRCODE_RAISE_EXCEPTION), errmsg("%s", msg), errdetail("%s", detail)))

#define STRERROR_EXCEPTION(msg)         \
    do {                                \
        char *strerr = strerror(errno); \
        CUSTOM_EXCEPTION(msg, strerr);  \
    } while (0);
...
static int copy_text_file(FILE *srcfile, FILE *dstfile, int start_line, int end_line);
...
static int get_descriptor(FILE *file, int max_linesize, int encoding)
{
    ...
    for (i = 0; i < MAX_SLOTS; i++) {
        if (GetSessionContext()->slots[i].id == INVALID_SLOTID) {
            GetSessionContext()->slots[i].id = ++GetSessionContext()->slotid;
            if (GetSessionContext()->slots[i].id == INVALID_SLOTID)
                GetSessionContext()->slots[i].id = ++GetSessionContext()->slotid; /* skip INVALID_SLOTID */
            GetSessionContext()->slots[i].file = file;
            GetSessionContext()->slots[i].max_linesize = max_linesize;
            GetSessionContext()->slots[i].encoding = encoding;
            return GetSessionContext()->slots[i].id;
        }
    }
...
static FILE *get_stream(int d, size_t *max_linesize, int *encoding)
{
    int i;
    ...
    for (i = 0; i < MAX_SLOTS; i++) {
        if (GetSessionContext()->slots[i].id == d) {
            if (max_linesize)
                *max_linesize = GetSessionContext()->slots[i].max_linesize;
            if (encoding)
                *encoding = GetSessionContext()->slots[i].encoding;
            return GetSessionContext()->slots[i].file;
        }
    }
...
Datum utl_file_fopen(PG_FUNCTION_ARGS)
{
    ...
    if (PG_NARGS() > 4 && !PG_ARGISNULL(4)) {
        ...
        if (encoding < 0)
            ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid encoding name \"%s\"", encname)));
    } else
    ...
    if (d == INVALID_SLOTID) {
        fclose(file);
        ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED), errmsg("program limit exceeded"),
...
Datum utl_file_is_open(PG_FUNCTION_ARGS)
{
    if (!PG_ARGISNULL(0)) {
        ...
        for (i = 0; i < MAX_SLOTS; i++) {
            if (GetSessionContext()->slots[i].id == d)
                PG_RETURN_BOOL(GetSessionContext()->slots[i].file != NULL);
        }
    }
...
static text *get_line(FILE *f, size_t max_linesize, int encoding, bool *iseof)
{
    int c;
    char *buffer;
    char *bpt;
    ...
    if (!eof) {
        ...
        decoded = (char *)pg_do_encoding_conversion((unsigned char *)buffer, size2int(csize), encoding,
                                                    GetDatabaseEncoding());
...
Datum utl_file_get_line(PG_FUNCTION_ARGS)
{
    ...
    if (iseof)
        ereport(ERROR, (errcode(ERRCODE_NO_DATA_FOUND), errmsg("no data found")));
...
/* encode(t, encoding) */
static char *encode_text(int encoding, text *t, size_t *length)
{
    char *src = VARDATA_ANY(t);
    char *encoded;

    encoded =
        (char *)pg_do_encoding_conversion((unsigned char *)src, VARSIZE_ANY_EXHDR(t), GetDatabaseEncoding(), encoding);
...
static size_t do_write(PG_FUNCTION_ARGS, int n, FILE *f, size_t max_linesize, int encoding)
{
...
static void do_new_line(FILE *f, int lines)
{
    int i;
    for (i = 0; i < lines; i++) {
#ifndef WIN32
 ...
 *          RETURNS NULL
 *
 * Close an open file. This function reset file handle to NULL on Orafce platform.
...
Datum utl_file_fclose(PG_FUNCTION_ARGS)
{
    ...
    for (i = 0; i < MAX_SLOTS; i++) {
        if (GetSessionContext()->slots[i].id == d) {
            if (GetSessionContext()->slots[i].file && fclose(GetSessionContext()->slots[i].file) != 0) {
                if (errno == EBADF)
            ...
            GetSessionContext()->slots[i].id = INVALID_SLOTID;
            PG_RETURN_NULL();
        }
    }
...
static void check_secure_locality(const char *path)
{
    ...
    if (SPI_processed == 0)
        ereport(ERROR, (errcode(ERRCODE_RAISE_EXCEPTION), errmsg(INVALID_PATH), errdetail("you cannot access locality"),
...
static char *safe_named_location(text *location)
{
    ...
    if (SPI_processed > 0) {
        char *loc = SPI_getvalue(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1);
        if (loc)
 ...
 */
static char *get_safe_path(text *location_or_dirname, text *filename)
{
    char *fullname;
    char *location;
    bool check_locality;
    ...
    if (filename == NULL || 0 == VARSIZE_ANY_EXHDR(filename)) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid parameter"),
...
Datum utl_file_fcopy(PG_FUNCTION_ARGS)
{
    ...
    if (end_line <= 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("end_line must be positive (%d passed)", end_line)));
