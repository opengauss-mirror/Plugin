...
static Node* a_plpgsql_pre_column_ref(ParseState* pstate, ColumnRef* cref);
...
#ifdef WHALE
static void ReplaceWhaleProBegin(char *proc_source);
#define  BEGIN_P_STR      " BEGIN_B_PROC " /* used in whale type proc body*/
    ...
    PLpgSQL_func_hashkey* hashkey, bool for_validator)
{
    ...
    ReplaceWhaleProBegin(proc_source);
#endif
 ...
 */
void a_plpgsql_parser_setup(struct ParseState* pstate, PLpgSQL_expr* expr)
{
    pstate->p_pre_columnref_hook = a_plpgsql_pre_column_ref;
    ...
    PG_TRY();
    {
        ...
        foreach(cell, parsetreeList) {
            ...
            queryList = pg_analyze_and_rewrite_params(parsetree, expr->query, 
                (ParserSetupHook)a_plpgsql_parser_setup, (void*)expr);
        }
        if (queryList == NULL) {
            ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
