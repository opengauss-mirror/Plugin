...
#include "pl_a_gram.hpp" /* must be after parser/scanner.h */
...
int plasql_yylex(void)
{
    ...
    plasql_yylval = aux1.lval;
    plasql_yylloc = aux1.lloc;
...
/*
 * Push back a single token to be re-read by next plasql_yylex() call.
 *
 ...
 */
void plasql_push_back_token(int token)
{
    TokenAuxData auxdata;

    auxdata.lval = plasql_yylval;
    auxdata.lloc = plasql_yylloc;
 ...
 * last returned by plasql_yylex()).
 * This is OK for syntax error messages from the Bison parser, because Bison
 * parsers report error as soon as the first unparsable token is reached.
 * Beware of using yyerror for other purposes, as the cursor position might
 * be misleading!
 */
void plasql_yyerror(const char* message)
{
    char* yytext = u_sess->plsql_cxt.core_yy->scanbuf + plasql_yylloc;
...
bool plpgsql_is_token_match2(int token, int token_next)
{
    ...
    if (token == tok) {
        ...
        tok_next = plasql_yylex();
        if (tok_next >= INT_MAX) {
            return false;
        }
        plasql_push_back_token(tok_next);
