...
#      3. plsql parser: (src/pl/plasql/src/) pl_gram.c pl_gram.h
#
...
while (my $indata = <$F>)
{
	...
	$indata =~ s/(char \*plasql_base_yytext;)/THR_LOCAL $1/;
	...
	$indata =~ s/(YYSTYPE plasql_yylval;)/THR_LOCAL $1/;
	$indata =~ s/(YYLTYPE plasql_yylloc;)/THR_LOCAL $1/;
