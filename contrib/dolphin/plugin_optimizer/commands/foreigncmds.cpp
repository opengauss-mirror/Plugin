 ...
 */
Datum transformGenericOptions(Oid catalogId, Datum oldOptions, List* options, Oid fdwvalidator, bool dup_toleration)
{
    ...
    foreach (optcell, options) {
        ...
        switch (od->defaction) {
            ...
            case DEFELEM_UNSPEC:
                if (cell != NULL) {
#ifdef DOLPHIN
                    /* gramma compat: if option exists and action is unspecified, action changes to SET */
                    if (dup_toleration) {
                        lfirst(cell) = od;
                        break;
                    }
...
#ifdef DOLPHIN
#define FOREIGN_SERVER_OPTION_USER "user"
#define FOREIGN_SERVER_OPTION_USERNAME "username"
#define FOREIGN_SERVER_OPTION_PASSWORD "password"
#define FOREIGN_SERVER_OPTION_DATABASE "database"
#define FOREIGN_SERVER_OPTION_SOCKET "socket"
#define FOREIGN_SERVER_OPTION_OWNER "owner"

/*
 * this function is used by function CreateForeignServer/AlterForeignServer, separating OPTIONS like USERNAME, PASSWORD
 * from stmt_options to auth_options, and discarding ignorable OPTIONS like DATABASE, OWNER and SOCKET.
 */
static List* ServerOptionFilter(List** stmt_options)
{
    List* auth_options = NIL;
    List* stmt_option_list = list_copy(*stmt_options);
    foreach_cell(cell, stmt_option_list) {
        DefElem* def = (DefElem*)lfirst(cell);
        if (strcmp(def->defname, FOREIGN_SERVER_OPTION_PASSWORD) == 0) {
            auth_options = lappend(auth_options, (DefElem*)lfirst(cell));
            *stmt_options = list_delete(*stmt_options, (DefElem*)lfirst(cell));
        } else if (strcmp(def->defname, FOREIGN_SERVER_OPTION_USER) == 0) {
            def->defname = FOREIGN_SERVER_OPTION_USERNAME;
            auth_options = lappend(auth_options, (DefElem*)lfirst(cell));
            *stmt_options = list_delete(*stmt_options, (DefElem*)lfirst(cell));
        } else if (strcmp(def->defname, FOREIGN_SERVER_OPTION_DATABASE) == 0 ||
                   strcmp(def->defname, FOREIGN_SERVER_OPTION_OWNER) == 0 ||
                   strcmp(def->defname, FOREIGN_SERVER_OPTION_SOCKET) == 0) {
            *stmt_options = list_delete(*stmt_options, (DefElem*)lfirst(cell));
            ereport(WARNING, (errmsg("Option %s will be deprecated.", def->defname)));
        }
    }
    list_free_deep(stmt_option_list);
    return auth_options;
}
#endif

#define FOREIGN_DATA_WRAPPER_MYSQL "mysql"
#define FOREIGN_DATA_WRAPPER_MYSQL_FDW "mysql_fdw"
#define USER_MAPPING_CURRENT_USER "current_user"
...
ObjectAddress CreateForeignServer(CreateForeignServerStmt* stmt)
{
     ...
     */
#ifdef DOLPHIN
    if (strcmp(stmt->fdwname, FOREIGN_DATA_WRAPPER_MYSQL) == 0) {
        stmt->fdwname = FOREIGN_DATA_WRAPPER_MYSQL_FDW;
    }
    bool is_mysql_fdw = (strcmp(stmt->fdwname, FOREIGN_DATA_WRAPPER_MYSQL_FDW) == 0);
#endif
...
#ifdef DOLPHIN
    /* separate options from stmt->options to auth_options */
    List* auth_options = is_mysql_fdw ? ServerOptionFilter(&stmt->options) : NIL;
#endif
...
#ifdef DOLPHIN
    if (is_mysql_fdw && auth_options != NIL) {
        /* advance command counter to make the new server visible, which can be used by the next create user mapping */
        CommandCounterIncrement();
        CreateUserMappingStmt* create_user_mapping_stmt = makeNode(CreateUserMappingStmt);
        create_user_mapping_stmt->username = USER_MAPPING_CURRENT_USER;
        create_user_mapping_stmt->servername = stmt->servername;
        create_user_mapping_stmt->options = auth_options;
        CreateUserMapping(create_user_mapping_stmt);
        ereport(WARNING, (errmsg("USER MAPPING for current user to server %s created.", stmt->servername)));
        list_free(auth_options);
        pfree_ext(create_user_mapping_stmt);
    }
#endif
    return myself;
}
...
ObjectAddress AlterForeignServer(AlterForeignServerStmt* stmt)
{
...
#ifdef DOLPHIN
    bool is_mysql_fdw = (strcmp(fdw->fdwname, FOREIGN_DATA_WRAPPER_MYSQL_FDW) == 0);
#endif
    ...
    if (stmt->options) {
        ...
        datum = transformGenericOptions(ForeignServerRelationId, datum, stmt->options, fdw->fdwvalidator, is_mysql_fdw);
        if (PointerIsValid(DatumGetPointer(datum))) {
...
#ifdef DOLPHIN
    /* CREATE or ALTER USER MAPPING if auth_options if not null */
    if (is_mysql_fdw && auth_options != NULL) {
        Oid useId;
        Oid umId;
        Relation user_mapping_rel;
        ForeignServer* srv = NULL;
        user_mapping_rel = heap_open(UserMappingRelationId, RowExclusiveLock);

        useId = GetUserOidFromMapping(USER_MAPPING_CURRENT_USER, false);
        srv = GetForeignServerByName(stmt->servername, false);
        user_mapping_ddl_aclcheck(useId, srv->serverid, stmt->servername);

        /* Check if user mapping exists and then choose CREATE or ALTER USER MAPPING action later */
        umId = GetSysCacheOid2(USERMAPPINGUSERSERVER, ObjectIdGetDatum(useId), ObjectIdGetDatum(srv->serverid));
        if (OidIsValid(umId)) {
            /* user mapping for current_user to given server exists, use ALTER USER MAPPING */
            AlterUserMappingStmt* alter_user_mapping_stmt = makeNode(AlterUserMappingStmt);
            alter_user_mapping_stmt->username = USER_MAPPING_CURRENT_USER;
            alter_user_mapping_stmt->servername = stmt->servername;
            alter_user_mapping_stmt->options = auth_options;
            AlterUserMapping(alter_user_mapping_stmt);
            ereport(WARNING, (errmsg("USER MAPPING for current user to server %s altered.", stmt->servername)));
            pfree_ext(alter_user_mapping_stmt);
        } else {
            /* user mapping doesn't exist, use CREATE USER MAPPING instead */
            CreateUserMappingStmt* create_user_mapping_stmt = makeNode(CreateUserMappingStmt);
            create_user_mapping_stmt->username = USER_MAPPING_CURRENT_USER;
            create_user_mapping_stmt->servername = stmt->servername;
            create_user_mapping_stmt->options = auth_options;
        ...
        list_free(auth_options);
        heap_close(user_mapping_rel, RowExclusiveLock);
    }
#endif
...
ObjectAddress AlterUserMapping(AlterUserMappingStmt* stmt)
{
    ...
    if (stmt->options) {
        ...
        /* Prepare the options array */
        datum = transformGenericOptions(UserMappingRelationId, datum, stmt->options, fdw->fdwvalidator, is_mysql_fdw);
