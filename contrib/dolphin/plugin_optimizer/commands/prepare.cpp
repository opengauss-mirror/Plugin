...
#ifdef DOLPHIN
static PreparedStatement* InsertIntoQueryHashTable(const char* stmt_name, CachedPlanSource* plansource, bool from_sql, bool* found)
#else
...
#ifdef DOLPHIN
    /*
     * we only let sql: 'prepare xx from stmt' to be replaced by new stmt in dolphin 
     * here is_share = true then from_sql must be false ,so we do not check again.
     * means when found a same prepare, and it is not from sql that we need erreport below. 
     */
    if (from_sql && found && entry != NULL) {
        entry->plansource = plansource;
        entry->from_sql = from_sql;
        entry->prepare_time = GetCurrentStatementStartTimestamp();
        entry->has_prepare_dn_stmt = false;
...
void StorePreparedStatement(const char* stmt_name, CachedPlanSource* plansource, bool from_sql)
{
    ...
    /* Add entry to hash table */
#ifdef DOLPHIN
    entry =InsertIntoQueryHashTable(stmt_name, plansource, from_sql, &found);
#else
    InsertIntoQueryHashTable(stmt_name, plansource, from_sql, &found);
#endif
