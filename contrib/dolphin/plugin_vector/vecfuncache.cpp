 ...
 * ---------------------------------------------------------------------------------------
 */
#include "./vecprimitive/int8.inl"
#include "./vecprimitive/int4.inl"
#include "./vecprimitive/float.inl"
#include "./vecprimitive/numeric.inl"
#include "./vecprimitive/varchar.inl"
#include "./vecprimitive/date.inl"
#include "./vecprimitive/timestamp.inl"
...
static VecFuncCacheEntry vec_func_table_plugin[] = {{1724,
                                                 {
...
void InitGlobalVecFuncMap()
{
    ...
    g_instance.plugin_vec_func_cxt.vec_func_plugin[DOLPHIN_VEC] =
        hash_create("VecFuncHash", 4096, &hash_ctl, HASH_ELEM | HASH_FUNCTION | HASH_CONTEXT);
    int nvecfunction = (sizeof(vec_func_table_plugin) / sizeof(VecFuncCacheEntry));
    for (i = 0; i < nvecfunction; i++) {
        entry =
            (VecFuncCacheEntry*)hash_search(g_instance.plugin_vec_func_cxt.vec_func_plugin[DOLPHIN_VEC],
                                            &vec_func_table_plugin[i].fn_oid, HASH_ENTER, &found);

        if (entry != NULL) {
            entry->fn_oid = vec_func_table_plugin[i].fn_oid;
            for (j = 0; j < FUNCACHE_NUM; j++) {
                entry->vec_fn_cache[j] = vec_func_table_plugin[i].vec_fn_cache[j];
                entry->vec_agg_cache[j] = vec_func_table_plugin[i].vec_agg_cache[j];
                entry->vec_sonic_agg_cache[j] = vec_func_table_plugin[i].vec_sonic_agg_cache[j];
                entry->vec_transform_function[j] = vec_func_table_plugin[i].vec_transform_function[j];
                entry->vec_sonic_transform_function[j] = vec_func_table_plugin[i].vec_sonic_transform_function[j];
            }
        }
    }
