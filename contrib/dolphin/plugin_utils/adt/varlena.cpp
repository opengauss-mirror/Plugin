...
Oid binary_need_transform_typeid(Oid typeoid, Oid* collation)
{
    ...
    if (*collation == BINARY_COLLATION_OID) {
        ...
        } else if (typeoid == BLOBOID || typeoid == BYTEAOID || typeoid == BINARYOID || typeoid == VARBINARYOID ||
...
Datum soundex_difference(PG_FUNCTION_ARGS)
{
    ...
    char* result1 = (char*)palloc0(Max(str_len1, min_sound_len));
    char* result2 = (char*)palloc0(Max(str_len2, min_sound_len));
    ...
    bool compare_as_times, bool compare_as_ts, PG_FUNCTION_ARGS)
{
    ...
    if (compare_as_times) {
        ...
        res_arg1_3 = varstr_cmp(cmp_args[ARG_0], strlen(cmp_args[ARG_0]),
            cmp_args[ARG_2], strlen(cmp_args[ARG_2]), PG_GET_COLLATION());
        if (res_arg1_2 != 0) {
...
char* AnyElementGetCString(Oid anyOid, Datum anyDatum, bool* hasError, bool* typIsVarlena)
{
    ...
    } else if (isVarlena && anyOid != NUMERICOID && !type_is_set(anyOid)) {
