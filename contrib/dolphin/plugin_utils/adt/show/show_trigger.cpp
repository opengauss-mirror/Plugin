    ...
    if (SRF_IS_FIRSTCALL()) {
        ...
        PG_TRY();
        {
            ...
            appendStringInfoString(stringInfo, "select CAST(t.tgname AS character varying)  as \"Trigger\",\n"
                                               ...
                                               "substring(pg_catalog.pg_get_triggerdef(t.oid) from 48)) + 47)\n"
                                               "                 AS \"Statement\",\n"
                                               "       CASE t.tgtype & 66 WHEN 2 THEN 'BEFORE' WHEN 64 THEN 'INSTEAD "
                                               "OF' ELSE 'AFTER' END  AS \"Timing\",\n"
                                               "       null                   as \"Created\",\n"
                                               "       null                   as \"sql_mode\",\n"
            ...
            if (SPI_execute(stringInfo->data, true, 0) != SPI_OK_SELECT) {
                ereport(ERROR,
                        (errcode(ERRCODE_DATA_EXCEPTION), errmsg("failed to exec query '%s'", stringInfo->data)));
            }
        }
        PG_CATCH();
        {
