...
- chara\(any\)
    ...
    ```
    b_compatibility_database=# select chara(77.121,83,81,'76');
     chara
    ------------
    MySQL
    (1 row)
...
- format\(val number, dec_num int \[,locale string\]\)
    ...
    注意：此format函数为对MySQL的format函数兼容，与openGauss原有的format函数语义不同。若想使用此语义，请创建B兼容模式数据库，启用B兼容性SQL引擎插件，并将B_FORMAT_MODE设置为TRUE.
    ...
    CREATE EXTENSION
    b_compatibility_database=# SET B_FORMAT_MODE = TRUE;
    SET
...
- quote(string text)
    ...
    ```
    openGauss=# SELECT quote(E'O\'hello');
       quote    
    ------------
     'O''hello'
    (1 row)
    ...
    ```
    openGauss=# SELECT quote('O\hello');
        quote    
    -------------
     E'O\\hello'
    (1 row)
...
- substr\(string \[from int\] \[for int\]\)
  ...
  ```
  openGauss=# SELECT SUBSTR('Thomas' from 2 for 3);
   substr 
  --------
   hom
  (1 row)
...
- mid\(string \[from int\] \[for int\]\)
  ...
  ```
  openGauss=# SELECT MID('Thomas' from 2 for 3);
   mid 
  -----
   hom
  (1 row)
...
- right\(str text, n int\)
    ...
    openGauss=# SELECT right('abcde', 0);
     right 
    -------
...
- rtrim\(string text \[, characters text\]\)
    ...
    -------
     trim
    (1 row)
    openGauss=# SELECT rtrim('trimxxxy', 'xy');
     rtrim  
    --------
     trimxx
    (1 row)
...
- trim\(\[leading |trailing |both\] \[characters\] from string\)
    ...
    ```
    openGauss=# SELECT trim(BOTH 'xy' FROM 'xyyTomxxy');
      trim 
    -------
      yTomx
    (1 row)
    ...
    ```
    openGauss=# SELECT trim(LEADING 'xy' FROM 'xyyTomxxy');
       trim  
    ---------
     yTomxxy
    (1 row)
    ...
    ```
    openGauss=# SELECT trim(TRAILING 'xy' FROM 'xyyTomxxy');
       trim  
    ---------
     xyyTomx
    (1 row)
...
- ltrim\(string \[, characters\]\)
    ...
    -------
     TRIM
    (1 row)
    openGauss=# SELECT ltrim('xyxTRIM','xy');
     ltrim 
    -------
     xTRIM
    (1 row)
