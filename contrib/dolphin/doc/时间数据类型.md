...
- 输入整数形式(YYYYMMDD, YYMMDD) 格式，需要加上强制类型转换符号 :: 或者使用 cast 函数, year 类型除外。
...
- 如果输入'0'，openGuass 会解析成为 2000 年；但是当输入的是整数0，openGauss 会解析成为 0，表示 year类型的 0 值。需要注意的是，输入整数形式需要使用强制转换符 :: 或者 cast 函数首先进行类型转换。
