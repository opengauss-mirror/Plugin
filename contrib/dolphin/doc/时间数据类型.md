...
| timestamp(p) | 日期和时间，带时区信息，p 表示精度                           | 8字节    | 4713 BC ~ 294276 AD                  | p表示小数点后的精度，取值范围为0~6，如不指定默认为 0| 输入必须为有效日期，不支持月份或者天数为零值；注意，timestamp 类型在原来 openGauss 数据库中表示不带时区的时间戳，兼容后的 timestamp 类型往 mysql 数据库靠拢，表示带时区信息的时间戳，因此存在兼容性问题 |
| year(w)      | 年份，w 表示 "display width"，year(4)、year 形式输出为 'YYYY' 形式，year(2) 形式输出为 'YY' | 2字节    | 1901 ~ 2155                          | -                                  | -                                                            |

备注
- 对于 mysql，如果使用时间类型(例如 timestamp、datetime、time)时不指定精度，则默认为 0，而我们兼容后的 timestamp、datetime、time 类型也使用相同的含义。例外：openGauss 有独特的 timestamp with(out) timezone 或者 time with(out) timezone 的语法，这种语法在 mysql 一端没有，如果使用这种语法，在省略精度的条件下，我们将会保留精度(最大为 6)；因此，timestamp without timezone 类似于 datetime(6)，timestamp with timezone 类似于 timestamp(6)
...
- 输入整数形式(YYYYMMDD, YYMMDD) 格式，需要加上强制类型转换符号 :: 或者使用 cast 函数, year 类型除外。同时，由于 mysql 原本的年份取值范围在 10000 以内，因此如果想要输入大于等于 10000 年份的日期，请使用 'YYYY-MM-DD' 这种格式，例如 '10100-12-12'。
- 允许输入 0000 年，同时在 openGauss 中，认为 0000 年为闰年，可以输入 2-29 日(mysql 不允许)
...
- 输入整数(如hhmmss)需要加上强制类型转换符号 :: 或者使用 cast 函数；当输入整数 0 时，代表的值为 '00:00:00'，也是 time 类型的零值，当输入负数时请勿使用 :: 符号进行类型转换，应该使用 cast 函数。
...
- 输入整数(如YYYYMMDDhhmmss)需要加上强制类型转换符号 :: 或者使用 cast 函数
...
- 如果输入'0'，openGuass 会解析成为 2000 年；但是当输入的是整数0，openGauss 会解析成为 0，表示 year类型的 0 值，输入整数无需使用 :: 强制转换类型。
