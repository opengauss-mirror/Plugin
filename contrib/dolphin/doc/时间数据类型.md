...
备注
- 注意，对于 mysql，在使用 CREATE TABLE 或者 ALTER TABLE 语句中，如果定义时间类型(例如 timestamp、datetime、time)列属性时不指定精度，则默认为 0。同时，使用 cast(expr as typename) 语法进行类型转换时，如果目标类型没有指定精度，那么默认精度也为 0。在 openGauss 兼容后维持这些特性，因此如果用户需要保留精度则需要显式使用 (typmod) 来指定。
