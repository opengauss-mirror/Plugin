...
select a from t1 force index(primary) where a = 2  order by a;
create index inx_t1 on t1(a);
select a from t1 force index(primary) where a = 2 order by a;
select a from t1 force index("inx_t1") where a = 2 order by a;
explain (costs off) select a from t1 force index(primary) where a > 2 order by a;
explain (costs off) select a from t1 force index("inx_t1") where a > 2 order by a;
explain (costs off) select a from t1 use index(primary) where a > 2 order by a;
explain (costs off) select a from t1 use index("inx_t1") where a > 2 order by a;

reset current_schema;
drop schema create_index_if_not_exists cascade;
