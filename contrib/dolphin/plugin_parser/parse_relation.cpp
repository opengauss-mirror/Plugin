...
/* Return true if the target entry was generated by SELECT * or rel.*. */
static bool target_entry_from_star(Query* query, AttrNumber resno)
{
    ListCell* startCell = list_head(query->starStart);
    ListCell* endCell = list_head(query->starEnd);

    while (startCell != NULL && endCell != NULL) {
        int start = lfirst_int(startCell);
        int end = lfirst_int(endCell);
        if (resno >= start && resno < end) {
            return true;
        }
        startCell = lnext(startCell);
        endCell = lnext(endCell);
    }
    return false;
}

/* Find the saved source-column name of a directly selected column. */
static char* get_attname_from_targetlist(Query* query, Oid rel_oid, int2 attnum)
{
    ...
    foreach (lc, query->targetList) {
        ...
        if (rte->rtekind == RTE_RELATION && rte->relid == rel_oid) {
            /* starStart/starEnd may be lost when a stored view is rewritten.
             * If so, an unchanged output name still identifies a star column;
             * an explicit alias is rejected when it differs from the source name. */
            char* sourceName = get_attname_from_rte(query, rel_oid, attnum);
            if (target_entry_from_star(query, tle->resno) ||
                (sourceName != NULL && strcmp(tle->resname, sourceName) == 0)) {
                return tle->resname;
            }
        }
    }
...
static bool CheckRelationColumnExists(Oid rel_oid, int2 attnum, Query* query, int2* newAttnum)
{
    ...
    /* A star-expanded target entry preserves the source name across reordering.
     * For an explicitly selected column, resname may be only a view-column alias,
    ...
    if (!HeapTupleIsValid(tuple)) {
#ifndef DOLPHIN
        /* A saved target name identifies the original column; do not bind it to
         * an unrelated column that later occupied the same physical attnum. */
        if (attnameFromTargetList) {
            return false;
        }
        /* If no saved target identity exists, retain the legacy attnum fallback. */
