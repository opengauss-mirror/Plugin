                                  ...
                                  List *seqoptions = NIL)
{
    ...
    Assert(!(isAutoinc && (forIdentity || forDIdentity)) && !(forIdentity && forDIdentity));
    ...
    if (forIdentity || forDIdentity) {
        ...
        if (TypeNameCheckIsUnsignedIntType(cxt, column->typname)) {
            ...
            maxvalue = (Node*)makeDefElem("maxvalue", (Node*)makeFloat(pstrdup(buf)));
        } else {
    ...
    int32 existPartNum, Const* lowBound, Const* upBound, bool needFree, bool isPartition)
{
    ...
    foreach (cell, pos) {
        ...
        switch (target_type) {
            ...
            default:
#ifdef DOLPHIN
                if (IsUnsignedIntType(target_type)) {
                    isinterval = false;
                    break;
                }
                ereport(ERROR,
...
static void TransformModifyColumnDatatype(CreateStmtContext* cxt, AlterTableCmd* cmd)
{
    ...
    /* pre-alter column type is an set, should drop it after */
#ifdef DOLPHIN
    bool old_set = DropTypeOwnedByTable(cxt, cmd->name);
#endif
...
static int identity_only(ColumnDef* column, CreateStmtContext* cxt, bool* seeDIdentity)
{
    ...
    foreach (clist, column->constraints) {
        Constraint* constraint = (Constraint*)lfirst(clist);
...
static void identity_type_dmod(ColumnDef* column, char* colname)
{
    ...
    if (newtypid != INT1OID &&
        ...
        newtypid != INT8OID &&
        newtypid != INT16OID &&
        (newtypid != NUMERICOID || s != 0)) {
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("Identity column '%s' data type invalid.", colname)));
    }
}

#ifdef DOLPHIN
/*
 * Create a partition key from the primary-key constraint when no explicit
...
static void CreatePartitionKeyFromIndexConstraints(PartitionState* partTableState, List* ixconstraints)
{
    ...
    foreach (ilc, ixconstraints) {
        ...
        IndexElem* elem = (IndexElem*)linitial(cons->keys);
        if (elem->name != NULL) {
