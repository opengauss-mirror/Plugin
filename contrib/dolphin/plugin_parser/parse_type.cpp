...
bool IsBSupportCharsetType(Oid typeOid)
{
    switch (typeOid) {
        case TEXTOID:
        case VARCHAROID:
        ...
        case BLOBOID:
            return true;
        default:
            if (IsBinaryType(typeOid) || type_is_enum(typeOid) || type_is_set(typeOid)) {
                return true;
            }
            return false;
    }
...
{
    return (typid == BLOBOID || typid == BYTEAOID || typid == BINARYOID ||
...
Oid DefineAnonymousEnum(TypeName* typname, Oid collations)
{
    ...
    enumNamespace = QualifiedNameGetCreationNamespace(typname->names, &enumName);
...
char* makeEnumTypeName(const char* relname, const char *colname, const char* schemaname) 
{
    ...
    nsOid = get_namespace_oid(schemaname, true);
    /* namespacename + "_" + tablename + "_" + typename + "_enum num" if conflict */
    for (;;) {

        /*  if pass == INT_MAX then error report */
        if ((pass + 1) < pass) {
        ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT),
                        errmsg("could not form enum type name for type \"%s\"",
                                colname)));
            break;
        }

        rc = sprintf_s(modlabel, NAMEDATALEN, "%u_anonymous_enum_%d", nsOid, ++pass);
        securec_check_ss(rc, "", "");

        /* add "_ENUM num" */
        
        arr = makeObjectName(relname, colname, modlabel);
