    ...
    switch (atttypid) {
        ...
        default:
            ...
            if (atttypid == LONGBLOBOID) {
                result =
                    coerce_type(NULL, node, con->consttype, atttypid, -1, COERCION_IMPLICIT, COERCE_IMPLICIT_CAST,
                                NULL, NULL, -1);
                break;
            }
#endif
    ...
    CoercionContext ccontext, CoercionForm cformat, char* fmtstr, char* nlsfmtstr, int location)
{
        ...
        if (ENABLE_B_CMPT_MODE) {
            ...
            if (!con->constisnull) {
                newcon->constvalue = stringTypeDatum_with_collation(targetType, DatumGetCString(con->constvalue),
                    fmtstr, nlsfmtstr, inputTypeMod, pstate != NULL && pstate->p_has_ignore, con->constcollid);
            } else {
                newcon->constvalue =
    ...
    if (inputTypeId == INT4OID && IsA(node, Const) && type_is_enum(targetTypeId)) {
        ...
        if (enumOrder == 0) {
            newcon->constvalue = stringTypeDatum(targetType, NULL, fmtstr, nlsfmtstr, -1);
            newcon->constisnull = true;
        } else {
            targetEnumLable = getEnumLableByOrder(targetTypeId, enumOrder);
            newcon->constvalue = stringTypeDatum(targetType, targetEnumLable, fmtstr, nlsfmtstr, -1);
        }
        cancel_parser_errposition_callback(&pcbstate);
    ...
    int32 targetTypMod, CoercionForm cformat, char* fmtstr, char* nlsfmtstr, int location, bool isExplicit)
{
    ...
    if (OidIsValid(funcId)) {
        ...
        AssertEreport((nargs < 2 || procstruct->proargtypes.values[1] == INT4OID ||
            procstruct->proargtypes.values[1] == OIDOID),
            MOD_OPT,
        ...
        (procstruct->proargtypes.values[2] == ANYELEMENTOID && procstruct->prorettype == ANYENUMOID)),
            MOD_OPT,
            "The number or type of parameters in the function is incorrect.");
#else
        AssertEreport((nargs < 2 || procstruct->proargtypes.values[1] == INT4OID),
    ...
    if (pathtype == COERCION_PATH_FUNC) {
        ...
        if (nargs >= 2) {
#ifdef DOLPHIN
            if (type_is_set(targetTypeId) || type_is_enum(targetTypeId)) {
#else
...
#ifdef DOLPHIN
    } else if (context != NULL && CHECK_PARSE_PHRASE(context, "CASE")) {
        ptype = choose_specific_expr_type(pstate, exprs, context);
#endif
    } else {
...
Node* coerce_to_common_type(ParseState* pstate, Node* node, Oid targetTypeId, const char* context)
{
    ...
    if (can_coerce_type(1, &inputTypeId, &targetTypeId, COERCION_IMPLICIT)) {
        ...
        node = coerce_type(pstate, node, inputTypeId, targetTypeId, -1, COERCION_EXPLICIT, COERCE_EXPLICIT_CAST,
        NULL, NULL, -1);
...
Oid findCastEnumFunction(Oid sourceTypeId)
{
...
Oid findEnumCastFunction(Oid targetTypeId)
{
    Oid funcid = InvalidOid;
