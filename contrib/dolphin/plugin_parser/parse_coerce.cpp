    ...
    CoercionContext ccontext, CoercionForm cformat, int location)
{
    ...
    if (inputTypeId == UNKNOWNOID && IsA(node, Const)) {
         ...
         */
        if (!con->constisnull) {
            newcon->constvalue = stringTypeDatum(targetType, DatumGetCString(con->constvalue), inputTypeMod, pstate != NULL && pstate->p_has_ignore);
        } else {
            newcon->constvalue = stringTypeDatum(targetType, NULL, inputTypeMod, pstate != NULL && pstate->p_has_ignore);
        }
