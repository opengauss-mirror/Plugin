#!/bin/bash
CMAKE_OPT="-DENABLE_MULTIPLE_NODES=OFF -DENABLE_PRIVATEGAUSS=OFF -DENABLE_THREAD_SAFETY=ON -DENABLE_LITE_MODE=$1 -DENABLE_MOT=$2"
cpus_num=$(grep -w processor /proc/cpuinfo|wc -l)
rm -f dolphin--4.5.sql
touch dolphin--4.5.sql
for i in `ls sql_script`; do cat sql_script/$i >> dolphin--4.5.sql; done
for i in `ls sql_script_post`; do cat sql_script_post/$i >> dolphin--4.5.sql; done
cat upgrade_script/dolphin--1.0--1.1.sql upgrade_script/dolphin--1.1--2.0.sql upgrade_script/dolphin--2.0--3.0.sql upgrade_script/dolphin--3.0--4.0.sql upgrade_script/dolphin--4.0--4.1.sql upgrade_script/dolphin--4.1--4.2.sql upgrade_script/dolphin--4.2--4.3.sql upgrade_script/dolphin--4.3--4.4.sql upgrade_script/dolphin--4.4--4.5.sql >> dolphin--4.5.sql
