...
rm -f dolphin--3.0.sql
touch dolphin--3.0.sql
for i in `ls sql_script`; do cat sql_script/$i >> dolphin--3.0.sql; done
for i in `ls sql_script_post`; do cat sql_script_post/$i >> dolphin--3.0.sql; done
cat upgrade_script/dolphin--1.0--1.1.sql upgrade_script/dolphin--1.1--2.0.sql upgrade_script/dolphin--2.0--3.0.sql >> dolphin--3.0.sql
