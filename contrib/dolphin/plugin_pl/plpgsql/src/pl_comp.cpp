            ...
            if (ALLOW_PROCEDURE_COMPILE_CHECK) {
                ...
                if (rec->expr) {
                    ...
                    if (rec->tupdesc) {
                        ...
                        rc = memset_s(nulls, rec->tupdesc->natts * sizeof(bool),
                                      true, rec->tupdesc->natts * sizeof(bool));
                        securec_check(rc, "\0", "\0");
...
void checkArrayTypeInsert(ParseState* pstate, Expr* expr)
{
    ...
    if (pstate->p_pre_columnref_hook == b_plpgsql_pre_column_ref && pstate->p_ref_hook_state != NULL) {
