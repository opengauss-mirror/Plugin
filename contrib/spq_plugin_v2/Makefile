...
all: extension spq-backup

# build extension
extension: $(spq_top_builddir)/src/include/spq_version.h
...
# generated headers are located in the build directory
	$(INSTALL_DATA) $(spq_top_builddir)/src/include/spq_version.h '$(DESTDIR)$(includedir_server)/'
# the rest in the source tree
	$(INSTALL_DATA) $(spq_abs_srcdir)/src/include/distributed/*.h '$(DESTDIR)$(includedir_server)/distributed/'
spq-backup: $(spq_top_builddir)/src/spq_backup/spq_backup.cpp
	$(MAKE) -C src/spq_backup/ all
...
install-backup:
	$(MAKE) -C src/spq_backup/ install


# Add to generic targets
install: install-extension install-headers install-backup
clean: clean-extension

# apply or check style
reindent:
	${spq_abs_top_srcdir}/ci/fix_style.sh
check-style:
	black . --check --quiet
	isort . --check --quiet
	flake8
	cd ${spq_abs_top_srcdir} && citus_indent --quiet --check
