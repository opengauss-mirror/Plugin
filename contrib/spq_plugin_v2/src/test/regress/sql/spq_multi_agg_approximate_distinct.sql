...
SET spq.coordinator_aggregation_strategy TO 'disabled';
...
SET spq.count_distinct_error_rate = 0.1;
SELECT count(distinct l_orderkey) FROM lineitem;

SET spq.count_distinct_error_rate = 0.01;
SELECT count(distinct l_orderkey) FROM lineitem;
...
SET spq.next_shard_id TO 20000000;
SET spq.next_placement_id TO 20000000;
...
SET spq.count_distinct_error_rate TO 0.01;
...
SET spq.limit_clause_row_fetch_count = 1000;
...
SET spq.count_distinct_error_rate = 0.0;
SELECT count(distinct l_orderkey) FROM lineitem;
