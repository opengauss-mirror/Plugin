...
CREATE TABLE partitioning_test(id int, value_1 int, time date) PARTITION BY RANGE (time) (
  PARTITION  partitioning_test_2010 VALUES LESS THAN ('2011-01-01'),
  PARTITION  partitioning_test_2017 VALUES LESS THAN ('2018-01-01')
);
...
SET spq.enable_repartition_joins to ON;
SELECT
	count(*)
FROM
(
...
SET spq.enable_repartition_joins to OFF;
