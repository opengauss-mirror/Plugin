...
SET spq.next_shard_id TO 360000;
...
SET spq.next_shard_id TO 360013;
...
-- first set spq.shard_count so we know the parameter works
SET spq.shard_count TO 10;
...
SELECT shard_count FROM spq_tables WHERE table_name::text = 'shard_count_table';
...
-- show the difference in pg_dist_colocation and spq_tables shard counts
SELECT
	(
		SELECT shardcount FROM pg_dist_colocation WHERE colocationid IN
		(
			SELECT colocation_id FROM spq_tables WHERE table_name = 'shard_split_table'::regclass
		)
	) AS "pg_dist_colocation",
	(SELECT shard_count FROM spq_tables WHERE table_name = 'shard_split_table'::regclass) AS "spq_tables";

CREATE TABLE shard_split_table_2 (a int, b int);
SELECT create_distributed_table ('shard_split_table_2', 'a', shard_count:=9);

SELECT a.colocation_id = b.colocation_id FROM spq_tables a, spq_tables b
	WHERE a.table_name = 'shard_split_table'::regclass AND b.table_name = 'shard_split_table_2'::regclass;

SELECT shard_count FROM spq_tables WHERE table_name = 'shard_split_table_2'::regclass;
...
SELECT shard_count FROM spq_tables WHERE table_name = 'shard_split_table_3'::regclass;
...
SELECT shardcount FROM pg_dist_colocation WHERE colocationid IN
(
	SELECT colocation_id FROM spq_tables WHERE table_name = 'shard_count_table_3'::regclass
);
