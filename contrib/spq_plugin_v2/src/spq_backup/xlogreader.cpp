...
static inline void prepare_invalid_report(XLogReaderState* state, char* fname,
                                          const size_t fname_len, const XLogSegNo segno)
{
    errno_t errorno = memset_s(fname, fname_len, 0, fname_len);
    securec_check(errorno, "", "");
    errorno = snprintf_s(fname, fname_len, fname_len - 1, "%08X%08X%08X",
                         state->readPageTLI, (uint32)((segno) / XLogSegmentsPerXLogId),
...
void report_invalid_record(XLogReaderState* state, const char* fmt, ...)
{
    ...
    va_start(args, fmt);
    rc = vsnprintf_s(state->errormsg_buf, MAX_ERRORMSG_LEN, MAX_ERRORMSG_LEN - 1, fmt,
                     args);
    va_end(args);
                                    ...
                                    Size alignedSize)
{
     ...
     */
    state->readBufOrigin =
        (char*)palloc_extended(XLOG_BLCKSZ + alignedSize, MCXT_ALLOC_NO_OOM);
...
bool allocate_recordbuf(XLogReaderState* state, uint32 reclength)
{
    ...
    newSize = state->isPRProcess ? Max(newSize, 512)
                                 : Max(newSize, 5 * Max(BLCKSZ, XLOG_BLCKSZ));
    ...
    *errormsg = NULL;
    ...
    readOff =
        ReadPageInternal(state, targetPagePtr,
                         Min(targetRecOff + SizeOfXLogRecord, XLOG_BLCKSZ), xlog_path);
    if (readOff < 0) {
        report_invalid_record(state, "read xlog page failed at %X/%X",
    ...
    if (total_len > state->readRecordBufSize && !allocate_recordbuf(state, total_len)) {
        ...
        report_invalid_record(state, "record length %u at %X/%X too long", total_len,
                              (uint32)(RecPtr >> 32), (uint32)RecPtr);
        goto err;
    }
    ...
    if (total_len > len) {
        ...
        errorno = memcpy_s(state->readRecordBuf, len,
        ...
        do {
            ...
            readOff = ReadPageInternal(
                state, targetPagePtr,
                Min(total_len - gotlen + SizeOfXLogShortPHD, XLOG_BLCKSZ), xlog_path);
            if (readOff < 0)
                goto err;
            ...
            if (readOff < (int)pageHeaderSize)
                readOff =
                    ReadPageInternal(state, targetPagePtr, pageHeaderSize, xlog_path);
        ...
        XLByteAdvance(state->EndRecPtr,
                      (pageHeaderSize + MAXALIGN(pageHeader->xlp_rem_len)));
    } else {
        /* Wait for the record data to become available */
        readOff = ReadPageInternal(state, targetPagePtr,
                                   Min(targetRecOff + total_len, XLOG_BLCKSZ), xlog_path);
        if (readOff < 0) {
            goto err;
        }
     ...
     */
    if (((XLogRecord*)record)->xl_rmid == RM_XLOG_ID &&
        ((XLogRecord*)record)->xl_info == XLOG_SWITCH) {
                            ...
                            char* xlog_path)
{
     ...
     */
    readLen = state->read_page(state, pageptr, Max(reqLen, (int)SizeOfXLogShortPHD),
    ...
    if (readLen < (int)XLogPageHeaderSize(hdr)) {
        readLen =
            state->read_page(state, pageptr, XLogPageHeaderSize(hdr), state->currRecPtr,
                             state->readBuf, &state->readPageTLI, xlog_path);
        if (readLen < 0) {
            goto err;
        }
    }
                           ...
                           XLogRecPtr PrevRecPtr, XLogRecord* record, bool randAccess)
{
    ...
    if (record->xl_tot_len < SizeOfXLogRecord ||
        record->xl_tot_len >= XLogRecordMaxSize) {
        report_invalid_record(state, "invalid record length at %X/%X: wanted %u, got %u",
                              (uint32)(RecPtr >> 32), (uint32)RecPtr,
    ...
    if (randAccess) {
        ...
        if (XLByteLE(RecPtr, xl_prev)) {
            report_invalid_record(state, "record with incorrect prev-link %X/%X at %X/%X",
...
bool ValidXLogRecord(XLogReaderState* state, XLogRecord* record, XLogRecPtr recptr)
{
    ...
    if (!EQ_CRC32C(record->xl_crc, crc)) {
        ...
        report_invalid_record(
            ...
            "record info: xl_info=%d, xl_prev=%X/%X, xl_rmid=%d, xl_tot_len=%u, xl_xid= "
            "%lu",
...
bool ValidXLogPageHeader(XLogReaderState* state, XLogRecPtr recptr, XLogPageHeader hdr)
{
    ...
    if (hdr->xlp_info & XLP_LONG_HEADER) {
        ...
        if (state->system_identifier && longhdr->xlp_sysid != state->system_identifier) {
             ...
             */
            ss_c =
                snprintf_s(fhdrident_str, sizeof(fhdrident_str),
                           sizeof(fhdrident_str) - 1, UINT64_FORMAT, longhdr->xlp_sysid);
#ifndef FRONTEND
            ...
            ss_c =
                snprintf_s(sysident_str, sizeof(sysident_str), sizeof(sysident_str) - 1,
                           UINT64_FORMAT, state->system_identifier);
#ifndef FRONTEND
            securec_check_ss(ss_c, "", "");
#else
            securec_check_ss_c(ss_c, "", "");
            ...
            report_invalid_record(state,
                                  "WAL file is from different database system: WAL file "
                                  "database system identifier is %s, pg_control "
                                  "database system identifier is %s.",
                                  fhdrident_str, sysident_str);
            return false;
        } else if (longhdr->xlp_seg_size != XLogSegSize) {
            report_invalid_record(state, "WAL file is from different database system: "
...
XLogRecPtr XLogFindNextRecord(XLogReaderState* state, XLogRecPtr RecPtr,
                              XLogRecPtr* endPtr, char* xlog_path)
{
    ...
    while (true) {
        ...
        readLen = ReadPageInternal(state, targetPagePtr,
                                   Max(targetRecOff, (int)SizeOfXLogLongPHD), xlog_path);
        if (readLen < 0) {
            goto out;
        }
    ...
    while (remaining > datatotal) {
        ...
        if (block_id == XLR_BLOCK_ID_DATA_SHORT) {
            ...
            if (!(fork_flags & BKPBLOCK_SAME_REL)) {
                uint32 filenodelen =
                    (hasbucket_segpage ? sizeof(RelFileNode) : sizeof(RelFileNodeOld));
...
char* XLogRecGetBlockImage(XLogReaderState* record, uint8 block_id, uint16* hole_offset,
                           uint16* hole_length)
{
    DecodedBkpBlock* bkpb = NULL;
...
int SimpleXLogPageRead(XLogReaderState* xlogreader, XLogRecPtr targetPagePtr, int reqLen,
...
tryAgain:
    ...
    if (xlogreadfd < 0) {
        ss_c = snprintf_s(xlogfname, MAXFNAMELEN, MAXFNAMELEN - 1, "%08X%08X%08X",
                          readprivate->tli,
        ...
        if (xlog_path != NULL) {
            ss_c = snprintf_s(xlogfpath, MAXPGPATH, MAXPGPATH - 1, "%s/%s", xlog_path,
                              xlogfname);
        } else {
            ss_c = snprintf_s(xlogfpath, MAXPGPATH, MAXPGPATH - 1, "%s/" XLOGDIR "/%s",
...
XLogRecord* XLogReadRecordFromAllDir(char** xlogDirs, int xlogDirNum,
                                     XLogReaderState* xlogReader, XLogRecPtr curLsn,
                                     char** errorMsg)
{
    XLogRecord* record = NULL;
...
void SSFindMaxXlogFileName(char* maxXLogFileName, char** xlogDirs, int xlogDirNum)
{
    ...
    for (int i = 0; i < xlogDirNum; i++) {
        ...
        while (subDir != NULL && (subDirEntry = readdir(subDir)) != NULL) {
            if (strlen(subDirEntry->d_name) == 24 &&
                strspn(subDirEntry->d_name, "0123456789ABCDEF") == 24 &&
                (strlen(maxXLogFileName) == 0 ||
                 strcmp(maxXLogFileName, subDirEntry->d_name) < 0)) {
                rc = strncpy_s(maxXLogFileName, MAXPGPATH, subDirEntry->d_name,
...
XLogRecPtr SSFindMaxLSN(char* workingPath, char* returnMsg, int msgLen,
                        pg_crc32* maxLsnCrc, char** xlogDirs, int xlogDirNum)
{
    XLogReaderState* xlogReader = NULL;
    XLogPageReadPrivate readPrivate = {.datadir = NULL, .tli = 0};
    XLogRecord* record = NULL;
    TimeLineID tli = 0;
    ...
    if (sscanf_s(maxXLogFileName, "%08X%08X%08X", &tli, &xlogReadLogid,
                 &xlogReadLogSeg) != 3) {
    ...
    rc = memset_s(&readPrivate, sizeof(XLogPageReadPrivate), 0,
                  sizeof(XLogPageReadPrivate));
    ...
    startLsn = (xlogReadLogSeg * XLogSegSize) +
               ((XLogRecPtr)xlogReadLogid * XLogSegmentsPerXLogId * XLogSegSize);
    ...
    while (true) {
        record =
            XLogReadRecordFromAllDir(xlogDirs, xlogDirNum, xlogReader, curLsn, &errorMsg);
        if (record == NULL) {
            break;
        }
                      ...
                      uint32* maxLsnLen, TimeLineID* returnTli, char* xlog_path)
{
    ...
    if (xlog_path != NULL) {
        ...
        rc = snprintf_s(xlogDirStr, MAXPGPATH, MAXPGPATH - 1, "%s/%s", workingPath,
                        XLOGDIR);
    }
    ...
    while ((dirEnt = readdir(xlogDir)) != NULL) {
        if (strlen(dirEnt->d_name) == 24 &&
            ...
            if (strlen(maxXLogFileName) == 0 ||
                strcmp(maxXLogFileName, dirEnt->d_name) < 0) {
                rc = strncpy_s(maxXLogFileName, MAXPGPATH, dirEnt->d_name,
...
XLogRecPtr FindMinLSN(char* workingPath, char* returnMsg, int msgLen, pg_crc32* minLsnCrc)
{
    ...
    while ((dirEnt = readdir(xlogDir)) != NULL) {
        if (strlen(dirEnt->d_name) == 24 &&
            strspn(dirEnt->d_name, "0123456789ABCDEF") == 24) {
            if (strlen(minXLogFileName) == 0 ||
                strcmp(minXLogFileName, dirEnt->d_name) > 0) {
                rc = strncpy_s(minXLogFileName, MAXPGPATH, dirEnt->d_name,
                               strlen(dirEnt->d_name) + 1);
#ifndef FRONTEND
                securec_check(rc, "", "");
#else
    ...
    if (sscanf_s(minXLogFileName, "%08X%08X%08X", &tli, &xlogReadLogid,
                 &xlogReadLogSeg) != 3) {
        rc = snprintf_s(returnMsg, msgLen, msgLen - 1,
                        "failed to translate name to xlog: %s\n", minXLogFileName);
#ifndef FRONTEND
    ...
    if (findValidXLogFile == false) {
        rc = snprintf_s(returnMsg, msgLen, msgLen - 1, "no valid record in pg_xlog.\n");
#ifndef FRONTEND
        securec_check_ss(rc, "", "");
#else
        securec_check_ss_c(rc, "", "");
...
bool XlogFileIsExisted(const char* workingPath, XLogRecPtr inputLsn, TimeLineID timeLine)
{
    ...
    rc = snprintf_s(xlogfname, MAXFNAMELEN, MAXFNAMELEN - 1, "%08X%08X%08X", timeLine,
                    ...
                    (uint32)((xlogSegno) % XLogSegmentsPerXLogId));
    securec_check_ss_c(rc, "", "");
    rc = snprintf_s(xlogfpath, MAXPGPATH, MAXPGPATH - 1, "%s/" XLOGDIR "/%s", workingPath,
                    xlogfname);
    securec_check_ss_c(rc, "", "");
...
char* GetRepOriginPtr(char* xnodes, uint64 xinfo, int nsubxacts, int nmsgs, int nrels,
                      int nlibrary, bool compress)
{
