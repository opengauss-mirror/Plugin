/* -------------------------------------------------------------------------
 *
 * receivelog.cpp - receive transaction log files using the streaming
 *                  replication protocol.
 *
 * Portions Copyright (c) 2025 Huawei Technologies Co.,Ltd.
 *
 * IDENTIFICATION
 *          src/spq_backup/receivelog.cpp
 * -------------------------------------------------------------------------
 */
...
typedef enum {
    DO_WAL_DATA_WRITE_DONE,
    DO_WAL_DATA_WRITE_STOP,
    DO_WAL_DATA_WRITE_ERROR
...
static long CalculateCopyStreamSleeptime(TimestampTz now,
                                         int standby_message_timeout_local,
                                         ...
                                         TimestampTz last_status)
{
    ...
    if (standby_message_timeout_local)
        status_targettime =
            last_status + (standby_message_timeout_local - 1) * (USECS_PER_SEC);
 ...
 */
static int open_walfile(XLogRecPtr startpoint, uint32 timeline, const char* basedir,
                        char* namebuf)
{
...
#define MAX_REALPATH_LEN 4096
    ...
    int nRet = snprintf_s(namebuf, MAXFNAMELEN, MAXFNAMELEN - 1, "%08X%08X%08X", timeline,
    ...
    if (statbuf.st_size != 0) {
        pg_log(PG_PRINT, _("%s: WAL segment %s is %d bytes, should be 0 or %lu\n"),
               progname, Lrealpath, (int)statbuf.st_size, XLogSegSize);
        if (close(f) != 0) {
            pg_log(PG_PRINT, _("%s: close file failed %s: %s\n"), progname, Lrealpath,
                   strerror(errno));
        }
...
public:
    void heartbeatSleep(void);
    void* heartbeatTimerHandler(void* data);
    void createHeartbeatTimer(PGconn* conn);
...
void HeartbeatTimer::heartbeatSleep(void)
{
...
void* heartbeatTimerThread(void* data)
{
    HeartbeatTimer* timer = (HeartbeatTimer*)data;
    return timer->heartbeatTimerHandler(nullptr);
}
 ...
 */
void* HeartbeatTimer::heartbeatTimerHandler(void* data)
{
    if (xlogconn == NULL) {
        return NULL;
    }
 ...
 * Avoid unnecessary timeouts.
 */
void HeartbeatTimer::createHeartbeatTimer(PGconn* conn)
{
    xlogconn = conn;
    timerFlag = 0;
    pthread_mutex_init(&heartbeatMutex, NULL);
    pthread_mutex_lock(&heartbeatMutex);
    if (pthread_create(&hearbeatTimerId, NULL, heartbeatTimerThread, this) != 0) {
...
void HeartbeatTimer::resumeHeartBeatTimer(void)
{
    pthread_mutex_unlock(&heartbeatMutex);
}
 ...
 */
void HeartbeatTimer::suspendHeartBeatTimer(void)
{
    /* wait for send finished */
    pthread_mutex_lock(&heartbeatMutex);
}

void HeartbeatTimer::closeHearBeatTimer(void)
{
 ...
 */
static bool close_walfile(int walfile, const char* basedir, char* walname,
                          bool segment_complete, XLogRecPtr pos)
{
    ...
    if (currpos == (off_t)XLogSegSize || segment_complete) {
        ...
        nRet = snprintf_s(oldfn, sizeof(oldfn), sizeof(oldfn) - 1, "%s/%s.partial",
                          basedir, walname);
        securec_check_ss_c(nRet, "", "");
        nRet = snprintf_s(newfn, sizeof(newfn), sizeof(newfn) - 1, "%s/%s", basedir,
                          walname);
        securec_check_ss_c(nRet, "", "");
        if (rename(oldfn, newfn) != 0) {
            pg_log(PG_PRINT, _("%s: could not rename file \"%s/%s\": %s\n"), progname,
                   basedir, walname, strerror(errno));
            return false;
        }
    } else {
 ...
 */
static bool localTimestampDifferenceExceeds(TimestampTz start_time, TimestampTz stop_time,
                                            int msec)
{
...
static bool sendReplyToSender(PGconn* conn, TimestampTz nowtime, bool replyRequested)
{
    ...
    if (PQputCopyData(conn, replybuf, sizeof(replybuf)) <= 0 || PQflush(conn)) {
        pg_log(PG_PRINT, _("%s: could not send feedback packet: %s"), progname,
               PQerrorMessage(conn));
        return false;
    }
 ...
 * Description    :
 *         Check if configured timeout has reached without receiving anything from server.
 * If yes then assume that connection broken. If timeout has not reached but half of
 * timeout has reached without receiving anything, then send a message to server along
 * with request for an immediate reply.
 */
static bool checkForReceiveTimeout(PGconn* conn)
{
    /*
    ...
    if (standby_message_timeout > 0) {
         ...
         */
        if (localTimestampDifferenceExceeds(last_recv_timestamp, nowtime,
                                            (standby_message_timeout / 2))) {
            if (ping_sent == false) {
...
/* do xlog write */
static int DoWALWrite(const char* wal_buf, int len, XLogRecPtr& block_pos,
                      const char* basedir, char* cur_wal_file, uint32 wal_file_timeline,
                      int& walfile, stream_stop_callback stream_stop, PGconn* conn,
                      XLogRecPtr endPos, HeartbeatTimer& timer)
{
    ...
    if (walfile == -1) {
        ...
        if (xlogoff != 0) {
            pg_log(PG_PRINT,
                   _("%s: received xlog record for offset %u with no file open\n"),
                   progname, xlogoff);
        ...
        if (lseek(walfile, 0, SEEK_CUR) != xlogoff) {
            pg_log(PG_PRINT, _("%s: got WAL data offset %08x, expected %08x\n"), progname,
    ...
    while (bytes_left) {
        ...
        if (write(walfile, wal_buf + bytes_written, bytes_to_write) != bytes_to_write) {
            pg_log(PG_PRINT, _("%s: could not write %u bytes to WAL file %s: %s\n"),
                   progname, bytes_to_write, cur_wal_file, strerror(errno));
            timer.suspendHeartBeatTimer();
            return DO_WAL_DATA_WRITE_ERROR;
        }
        ...
        if (block_pos % XLogSegSize == 0) {
            ...
            if (stream_stop != NULL) {
                 ...
                 * told us to.
                 */
                if (stream_stop(block_pos, wal_file_timeline, true, endPos)) {
                    timer.suspendHeartBeatTimer();
                       ...
                       int standby_message_timeout_local, bool rename_partial)
{
    ...
    if (replication_slot != NULL) {
        ...
        ss_c = snprintf_s(slotcmd, MAXPGPATH, MAXPGPATH - 1, "SLOT \"%s\" ",
                          replication_slot);
    ...
    if (sysidentifier != NULL) {
        ...
        if (strcmp(sysidentifier, PQgetvalue(res, 0, 0)) != 0) {
            pg_log(PG_WARNING, _("system identifier does not match between base backup "
                                 "and streaming connection\n"));
            PQclear(res);
            return false;
        }
        if (timeline != (unsigned int)atoi(PQgetvalue(res, 0, 1))) {
            pg_log(PG_WARNING, _("timeline does not match between base backup and "
    ...
    if (access(basedir, F_OK) != 0 && pg_mkdir_p((char*)basedir, S_IRWXU) == -1) {
    ...
    ss_c = snprintf_s(query, MAXPGPATH, MAXPGPATH - 1, "START_REPLICATION %s%X/%X",
                      slotcmd, (uint32)(startpos >> 32), (uint32)startpos);
    securec_check_ss_c(ss_c, "", "");
    ...
    timer.createHeartbeatTimer(conn);
    /*
    ...
    while (1) {
         ...
         */
        if ((stream_stop != NULL) && stream_stop(blockpos, timeline, false, *endPos)) {
            timer.closeHearBeatTimer();
            // xlogconn = NULL;
            if (walfile != -1)
                /* Potential error message is written by close_walfile */
                return close_walfile(walfile, basedir, current_walfile_name,
                                     rename_partial, blockpos);
            return true;
        }
        ...
        if (checkForReceiveTimeout(conn) == false) {
            pg_log(PG_WARNING, _("checkForReceiveTimeout failed 1\n"));
            goto error;
        }
        ...
        if (r == 0) {
            ...
            if (standby_message_timeout_local) {
#ifdef HAVE_INT64_TIMESTAMP
                timeout.tv_sec = CalculateCopyStreamSleeptime(
                    local_now, standby_message_timeout_local / 2, last_status);
#else
                timeout.tv_sec =
                    last_status + (standby_message_timeout_local / 2) - local_now - 1;
#endif
                if (timeout.tv_sec <= 0)
            ...
            if (r == 0 || (r < 0 && errno == EINTR)) {
                 ...
                 * and then either deliver a status packet to the server or just go back
                 * into blocking.
                 */
                if (checkForReceiveTimeout(conn) == false) {
                    pg_log(PG_WARNING, _("checkForReceiveTimeout failed 2\n"));
                    goto error;
                }
                /* Set the last reply timestamp */
                ping_sent = false;
            ...
            if (PQconsumeInput(conn) == 0) {
                pg_log(PG_WARNING,
                       _("Could not receive data from WAL stream, please check network "
                         "state and configuration, e.g "
                         "timeout setting: %s\n"),
                       PQerrorMessage(conn));
                goto error;
            }
        ...
        if (r == -2) {
            pg_log(PG_WARNING,
                   _("Could not read copy data, please check network state and "
                     "configuration: %s\n"),
                   PQerrorMessage(conn));
            goto error;
        }
        ...
        if (copybuf[0] == 'k') {
            ...
            ret = memcpy_s(&keepalive, sizeof(PrimaryKeepaliveMessage), copybuf + 1,
                           sizeof(PrimaryKeepaliveMessage));
            securec_check(ret, "\0", "\0");

            /* If as part of keepalive message from sender, an immediate reply is
             * requested then send the same to sender.
             */
            if (keepalive.replyRequested) {
                local_now = localGetCurrentTimestamp();
                if (sendReplyToSender(conn, local_now, false) == false) {
                    pg_log(PG_PRINT, _("sendReplyToSender failed\n"));
                    goto error;
                }
        ...
        ret = DoWALWrite((const char*)wal_data_buf, bytes_left, blockpos,
                         (const char*)basedir, current_walfile_name, timeline, walfile,
                         stream_stop, conn, *endPos, timer);
