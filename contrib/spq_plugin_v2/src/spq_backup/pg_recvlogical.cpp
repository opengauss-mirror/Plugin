/* -------------------------------------------------------------------------
 *
 * pg_recvlogical.cpp - receive data from a logical decoding slot in a streaming fashion
 *                      and write it to to a local file.
 *
 * Portions Copyright (c) 2025 Huawei Technologies Co.,Ltd.
 *
 * IDENTIFICATION
 *       src/spq_backup/pg_recvlogical.cpp
 * -------------------------------------------------------------------------
 */
...
static void usage(void)
{
    ...
    printf(_("  -F  --fsync-interval=INTERVAL\n"
             "                         frequency of syncs to the output file (in "
             "seconds, defaults to 10)\n"));
    printf(_("  -o, --option=NAME[=VALUE]\n"
             "                         Specify option NAME with optional value VAL, to "
             "be passed\n"
             "                         to the output plugin\n"));
    printf(_("  -P, --plugin=PLUGIN    use output plugin PLUGIN (defaults to "
             "mppdb_decoding)\n"));
    printf(_("  -s, --status-interval=INTERVAL\n"
             "                         time between status packets sent to server (in "
             "seconds, defaults to 10)\n"));
    printf(_("  -S, --slot=SLOT        use existing replication slot SLOT instead of "
             "starting a new one\n"));
    printf(_("  -I, --startpos=PTR     Where in an existing slot should the streaming "
             "start\n"));
    printf(_("  -r, --raw              parallel decoding output raw results without "
             "converting to text format\n"));
    printf(_("\nAction to be performed:\n"));
    printf(_("      --create           create a new replication slot (for the slotname "
             "see --slot)\n"));
    printf(_("      --start            start streaming in a replication slot (for the "
             "slotname see --slot)\n"));
    printf(_("      --drop             drop the replication slot (for the slotname see "
             "--slot)\n"));
    printf(_("\n"));
}
...
static bool sendFeedback(PGconn* conn, int64 now, bool force, bool replyRequested)
{
     ...
     * kill us.
     */
    if (!force && XLByteEQ(last_written_lsn, output_written_lsn) &&
        !XLByteEQ(last_fsync_lsn, output_fsync_lsn))
        return true;

    if (verbose)
        fprintf(stderr,
                _("%s: confirming write up to %X/%X, flush to %X/%X (slot %s) %ld\n"),
                progname, (uint32)(output_written_lsn >> 32), (uint32)output_written_lsn,
                (uint32)(output_fsync_lsn >> 32), (uint32)output_fsync_lsn,
                replication_slot, now);
...

    {
        ...
        replymsg->applyRead =
            output_fsync_lsn;     /* applyRead: walsender need it to update lsn */
    ...
    if (PQputCopyData(conn, replybuf, len) <= 0 || PQflush(conn)) {
        fprintf(stderr, _("%s: could not send feedback packet: %s"), progname,
                PQerrorMessage(conn));
        return false;
    }
...
static int sendStartReplicationCmd()
{
    ...
    if (verbose)
        fprintf(stderr, _("%s: starting log streaming at %X/%X (slot %s)\n"), progname,
                (uint32)(startpos >> 32), (uint32)startpos, replication_slot);
...
static void StreamLogicalLog(void)
{
    ...
    while (!time_to_abort) {
        ...
        if (outfd != -1 &&
            feTimestampDifferenceExceeds(output_last_fsync, now, fsync_interval)) {
            if (!OutputFsync(now))
                goto error;
        }
        ...
        if (r == 0) {
            ...
            if (standby_message_timeout)
                message_target =
                    last_status + (standby_message_timeout - 1) * ((int64)1000);
        ...
        if (r == -2) {
            fprintf(stderr, _("%s: could not read COPY data: %s"), progname,
                    PQerrorMessage(conn));
            goto error;
        }
        ...
        if (copybuf[0] == 'k') {
            ...
            errorno = memcpy_s(&keepalive_message, sizeof(PrimaryKeepaliveMessage),
                               &copybuf[pos], sizeof(PrimaryKeepaliveMessage));
        ...
        if (write(outfd, "\n", 1) != 1) {
            fprintf(stderr, _("%s: could not write %d bytes to log file \"%s\": %s\n"),
...
static int getOptions(const int argc, char* const* argv)
{
    ...
    int option_index;
    uint32 hi, lo;
    while ((c = getopt_long(argc, argv, "f:F:nvd:h:o:p:U:wWP:rs:S:I:", long_options,
                            &option_index)) != -1) {
        switch (c) {
            /* general options */
            case 'r':
            ...
            case 's':
                ...
                if (standby_message_timeout < 0 ||
                    standby_message_timeout > PG_INT32_MAX) {
...
int main(int argc, char** argv)
{
     ...
     */
    {
        ...
        if (PQntuples(res) != 1 || PQnfields(res) < 4) {
            fprintf(stderr,
                    _("%s: could not identify system: got %d rows and %d fields, "
    ...
    if (do_drop_slot) {
        ...
        rc = snprintf_s(query, sizeof(query), sizeof(query) - 1,
                        "DROP_REPLICATION_SLOT \"%s\"", replication_slot);
        ...
        if (PQntuples(res) != 0 || PQnfields(res) != 0) {
            fprintf(stderr,
                    _("%s: could not stop logical rep: got %d rows and %d fields, "
    ...
    if (do_create_slot) {
        ...
        rc = snprintf_s(query, sizeof(query), sizeof(query) - 1,
                        "CREATE_REPLICATION_SLOT \"%s\" LOGICAL %s", replication_slot,
                        plugin);
        securec_check_ss_c(rc, "\0", "\0");
        ...
        if (PQntuples(res) != 1 || PQnfields(res) != 4) {
            fprintf(stderr,
                    _("%s: could not init logical rep: got %d rows and %d fields, "
                      "expected %d rows and %d fields\n"),
                    progname, PQntuples(res), PQnfields(res), 1, 4);
            disconnect_and_exit(1);
        }
