...
#define assert_valid_hash_key2(type, field1, field2)                             \
    static_assert(                                                               \
        sizeof(type) == sizeof(((type){0}).field1) + sizeof(((type){0}).field2), \
...
#define foreach_htab(var, status, htab)                                              \
    ...
    for ((var) = static_cast<decltype(var)>(hash_seq_search(status)); (var) != NULL; \
         (var) = static_cast<decltype(var)>(hash_seq_search(status)))
...
#define CreateSimpleHash(keyType, entryType) \
    CreateSimpleHashWithNameAndSize(keyType, entryType, #entryType "Hash", 32)

/*
 * Same as CreateSimpleHash but allows specifying the name
 */
#define CreateSimpleHashWithName(keyType, entryType, name) \
    CreateSimpleHashWithNameAndSize(keyType, entryType, name, 32)
...
#define CreateSimpleHashWithSize(keyType, entryType, size) \
    CreateSimpleHashWithNameAndSize(keyType, entryType, #entryType "Hash", size)

#define CreateSimpleHashWithNameAndSize(keyType, entryType, name, size)               \
    CreateSimpleHashWithNameAndSizeInternal(sizeof(keyType), sizeof(entryType), name, \
                                            size)
...
#define CreateSimpleHashSet(keyType) \
    CreateSimpleHashWithName(keyType, keyType, #keyType "HashSet")
