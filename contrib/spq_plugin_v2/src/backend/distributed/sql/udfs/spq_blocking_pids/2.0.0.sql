CREATE FUNCTION pg_catalog.spq_blocking_pids(pBlockedPid integer)
RETURNS int4[] AS $$
  DECLARE
  ...
  BEGIN
    ...
    SELECT array_agg(global_pid) INTO mRemoteBlockingPids FROM (
      WITH activeTransactions AS (
        ...
        SELECT blocking_global_pid FROM spq_internal_global_blocked_processes()
...
REVOKE ALL ON FUNCTION spq_blocking_pids(integer) FROM PUBLIC;
