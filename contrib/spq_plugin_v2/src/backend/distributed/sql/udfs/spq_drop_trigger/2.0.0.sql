CREATE OR REPLACE FUNCTION pg_catalog.spq_drop_trigger()
...
BEGIN
    ...
    LOOP
        ...
        -- i.e. we call spq_drop_all_shards with drop_shards_metadata_only parameter set to true
        IF dropped_table_is_a_partition
        THEN
            PERFORM spq_drop_all_shards(v_obj.objid, v_obj.schema_name, v_obj.object_name, drop_shards_metadata_only := true);
        ELSE
            PERFORM spq_drop_all_shards(v_obj.objid, v_obj.schema_name, v_obj.object_name, drop_shards_metadata_only := false);
        END IF;
    ...
    FOR v_obj IN SELECT * FROM pg_event_trigger_dropped_objects()
    LOOP
        -- remove entries from __$spq$__.pg_dist_object for all dropped root (objsubid = 0) objects
        PERFORM spq_unmark_object_distributed(v_obj.classid, v_obj.objid, v_obj.objsubid);
    END LOOP;
...
END;
$cdbdt$;
COMMENT ON FUNCTION pg_catalog.spq_drop_trigger()
