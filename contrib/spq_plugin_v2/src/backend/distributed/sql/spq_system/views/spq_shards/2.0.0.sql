CREATE OR REPLACE VIEW __$spq$__.spq_shards AS
SELECT
     ...
     CASE WHEN partkey IS NOT NULL THEN 'distributed'
      ...
      ELSE 'distributed' END AS table_type,
...
LEFT JOIN
   (SELECT shard_id, max(size) as size from spq_shard_sizes() GROUP BY shard_id) as shard_sizes
ON
...
ALTER VIEW __$spq$__.spq_shards SET SCHEMA pg_catalog;
GRANT SELECT ON pg_catalog.spq_shards TO public;
