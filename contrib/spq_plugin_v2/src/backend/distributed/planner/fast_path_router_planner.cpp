 ...
 *
 */
PlannedStmt* FastPathPlanner(Query* originalQuery, Query* parse,
                             ParamListInfo boundParams)
{
     ...
     */
    parse->targetList = (List*)eval_const_expressions(NULL, (Node*)parse->targetList);
 ...
 */
PlannedStmt* GeneratePlaceHolderPlannedStmt(Query* parse)
{
    ...
    plan->targetlist =
        static_cast<List*>(copyObject(FetchStatementTargetList((Node*)parse)));
...
#endif
    result->rtable = static_cast<List*>(copyObject(parse->rtable));
 ...
 */
bool FastPathRouterQuery(Query* query, Node** distributionKeyValue)
{
    FromExpr* joinTree = query->jointree;
    Node* quals = NULL;

    if (!Session_ctx::Vars().EnableFastPathRouterPlanner) {
        return false;
    }
     ...
     */
    if (query->cteList != NIL || query->hasSubLinks || query->setOperations != NULL ||
    ...
    if (joinTree == NULL || (IsCitusTableTypeCacheEntry(cacheEntry, DISTRIBUTED_TABLE) &&
                             joinTree->quals == NULL)) {
        return false;
    }
     ...
     *
     *	We're also not allowing any other appearances of the distribution key in the
     *quals.
     *
