    ...
    if (message && error) {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("Memory constraint error: %s (errno %d)", message, error)));
    } else if (message) {
        ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                        errmsg("Memory constraint error: %s", message)));
    } else if (error) {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("Unknown function failed with memory constraint error (errno %d)",
                        error)));
    } else {
        ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                        errmsg("Unknown function failed with memory constraint error")));
    }
}
 ...
 */
int64 SafeStringToInt64(const char* str)
{
    char* endptr;
    errno = 0;
 ...
 * https://stackoverflow.com/a/26083517/2570866
 */
int32 SafeStringToInt32(const char* str)
{
    char* endptr;
    errno = 0;
 ...
 * https://stackoverflow.com/a/26083517/2570866
 */
uint64 SafeStringToUint64(const char* str)
{
    char* endptr;
    errno = 0;
               ...
               int (*comp)(const void*, const void*))
{
    if (count > RSIZE_MAX_MEM) {
        ereport_constraint_handler("SafeQsort: count exceeds max", NULL, ESLEMAX);
    }

    if (size > RSIZE_MAX_MEM) {
        ereport_constraint_handler("SafeQsort: size exceeds max", NULL, ESLEMAX);
    }
    if (size != 0) {
        if (ptr == NULL) {
...
{
    if (count > RSIZE_MAX_MEM) {
        ereport_constraint_handler("SafeBsearch: count exceeds max", NULL, ESLEMAX);
    }

    if (size > RSIZE_MAX_MEM) {
        ereport_constraint_handler("SafeBsearch: size exceeds max", NULL, ESLEMAX);
    }
    if (size != 0) {
        if (key == NULL) {
 ...
 */
int SafeSnprintf(char* restrict buffer, rsize_t bufsz, const char* restrict format, ...)
{
    /* failure mode 3 */
    if (buffer == NULL) {
    ...
    /* failure mode 4 */
    if (bufsz == 0) {
        ereport_constraint_handler("SafeSnprintf: bufsz is 0", NULL, ESZEROL);
    }

    if (bufsz > RSIZE_MAX_STR) {
        ereport_constraint_handler("SafeSnprintf: bufsz exceeds max", NULL, ESLEMAX);
    }
