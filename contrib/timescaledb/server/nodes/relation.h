...
typedef struct AggClauseCosts {
    ...
    bool hasNonPartial;//tsdb
    bool hasNonSerial;//tsdb
} AggClauseCosts;
...
typedef struct PlannerInfo {
    ...
    List* processed_tlist;//tsdb
    struct PathTarget *upper_targets[6];//tsdb,长度原本是UPPERREL_FINAL+1
    Bitmapset *outer_params;//tsdb
    List *upper_rels[6]; //tsdb,长度原本是UPPERREL_FINAL+1
} PlannerInfo;
...
typedef struct RelOptInfo {
    ...
    List *partial_pathlist;//tsdb
    bool consider_parallel;//tsdb
    bool consider_startup;//tsdb
    bool consider_param_startup;//tsdb
    struct PathTarget *reltarget; //tsdbb
} RelOptInfo;
...
typedef struct Path {
    ...
    PathTarget *pathtarget; //tsdb
    bool parallel_aware;//tsdb
    bool parallel_safe;//tsdb
    int parallel_workers;//tsdb
    struct PathTarget *reltarget; //tsdb
} Path;
